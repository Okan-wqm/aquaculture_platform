...
import { MattilsynetApiService, type MattilsynetBasePayload } from '../mattilsynet-api.service';
...
} as Partial<MattilsynetBasePayload> as ValidatedPayload<MattilsynetBasePayload>;
...
describe('MattilsynetApiService submit breaker (FARM-MEDIUM-172)', () => {
  ...
  it('THROWS inside the breaker fn on a 5xx so the breaker counts it', async () => {
    ...
    const result = await service.submitByType('tenant-A', RegulatoryReportType.SEA_LICE, PAYLOAD);
