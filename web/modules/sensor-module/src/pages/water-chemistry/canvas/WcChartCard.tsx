...
const WcChartCard = ({
  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
      <div className="flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 px-3 py-1.5">
        <span
          className="drag-handle flex-1 cursor-move truncate text-sm font-medium text-gray-800 dark:text-gray-200"
          title={card.title}
        >
          {card.title}
        </span>
        <Select
          ...
          options={(Object.keys(CHART_LABELS) as ChartType[]).map((t) => ({
            value: t,
            label: CHART_LABELS[t],
          }))}
        />
        <Button variant="ghost" type="button" onClick={onConfigure} title="Configure">
          ⚙
        </Button>
        <Button variant="ghost" type="button" onClick={onRemove} title="Remove">
          ✕
        </Button>
      </div>
      <div className="min-h-0 flex-1 space-y-3 overflow-auto p-2">
        {inputs && outputs ? (
          ...
          <div className="flex h-full items-center justify-center rounded border border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 p-3 text-center text-xs text-gray-500 dark:text-gray-400">
