...
`>400` hard review gate; generated/declarative migration istisnası açık gerekçe ister. God
service/controller, ters dependency ve kontrolsüz complexity kabul edilmez.

## Durum, kanıt ve kapanış sözleşmesi

Sprint state'i yalnız `PLANNED -> READY -> IN_PROGRESS -> VERIFYING -> DONE`; side state'ler
`BLOCKED`, `SUPERSEDED`. `events.jsonl` append-only, hash-chained authority'dir. Evidence manifest
proof record'dur; `PROGRESS.md` yalnız generator projection'ıdır. D0 görünümü S01 generator'ı
gelene kadar açıkça bootstrap projection olarak işaretlidir.
...
Proof class'ları `code_proven`, `live_proven`, `operator_attested`. Her kayıt claim, target SHA,
origin-main reachability, authority path/digest, producer, gerekli reviewer, exact command/workflow,
UTC start/end, exit/verdict, artifact URI/digest, freshness, gerekli negative control ve linked
finding içerir. Producer kendi artifact'ını onaylayamaz.
...
| `ACC-READ-001`      | Kaynak modülleri cohesive, dependency-direction uyumlu; `<=250` hedefi ve `>400` hard gate uygulanır.      |
...
| ------- | ------------------------------------------------------------------------------------------- | ------------------ |
| `OP-01` | Platform Security: ayrı Publisher/Merge GitHub App permission ve branch-protection export'u | S25, S31, S52      |
| `OP-02` | Platform Ops: production droplet'tan ayrı worker VM, UID/network/resource isolation         | S18, S41           |
| `OP-03` | Security: PKI/KMS, cert CN, pinned attestor/subject bindings ve rotation drill              | S04, S05, S28      |
| `OP-04` | Data Ops: bağımsız Postgres, object store, backup/PITR ve retention policy                  | S03, S41, S61      |
| `OP-05` | AI Ops: Codex CLI ve Claude Code CLI subscriptions, live rate/concurrency limits            | S14, S20, S21      |
| `OP-06` | Release Owner: human deploy/release ve rollback/on-call authority                           | S54, S69           |
| `OP-07` | SRE: ölçülmüş SLO, RPO/RTO, capacity ve burn-in minimumları                                 | S47, S55, S62, S63 |
| `OP-08` | Privacy/Legal: retention, incident capture, legal hold ve deletion-proof onayı              | S14, S39           |
...
| ------ | --------------------------------------------------------------------------------- | ---------- | ------------------------- |
| S01    | Progress event/evidence schema, projection contract ve false-completion invariant | D0 merged  | `ACC-S01`, `ACC-EVD-001`  |
| S02    | Inert Nx/NestJS `aria-service` scaffold ve kernel boundaries                      | S01        | `ACC-S02`, `ACC-READ-001` |
| S03    | Bağımsız Postgres/object-store schema, migration ve repository contracts          | S02, OP-04 | `ACC-S03`, `ACC-ISO-001`  |
| S04    | Immutable identities, access predicate ve workspace allowlist                     | S03, OP-03 | `ACC-S04`, `ACC-TCB-001`  |
| S05    | Protected TCB/policy registry, risk taxonomy, non-self-modification               | S04        | `ACC-S05`, `ACC-TCB-001`  |
| S06    | Read-only GraphQL overview/status/timeline surfaces                               | S03-S05    | `ACC-S06`, `ACC-ISO-001`  |
| S07    | Federated read-only ARIA web shell/status/result views                            | S06        | `ACC-S07`, `ACC-READ-001` |
| S08    | P01 adversarial review, restore/fail-closed gate ve evidence seal                 | S01-S07    | `ACC-S08`, `ACC-EVD-001`  |
...
| S13    | Execution olmadan mission planı ve acceptance/test synthesis     | S11-S12           | `ACC-S13`, `ACC-TCB-001` |
...
| S16    | P02 adversarial read-only/no-side-effect proof                   | S09-S15           | `ACC-S16`, `ACC-EVD-001` |
...
| S18    | Separate worker VM, mTLS identity ve confinement             | S17, OP-02     | `ACC-S18`, `ACC-SEP-001` |
...
| S23    | Artifact admission, secret/SSRF/exfiltration/budget controls | S19-S22        | `ACC-S23`, `ACC-EVD-001` |
| S24    | `EXECUTE_NO_PUSH` adversarial pressure/recovery gate         | S17-S23        | `ACC-S24`, `ACC-EVD-001` |
...
| S27    | Typed independent attestations ve producer/reviewer separation | S26        | `ACC-S27`, `ACC-SEP-001` |
| S28    | Operation/workspace/SHA/payload/policy-bound step-up grants    | S27, OP-03 | `ACC-S28`, `ACC-TCB-001` |
| S29    | Policy-attestor, deterministic admission ve fail-closed schema | S27-S28    | `ACC-S29`, `ACC-TCB-001` |
| S30    | Merge-evaluation dossier; merge execution yok                  | S26-S29    | `ACC-S30`, `ACC-REL-001` |
| S31    | Async merge effect protocol/readback, execution disabled       | S30, OP-01 | `ACC-S31`, `ACC-EVD-001` |
| S32    | `PR_OPEN` gate; merge disabled                                 | S25-S31    | `ACC-S32`, `ACC-REL-001` |
...
| S34    | Producer/challenger/judge/appellate separation ve quorum           | S33            | `ACC-S34`, `ACC-SEP-001` |
...
| S39    | Observability, incident capture, retention/redaction/kill switch   | S36-S38, OP-08 | `ACC-S39`, `ACC-TCB-001` |
| S40    | Adversarial gate; unresolved load-bearing challenge sıfır          | S33-S39        | `ACC-S40`, `ACC-EVD-001` |
...
| Sprint | Objective                                                          | Dependency        | Acceptance                |
| ------ | ------------------------------------------------------------------ | ----------------- | ------------------------- |
| S41    | Independent-state shadow control plane, write yok                  | S40, OP-02, OP-04 | `ACC-S41`, `ACC-ISO-001`  |
| S42    | Legacy outcome salt-okunur karşılaştırması, coupling yok           | S41               | `ACC-S42`, `ACC-ISO-001`  |
| S43    | `PR_ONLY`; PR create/update var, merge yok                         | S41-S42           | `ACC-S43`, `ACC-REL-001`  |
| S44    | Quota/concurrency/reservation/circuit breaker/host protection      | S43               | `ACC-S44`, `ACC-EVD-001`  |
| S45    | Crash/replay/lease/reconcile/disaster restore                      | S41-S44           | `ACC-S45`, `ACC-EVD-001`  |
...
| S48    | Burn-in gate; current evidence olmadan merge promotion yok         | S47               | `ACC-S48`, `ACC-EVD-001`  |
...
| S50    | Single-use autonomous merge permit issue/consume          | S49            | `ACC-S50`, `ACC-SEP-001`  |
| S51    | Per-base serialization/idempotency/duplicate-effect proof | S50            | `ACC-S51`, `ACC-EVD-001`  |
| S52    | Merge execution, 202/409 ve read-after-write reconcile    | S51, OP-01     | `ACC-S52`, `ACC-REL-001`  |
...
| S55    | Low-risk canary cohort ve supervised outcome evidence     | S49-S54, OP-07 | `ACC-S55`, `ACC-LIVE-001` |
| S56    | `MERGE_CANARY` rollback/restore/capacity gate             | S55            | `ACC-S56`, `ACC-EVD-001`  |
...
| Sprint | Objective                                               | Dependency     | Acceptance                |
| ------ | ------------------------------------------------------- | -------------- | ------------------------- |
| S57    | Protected medium-risk taxonomy/prohibited boundary      | S56            | `ACC-S57`, `ACC-TCB-001`  |
| S58    | Bounded multi-authority medium-risk authorization       | S57            | `ACC-S58`, `ACC-SEP-001`  |
| S59    | Multi-repository onboarding ve identity collision tests | S58            | `ACC-S59`, `ACC-ISO-001`  |
| S60    | Portable config/package/runtime deployment contract     | S59            | `ACC-S60`, `ACC-READ-001` |
| S61    | Backup/PITR/object recovery/regional rebuild drills     | S60, OP-04     | `ACC-S61`, `ACC-LIVE-001` |
| S62    | Loaded capacity/queue/SLO/rate/cost envelopes           | S60-S61, OP-07 | `ACC-S62`, `ACC-LIVE-001` |
| S63    | Supervised medium-risk burn-in ve outcome minimums      | S57-S62, OP-07 | `ACC-S63`, `ACC-LIVE-001` |
| S64    | Medium-risk promotion ve portability gate               | S63            | `ACC-S64`, `ACC-EVD-001`  |
...
| S66    | Protected prohibited categories/non-bypass/policy ownership      | S65        | `ACC-S66`, `ACC-NOHR-001` |
| S67    | Threat model/abuse cases/authority-compromise drills             | S66        | `ACC-S67`, `ACC-EVD-001`  |
| S68    | Disaster/credential revoke/provider outage/recovery exercises    | S67        | `ACC-S68`, `ACC-LIVE-001` |
| S69    | Human release gate ve exact deployed SHA proof under pressure    | S68, OP-06 | `ACC-S69`, `ACC-REL-001`  |
| S70    | Fresh twelve-agent full-system audit ve appellate review         | S65-S69    | `ACC-S70`, `ACC-EVD-001`  |
| S71    | Signed go/no-go dossier, unresolved risks/operator prerequisites | S70        | `ACC-S71`, `ACC-NOHR-001` |
| S72    | Program closeout; low/medium bounds kayıtlı, high-risk disabled  | S71        | `ACC-S72`, `ACC-NOHR-001` |

## Phase gate'leri

| Gate | Minimum exit evidence                                                                              |
| ---- | -------------------------------------------------------------------------------------------------- |
| P01  | Access/TCB isolation, migration restore, read-only API/UI ve adversarial seal `code_proven`        |
| P02  | Immutable snapshot-to-plan provenance ve hiçbir side effect olmadığını gösteren negative control   |
| P03  | Separate-worker containment, broker contract, TDD/artifact admission ve `EXECUTE_NO_PUSH` recovery |
| P04  | Publisher/attestor/step-up ayrımı, PR readback; merge capability'nin erişilemezliği                |
| P05  | 12-role campaign, deterministic oracle, 88-row executable coverage, zero load-bearing challenge    |
| P06  | Sustained shadow/PR_ONLY `live_proven`, restore ve kill/freeze drill; merge disabled               |
| P07  | Low-risk canary outcome, duplicate-effect/rollback/capacity proof; human release korunmuş          |
| P08  | Medium-risk quorum, portability/DR/capacity ve minimum supervised outcome evidence                 |
| P09  | Signed dossier; high-risk execution/merge path'inin disabled/non-bypass proof'u                    |
...
Başlangıç `origin/main` exact SHA'sı note'a yazılır. TDD kırmızısı gözlenir; yalnız green commits
...
P01-P05 hiçbir production autonomy vermez. P06 shadow ve `PR_ONLY`, P07 yalnız protected low-risk
canary merge, P08 yalnız bounded multi-authority medium-risk yetki sağlar. Her aşama güncel evidence,
capacity, restore ve rollback gate'ine bağlıdır. UI yalnız yeni API'ya bağlanır. Legacy sistem aktif
ve bağımsız kalır; removal/cutover bu programın etkisi değildir.
