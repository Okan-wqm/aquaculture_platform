 ...
 * Process diyagramını SCADA edge device'a deploy etme dialog'u
 */

import React, { useState, useEffect } from 'react';
import { Monitor, Wifi, WifiOff, Upload, X, Loader2, CheckCircle, AlertCircle } from 'lucide-react';
...
interface DeployToEdgeDialogProps {
  processId: string;
  processName: string;
  isOpen: boolean;
  onClose: () => void;
...
export const DeployToEdgeDialog: React.FC<DeployToEdgeDialogProps> = ({
  ...
  const deployMutation = useDeployProcessToEdge();
  ...
  const handleDeploy = async () => {
    ...
    try {
      const result = await deployMutation.mutateAsync({ processId, deviceId: selectedDeviceId });
      if (result.success) {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[85vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
            ...
            <p className="font-medium text-gray-900">{processName}</p>
          </div>
          ...
          {deploySuccess && (
            <div className="p-3 rounded-lg flex items-center gap-2 bg-green-50 text-green-700 border border-green-200">
              ...
              <span className="text-sm font-medium">Deploy basariyla baslatldi!</span>
            </div>
          )}
