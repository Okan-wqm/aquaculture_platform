/**
 * Synchronous SHA-256 over the exact UTF-8 bytes of a JavaScript string.
 *
 * The event-contracts package is consumed by Node and browser bundles, so this
 * helper deliberately depends on neither `node:crypto` nor the asynchronous
 * Web Crypto API. Inputs at this boundary are capped at 256 KiB.
 */
export function sha256Utf8Hex(value: string): string {
  ...
  writeUint32BigEndian(padded, paddedLength - 8, highLength);
  writeUint32BigEndian(padded, paddedLength - 4, lowLength);
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choose + (SHA256_ROUND_CONSTANTS[index] ?? 0) + (schedule[index] ?? 0)) >>> 0;
