...
- Cross-tenant tables KEEP `schema: 'farm'`. The authoritative set is farm's `MODULE_SCHEMAS[].infrastructureTables` in `libs/backend-common/src/database/schema-manager.service.ts`: <!-- infra-tables:farm -->`migrations`, `farm_outbox`, `outbox_events`, `inbox_messages`, `event_dlq`, `tenant_erasure_audit`, `farm_audit_logs`, `tenant_localization`, `feeding_job_runs`, `tenant_erasure_target_proofs`<!-- /infra-tables -->. That list is proven against the registry by `tests/invariants/nested-steering-parity.spec.ts` — edit the registry, never this copy. (`farm_outbox` is a legacy registry alias; the live entity `outbox/farm-outbox.entity.ts` declares `name: 'outbox_events'`.)
- The per-table `schema:` rule is enforced for farm-service by `tests/invariants/entity-schema-declaration.spec.ts` (both directions, source text, every PR); the live-DDL direction (cross-tenant tables MUST carry `schema:` in the database) by `e2e/tests/integration/schema-invariants.spec.ts` B.1/B.2 in `db-migration-check.yml`. farm-service owns no private copy of that spec — its former `tenant-schema-routing.architecture.spec.ts` was a weaker duplicate (ORPHAN-MEDIUM-118) in a lane no workflow ran.
