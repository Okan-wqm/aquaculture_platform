...
**Still open under this finding (owner okan, deadline 2026-12-31):** `admin.plan_definitions` and
`admin.custom_plans` have not moved. `billing.plans.pricing` is still a jsonb price matrix and
`admin.custom_plans.modules` still holds per-module subtotals in jsonb (nine of the twenty-three
remaining allowlist entries between them), and `stripeProductId` / `stripePriceIds` still have two
writable homes because `admin.plan_definitions` duplicates `billing.plans`.
...
**Fix:** one forward migration per class (timestamptz, uuid tenantId, numeric money, real arrays,
inet) landed together with `{ type: 'timestamptz' }` on every decorator; `SCHEMA_DRIFT_FATAL` in
production.
**Gate:** no admin column is `timestamp without time zone`; decorator lint bans bare
`@CreateDateColumn()` / `@UpdateDateColumn()`; CHECK presence invariant on money / enum / state
columns.
