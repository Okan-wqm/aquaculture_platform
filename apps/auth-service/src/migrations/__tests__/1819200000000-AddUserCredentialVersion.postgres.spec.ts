...
describe('AddUserCredentialVersion1819200000000 on real Postgres', () => {
  ...
  it('advances the version once per credential-bearing change: password, role, tenant, deactivation', async () => {
    await requireAdmin().query(`UPDATE auth.users SET password = 'p1:$2b$hash-two' WHERE id = $1`, [
      userId,
    ]);
    expect(await readVersion()).toBe(2);

    await requireAdmin().query(`UPDATE auth.users SET role = 'MODULE_USER' WHERE id = $1`, [
      userId,
    ]);
  ...
  it('is owned by the database: a value supplied by the application is discarded', async () => {
    await requireAdmin().query(`UPDATE auth.users SET "credentialVersion" = 99 WHERE id = $1`, [
      userId,
    ]);
    expect(await readVersion()).toBe(5);
  ...
  it('down restores the pre-migration catalog', async () => {
    await withQueryRunner((queryRunner) =>
      new AddUserCredentialVersion1819200000000().down(queryRunner),
    );
