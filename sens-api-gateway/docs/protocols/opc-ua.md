...
A separate OPC UA **server** role (feature `opc-ua-server`, crate `async-opcua`) is off by default and is not part of the RC4 `scada-display` release tier; this chapter covers the **client** role only.

RC4 operator guidance lives in `docs/runbooks/edge-gateway-opcua.md`. For `agent-v2.0.0-rc4`, treat the client as `SecurityPolicy#None` unless a later runtime PR proves signed/encrypted chunk support.
