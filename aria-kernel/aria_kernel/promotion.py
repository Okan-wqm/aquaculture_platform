...
def promote_tool(
    ...
    *,
    ...
    fixture_passed = latest_fixture_pass(tool_id, base_dir=base_dir)
    if target_status == "SHADOW" and tool["status"] == "CALIBRATE" and not fixture_passed:
        raise GovernanceError("CALIBRATE -> SHADOW requires the latest fixture suite to pass")
    if target_status == "ACTIVE":
        if not fixture_passed:
            raise GovernanceError("SHADOW -> ACTIVE requires the latest fixture suite to pass")
