...
test('real MFA enrollment, public login and step-up preserve session identity under tenant RLS', async ({ request }) => {
  ...
  try {
    ...
    const setup = await client.mutate<{ setupMfa: { secret: string; recoveryCodes: string[] } }>(
      'mutation { setupMfa { secret recoveryCodes } }', {}, { token: user.token },
    );
    ...
    const confirmed = await client.mutate<{ verifyMfaSetup: { success: boolean } }>(
      ...
      { input: { code: authenticatorCode(enrollment.secret) } }, { token: user.token },
    );
    ...
    if (!challenge || !loginCode || !stepUpCode) throw new Error('Missing MFA challenge or recovery codes');
    const before = await db.query<{ count: string }>(
      'SELECT COUNT(*)::text AS count FROM auth.refresh_tokens WHERE "userId" = $1', [user.id]);
    const verified = await client.mutate<{ verifyMfaLogin: { accessToken: string; refreshToken: string } }>(
    ...
    const elevated = await client.mutate<{ mfaStepUp: { accessToken: string; refreshToken: string } }>(
      'mutation StepUp($input: MfaStepUpInput!) { mfaStepUp(input: $input) { accessToken refreshToken } }',
      { input: { code: stepUpCode } }, { token });
    ...
    expect(elevatedClaims).toMatchObject({ sub: user.id, jti: original.jti, iat: original.iat, mfaVerified: true });
