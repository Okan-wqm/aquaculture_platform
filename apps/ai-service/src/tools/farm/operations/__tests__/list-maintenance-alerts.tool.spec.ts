...
import { ListMaintenanceAlertsTool } from '../list-maintenance-alerts.tool';
...
describe('ListMaintenanceAlertsTool', () => {
  let send: jest.Mock;
  let tool: ListMaintenanceAlertsTool;

  beforeEach(() => {
    send = jest.fn().mockReturnValue(of({ ok: true, data: { items: [], truncated: false } }));
    tool = new ListMaintenanceAlertsTool({ send });
  });

  it('is a module-scoped farm read tool', () => {
    const metadata = tool.getMetadata();
    expect(metadata.name).toBe('list_maintenance_alerts');
  ...
  it('applies defaults and sends the contract subject with the context tenant', async () => {
    ...
    expect(send).toHaveBeenCalledWith(FARM_AI_QUERY_SUBJECTS.MAINT_SCHEDULE_ALERTS, {
      ...{ limit: 20 },
  ...
  it('forwards explicit fields (limits clamped to the contract cap)', async () => {
    await tool.execute({ limit: 9 }, CTX);
