...
import type { GetMyWeeklyPlanQuery, GetMyWeeklyPlanQueryVariables } from '@/generated/graphql';
...
const MY_WEEKLY_PLAN_QUERY: TypedDocumentNode<GetMyWeeklyPlanQuery, GetMyWeeklyPlanQueryVariables> =
  gql`
...
async function fetchMySchedule(weekStartDate: string): Promise<WeeklyPlan | null> {
  const result = await graphqlRequest(MY_WEEKLY_PLAN_QUERY, { weekStartDate });
