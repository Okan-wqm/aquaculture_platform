...
export type PaginatedModules = IStandardPaginatedResult<ModuleDto>;
...
export interface ModuleTenantSummary {
  id: string;
  name: string;
  slug: string;
  status: string;
...
export class ModulesService {
  ...
  async getModuleTenants(
    ...
    page = 1,
    limit = 50,
  ): Promise<IStandardPaginatedResult<ModuleTenantSummary>> {
    const offset = (page - 1) * limit;

    try {
      const [tenants, countResult] = await Promise.all([
        this.dataSource.query<ModuleTenantSummary[]>(
          `
          SELECT
            t.id,
            t.name,
  ...
  async getAssignments(
    ...
    page = 1,
    limit = 50,
  ): Promise<IStandardPaginatedResult<ModuleAssignmentSummary>> {
    const offset = (page - 1) * limit;
    ...
    try {
      const [assignments, countResult] = await Promise.all([
        this.dataSource.query<ModuleAssignmentSummary[]>(
          `
          SELECT
            tm.id,
