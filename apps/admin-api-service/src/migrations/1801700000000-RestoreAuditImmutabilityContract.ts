...
/**
 * RestoreAuditImmutabilityContract1801700000000
 * ============================================================================
 *
 * Restores the two-trigger append-only contract on `shared.audit_logs` and `admin.audit_logs` that the
 ...
 * its own incident.
 *
 * `shared.audit_logs` fared worse still: the baseline carries no function and no
 * trigger for it at all, so the canonical cross-service audit table lost its
 * immutability guarantee outright.
 *
 ...
 */
export class RestoreAuditImmutabilityContract1801700000000 implements MigrationInterface {
  name = 'RestoreAuditImmutabilityContract1801700000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    for (const table of [
      { schema: 'shared', table: 'audit_logs' },
