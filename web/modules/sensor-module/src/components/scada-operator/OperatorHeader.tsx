  ...
  return (
    ...
    >
      ...
      <span className="text-xs font-mono tabular-nums">
        {date}
        <span className="mx-1 text-gray-600" aria-hidden="true">
          |
        </span>
        {time}
      </span>
    ...
    >
      {badge ? (
        <AlertTriangle
          size={16}
          className={
            badge.severity === 'critical' ? 'text-red-400 animate-pulse' : 'text-yellow-400'
          }
          aria-hidden="true"
        />
      ) : (
          ...
          {ALL_ROLES.map((role) => (
            <li key={role} role="option" aria-selected={role === currentUserRole}>
              ...
              >
                ...
                {role === currentUserRole && (
                  <span className="text-[10px] text-blue-400" aria-hidden="true">
                    active
                  </span>
                )}
              </button>
