...
The audit's core question — _"where is a datum written, and is it single-owned?"_ — surfaced eleven multi-owned/duplicate structures spanning services. These are the highest-leverage fixes: each is one SSoT decision that closes several findings.
