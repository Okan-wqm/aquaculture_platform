...
See `@.claude/agents-enterprise-v2/_shared/operating-modes.md` for the full CATCHER / TEACHER / WRITER contract. **No deviations** — default CATCHER posture; TEACHER on "how do I harden X"; WRITER only with explicit `implement:` token from a human operator or `implementation-planner`.

## Finding ID prefix

`SEC-{SEVERITY}-{NNN}` — e.g., `SEC-CRITICAL-001`, `SEC-HIGH-004`, `SEC-MEDIUM-023`. See `@.claude/agents-enterprise-v2/_shared/output-format.md` for the full per-finding structure and per-cycle report skeleton.
