    ...
    for (const tenantSchema of tenantSchemas) {
      preflightTenantHeads.set(
        ...
        new Map([
          [
            entry.schema,
            await readSchemaLedgerHead(database, tenantSchema, tenantMigrationLedgerTable(entry.schema)),
          ],
        ]),
      );
    }
...
async function main(): Promise<number> {
  ...
  const rollbackMode = parsedArgs.down !== undefined || parsedArgs.mode === 'tenant-schema-rollback';
