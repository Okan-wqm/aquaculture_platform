...
## ORPHAN-MEDIUM-308 — admin-panel TenantManagementPage.spec.tsx is 16/33 red independent of jest-dom wiring — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `web/modules/admin-panel/vite.config.ts` wires `setupFiles: ['./src/test-setup.ts']`. The jest-dom wiring fix merged in `8d1b342ed` (PR #830); the registry entry was left `IN-PROGRESS` at raise-time. This finding covered ONLY the wiring — the 16 residual data-mock failures remain a separate undiagnosed concern (not reopened here).
...
## ORPHAN-MEDIUM-314 — gateway-api reimplements unknown-rejection normalization ad hoc at 9 sites, one variant losing the object case — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `toError()` lives in `apps/gateway-api/src/common/error-normalization.ts` and is imported + used at all four cited sites (`upload.controller.ts`, `circuit-breaker.service.ts`, `timeout.middleware.ts`, `redis-io.adapter.ts`). Consolidation fix merged in `8d1b342ed` (PR #830); the registry was left `IN-PROGRESS`. Note: that commit's `Closes:` trailer cites the pre-renumbering id `ORPHAN-MEDIUM-309` (merge-train collision renumbering), recorded in `tests/invariants/three-store-invariants.spec.ts` `LEGACY_TRAILER_DRIFT`.
...
## ORPHAN-MEDIUM-310 — RedisService.keyPrefix uses `||` instead of `??`, silently discarding an explicit empty-string prefix — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `libs/backend-common/src/redis/redis.service.ts:24` reads `this.keyPrefix = options.keyPrefix ?? 'aqua:';` (nullish, not falsy). Fix merged in `8d1b342ed` (PR #830); the registry was left `IN-PROGRESS`.
...
## ORPHAN-MEDIUM-311 — e2e TestDatabase hardcoded DEFAULT_DATABASE_URL silently drifted from the actual docker-compose port — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `e2e/helpers/db.helper.ts`'s `resolveDefaultDatabaseUrl()` calls `getRequiredE2eDatabaseUrl()` (env var → `.env` → docker-compose-derived) FIRST, so the SSoT is the live compose file (`docker-compose.infra.yml` maps `"5433:5432"`), not a hand-copied literal. The remaining `localhost:5432` constant is an explicitly-documented last-resort fallback (reachable only if both env and a readable compose file are absent) — the silent-drift root cause is fixed. Fix merged in `8d1b342ed` (PR #830); the registry was left `IN-PROGRESS`.
...
## ORPHAN-MEDIUM-322 — aquamobil: 10 of ~65 void-wrapped async calls were genuinely unguarded (2 broke their own "never throws" contract) — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `useMarkRead.ts`/`useEditMessage.ts` now guard every `addToQueue()` inside try/catch (honoring the never-throws contract); `AccountPage.tsx`'s clear-queue action uses `runAsyncAction('account-clear-empty-queue')`; `ChatRoomPage.tsx`/`AiChatPage.tsx` send paths use `runAsyncAction(handleSend, ...)`. Fix merged in `8d1b342ed` (PR #830); the registry was left `IN-PROGRESS`. Note: that commit's `Closes:` trailer cites the pre-renumbering id `ORPHAN-MEDIUM-313` (merge-train collision renumbering), recorded in `LEGACY_TRAILER_DRIFT`.
...
## ORPHAN-MEDIUM-315 — 5 real CI failures surfaced by PR #830 (registry state-machine violation + 2 tsconfig-scope gaps + 1 finding-registry hygiene issue) — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `e2e/types.d.ts`, `tools/testing/tsconfig.json`, and `web/apps/aquamobil/tsconfig.sw.json` all exist and are wired; PR #830 (`8d1b342ed`) merged with CI green. The described CI-surface gaps are closed; the registry was left `IN-PROGRESS`.
...
## ORPHAN-MEDIUM-316 — e2e/project.json registration silently exposed live-infra-dependent test scripts to the generic nx affected test sweep — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `e2e/package.json` declares `"nx": { "includedScripts": [] }`, disabling Nx package.json-script auto-inference, and `e2e/project.json` declares only the explicit `lint` target. Live-infra test scripts are no longer exposed to the generic `nx affected -t test` sweep. Fix merged in `8d1b342ed` (PR #830); the registry was left `IN-PROGRESS`.
...
## ORPHAN-MEDIUM-321 — aquamobil silently swallowed 9 background-operation failures with zero observability — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — the previously silent `.catch(() => undefined)` sites now log: `useAuth.tsx` (five `logger.error('[auth-logout] ...')` catches + `runAsyncAction`), `useChannels.ts`/`useMessages.ts` (`logger.error('[useChannels|useMessages] ...')`), and `messaging-sw.ts` (`logger.error('[sw-fetch|sw-revalidate] ...')`). Fix merged in `8d1b342ed` (PR #830); the registry was left `IN-PROGRESS`. Note: that commit's `Closes:` trailer cites the pre-renumbering id `ORPHAN-MEDIUM-312` (merge-train collision renumbering), recorded in `LEGACY_TRAILER_DRIFT`.
...
## ORPHAN-MEDIUM-323 — CURRENT_STATE.md's pinned ARIA authority hash went stale after merging main's ARIA changes, breaking invariants-fast + aria-merge-authority — RESOLVED (state-drift close 2026-07-13; fix on main via 8d1b342ed / PR #830)

**RESOLVED 2026-07-13 (registry state-drift close):** verified on current main — `tests/invariants/aria-doc-runtime-ssot.spec.ts` PASSES (the pinned authority hash in `docs/aria/CURRENT_STATE.md` matches the current tree). This is per-PR regen churn; the hash was regenerated in `8d1b342ed` (PR #830) and has stayed in sync on main. The registry was left `IN-PROGRESS`.
