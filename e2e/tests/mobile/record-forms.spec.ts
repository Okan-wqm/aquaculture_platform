...
test.describe('AquaMobil record forms (online lane)', () => {
  ...
  test('an online cull record drains through the queue to the batch counters', async ({ page, request }) => {
    const client = new GraphQLTestClient(request);
    client.setToken(seed.adminApiToken);
    ...
    await expect
      .poll(
        async () => (await getBatchCounters(client, seed.adminApiToken, farm.batchId)).currentQuantity,
