/**
 * Scheduled operation K-3 dry-run: PAUSED atamalar için plan HESABI — persist YOK.
 *
 * Pinlenen sözleşme:
...
const mockReadTenantMutationInstantV1 = jest.fn();

jest.mock('@aquaculture/backend-common/database', () => ({
  ...jest.requireActual('@aquaculture/backend-common/database'),
  readTenantMutationInstantV1: mockReadTenantMutationInstantV1,
...
import { FEEDING_UTC_TIMEZONE } from '@aquaculture/feeding-contracts';
...
function makeService(fixture: DryRunFixture): {
  service: ScheduledFeedingOperationExecutor;
  ...
  const service = createScheduledFeedingOperationTestExecutor({
    ...
    temperatureService: mock<WaterTemperatureService>({ getEffectiveTemperaturesForUnits }),
...
describe('ScheduledFeedingOperationExecutor.dryRunForTargets (K-3)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockReadTenantMutationInstantV1.mockResolvedValue(feedingProtocolTestMutationInstant());
  });
  ...
  it('paused atama için GERÇEK plan hesabı döner — persist ve event YOK', async () => {
    ...
    const results = await service.dryRunForTargets(TENANT, [
      { siteId: SITE, timezone: FEEDING_UTC_TIMEZONE, planDate: '2026-07-20' },
    ]);
