  ...
  async execute(command: TransferBatchCommand): Promise<Batch> {
    ...
    const transferredBatch = await runInTenantTransaction(
      ...
      async (queryRunner) => {
        ...
        // Kilit sırası servis tarafından unitId-sıralı garanti edilir
        // (FARM-MEDIUM-275): payload sırası karşılıklı transferlerde AB-BA
        // penceresi açıyordu.
        await this.dayPlanRecalc.recalcForUnits(
          queryRunner.manager,
          tenantId,
