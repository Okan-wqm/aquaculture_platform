...
export function snapshotCredentialProof(user: User): CredentialProof {
  ...
  return Object.freeze({ [credentialProofBrand]: true as const, id: user.id,
    role: user.role, tenantId: user.tenantId ?? null, isActive: user.isActive,
...
export function credentialProofFromClaims(claims: unknown): CredentialProof {
  if (!claims || typeof claims !== 'object' ||
      !('id' in claims) || typeof claims.id !== 'string' || !claims.id ||
      !('role' in claims) || !Object.values(Role).some((role) => role === claims.role) ||
      !('tenantId' in claims) || !(claims.tenantId === null || typeof claims.tenantId === 'string') ||
      !('isActive' in claims) || typeof claims.isActive !== 'boolean' ||
      !('credentialVersion' in claims) || typeof claims.credentialVersion !== 'number' ||
      !Number.isSafeInteger(claims.credentialVersion) || claims.credentialVersion < 1) {
    throw new ForbiddenException('User credential state is unavailable');
  }
  ...
  return Object.freeze({ [credentialProofBrand]: true as const, id: claims.id, role,
    tenantId: claims.tenantId, isActive: claims.isActive, credentialVersion: claims.credentialVersion });
}

export function assertCredentialProof(proof: CredentialProof, user: User): void {
  if (proof.id !== user.id || proof.role !== user.role ||
      proof.tenantId !== (user.tenantId ?? null) || proof.isActive !== user.isActive ||
...
export class LockedAuthContext {
  ...
  static async lock(manager: EntityManager, subject: CredentialProof | string): Promise<LockedAuthContext> {
    ...
    const discovered = await manager.findOne(User, { where: { id }, select: { id: true, tenantId: true } });
    if (!discovered) throw new ForbiddenException('Authentication failed');
    const tenant = discovered.tenantId
      ? await manager.findOne(Tenant, { where: { id: discovered.tenantId }, lock: { mode: 'pessimistic_read' } })
      : null;
  ...
  assertSessionAdmission(): void {
    ...
    if (!this.user.isActive || this.user.isLocked() ||
