...
| RPT-007 | MEDIUM | Harvest `QualityGrade` (PREMIUM/A/B/C/REJECT) ≠ Norwegian kvalitetsklasser (superior/ordinær/produksjonsfisk/utkast) | Two taxonomies grew independently | Phase 2 DONE: `harvest_records.quality_class` added (`1803100000000`), backfilled from `qualityGrade`. Phase 4: **retire the 5-level grade** (operator decision — regulator format wins, plan verdict). `quality_class` becomes the sole stored + input taxonomy; the `qualityGrade` column, enum, and grade→class map are dropped. **Accepted tradeoff:** the grade→class map is lossy (PREMIUM+GRADE_A→SUPERIOR), so historical Premium/Grade-A granularity is not preserved — this is a deliberate product change, executed across the harvest domain (inputs, stats grouped by class, filters) + both web apps. Note: the `BatchHarvested` wire event never carried the grade, so no event-contract change. |
