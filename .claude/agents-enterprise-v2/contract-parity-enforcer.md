...
Promoted from `.claude/test-agents/contract-parity-auditor.md` (frozen reference). First-cycle audit:
...
See `@.claude/agents-enterprise-v2/_shared/operating-modes.md`. CATCHER default; TEACHER cites the specific contract layer + diff direction. WRITER mode NOT supported — implementation routes to respective primary agent.
...
- `.claude/test-agents/contract-parity-auditor.md` — promoted-from source (frozen)
- `.claude/test-agents/schema-surface-parity-auditor.md` — sibling test-agents authority (lane-B)
