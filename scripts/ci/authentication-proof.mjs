...
if (![base, head, prHead].every((sha) => /^[0-9a-f]{40}$/.test(sha))) throw new Error('Immutable base/head identities required');
...
const changed = execFileSync('git', ['diff', '--name-only', `${base}...${head}`], { encoding: 'utf8' })
  .split('\n').filter((file) => /\.(spec|test)\.[cm]?[tj]sx?$/.test(file)).map((file) => resolve(file));
const receipt = { schema_version: 1, base_sha: base, pr_head_sha: prHead, tested_merge_sha: head,
  ...
  inventory_sha256: createHash('sha256').update(inventoryText).digest('hex'), lane, checks: [] };
mkdirSync('artifacts/authentication-proof', { recursive: true });
let failed = false;
for (const check of checks) {
  process.stdout.write(`::group::${check.id}\n`);
  const result = spawnSync(check.command[0], check.command.slice(1), {
    cwd: check.cwd, stdio: 'inherit', env: { ...process.env, NX_DAEMON: 'false', NX_NO_CLOUD: 'true' },
  });
  process.stdout.write('::endgroup::\n');
  const evidence = { id: check.id, command: check.command, success: result.status === 0 && !result.error, exit_code: result.status };
  if (check.report) {
    try {
  ...
  writeFileSync(`artifacts/authentication-proof/${lane}-receipt.json`, `${JSON.stringify(receipt, null, 2)}\n`);
