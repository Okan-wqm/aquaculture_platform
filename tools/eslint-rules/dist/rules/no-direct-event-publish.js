...
exports.default = createRule({
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'Direct calls to eventBus.publish() / natsClient.publish() outside @platform/outbox bypass the transactional-outbox guarantee (ADR-006, DATA-HIGH-004).',
        },
        schema: [],
        messages: {
            directEventBusPublish: 'Direct eventBus.publish() call outside @platform/outbox bypasses the transactional-outbox guarantee (ADR-006, DATA-HIGH-004). Write to the outbox entity within the same transaction; the outbox worker publishes to NATS in a separate step with at-least-once semantics. Reference: platform/libs/outbox/src/outbox-worker.service.ts.',
            directNatsClientPublish: 'Direct natsClient / natsConnection .publish() call outside @platform/outbox is a CRITICAL bypass of the outbox pipeline. The at-most-once delivery path this creates desynchronises the write-model from the event stream on any network blip. Route through @platform/outbox (OutboxEntityBase subclass + outbox worker).',
        },
    },
    defaultOptions: [],
    create(context) {
        const filename = context.getFilename();
        if (OUTBOX_EXEMPT_PREFIX.test(filename))
            return {};
        if (TEST_FILE_PATTERNS.some((re) => re.test(filename)))
            return {};
        return {
            CallExpression(node) {
                ...
                if (property.type !== 'Identifier' || property.name !== 'publish')
                    return;
...
//# sourceMappingURL=no-direct-event-publish.js.map