...
import { generateServiceIdentityHeadersV2, parseServiceIdentityKeyring } from '../utils/service-identity.util';
...
export async function signedFetch(
  ...
  void _s; void _t; void _sec; void _kid; void _aud; void _q; void _ct; void _ah; void _et; void _nonce; void _h;
...
function resolveServiceIdentitySigningMaterial(
  ...
  if (secretOverride) {
    if (isProduction) {
      throw new Error('[signed-http-client] secret overrides are forbidden in production; use SERVICE_IDENTITY_KEYRING');
    }
  ...
  if (keyring.length > 0) {
    ...
    if (isProduction && !configuredKid) {
      throw new Error('[signed-http-client] SERVICE_IDENTITY_SIGNING_KID is required in production');
    }
    const selected = configuredKid
      ? keyring.find((entry) => entry.kid === configuredKid)
      : keyring.find((entry) => entry.status === 'active') ?? keyring[0];
    if (!selected || selected.status === 'disabled') {
      throw new Error('[signed-http-client] SERVICE_IDENTITY_SIGNING_KID does not resolve to an enabled key');
    }
    return { keyId: selected.kid, secret: selected.secret };
  }

  const devSecret = process.env['SERVICE_IDENTITY_SIGNING_SECRET'] ?? process.env['INTERNAL_SERVICE_SECRET'];
