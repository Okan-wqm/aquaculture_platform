...
## ORPHAN-HIGH-217 — production deploy health-gate crashes on `ERR_MODULE_NOT_FOUND: js-yaml`: deploy checkout never provisions node_modules → false `critical_health` + `rollback_failed` on every deploy
...
Status: RESOLVED (2026-06-29; fix branch `fix/deploy-health-gate-js-yaml`). Registry: orphan-findings.md only.

---

## ORPHAN-MEDIUM-218 — admin-api Scheduler logs `permission denied for schema messaging` every ~5 min (admin_service lacks messaging-schema grant)
