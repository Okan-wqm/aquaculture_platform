...
export class FeederCalibration {
  ...
  @Column({ type: 'decimal', precision: 5, scale: 2, name: 'feed_size_mm' })
  feedSizeMm: number;

  @Column({ type: 'varchar', length: 100, nullable: true, name: 'feed_size_label' })
  feedSizeLabel: string;

  @Column({ type: 'decimal', precision: 8, scale: 2, name: 'grams_per_dispensing' })
  gramsPerDispensing: number;

  @Column({ type: 'decimal', precision: 8, scale: 2, name: 'silo_capacity_kg' })
