...
if (mode === 'commit-msg') {
  ...
  if (!/^(?:Merge |Revert |(?:fix|feat|refactor|security|test|chore)(?:\([^)]+\))?!?: )/.test(subject)) {
  ...
  if (/^(fix|security|refactor\(agentic,phase-|feat)/.test(subject)) {
    ...
    if (trailers.length === 0) fail('this commit requires a Closes: review-path#finding-id trailer');
    for (const [, review, finding] of trailers) {
      if (review.includes('..') || !/^(docs\/reviews\/|aria-findings\/|aria-debts\/)/.test(review) || !existsSync(review)) {
      ...
      if (review.startsWith('docs/reviews/')) {
        ...
        const registered = registry.split('\n').filter(Boolean).some((line) => JSON.parse(line).id === finding);
  ...
  for (const file of files) {
    if (/(^|\/)\.env(?:$|\.(?!example$|template$|sample$))/.test(file) || /(?:^|\/)(?:id_rsa|id_ed25519)$/.test(file)) {
      fail(`secret-bearing file must not be committed: ${file}`);
    }
  }
} else if (mode === 'pre-push' || mode === 'post-merge') {
  process.stdout.write('Local metadata checks only; required hosted-validation must pass before merge or deploy.\n');
} else {
