...
## EDGE-HIGH-008 — bundle apply phase is not all-or-nothing on a runtime apply fault — OPEN (design decision pending)
**Evidence:** the verify/stage phase is genuinely pure (a bad checksum/signature/tenant-binding fails staging and applies nothing — SENSOR-HIGH-005's "broken checksum applies nothing" holds), but once staged the apply loop (`bundle_deploy.rs:359-413`) writes each artifact and, on the first `Err`, returns `{ phase: "failed", stage: "apply", applied* }` WITHOUT restoring the already-applied artifacts (`bundle_deploy.rs:396` comment: "instead of pretending atomicity"). SENSOR-HIGH-005 is RESOLVED with a note claiming "atomic apply", so that claim is overstated for the apply-phase-fault case, and the Faz-5 plan's "FAILED → hiçbir şey uygulanmaz" is not met when a fault hits mid-apply.
**Rule:** a release bundle billed as an atomic edge apply must be all-or-nothing — after a mid-apply fault the device is rolled back to its pre-bundle state or driven to a declared safe state; honest partial-apply reporting is not atomicity.
**Resolution options (design-laden, tracked pending decision):** (A) best-effort rollback restoring each already-applied artifact's prior version from the edge's versioned store, with a distinct `rolled_back` vs `failed` ack when rollback itself faults; (B) drive the device to a declared safe state on partial apply; or (C) accept honest-partial-apply as the documented design and correct SENSOR-HIGH-005's atomicity claim + the Faz-5 plan wording. Owner: edge-expert. Deadline: 2026-07-22.
