...
export default createRule<Options, MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        'Operator/tenant-controlled outbound HTTP must go through SsrfValidatorService.safeFetch (IP-pinned); bare global fetch() re-opens the DNS-rebinding window (SENSOR-CRITICAL-002).',
    },
    schema: [],
    messages: {
      bareFetchInSensitiveZone:
        'Bare global fetch() on a non-literal URL in an SSRF-sensitive file re-opens the DNS-rebinding window that safeFetch closes. Route operator/tenant-controlled requests through SsrfValidatorService.safeFetch — it validates the host and pins the connection to the validated IP. SENSOR-CRITICAL-002.',
      removedGetSafeFetchOptions:
        'getSafeFetchOptions() was removed — it only set redirect:"error" and did NOT pin the resolved IP. Use SsrfValidatorService.safeFetch, which validates the host and pins the connection to the validated IP. SENSOR-CRITICAL-002.',
    },
  },
  defaultOptions: [],
  create(context) {
