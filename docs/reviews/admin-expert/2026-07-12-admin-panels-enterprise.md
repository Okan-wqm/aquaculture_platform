...
this review does not reuse 003; see the ID-collision note below for the final numbering.

**ID-collision note (2026-09-04 re-derivation):** this review was written on a branch that
rotted before merge. While it sat, `main` allocated six of its ids to DIFFERENT findings
(`ADMIN-HIGH-004/005/006/007`, `ADMIN-MEDIUM-001/002` — pagination-result authority, SQL
identifier interpolation, the compliance-page React-child crash, the controller-guard root
context, the tenant integration harness, the email circuit-breaker specs). To keep one id
meaning exactly one finding, this document's six were renumbered above each domain's
high-water mark: HIGH 004→016, 005→017, 006→018, 007→019; MEDIUM 001→020, 002→021. Every
other id in this document is unchanged and is the id carried in `findings.jsonl`.

---

## Implemented in this cycle

## ADMIN-HIGH-016 — tenant-admin specs never run in CI (missing nx `test` target)
...
## ADMIN-HIGH-017 — admin-panel baseline spec red (Turkish placeholder drift)
...
- web/modules/admin-panel/src/pages/**tests**/TenantManagementPage.spec.tsx:196
- web/modules/admin-panel/src/pages/**tests**/TenantManagementPage.spec.tsx:208
...
## ADMIN-HIGH-018 — dead non-tenant-scoped query-key factory in tenant-admin
...
## ADMIN-HIGH-019 — admin-panel dead parallel react-query/GraphQL layer with undeclared dependency
...
enforcement. The retired admin-api `TenantSecurityConfig` shape still _synthesizes_ defaults
...
class as ADMIN-HIGH-016/017 and SENSOR-MEDIUM-023.
...
## ADMIN-MEDIUM-028 — CRIT-04 deviation: 5 hooks bypass the lib/api.ts SSoT
...
## ADMIN-MEDIUM-021 — native confirm()/alert() in 9 admin-panel pages
...
## ADMIN-MEDIUM-022 — no working toast system; useToast is per-component state
...
## ADMIN-MEDIUM-023 — local MetricCard duplicate in BillingDashboardPage
...
## ADMIN-MEDIUM-024 — performance dashboard placeholder despite fetched trend data
...
## ADMIN-MEDIUM-026 — Turkish UI-string leaks across both panels
...
already exists to host bulk operations. (Per-user role _change_ already exists via
...
Found during the ADMIN-HIGH-017 spec repair: the old spec tested a "quick view" flow the page
...
## ADMIN-LOW-025 — AdminLayout decorative chrome and hardcoded docs link
...
(`tenant-api.service.ts:148`) removed after the ADMIN-MEDIUM-028 hook migration.
...
## ADMIN-LOW-027 — vapor "Appearance" section in tenant settings
...
Discovered while closing ADMIN-MEDIUM-026: ASCII-transliterated Turkish with no special characters
