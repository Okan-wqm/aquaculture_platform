  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
      <div className="flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 px-3 py-1.5">
        <span className="drag-handle flex-1 cursor-move truncate text-sm font-medium text-gray-800 dark:text-gray-200" title={card.title}>
          {card.title}
        </span>
        <select
          value={card.chartType}
          onChange={(e) => onChange({ chartType: e.target.value as ChartType })}
          className="rounded border border-gray-300 dark:border-gray-600 px-1 py-0.5 text-xs"
