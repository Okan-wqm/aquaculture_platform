...
describe('INVARIANT: /security/audit reads immutable audit logs', () => {
  it('uses AuditLogService for list, summary, and entity history reads', () => {
    ...
    // The canonical envelope, not a service-local near-copy. APA-283 deleted
    // `PaginatedAuditLogs` (and four siblings) — each was
    // IStandardPaginatedResult minus hasNextPage/hasPreviousPage, understating
    // what its own producer already returned.
    expect(controller).toContain('IStandardPaginatedResult<AuditLog>');
    expect(controller).not.toContain('PaginatedAuditLogs');
...
/**
 * APA-232 / APA-233 — the compliance surface must consume the shape the
 * backend actually persists and sends.
 *
 ...
 *
 * @see docs/reviews/claude/2026-07-20-admin-panel-e2e-audit/findings/security.md#APA-232
 */
describe('INVARIANT: compliance checks consume the canonical ComplianceCheckResult', () => {
  ...
  it('reads report findings through the render-safety guard, like its sibling branch', () => {
    ...
    // complianceResults branch did not, which is the whole of APA-233.
