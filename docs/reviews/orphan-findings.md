...
**3. config `configurations`/`effectiveConfiguration` engine (INFRA-HIGH-001) — SUSPECT, needs dedicated verification (NOT concluded here).** The finding says "write-only, no consumer", but config-service HAS a read surface (`configuration/configuration.resolver.ts` + `effective-configuration.dto.ts`) and multiple apps reference config-service/effective config (admin-api, observability, gateway, farm harvest listener) — several may be federation/registration string-matches rather than real consumers. Like `alert_incidents`, the "no consumer" claim is at least partially contradicted by the code and must be adjudicated call-by-call. Deferred to a dedicated pass because config-service is an active concurrent-work + erasure-onboarding surface (collision risk this session).
