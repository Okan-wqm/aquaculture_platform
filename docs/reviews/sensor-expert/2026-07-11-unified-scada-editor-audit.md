...
### [SENSOR-HIGH-056] Deleting a SCADA package never removes it from devices: deployed HMIs keep rendering, alarming and actuating from a package the tenant believes deleted

- **File:** `apps/sensor-service/src/process/services/scada-package.service.ts`, `apps/sensor-service/src/process/entities/scada-deploy-log.entity.ts`, `apps/sensor-service/src/process/services/scada-deploy-log.service.ts`, `apps/sensor-service/src/database/migrations/1806200000000-AddUndeployScadaDeployStatuses.ts`, `apps/sensor-service/src/process/resolvers/process.resolver.ts`, `apps/sensor-service/src/ingestion/mqtt-listener.service.ts`, `libs/sensor-contracts/src/schemas/undeploy-scada-package.schema.ts`, `sens-api-gateway/src/{scada_db.rs,scada_server.rs,commands/{system.rs,dispatch_lifecycle.rs,catalog.rs}}`
- **Category:** Workflow / lifecycle completeness (WF-011)
- **Description:** `deleteScadaPackage` was a pure DB archive. Devices the package had been deployed to were never told: the local HMI kept rendering screens, the alarm engine kept evaluating its rules, and control widgets kept actuating hardware — from a package the tenant believed deleted. The edge even had a suitable primitive (`clear_package`, built for bundle rollback) that NO command could reach — and it left the SQLite `is_active` row set, so even a hypothetical clear would resurrect the package on the next agent restart (startup reloads the active package from SQLite).
- **Impact:** Physical screens and actuation surfaces survived deletion indefinitely; the cloud had no record that they did.
- **Recommendation (landed):** A full `undeploy_scada_package` command contract, rolled out contracts→edge→cloud. **Contracts:** `UNDEPLOY_SCADA_PACKAGE_PARAMS_SCHEMA` (`{packageId (UUID), reason?}`, `additionalProperties:false`) + validator + shared fixture wired into both parity legs. **Edge:** `ScadaDb::deactivate_package()`; `clear_package` now ALSO deactivates the SQLite active row (fixing the restart-resurrection hole for bundle rollback too — history rows stay, append-only); `cmd_undeploy_scada_package` reuses `clear_package` under the deploy lock; dispatch arm + catalog `entry!` with the SAME `DeployProgram` permission class / ProgramDeploy audit taxonomy / legacy policy as its deploy counterpart (EDGE-HIGH-003 discipline) + `MUTATING_WIRE_NAMES`. **Cloud:** delete derives target devices from the deploy logs (LATEST row per device; FAILED/ROLLED_BACK/UNDEPLOY_SENT/UNDEPLOYED skip), publishes a contract-validated envelope per device with its own commandId, logs `UNDEPLOY_SENT` rows (new enum values via a type-presence-guarded, statement-autocommit `ALTER TYPE ADD VALUE` migration — blue-green safe, no-op down), and archives REGARDLESS (offline/no-MQTT/publish-throw become per-device result notes; a failure on device A never stops device B; archived re-delete is an idempotent no-op; absent optional deps degrade to archive-only). The resolver reports per-device outcomes honestly; the MQTT listener settles acks to `UNDEPLOYED`/`FAILED`. Archived packages remain un-redeployable (`assertPackageDeployable`), so best-effort cannot regress into a live redeploy.

### [SENSOR-LOW-010] validatePagination treats an explicit limit of 0 as "unset", defeating its own minimum clamp

- **File:** `apps/sensor-service/src/sensor/validation/input-sanitizer.ts`
- **Category:** Input validation (baseline test triage)
- **Description:** `validatePagination` computed `Math.max(1, Math.floor(limit || 20))` — the `||` coerces an explicit `0` into the default `20`, so the `Math.max(1, …)` floor is unreachable dead code. The sibling `validateLimit` implements the intended semantics correctly (null-check then clamp), and the unit test (`validatePagination(1, 0) → limit 1`) encoded the same intent; the function shipped disagreeing with both.
- **Impact:** Minor — an explicit `limit: 0` silently became 20 instead of clamping to 1; the suite carrying the correct expectation was red from birth.
- **Recommendation (landed):** Mirror `validateLimit`'s pattern with a NaN guard: absent/NaN → default 20; an EXPLICIT value clamps into 1..100. `page || 1` deliberately untouched (default equals the min clamp — no observable defect).

### [SENSOR-MEDIUM-022] QUICK_STOP and COAST_STOP are advertised VFD command types with no dispatch case — clients sending them get "Unknown command"

- **File:** `apps/sensor-service/src/vfd/services/vfd-command.service.ts`, `apps/sensor-service/src/vfd/entities/vfd.enums.ts`
- **Category:** Command dispatch completeness (baseline test triage)
- **Description:** The `VfdCommandType` enum (GraphQL-exposed) advertises 11 executable commands; the service switch handled 9. `QUICK_STOP` and `COAST_STOP` fell into `default → "Unknown command"` although the brand configs carry wire values for both (Danfoss `QUICK_STOP: 0x042f`/`COAST: 0x0437`, ABB/Schneider `QUICK_STOP: 0x0002`, Mitsubishi `COAST_STOP: 0x0200`) and the CiA402 constants exist (`QUICK_STOP 0x0002`, `DISABLE_VOLTAGE 0x0000`).
- **Impact:** Two documented drive commands were unusable end to end; the failing baseline test for QUICK_STOP encoded the correct expectation.
- **Recommendation (landed):** `executeQuickStop` (brand `QUICK_STOP` || CiA402 `0x0002`) and `executeCoastStop` (brand `COAST` || `COAST_STOP` || `0x0000` OFF2) cloned from the `executeStop` shape; `executeEmergencyStop` untouched (sharing the quick-stop wire value is deliberate — the distinct command types preserve operator intent in results/audit). The `default` arm is now a compile-time `never` guard, so a future enum member without a dispatch case is a type error instead of a runtime "Unknown command".

### [SENSOR-HIGH-057] SET_FREQUENCY passes ANY numeric value to the physical drive when the speed-reference mapping carries no bounds

- **File:** `apps/sensor-service/src/vfd/services/vfd-command.service.ts`, `apps/sensor-service/src/vfd/entities/vfd-register-mapping.entity.ts`
- **Category:** Physical-actuation safety (baseline test triage)
- **Description:** Frequency range validation only fired when `speedRefMapping.minValue/maxValue` were set — but those columns are nullable, so an unbounded mapping let 600 Hz or −10 Hz reach the wire unvalidated (the adapters are pure transport; no other layer checks). Devices carry no frequency fields, so nothing else could catch it.
- **Impact:** An out-of-envelope frequency write could reach a physical drive; the two failing baseline validation tests encoded the correct expectation.
- **Recommendation (landed):** The service stays the single choke point: non-finite values rejected; mapping-configured bounds win when present (Rockwell's signed −500..500 stays legitimate); an UNBOUNDED mapping now falls back to a conservative absolute envelope of 0..400 Hz (the widest Hz bound in the shipped brand register data — Danfoss and Yaskawa both cap at 400 Hz; min 0 because SET_FREQUENCY is a magnitude, direction is REVERSE). Throws surface to callers as `{success:false, error}` per the service's swallow-into-result contract.

### [SENSOR-MEDIUM-023] Seven sensor-service baseline suites shipped red from birth: 25 stale/broken tests masked the CI signal and hid three real defects

- **File:** `apps/sensor-service/src/vfd/entities/__tests__/vfd.enums.spec.ts`, `apps/sensor-service/src/vfd/adapters/__tests__/{base-vfd,vfd-modbus-tcp}.adapter.spec.ts`, `apps/sensor-service/src/vfd/services/__tests__/vfd-command.service.spec.ts`, `apps/sensor-service/src/sensor-type/__tests__/{channel-detection,sensor-type}.service.spec.ts`
- **Category:** Test-signal integrity (baseline triage; 28 failures total = 25 stale tests + SENSOR-LOW-010/MEDIUM-022/HIGH-057)
- **Description:** Source and tests landed together in one squash commit already disagreeing. The stale classes: (1) wrong literals — Siemens baud 19200 vs the deliberate 9600 factory default; a CRC16 expectation byte-swapped into wire order (`0xC5CD` vs the integer `0xCDC5`); an outdated `'host is required'` message. (2) A VESTIGIAL `modbus-serial` mock — the adapter speaks raw MBAP over `net` sockets, so 12 I/O tests attempted real TCP to an RFC-1918 address that the deliberate SSRF guard (SVD-HIGH-001) rightly denies with the opaque `'Connection failed'`; one test still asserted the leaky `'Connection refused'` the oracle-suppression design removed. (3) The command-service spec never migrated to the pooled-connection/result-object redesign (throw expectations, per-command disconnect assertions, a mock that never supplied `latencyMs`). (4) A shared module-level proposal fixture mutated by the service tripped the deliberate idempotency guard in the second test; two repo mocks were not array-aware for batch saves; two assertions counted `TenantScopedRepository.saveMany`'s internal `create` call.
- **Impact:** `apps/sensor-service` could never go green, so ANY new red test drowned in the baseline noise — the exact condition under which the three real defects above survived.
- **Recommendation (landed):** Literals corrected with WHY comments; the modbus-tcp spec rebuilt on a `FakeModbusSocket` (`jest.mock('net')` overriding ONLY `Socket`, real exports preserved) that answers the adapter's real MBAP frames, plus an `SsrfValidatorService.prototype.validateHost` spy for the guard (default allow; per-test deny for the opaque-error paths; one test exercises the REAL denylist unstubbed); the command-service spec migrated to the pooled/result-object contract (pool-reuse + closeConnection-release tests replace per-command disconnect; COAST_STOP happy path added); per-test `makeProposal()` factory; array-aware `create`/`save` mocks; batch assertions target persisted `save` args instead of repository internals. Suite state: 58/58 suites, 869/869 tests green.
