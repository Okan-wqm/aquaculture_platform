...
const CalibrationHistoryRenderer: React.FC<WidgetRendererProps> = ({
  config,
  width,
  height,
  isEditing,
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        {rows.slice(0, visibleCount).map((row: any, i: number) => (
          ...
          >
            ...
            <div style={{ width: colW, color: colors.neutral[700], fontFamily: 'monospace' }}>
              {row.offset}
            </div>
            <div
              style={{
                width: colW,
                color: row.result === 'Success' ? colors.success[600] : colors.error[600],
                fontWeight: 500,
              }}
            >
              {row.result}
