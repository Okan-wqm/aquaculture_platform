...
describe('WebAuthnService security ceremony (AUDIT-HIGH-009)', () => {
  ...
  const credRepo = { findOne: jest.fn(), count: jest.fn(), find: jest.fn(), create: jest.fn(), save: jest.fn() };
  ...
  beforeEach(async () => {
    ...
    const redis = {
      ...
      set: jest.fn((k: string, v: string) => { store.set(k, v); return Promise.resolve(); }),
      del: jest.fn((k: string) => { store.delete(k); return Promise.resolve(); }),
      delete: jest.fn((k: string) => { store.delete(k); return Promise.resolve(); }),
    };
  ...
  describe('generateRegistrationChallenge', () => {
    it('rejects an unknown user', async () => {
      ...
      await expect(service.generateRegistrationChallenge('nobody')).rejects.toThrow('User not found');
    });
    ...
    it('enforces the per-user credential cap', async () => {
      ...
      await expect(service.generateRegistrationChallenge('u1')).rejects.toThrow(BadRequestException);
    });
