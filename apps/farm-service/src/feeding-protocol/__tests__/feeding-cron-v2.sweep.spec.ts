...
function makeHarness(fixture: SweepFixture) {
  ...
  const outboxPublisher = stub<OutboxPublisher>({
    // `OutboxPublisher.enqueue` resolves to void — the double declares that
    // return type instead of casting its way out of one.
    enqueue: jest.fn(async (event: { eventType: string; unitCode?: string }): Promise<void> => {
      enqueued.push(event);
    }),
  });
    ...
    const harness = makeHarness({
      ...
      dayPlans: [
        stub<FeedingDayPlan>({
          ...
          snapshot: stub<DayPlanSnapshot>({ expectedFcr: 1.5 }),
          resolution: stub<DayPlanResolution>({ expectedFcr: 1.5 }),
        }),
      ],
