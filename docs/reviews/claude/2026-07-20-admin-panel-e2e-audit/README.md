...
- Findings total: **383** — with root-cause+fix design: **85**, REFUTED: **1**, PENDING (verification + fix design queued in staged continuation): **297**
- Current severity distribution (verified where available): **CRITICAL: 26**, **HIGH: 114**, **MEDIUM: 159**, **LOW: 83**
...
| APA-005 | MEDIUM | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | Service Status card can never show 'unhealthy' for remote services — a dead service renders as 'degr |
| APA-006 | MEDIUM | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | Total backend failure renders as an all-zero dashboard with no error message |
| APA-007 | MEDIUM | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | Cache card and 'Clear Cache' button are unreachable dead UI; backing /debug endpoints are 404 by def |
| APA-008 | MEDIUM | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | Circuit Breakers panel shows only the SMTP breaker — the platform's real cross-service breakers are  |
| APA-009 | MEDIUM | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | GET /system/services/health probes 12 services sequentially with 3s timeouts — worst case ~36s respo |
| APA-010 | LOW | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | 'System Resources' card shows only admin-api-service's own process stats, labeled as system-wide |
| APA-011 | LOW | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | 'Logins (Last 24h)' counts users with a recent lastLoginAt, not login events |
| APA-012 | LOW | ⏳ pending | [dashboard](findings/dashboard.md) | AdminDashboard | AbortController is never wired to fetch — in-flight requests survive unmount and stale-abort |
...
| APA-014 | LOW | ⏳ pending | [dashboard](findings/dashboard.md) | (cross-cutting) | Frontend double-submit CSRF is a no-op: admin-api never issues nor validates the XSRF-TOKEN cookie |
| APA-015 | LOW | ⏳ pending | [dashboard](findings/dashboard.md) | (cross-cutting) | Auth/envelope/routing chain verified sound for the dashboard (informational) |
...
| APA-018 | MEDIUM | ⏳ pending | [tenants](findings/tenants.md) | TenantManagementPage | Trial badge and Last Activity column render fields the list DTO never carries |
| APA-019 | MEDIUM | ⏳ pending | [tenants](findings/tenants.md) | TenantManagementPage | Stats cards stay stale up to 1 hour after lifecycle actions despite FE cache-busting |
| APA-020 | LOW | ⏳ pending | [tenants](findings/tenants.md) | TenantManagementPage | Per-row suspend/activate UI is unreachable dead code on the list page |
| APA-021 | LOW | ⏳ pending | [tenants](findings/tenants.md) | TenantManagementPage | tenants API surface exposes endpoints that cannot work |
...
| APA-023 | MEDIUM | ⏳ pending | [tenants](findings/tenants.md) | CreateTenantPage | wait_for_onboarding_ack has no pending/requeue semantics even if the transport were wired |
| APA-024 | LOW | ⏳ pending | [tenants](findings/tenants.md) | CreateTenantPage | 'begin_provisioning' step is executed but missing from the seeded step catalog |
| APA-025 | LOW | ⏳ pending | [tenants](findings/tenants.md) | CreateTenantPage | Success screen shows the locally computed price, not the provisioned subscription's amount |
...
| APA-028 | MEDIUM | ⏳ pending | [tenants](findings/tenants.md) | TenantDetailPage | Overview renders lastActivityAt which was removed from the backend contract — always '-' |
| APA-029 | LOW | ⏳ pending | [tenants](findings/tenants.md) | TenantDetailPage | Note update/delete ownership violations surface as HTTP 500, and user-stats errors are silently zero |
...
| APA-031 | MEDIUM | ⏳ pending | [tenants](findings/tenants.md) | (cross-cutting) | Hand-written FE tenant types drift from backend DTOs in both directions (no codegen) |
...
| APA-078 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | BillingDashboardPage | Five of nine dashboard metrics are hardcoded null and render permanent N/A |
| APA-079 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | BillingDashboardPage | Export Report button and Revenue Trend chart are dead UI |
| APA-080 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | BillingDashboardPage | Recent Transactions swallows all errors and shows empty state on 500 |
| APA-081 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | BillingDashboardPage | Invoice statuses 'sent'/'draft'/'partially_paid'/'overdue' all misrender as 'failed' transactions |
...
| APA-083 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | InvoicesPage | No pagination — list hard-capped at 100 invoices with no offset controls |
| APA-084 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | InvoicesPage | Search fires a full list+stats refetch on every keystroke with no debounce |
| APA-085 | LOW | ⏳ pending | [billing-core](findings/billing-core.md) | InvoicesPage | Void offered for partially_paid invoices but backend always rejects it |
| APA-086 | LOW | ⏳ pending | [billing-core](findings/billing-core.md) | InvoicesPage | MarkInvoicePaidDto accepts amount 0, creating a $0 payment that flips status to partially_paid |
| APA-087 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | PaymentsPage | Invoice-ID filter throws 500 on every keystroke of a non-UUID value |
| APA-088 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | PaymentsPage | Refund history is never returned — payment detail modal's Refund History section is permanently dead |
| APA-089 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | PaymentsPage | Stat cards (Succeeded/Refunded/Net) computed from the current 50-row page while Total Payments is th |
| APA-090 | LOW | ⏳ pending | [billing-core](findings/billing-core.md) | PaymentsPage | FE PaymentOverview type omits invoiceNumber; page reads it through an inline cast |
...
| APA-092 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | BillingReportsPage | Refund count additionally capped at 100 rows and export is summary-only |
...
| APA-094 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | (cross-cutting) | Financial mutation endpoints use interface-typed @Body DTOs, silently bypassing the global Validatio |
| APA-095 | MEDIUM | ⏳ pending | [billing-core](findings/billing-core.md) | (cross-cutting) | Multi-currency amounts are summed into single totals and always rendered as USD |
| APA-096 | LOW | ⏳ pending | [billing-core](findings/billing-core.md) | (cross-cutting) | FE double-submit CSRF token is decorative — admin-api never sets or verifies XSRF-TOKEN |
| APA-097 | LOW | ⏳ pending | [billing-core](findings/billing-core.md) | (cross-cutting) | Positive verification: auth, routing and envelope chain are sound for this section |
...
| APA-100 | MEDIUM | ⏳ pending | [billing-plans](findings/billing-plans.md) | SubscriptionManagementPage | Cancel leaves status 'active' — UI appears to have failed |
| APA-101 | LOW | ⏳ pending | [billing-plans](findings/billing-plans.md) | SubscriptionManagementPage | Search refetches subscriptions+stats on every keystroke |
...
| APA-103 | MEDIUM | ⏳ pending | [billing-plans](findings/billing-plans.md) | PlanManagementPage | POST/PUT /billing/plans bodies are completely unvalidated |
| APA-104 | LOW | ⏳ pending | [billing-plans](findings/billing-plans.md) | PlanManagementPage | FE PlanLimits type drifts from backend PlanLimits |
| APA-105 | LOW | ⏳ pending | [billing-plans](findings/billing-plans.md) | PlanManagementPage | console.error used (banned by repo lint rules) |
...
| APA-107 | MEDIUM | ⏳ pending | [billing-plans](findings/billing-plans.md) | DiscountCodePage | Generate->Create silently mutates the code (underscore stripped) |
| APA-108 | MEDIUM | ⏳ pending | [billing-plans](findings/billing-plans.md) | DiscountCodePage | applyDiscount redemption counting is not atomic |
| APA-109 | LOW | ⏳ pending | [billing-plans](findings/billing-plans.md) | DiscountCodePage | List capped at 50 codes with no pagination UI |
| APA-110 | LOW | ⏳ pending | [billing-plans](findings/billing-plans.md) | DiscountCodePage | getDiscountRedemptions FE contract mismatch (unused endpoint) |
| APA-111 | MEDIUM | ⏳ pending | [billing-plans](findings/billing-plans.md) | ModulePricingPage | pricingMetrics accepted with only @IsArray — no element validation |
...
| APA-174 | LOW | ⏳ pending | [messaging-monitoring](findings/messaging-monitoring.md) | MessagingRetentionPage | Hardcoded 'Next cleanup: 02:00 UTC' chip |
| APA-175 | LOW | ⏳ pending | [messaging-monitoring](findings/messaging-monitoring.md) | MessagingAiDashboardPage | Static claims about backend infrastructure are unverifiable and can silently rot |
...
| APA-177 | MEDIUM | ⏳ pending | [messaging-monitoring](findings/messaging-monitoring.md) | MessagingAiPersonasPage | Mandatory tenant UUID input has zero effect — backend registry ignores tenantId |
| APA-178 | LOW | ⏳ pending | [messaging-monitoring](findings/messaging-monitoring.md) | MessagingAiPersonasPage | '+ Add Custom Persona' button opens a future-release placeholder |
...
| APA-182 | MEDIUM | ⏳ pending | [messaging-monitoring](findings/messaging-monitoring.md) | (cross-cutting) | Hand-written FE types systematically expect enrichment (tenantName/userName/channelName) that no lay |
| APA-183 | MEDIUM | ⏳ pending | [messaging-monitoring](findings/messaging-monitoring.md) | (cross-cutting) | Documented DB-level dual-approver CHECK constraint is missing from the active migration ledger |
| APA-184 | LOW | ⏳ pending | [messaging-monitoring](findings/messaging-monitoring.md) | (cross-cutting) | Auth/guard chain and schema discipline are correct across the section |
...
| APA-189 | MEDIUM | ⏳ pending | [support](findings/support.md) | TicketsPage | Ticket numbers generated from an in-memory counter: guaranteed unique-constraint collision after ser |
| APA-190 | MEDIUM | ⏳ pending | [support](findings/support.md) | TicketsPage | Internal notes and system status-change comments count as SLA 'first response' |
| APA-191 | MEDIUM | ⏳ pending | [support](findings/support.md) | TicketsPage | FE contract drift on stats/by-category, stats/by-priority, sla-risk and satisfaction endpoints |
| APA-192 | MEDIUM | ⏳ pending | [support](findings/support.md) | TicketsPage | All mutation failures swallowed with console.error; no user feedback or rollback |
| APA-193 | LOW | ⏳ pending | [support](findings/support.md) | TicketsPage | commentCount hardcoded to 0 in the ticket list |
...
| APA-198 | MEDIUM | ⏳ pending | [support](findings/support.md) | MessagingPage | Thread list tenant name always 'Unknown Tenant' |
| APA-199 | MEDIUM | ⏳ pending | [support](findings/support.md) | MessagingPage | New Conversation takes free-text Tenant ID: non-UUID input 500s, no tenant existence check |
| APA-200 | LOW | ⏳ pending | [support](findings/support.md) | MessagingPage | New thread double-counts the first message as unread (unreadTenantCount = 2) |
...
| APA-205 | MEDIUM | ⏳ pending | [support](findings/support.md) | AnnouncementsPage | Publish/cancel/delete failures are silent |
| APA-206 | LOW | ⏳ pending | [support](findings/support.md) | AnnouncementsPage | FE AnnouncementType includes 'success' which the backend does not support |
...
| APA-208 | HIGH | ⏳ pending | [support](findings/support.md) | OnboardingPage | Field drift blanks the core renders: 'undefined% complete', empty step names, dead Needs-Attention f |
| APA-209 | HIGH | ⏳ pending | [support](findings/support.md) | OnboardingPage | Stats header shows 'Total Tenants: NaN' — backend has no 'stalled' bucket |
| APA-210 | MEDIUM | ⏳ pending | [support](findings/support.md) | OnboardingPage | Training resources and step tutorial links are hardcoded with dead URLs |
| APA-211 | MEDIUM | ⏳ pending | [support](findings/support.md) | OnboardingPage | sendWelcomeEmail returns fake success: no email is ever sent but welcomeEmailSent is persisted true |
| APA-212 | LOW | ⏳ pending | [support](findings/support.md) | OnboardingPage | Skipping a required step throws a plain Error -> 500 instead of 400 |
...
| APA-216 | MEDIUM | ⏳ pending | [support](findings/support.md) | (cross-cutting) | Guard posture verified sound; schema/migration parity verified sound (no finding — audit confirmatio |
...
| APA-224 | HIGH | ⏳ pending | [security](findings/security.md) | AuditTrailPage | Action filter can never match: lowercase FE values vs UPPERCASE backend enum |
| APA-225 | HIGH | ⏳ pending | [security](findings/security.md) | AuditTrailPage | Alert Rules tab is decorative: in-memory rules, evaluation never runs, channels are stubs |
...
| APA-231 | HIGH | ⏳ pending | [security](findings/security.md) | CompliancePage | Typing in the search box 400s the whole data-request list |
...
| APA-246 | HIGH | ⏳ pending | [security](findings/security.md) | (cross-cutting) | Security alerting/notification layer is stubbed end-to-end |
...
| APA-251 | CRITICAL | ⏳ pending | [xc-routing-nav](findings/xc-routing-nav.md) | (cross-cutting) | Feature-toggle switch (FeatureTogglesPage primary action) calls a route that does not exist on the b |
| APA-252 | HIGH | ⏳ pending | [xc-routing-nav](findings/xc-routing-nav.md) | (cross-cutting) | docker-compose.prod.yml stack routes /api/ to gateway-api, which has no admin proxy — entire admin p |
...
| APA-262 | HIGH | ⏳ pending | [system-mgmt](findings/system-mgmt.md) | FeatureTogglesPage | Feature toggles are persisted but consumed by nothing - no gating code exists anywhere |
...
| APA-265 | CRITICAL | ⏳ pending | [system-mgmt](findings/system-mgmt.md) | MaintenancePage | Maintenance list is always empty: FE expects a bare array but receives {items,total} |
| APA-266 | CRITICAL | ⏳ pending | [system-mgmt](findings/system-mgmt.md) | MaintenancePage | Schedule Maintenance always 400s: payload includes createdBy which CreateMaintenanceDto does not whi |
...
| APA-275 | CRITICAL | ⏳ pending | [system-mgmt](findings/system-mgmt.md) | ErrorTrackingPage | Page crashes on any successful load once data exists: reads .data from an {items,total} response |
| APA-276 | HIGH | ⏳ pending | [system-mgmt](findings/system-mgmt.md) | ErrorTrackingPage | Acknowledge action 404s (route mismatch) and Resolve action 400s (non-whitelisted body field); both  |
...
| APA-279 | CRITICAL | ⏳ pending | [system-mgmt](findings/system-mgmt.md) | JobQueuePage | Jobs list is always empty: getJobs reads response.data from an {items,total} payload and the guard s |
...
| APA-288 | CRITICAL | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | DB append-only trigger on impersonation_sessions makes every session-lifecycle mutation fail (end/te |
| APA-289 | CRITICAL | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | Impersonation access chain is not wired end-to-end: the issued token is discarded and nothing can co |
| APA-290 | HIGH | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | Revoke Permission always 404s: FE sends the permission row id where the route requires superAdminId |
...
| APA-295 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | End/Extend failures are swallowed silently, and the operator-override Terminate flow is unreachable  |
| APA-296 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | Grant form drift: duration input allows 15-480 min vs backend @Max(60), and 'Allowed Actions' checkb |
| APA-297 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | Stats mislabeled and aggregates truncated to first 20 sessions (no pagination) |
| APA-298 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | GET /impersonation/permissions?tenantId=... 500s: ANY() applied to a jsonb column |
| APA-299 | LOW | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | targetTenantName is client-supplied and stored unverified into the session/audit record |
| APA-300 | LOW | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | ImpersonationPage | Top-admins emails render 'Unknown' going forward (H-08 removed email from JWT) |
...
| APA-302 | HIGH | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | DebugToolsPage | Cache management is a placebo even when enabled: all invalidation paths are logger no-ops, and the F |
| APA-303 | HIGH | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | DebugToolsPage | Cache entries/stats are DB snapshots that nothing ever writes, with a fabricated hit-rate formula —  |
| APA-304 | HIGH | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | DebugToolsPage | 'Active Connections' table is fabricated in the FE: hardcoded database/user/application/state values |
| APA-305 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | DebugToolsPage | Log Viewer and Config Viewer tabs are client-side TODO stubs (NOT_WIRED features shipped in the UI) |
| APA-306 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | DebugToolsPage | SQL Query Executor is a facade: warns about executing on the production database, then always throws |
| APA-307 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | DebugToolsPage | Route-declaration-order shadowing in DebugToolsController makes two endpoints unreachable |
| APA-308 | LOW | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | DebugToolsPage | Cache key is URL-decoded twice (FE encode -> Express decode -> controller decodeURIComponent) |
...
| APA-311 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | (cross-cutting) | Hand-written FE types with no codegen have drifted to the point of conceptual inversion |
| APA-312 | MEDIUM | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | (cross-cutting) | Backend feature availability (env flags, nginx blocks) is invisible to the admin-panel: pages ship f |
| APA-313 | LOW | ⏳ pending | [impersonation-debug](findings/impersonation-debug.md) | (cross-cutting) | NestJS static-segment routes declared after parameterized siblings are shadowed — recurring pattern  |
...
| APA-317 | HIGH | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Suspend/Activate schema buttons silently do nothing — backend always throws 409 and FE has no .catch |
| APA-318 | HIGH | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Slow Queries panel can never display data — backend returns a SlowQueryResult object, FE expects an  |
| APA-319 | HIGH | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Database Health 'Slow Queries' check uses an inverted time filter — counts everything EXCEPT the las |
...
| APA-321 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Backup Schedule card always shows 'Not configured' + suspended — response shape drift |
| APA-322 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Point-in-Time Recovery modal is not wired — inputs are dead and the API is never called |
| APA-323 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Migrations tab is vestigial — registry is permanently empty and all run/rollback/batch endpoints unc |
| APA-324 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Index Recommendations render an empty SQL block — backend has no createStatement field |
| APA-325 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | Schemas tab stats may be stale/zero — tenant_schemas sizeBytes/tableCount are ledger columns with no |
| APA-326 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseManagementPage.tsx | FE api layer declares ~14 endpoints that do not exist on the backend (404 if ever used) plus hand-wr |
...
| APA-330 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseExplorerPage.tsx | insert/update responses return the raw RETURNING * row unmasked |
| APA-331 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseExplorerPage.tsx | Write UI (New Row / Edit / Delete) is always rendered but always 403 in production |
| APA-332 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseExplorerPage.tsx | Raw SQL endpoint returns rows unmasked and its catalog blocklist misses unqualified references |
| APA-333 | MEDIUM | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | DatabaseExplorerPage.tsx | Personal PII is not masked — masking covers credentials only, and export pulls up to 10K rows |
...
| APA-336 | HIGH | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | (cross-cutting) | Global ResponseInterceptor has no StreamableFile/binary bypass — every file-download endpoint in adm |
| APA-337 | HIGH | ⏳ pending | [database-mgmt](findings/database-mgmt.md) | (cross-cutting) | Admin-panel 'Backups' feature is a paper capability in production — runtime dependencies absent from |
...
| APA-340 | CRITICAL | ⏳ pending | [settings-email-audit](findings/settings-email-audit.md) | SystemSettingsPage.tsx | Email, Security, Rate-Limit and Maintenance settings persist but nothing enforces or consumes them ( |
...
| APA-356 | HIGH | ⏳ pending | [settings-email-audit](findings/settings-email-audit.md) | AuditLogPage.tsx | Every filter on the audit page returns 400 — @Query() DTO + forbidNonWhitelisted rejects the filter  |
| APA-357 | HIGH | ⏳ pending | [settings-email-audit](findings/settings-email-audit.md) | AuditLogPage.tsx | Export always fails (limit=10000 > @Max(100)); even if accepted it would silently truncate to 100 ro |
| APA-358 | HIGH | ⏳ pending | [settings-email-audit](findings/settings-email-audit.md) | AuditLogPage.tsx | Severity vocabulary drift: FE uses low/medium/high/critical, backend enum is info/warning/critical |
...
| APA-376 | HIGH | ⏳ pending | [xc-db-parity](findings/xc-db-parity.md) | (cross-cutting) | Auth-schema RBAC + invitation DDL is owned by admin-api's migration chain (ownership inversion, depl |
