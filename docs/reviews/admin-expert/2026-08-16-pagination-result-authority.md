...
## ADMIN-HIGH-004 - every tier declares its own paginated result shape
...
`platform/libs/pagination-contracts` alone — a versioned result authority
with a single constructor. `expectedTotalPages` fixes the zero-row
disagreement in one place: `Math.max(1, Math.ceil(total / limit))`, so an
empty page is page 1 of 1 rather than page 1 of 0.

Nothing imports it. That is not a convenience, it is the boundary: the
authority's `items` is `readonly`, so the moment
`libs/backend-common/src/pagination` is rebuilt on it, `readonly`
propagates into every existing consumer. farm-service resolvers already
return the old shape into GraphQL response types that declare mutable
arrays, and `nx build farm-service` fails with twelve TS4104/TS2322 errors
across chemical, department, equipment, farm, feed, growth, species,
supplier, system and tank — plus batch, where the whole result type stops
being assignable.

The NestJS bridge is therefore **not** an inert step. It belongs with the
migration it breaks, not with the lib.

### What remains, and the gate that will close it

The `libs/backend-common/src/pagination` bridge plus the producer migration
across the four consumer roots, landing together with
`tests/invariants/admin-pagination-ssot.spec.ts`. That invariant scans every
production file under those roots and fails on a local result-shape
declaration or a hand-written result object, so it can only pass once the
migration is complete — which is why neither it nor the bridge is in the
first commit.
