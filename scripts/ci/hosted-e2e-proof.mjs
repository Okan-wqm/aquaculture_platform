...
if (
  ...
  process.env.HOSTED_E2E_ISOLATED !== 'true'
) {
  throw new Error('Browser proof requires the isolated hosted E2E stack');
}
const head = process.env.HEAD_SHA;
if (
  ![head, process.env.BASE_SHA, process.env.PR_HEAD_SHA].every((sha) =>
    /^[0-9a-f]{40}$/.test(sha),
  ) ||
  execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim() !== head
)
  throw new Error('E2E candidate identity mismatch');
const lanes = [
  {
    name: 'security',
    ...
    files: [
      ...
      'mfa-login.spec.ts',
      'header-spoofing.spec.ts',
      'graphql-limits.spec.ts',
      'rate-limiting.spec.ts',
      'rbac-escalation.spec.ts',
      'tenant-isolation.spec.ts',
    ],
  },
  {
    name: 'mobile',
    config: 'playwright.aquamobil.config.ts',
    project: 'aquamobil-mobile',
    files: [
      'login.spec.ts',
      'alerts-ack.spec.ts',
      'messaging-smoke.spec.ts',
      'ai-action-confirm.spec.ts',
      'record-forms.spec.ts',
      'offline-sync-roundtrip.spec.ts',
    ],
  },
];
...
const receipt = {
  ...
  images_receipt_sha256: createHash('sha256').update(imageReceipt).digest('hex'),
  suites: [],
};
let failed = false;
for (const lane of lanes) {
  const actualFiles = readdirSync(`e2e/tests/${lane.name}`)
    .filter((file) => file.endsWith('.spec.ts'))
    .sort();
  if (JSON.stringify(actualFiles) !== JSON.stringify([...lane.files].sort()))
    throw new Error(`E2E inventory differs from ${lane.name} source suites`);
  for (const file of lane.files) {
    const reset = spawnSync('node', ['scripts/ci/hosted-e2e-stack.mjs', 'reset-rate-limits'], {
      stdio: 'inherit',
    });
