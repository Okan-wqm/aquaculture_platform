...
**Owner:** Okan
**Related ADRs:** ADR-008 (guard strategy defense-in-depth), ADR-011 (schema ownership), ADR-025 (Rust sidecar architecture), SEC-M16 (tenant-scoped cache key)
**Related plans:** `/root/.claude/plans/snappy-sniffing-pine.md` Kör Nokta 1

---

## Context (WHY)

NestJS services enforce tenant isolation at three layers:

a. **Schema-per-tenant** (`tenant_<hex>` schema derived from `TenantId`).
b. **`getScopedRepository()`** in TypeORM — every transaction issues `SET LOCAL app.current_tenant = $1`.
c. **RLS policies** (Row Level Security) that filter by `current_setting('app.current_tenant')::uuid`.

CLAUDE.md elevates this to "defense-in-depth" — a single-layer bypass does not breach isolation.

`apps/sensor-ingestion/src/persistence.rs` uses `tokio-postgres` directly: `COPY ... FROM STDIN BINARY` → `batch_execute(upsert_sql + truncate_sql)`. **No `SET LOCAL app.current_tenant` is issued**. Today isolation is preserved **only by the schema name** (layer a alone). A future regression — a `SchemaName::from_raw(user_input)` backdoor, a parser bug that mis-derives the schema, or a cross-schema query by a future control-plane feature — would not be caught by layer (b) or (c) because neither is active on the Rust path.

The audit matched every assumption to file:line and the gap is verified. The fix must be architectural (CLAUDE.md); runtime checks in application code are bypassable by definition.
...
1. `crates/tenant-context/src/scoped_tx.rs` exposes `ScopedTx<'t>` — a wrapper around `tokio_postgres::Transaction` that:
   ...
   - Exposes the query API to callers; never re-exports the raw `Transaction`.
...
- Migration V017 is bidirectional but requires the DB role to own `sensor.sensor_metrics`. Deploy runbook must verify this.
- Test suites that write to `sensor.sensor_metrics` without the wrapper (legacy fixtures) will fail after V017 — must be converted to `ScopedTx`.

**Neutral:**
- `current_setting('app.current_tenant', true)::uuid` with `missing_ok=true` returns NULL when unset; policy `USING (tenant_id = NULL)` evaluates to NULL → row excluded. INSERT without `SET` returns `new row violates RLS policy` — consistent fail-closed behaviour both sidecar and tests can rely on.
...
- Schema drift validator (ADR-012) gains an RLS-policy-presence check — boot fails if V017 is missing in an expected environment.
