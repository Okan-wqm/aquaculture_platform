...
import { installSourceSchemaWriteGuards } from './source-schema-write-guards.helper';
...
export class SourceSchemaWriteGuardService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    assertRuntimeDdlAllowed({
      serviceName: 'source-schema-write-guard',
      operation: 'source schema write guard trigger install',
    });
  ...
  async installWriteGuards(): Promise<void> {
    ...
    await installSourceSchemaWriteGuards(this.dataSource, {
      sourceSchema,
      logger: {
  ...
  async disableGuards(sourceSchema: string, tables: string[]): Promise<void> {
    assertRuntimeDdlAllowed({
      serviceName: 'source-schema-write-guard',
      operation: 'source schema write guard trigger disable',
    });
  ...
  async enableGuards(sourceSchema: string, tables: string[]): Promise<void> {
    assertRuntimeDdlAllowed({
      serviceName: 'source-schema-write-guard',
      operation: 'source schema write guard trigger enable',
    });
