...
export function createAliasLimitPlugin(): ApolloServerPlugin {
  ...
  return {
    // Apollo plugin: requestDidStart returns Promise<listener>; the listener's
    // didResolveOperation returns Promise<void>. We use Promise.resolve(...)
    // to satisfy the contract without an empty `async` (require-await).
    requestDidStart() {
