...
mod tests {
    ...
    async fn confirm_active_returns_503_when_cell_empty() {
        ...
        let resp = confirm_active_handler(Extension(cell), Extension(HealthBindIsLoopback(true)), axum::http::HeaderMap::new())
            .await
            .into_response();
    ...
    async fn confirm_active_with_auth_rejects_wrong_hmac() {
        ...
        let resp = confirm_active_handler(Extension(cell), Extension(HealthBindIsLoopback(true)), headers)
            .await
            .into_response();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED);
    ...
    async fn keyless_confirm_active_fails_closed_on_non_loopback_bind() {
        ...
        let total = health
            ...
            .map(|l| l.ends_with(" 1"))
            .unwrap_or(false);
        assert!(total, "keyless external refusal must bump the rejected metric");
