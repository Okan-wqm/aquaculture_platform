...
export class Baseline1800000000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "observability"."schema_object_type_enum" AS ENUM('table', 'column', 'index', 'constraint', 'enum', 'policy'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "observability"."schema_object_action_enum" AS ENUM('created', 'altered', 'dropped', 'renamed'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;`);
        ...
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "observability"."migration_event_type_enum" AS ENUM('start', 'applied', 'failed', 'skipped', 'validator_clean', 'validator_warn', 'validator_error'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;`);
        ...
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "observability"."emergency_override_kind_enum" AS ENUM('drift_fatal_bypass', 'migration_skip', 'validator_disable'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;`);
