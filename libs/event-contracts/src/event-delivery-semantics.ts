 ...
 *   `event_dlq` shelf before terminating it. Registering `DeadLetterModule` is
 ...
 * must therefore own an `event_dlq` shelf). Unknown types default to `false`:
...
export function requiresDurableDelivery(eventType: string): boolean {
  return (
    FARM_SIGNAL_DELIVERY_SEMANTICS[eventType as ConsumedFarmSignalEventType] === 'one_shot'
  );
}
