...
- Add grouped comment block: _"OpenTelemetry coupled-release family — bump all four crates atomically OR none."_
...
- If yes: drop `process` feature; comment update: _"Capability drop via libc::prctl direct FFI; nix::process feature not used."_
...
**Evidence:** Graceful-shutdown sequence step 4 claims _"Flush offline queue to disk (WAL checkpoint + fsync)"_. Actual code:
...
- Ultra-plan Batches #235 (C-2 finding board), #247 (C-4 STRIDE), #248 (C-5 supply chain SBOM + cosign + SLSA L3), #282-#288 (G-_ cross-repo platform), #289-#300 (F-_ E2E + release) remain as claimed — genuine net-new architectural work.
...
**Severity: LOW** — repo hygiene only. No correctness, security, or test-coverage implication. Documented per user policy: _gördüğüm hiçbir problemi senin ilgili olmasa bile not al_.
...
**Severity: LOW** — git hygiene only. No correctness, security, or test-coverage implication. Documented per user policy: _gördüğüm hiçbir problemi senin ilgili olmasa bile not al_.
...
- churn _and_ a correlated failing e2e test suite, OR
- churn _and_ a spike in open AUDIT-\* findings on the same files, OR
- churn _and_ a rise in lint-warning count for the service,
...
billing-service (added 2026-04-27 during PR #159 audit): 3. `apps/billing-service/src/billing/entities/subscription-module-item.entity.ts` — reachable only via parent `Subscription` (scoped by `subscriptionId` FK).
...
The scoped repo auto-injects `{ ..., tenantId }` into every save / update / create / delete. DeviceIoConfig has no `tenantId` column → the INSERT would fail at runtime with a `column "tenantId" does not exist` Postgres error. Wrapping with `tenantManagerRepo()` is therefore _structurally incorrect_ until the column exists.
...
- Skips test/**tests**/**mocks**/migrations
...
```typescript
export class AddTenantsCustomDomainPartialUnique1787300000000 implements MigrationInterface {
...
Severity: CRITICAL. Every postgres container restart with a non-empty PGDATA volume leaves the platform's schemas / roles / functions / shared._ tables in whatever state they were last manipulated, with no automatic recovery path. After any `DROP SCHEMA … CASCADE` operation (day-one reset, schema-recovery hand-run, blue-green migration testing) the postgres container's docker-entrypoint silently SKIPS `/docker-entrypoint-initdb.d/_.{sh,sql}` because PGDATA is detected as pre-initialized — the upstream Postgres image's documented contract is "init-scripts run ONCE on initdb, never again."
...
## ORPHAN-MEDIUM-089 — messaging-service /metrics serves only the domain registry; http*\*/nodejs*\* families absent
...
**Problem (root cause):** the gate measures whether a change introduces _new_ lint errors by linting each changed file at base and at head and diffing the per-rule counts. But it linted the two sides with DIFFERENT configs:
...
> **D2 / CRITICAL-002 traceability note (not a registry close):** the gateway rate-limit consolidation (PR #457, `0c2370b04`) — which also fixed the gateway's fail-OPEN Redis store — is fully traced by its own commit message + PR. It is NOT seeded as a closable registry finding because the three-store invariant requires a closing commit to carry a `Closes: …#<id>` trailer _at commit time_; a finding seeded post-merge cannot be closed against an already-merged commit (no amend/force-push). The SEC-MEDIUM-001/002 closes in this same registry pass ARE valid because D1's commit (`bc79457d5`) carried their `Closes:` trailers.
...
**Effect:** none today (the overlapping common classes are always present). The latent risk: a lib that introduces a class used _nowhere else_ (e.g. an unusual `bg-fuchsia-300`) would silently fail to render. Pre-existing — identical behavior under v3.
...
- **Deploy gate** (`release-verification` → `npm run type-check` → `tools/scripts/type-check-all.mjs`) type-checks each project's _canonical_ config — `tsconfig.app.json`/`tsconfig.lib.json`/`tsconfig.json`, first match. For a lib whose base `tsconfig.json` does `include: ["src"]` and has NO lib/app variant, that canonical config sweeps `src/__tests__/*.spec.ts` _without_ test types.
...
**Problem:** While collapsing the FIVE per-plan _limit_ catalogs into the canonical
`PLAN_CATALOG` SSoT (ADR-037, `libs/event-contracts/src/billing/plan-catalog.ts`),
a sibling drift remains: per-plan _feature_ booleans (`TenantFeatures`:
...
## ORPHAN-MEDIUM-185 — auth/gateway/sensor metrics served from bespoke @Controller('metrics') miss default http*/nodejs* series (089-siblings)
...
Verification: billing + gateway + service-catalog `tsc` 0; billing factory 5/5; gateway 969/969; registry/catalog invariant shard green incl. the new one. Regenerated provenance artifacts (apollo-router/_ + deploy/_ + federated-subgraphs.generated.ts) are mechanical hash re-pins (no topology/functional change).
...
Found 2026-06-28 (tenant-panel WS-5 gap audit vs main). equipment*types is cloned PER-TENANT (the entity omits `schema:`, MODULE_SCHEMAS clones it into each `tenant*<uuid>`), and the EquipmentTypeLookupService reads the per-tenant copy under tenant context — but the GraphQL `equipmentTypes`/`equipmentType`resolvers carried`@SkipTenantGuard` (`apps/farm-service/src/equipment/equipment.resolver.ts:162,176`) and the handler cached results in a process-wide Map keyed by the serialized FILTER ONLY (`get-equipment-types.handler.ts:47`), so the FIRST tenant's result was served to EVERY other tenant. Two read paths (GraphQL global vs lookup per-tenant) hit different physical tables. **Operator decision: per-tenant catalog. Status: RESOLVED (2026-06-28 — removed `@SkipTenantGuard`from both resolvers so they run tenant-scoped (search_path →`tenant\_<uuid>.equipment_types`, the same table the lookup service reads); DELETED the tenant-blind in-process cache (the leak's root cause — equipment_types is small reference data and React Query already caches it per-tenant client-side via a tenant-scoped query key); gated the FE `useEquipmentTypes`hook on`!!token && !!tenantId`. The `farm-service-tenant-isolation` invariant already allows equipmentType repo reads without a tenantId where-clause (search_path isolation), so the change is invariant-safe.)\*\* Note: that invariant's allowlist comment still calls equipmentType a "global catalogue" — semantically stale post-change (doc only). (Renumbered from ORPHAN-MEDIUM-196 at merge — main's 196 was concurrently taken by the deploy isolated-checkout finding.)
...
2. Delete the dead auth._ writer services above; audit for any other admin-api writers/locks of `auth._`.
...
## ORPHAN-HIGH-327 — main deploy pipeline blocked since #830: alert-engine **tests** support mocks leak into the app tsconfig and fail release-verification — IN-PROGRESS (this PR)
...
**Evidence:** `AlarmEngineService` (`apps/sensor-service/src/scada-runtime/services/alarm-engine.service.ts`) is an `@Injectable()` singleton with ONE 1 Hz eval loop, ONE `rules` set, and ONE bound `tenantId`; `ScriptEngineService` likewise binds a single tenant. After ORPHAN-CRITICAL-339 the persistence layer is tenant-safe and fail-closed, so no data can cross tenants — but the engine can only be _activated_ for one tenant per process at a time. Serving multiple concurrent tenants from one sensor-service process would require the second tenant's activation to overwrite the first's `setTenantId`/`setAlarmRules`.
...
**DB-ADMIN-HIGH-003 (suspension metadata written to transient fields) + HIGH-004 (admin dual-writes auth.tenants) — CONFIRMED; both are the DEFERRED auth.tenants-ownership work (P1-A4).** **RESOLVED by Faz 3 (feat/auth-tenants-ownership, owner-approved lane):** auth-service migration `1807100000000-AddTenantSuspensionAudit` adds the nullable trio; `transitionTenantStatus` persists it on →SUSPENDED and nulls on →ACTIVE (`command.actor.id` already in the command metadata — no contract change); all four admin dual-writes removed (NATS-reply → fresh re-read, synchronous contract kept); admin's fake transient props became real READ-ONLY columns; `lastActivityAt` removed (different semantics, never populated); NEW invariant `admin-no-auth-tenants-writes.spec` (erasure-handler allowlisted with tracked migration note). BONUS root-cause: the dual-write held a FOR-UPDATE row lock ACROSS the NATS request while the owner takes its own lock — a structural cross-service lock convoy; the new shape (unlocked pre-read → command → local txn) removes it. Validation: full invariants 1880/1880; auth+admin tsc clean; suites green incl. the repaired tenant.integration (ORPHAN-MEDIUM-372 item 1). `tenant.entity.ts:132-140` declares `suspendedAt/suspendedReason/suspendedBy/lastActivityAt` with NO `@Column` (commented "NOT in the database"); `suspend-tenant.handler` assigns them then `save(Tenant)` — TypeORM drops the unmapped props, so they never persist and always read `undefined`. The correct fix adds real columns on `auth.tenants` **routed through auth-service** (auth owns that table — admin's `Tenant` is a read model, and HIGH-004 is the same handler directly `save(Tenant)`-ing into `auth` schema = an SSoT fork) or hydrates from `tenant_activities`. This is precisely the auth/RBAC-adjacent A4 work the platform owner chose to DEFER in this hostile-environment session; not safe to do unilaterally here.
...
**Scope:** platform owner chose DROP over wire ([[ORPHAN-HIGH-356]] item 2; live DB: 0 rows). Remove the orphan DMS: `document/` domain dir (entity+module), farm*documents references in `common/file-cleanup/*` + scheduler cron wiring, `MODULE_SCHEMAS['farm']` registration if present, and a NEW drop migration (never edit `CreateFarmDocuments` — immutability) with a 0-row guard (ABORT if any environment has data) + tenant-fanout drop across tenant schemas.
**RESOLVED (this PR, lead-verified):** migration `1805300000000-DropFarmDocuments` — `current_schema()`-relative (no cross-schema DDL; avoids the #926 outage class — db-migrate fan-out delivers the farm pass + each tenant pass), guards: skip-if-absent, `RAISE EXCEPTION` if ANY rows (data-loss guard for other environments), `DROP TABLE IF EXISTS` with DESTRUCTIVE marker, per-type pg_depend zero-dependents probe for the three `farm_documents*\*`enums (live catalog showed they are per-schema-LOCAL — each tenant clone references its own copies),`postCondition()`refuses the ledger row unless the table is gone. **The exact shipped SQL was exercised in a throwaway pg16 container** (row-guard abort, farm-pass isolation, dependent-enum skip, idempotent replay, postCondition — all verified; live DB untouched). Code:`document/`dir + farm-document cleanup provider removed; file-cleanup framework intact (BatchDocument + Chemical providers live); cron tenant-discovery re-anchored;`MODULE_SCHEMAS['farm']` entry removed; tenant-isolation pins 86→85 / 183→182. Validation: full invariants 1876/1876; farm + backend-common tsc clean; file-cleanup/minio-orphan specs 9/9.
**Owner:** farm-expert. **Deadline:** closed by this PR's merge.

## ORPHAN-HIGH-370 — admin-panel P2 contract repairs: impersonation FE rename + tenant list tier/counts (DB-ADMIN-HIGH-001/005) — RESOLVED (this PR)
...
- HIGH-005: `ListTenantsHandler` maps to the existing `TenantListItemDto` with `tier` (from `plan`) and `farmCount`/`sensorCount` via ONE batched round-trip (single SQL across the page's tenant schemas with information_schema existence guards — the batched form of `countTenantResource`; no per-row N+1).
  **RESOLVED (this PR, lead-verified) — the lane found the surface was worse than audited:**
- Impersonation **start was fully broken in prod-shape** (the FE sent `adminId`-era fields; the backend `StartImpersonationDto` + `forbidNonWhitelisted` rejected the payload) and the session **list always rendered empty** (FE read `.data` where the backend returns `{items,total}`). Both fixed with the contract rename; a 7-value reason enum select added to match the DTO.
- **Four MORE token-leak read paths** the DB-ADMIN-HIGH-002 sweep missed: `endImpersonation`/`terminateSession`/`extendSession`/`getImpersonationStats.recentSessions` returned raw entities (plaintext `originalSessionToken` + token hash). All now return `toSafeImpersonationSession(...)`, pinned by the extended token-redaction spec.
- Tenant list: `PaginatedResult<TenantListItemDto>` with `tier` materialized as an own property (serialization proven via JSON round-trip spec) + farm/sensor counts in exactly 2 round-trips/page (information_schema existence probe + one UNION ALL count over existing schemas only; ids `isValidUUID`-checked → canonical `getTenantSchemaName` → `assertSafeSchemaName` before inlining; unprovisioned tenant → 0). Specs pin the batching (query called exactly 2×/1×/0×).
- Validation: admin-api tsc 0; admin-panel tsc 0; impersonation suites 124/124; list suites green.
  **Owner:** admin-expert. **Deadline:** closed by this PR's merge.

## ORPHAN-MEDIUM-372 — admin impersonation/tenant surfaces: four pre-existing contract defects surfaced by the Faz 4 lane (OPEN)

**Discovered:** 2026-07-12 by the Faz 4 lane while repairing DB-ADMIN-HIGH-001/005 (all verified pre-existing on the base; none introduced or fixed by that PR):

1. `tenant/__tests__/tenant.integration.spec.ts` fails on the base: `SuspendTenantHandler` requires `OutboxPublisher` which the spec's TestingModule never provides (28 tests) — the handler gained the dependency after the spec was written. Fix = provide the publisher mock (or a factory) in the TestingModule; belongs with the Faz 3 auth.tenants work that touches these handlers anyway.
2. `ImpersonationPermission` FE read model is invented (FE `tenantName`/`grantedByEmail`/`allowedActions`/`revokedAt` vs backend `allowedTenants`/`maxSessionDurationMinutes`/`isActive`), and FE `revokePermission` passes `permission.id` where the backend path param is `superAdminId` — the permissions half of DB-ADMIN-HIGH-001, untouched by the sessions repair.
...
## ORPHAN-HIGH-373 — Faz 2b: config-service WIRE — seed the legacy settings + first real consumer (INFRA-HIGH-001; owner decision: wire) — RESOLVED (this PR)
...
**RESOLVED (this PR, lead-verified):** seed migration `1805400000000-SeedPlatformConfigurations` (35 rows, SYSTEM-tenant, idempotent ON CONFLICT, tx-local RLS pin; secrets seeded EMPTY — no plaintext); admin-panel `SystemSettingsPage` reads `effectiveConfigurationsByService('platform')` + writes `setConfiguration` via the module's sanctioned `useAdminQuery/useAdminMutation` pattern (maintenance mode now actually persists; 13 dead 410 REST methods removed). FOUR mandatory config-service enablers landed with WHY+specs: SUPER_ADMIN (tenantless) scope→SYSTEM_TENANT_ID resolution; tx-local RLS-scoped read/upsert boundaries (`rls-scoped-session.ts` — pool-pinned GUC could never expose the SYSTEM fallback partition); upsert no longer downgrades value_type to string; empty-secret reads return value:null (unset vs stored distinguishable). Live proof: config subgraph reachable (ServiceIdentityGuard 400 — correct gating). Validation: config 29/29 jest, admin-panel new spec 10/10 (26 others pre-existing on base via stash), tsc ×3 clean, migration-lint clean.
**INCIDENT surfaced during live verification (separate from this lane):** farm-service crash-looping on `farm.farm_documents` drift → supergraph composition down. Root cause: the `f995d8e54` farm IMAGE contains stale compiled dist (git tree is correct — entity absent); healed by the next full rebuild/deploy. Tracked in [[ORPHAN-HIGH-377]].

## ORPHAN-MEDIUM-374 — hr GOAL surface: competencyRatings scalar selection + the stale-SDL false-positive corrections — RESOLVED (this PR)
...
**RESOLVED (this PR, lead-verified):** `competencyRatings` now sub-selects all six `CompetencyRating` fields; no other scalar-selected object lurks in the fragment; fresh supergraph + full codegen green (drift gate PASS, baseline 0); synthesis §D corrected (the "5 ops absent" half = FALSE POSITIVE from the stale Jun-19 SDL, all five live at `performance.resolver.ts:173/186/201/278/344`).

## ORPHAN-HIGH-375 — GraphQL FRAGMENT content is validated by NEITHER gate (structural hole) + two live undetected drifts — OPEN

**Discovered:** 2026-07-12 by the Faz 8a lane's ratchet-gap analysis (why the competencyRatings drift survived): `scripts/ci/validate-graphql-operations.mjs` only extracts documents containing `query|mutation|subscription` (fragment-only `gql` files like `fragments.ts` are never validated) and strips `${FRAGMENT}` interpolations assuming "codegen owns fragment wiring" — but `codegen.ts` (S1 scope note, lines 34-47) removed the module-documents output precisely because module drift aborted codegen. Net: fragment content across `web/modules`/`web/shell`/`web/shared-ui`/`mcp` has NO validating gate.
**Live proof (found via a scratch full-fragment validator; NOT yet fixed):** (1) `LeaveBalanceFull` selects `leaveType` where the hr subgraph serves `leaveTypeId` — invalidates 4 leave operations in `leave.operations.ts`; (2) `GetCurrentRotation` spreads `WorkRotationFull` on type `RotationDetail`. Both would 400 at the gateway.
**Fix direction:** fragment-aware validation (cross-file fragment registry in the validator) OR fix remaining module drift and restore a per-module codegen documents output. The two live drifts need their own repair commits (they touch consuming components).
**Owner:** frontend-expert (gate) + hr-expert (the two drifts). **Deadline:** tracked (2026-10-15).

## ORPHAN-MEDIUM-376 — two pre-existing auth/admin spec failures surfaced by the Faz 3 lane (OPEN)
...
**Owner:** infra-expert. **Deadline:** provenance guard tracked (2026-10-15); incident heal verified by the running watcher.
**RE-DIAGNOSIS (2026-07-13, recurrence forensics):** the "stale-dist image" theory was an artifact of the inspection method — `docker run <release-tag>` inspected the droplet's LOCAL tag, which the deploy rollback had re-pointed at the pre-#946 image (that image legitimately contains the entity). Registry digests prove each release was a distinct real build (f995→df2e981f, 388117baa→a086f762, 27dfb0aa9→6606e964, e8a67919b→24bc4d28), and direct inspection of the registry's e8a67919b image shows NO stale entity. The true defect is the droplet rollback mechanism — see [[ORPHAN-HIGH-381]]. The originally-proposed image-vs-tree provenance guard is NOT the fix (considered and rejected — the images were never wrong); the rollback migration-boundary guard is.

## ORPHAN-HIGH-378 — shared.user_permissions dead parallel permission catalog retired (A3)

**Discovered:** 2026-07-11 by the database E2E audit (Faz 8-A3 of the remediation plan). The platform carried TWO permission catalogs: the live tenant RBAC in auth-service (`auth.tenant_role_permissions.panel_permissions`, GraphQL-surfaced, consumed by tenant-admin FE and enforced by `@RequireTenantPermission` guards + token claims) AND `shared.user_permissions` — an admin-api-owned per-user JSONB checkbox catalog exposed via 5 REST endpoints (`POST /users/tenant/invite`, `GET /users/permission-categories`, `GET/PUT /users/:id/permissions`, `GET /users/tenant/users-with-permissions`).
**Evidence (read-only scout, 2026-07-11):** ZERO frontend callers of the 5 endpoints across web shell, all federated modules (incl. tenant-admin + admin-panel), aquamobil, and mcp; 1 live row on the production droplet (stale pre-RBAC artifact); no backend reader outside the admin-api users module (the "every service reads it" comment in `generate-init-schemas.ts` was aspirational — no guard or middleware ever selected from the table). Two writable authorities with one unread = split-brain by construction.
**Fix (this PR, ADR-042):** deleted the 5 REST handlers + `UserPermissions` entity + `UserPermissionsService` + `invite-user.dto.ts` DTOs + module wiring + spec (the SUPER_ADMIN `POST /users/invite` / `UserProvisioningService` path used by admin-panel is untouched); admin-api migration `1801500000000-DropRetiredUserPermissions` archives all rows into `admin.retired_config_backups` (RLS-bypassed, count-asserted, idempotent) then drops `shared.user_permissions` (+ any `public` remnant); canonical shared set shrunk 5→4 across the full W5 gate chain (PROTECTED_TABLES, generate-init-schemas SHARED_SCHEMA_TABLES, platform-bootstrap 006 SQL + service post-condition, schema-invariants B.3, critical-infra-ssot incl. a new negative recreate-lock, db-migrate specs) + ADR-011 amendment note + runbook/skill/knowledge/agent-roster list updates.
**Owner:** admin-expert + data-expert. **RESOLVED (this PR).**

## ORPHAN-MEDIUM-379 — farm_workers placeholder-PII columns dropped (A8 / DB-FARMPLAT-MEDIUM-001) — RESOLVED (this PR)

**Discovered:** 2026-07-11 by the Lane-D database E2E audit (db-audit-farm-platform). `farm.farm_workers` persisted five NOT NULL columns that were write-only placeholder sinks: `address`, `dateOfBirth`, `nationalId`, `employmentType`, `baseSalary`. Evidence: since the worker module's introduction (00139d810, v1.4.1) `CreateWorkerInput` accepted none of them; `create-worker.handler.ts` synthesized the same constants on every insert purely to satisfy NOT NULL (address `{'-','-','-','-','TR'}`, dateOfBirth `'1990-01-01'`, nationalId `'-'`, employmentType `'full_time'`, baseSalary `0`); `update-worker.handler.ts` never touched them; `worker.resolver.ts` never returned them; no query filtered on them. Three of the five were AES-256-GCM-encrypted fake PII — encryption theater over synthesized constants. Deep worker PII belongs to `hr.employees`, the platform's worker-PII SSoT.

**Keep-set (verified consumed, NOT dropped):** `contactInfo` — the audit's "write-only" claim is FALSE for this column: `worker.resolver.ts` reads `contactInfo.phone` in all three response mappings and `update-worker.handler.ts` merges user-supplied phone/email into it (real data, real read path). `department` — returned by the resolver, selected by `web/modules/farm-module/src/hooks/useWorkers.ts` GraphQL documents, and backs the `(tenantId, department)` index.

**RESOLVED (this PR):** entity fields + placeholder synthesis deleted; `DropFarmWorkerPlaceholderPii1805500000000` drops the five columns per tenant-fanout pass (current_schema()-relative, `DROP COLUMN IF EXISTS`, postCondition-guarded). No archive step: every value was a synthesized constant — nothing real to preserve (structural code evidence above; a data-level guard is impossible because the ciphertext columns cannot be inspected in SQL). Regression guard added in `create-worker.handler.spec.ts`.
**Owner:** farm-expert. **Deadline:** this PR.

## ORPHAN-MEDIUM-380 — ai.conversation_turns durable per-turn cost ledger added; cache-creation tokens now billed (DB-PEOPLE-MEDIUM-002) (RESOLVED — this PR)

**Discovered:** 2026-07-11 by the Lane-D database E2E audit (db-audit-people-messaging, DB-PEOPLE-MEDIUM-002). AI cost tracking had no durable per-invocation record: enforcement rode the ephemeral Redis counter `ai:tokens:{tenant}:{YYYY-MM}` (TokenBudgetService) and the mutable aggregate `agent_conversations.totalTokens`; the layer-1-ai knowledge SSoT specifies an immutable per-turn audit table `ai.conversation_turns`. Additionally, the budget accounting summed only input+output, leaving cache-CREATION tokens (billed ~1.25x input by the provider) entirely unbilled.
**Fix (this PR):** per-tenant `conversation_turns` table (entity `apps/ai-service/src/cost/conversation-turn.entity.ts`, migration `1802100000000-CreateConversationTurns`, MODULE_SCHEMAS['ai'].tables entry) + append-only `TurnLedgerService` writing one row per completed invocation via the tenant-scoped repository, with costUsd from the new `cost/model-pricing.ts` catalog (all four token classes billed); AgentRunnerService budget total now includes cacheCreation. Ledger failures log loudly and never fail the chat path; Redis stays the fast enforcement cache.
**Owner:** ai-service domain. **Status:** RESOLVED (this PR).

## ORPHAN-HIGH-381 — deploy rollback re-tags releases across destructive-migration boundaries and captures crash-looping containers as rollback points (self-sustaining prod outage) — RESOLVED (this PR)

**Discovered:** 2026-07-13 ~02:00Z while diagnosing the farm-service crash-loop recurrence (whole-GraphQL 503 for ~3h). Forensics on the droplet: EVERY local farm release tag (f995d8e540326, 27dfb0aa9, e8a67919b, latest, plus four `rollback-*` retags) pointed at ONE image ID `2f132659b842` — the pre-#946 image whose `farm_documents` entity can never boot against the forward-migrated DB — while the registry held distinct, healthy builds for those same tags (verified by digest and by direct dist inspection of e8a67919b = clean). Mechanism, from `scripts/deploy/droplet-up.sh`:

1. `capture_rollback_manifest` captures whatever containers are running — INCLUDING crash-looping ones — as "the previous good image" (no health gate).
2. `rollback_deployed_services` re-tags `<svc>:<release-sha>` to that captured image ID on any post-deploy gate failure — even when db-migrate just applied a destructive migration (#946 dropped `farm.farm_documents`), so the restored image is structurally incompatible with the DB and crash-loops.
3. The next deploy captures the crash-looping image as its rollback point → every subsequent gate failure restores the same broken image under the newest tag. Death spiral; release-verification failed for 31088c459 exactly this way.
   **Live recovery (user-approved, 2026-07-13 ~02:15Z):** re-tagged the registry-pulled clean image locally (`incident-clean-e8a67919b` = 24bc4d28) + compose override pin → farm HEALTHY (restarts=0); gateway restart → supergraph composed, `/health/ready` 200. Login smoke green.
   **Fix (this PR):** (1) `capture_rollback_manifest` refuses to capture non-running/unhealthy containers — a broken image can never become a restore target; (2) `rollback_deployed_services` refuses image rollback when this release applied ≥1 DB migration (count parsed from db-migrate's structured completion line; fail-closed on unparseable) — forward-only DB means forward-only images, fix-forward is the only safe recovery; (3) `workflow_dispatch`/`workflow_call` input `no_cache` on deploy-digitalocean.yml for operator-triggered clean rebuilds. NOT fixed here, left tracked: droplet-side GHCR pull auth expires outside deploy windows (`docker pull` → denied), which forced the local-retag recovery path; owner infra-expert, deadline 2026-10-15.
   **Owner:** infra-expert. **Deadline:** RESOLVED on merge of this PR.

## ORPHAN-HIGH-382 — LoRaWAN `CodecType::Custom` was a dead decoder slot; per-vendor payload formats had nowhere to run

**Discovered:** 2026-07-13 by the WASM-adoption research (Phase 3). `sens-api-gateway/src/lora/codec.rs` dispatches `CodecType::Custom { decoder_name }` — the config slot every unrecognised codec name resolves to (`src/lora/mod.rs`) — to a `warn!` + empty `Vec`, with a code comment earmarking it for "Lua veya WASM tabanlı" integration. Per-vendor LoRa payload formats are foreign, frequently-changing code; running them as native Rust would be a memory-safety and supply-chain risk on the edge device, so the slot stayed dead and custom-codec devices produced no telemetry.

**Resolution:** the `wasm-codec` feature (`src/lora/wasm_decoder.rs`) fills the arm with a sandboxed `wasmi` interpreter — each decoder runs in a fresh, per-invocation store with a fuel budget and a 1 MiB memory cap (the decode path is synchronous on the LoRa actor loop, so bounding is cooperative — a tokio timeout cannot preempt it). Fail-soft: any trap, fuel exhaustion, ABI violation or malformed output yields an empty result, matching the built-in codecs. Output is capped (≤32 records, ≤64-byte names) and NaN/Inf-filtered; the host prepends `tag_prefix`. Decoders load once from `lorawan.wasm_decoder_dir` (same trust level as the device YAML); the signed cloud→edge deploy path (mirroring the ST-bytecode command/registry/store machinery) is tracked separately.

**Note:** the branch commit that closed this finding (`feat(lora)` 5334a47a) referenced `ORPHAN-HIGH-378`, which independently landed on `main` (shared.user_permissions retirement) during concurrent development; this finding was renumbered to 382 on merge, and the pushed commit's trailer cannot be amended under the force-push ban (its SHA is allowlisted in `commit-msg-validator.ts`).

**Owner:** edge-expert. **Deadline:** signed-deploy follow-on tracked (2026-10-15). Feature is off by default (no wasmi symbols in the default fleet binary); vendor SDK + ABI contract at `sensorprotocols/wasm-decoder-template/`.

## ORPHAN-HIGH-383 — Rust and TS alarm engines silently drifted on the same decision math (epsilon, deadband boundary, hidden floor, delay truncation)

**Discovered:** 2026-07-13 by the WASM-adoption research (Phase 4). The edge alarm engine (`sens-api-gateway/src/alarm_engine.rs`) and the SCADA-runtime alarm engine (`apps/sensor-service/src/scada-runtime/services/alarm-engine.service.ts`) independently re-implemented the SAME decision predicates and had DRIFTED: `==`/`!=` compared with `f64::EPSILON` (effectively exact) on the edge vs `1e-4` in the runtime; deadband CLEAR boundaries were INCLUSIVE (`>=`/`<=`) on the edge vs EXCLUSIVE (`<`/`>`) in the runtime; the edge carried a hidden `.max(0.01)` deadband floor for `==`/`!=` the runtime never had; and the edge truncated the delay to whole seconds (`Instant::elapsed().as_secs()`) vs millisecond precision (`Date.now()`) in the runtime. Two engines, one spec, no shared code — drift by construction, with no test coverage on the Rust side.

**Fix (this PR):** a single pure Rust kernel `crates/alarm-core` (`evaluate_condition`, `is_outside_deadband`, `delay_elapsed`) is the drift-zero SSoT. The edge engine consumes it natively; the NestJS runtime consumes it via a WebAssembly build (`crates/alarm-core-wasm` → `@platform/alarm-core`, mirroring the Phase-2 `protocol-codec` toolchain). A shared fixture suite (`libs/sensor-contracts/fixtures/alarm/decision-core.json`) is asserted by BOTH a Rust golden test and a TS jest twin, so any future divergence fails in exactly one language. The alarm state machines (edge 2-state vs runtime 4-state + ack modes + actions) are deliberately NOT unified — only the stateless decision predicates are shared.

**Canonical semantics (deliberate, documented — the SCADA-runtime semantics are canonical, the richer 1 Hz engine):** `==`/`!=` use an explicit, caller-supplied epsilon (default `1e-4`); deadband clear boundaries are EXCLUSIVE (no chatter at the band edge); the hidden `.max(0.01)` floor is REMOVED (no silent minimum); `deadband == 0` clears immediately (no hysteresis); the delay gate is millisecond-precision with no integer-second truncation. The behaviour change lands ON THE EDGE (aligned to the runtime); the runtime's field behaviour is unchanged (it already matched the canon), now routed through the shared kernel.

**Owner:** sensor-expert + edge-expert. **Deadline:** RESOLVED (this PR); per-rule epsilon override is a trivial future extension (the kernel already takes epsilon as a parameter).

## ORPHAN-MEDIUM-382 — billing.tenant_usage_metrics dead parallel usage model retired; usage_aggregations declared SSoT (A6 / DB-IDENT-MEDIUM-002)

**Discovered:** 2026-07-11 by the Lane-D database E2E audit (db-audit-identity-billing, DB-IDENT-MEDIUM-002). billing carried TWO overlapping usage-metering models: (a) `billing.tenant_usage_metrics` (TenantUsageMetrics entity, per-module `ModuleUsageMetrics` jsonb, `includedQuantities`/`overageQuantities`) and (b) `billing.usage_aggregations` + `billing.usage_hourly_data` (metering module). Model (a) had NO writer anywhere in the codebase — `UsageMeteringService.recordUsage` feeds only the in-memory meter state + `usage.recorded` events, which `UsageAggregatorService.persistDirtyData()` upserts into model (b) — and 0 live rows on the production droplet. Every read surface of (a) therefore always rendered the zero-state: `GetTenantBillingHandler` (tenant-admin billing page usage cards + planLimits current\*) and admin-api's `GET /billing/usage/tenant/:tenantId/metrics` (readonly projection with zero frontend callers). Model (b) is the one invoices are calculated from (`MeteredBillingService.calculateBilling`) and the one the admin usage dashboard reads — two persisted authorities, one unread and unwritten = split-brain by construction.
**Fix (this PR):** `usage_aggregations`/`usage_hourly_data` declared the single persisted usage SSoT. `GetTenantBillingHandler` repointed to it — month-to-date sums + gauge levels via new DB-backed `UsageAggregatorService.getPersistedMonthUsage()` (hourly buckets, the granularity the live event path writes), included quantities from `MeteredBillingService.getPricingModel(planTier)` (the same source billing calculates against; the old `sub.limits.maxSensors` fallback conflated sensor COUNT with a readings/month quota, and `maxStorage` was a hardcoded 0). GraphQL contract shape unchanged — tenant-admin FE untouched. Admin-api's dead read surface deleted (endpoint + service method + `TenantUsageMetricsReadOnly` entity + module wiring; admin-panel never called it). Billing migration `1801700000000-DropRetiredTenantUsageMetrics` archives any rows into new `billing.retired_usage_metrics_backup` (jsonb, count-asserted, idempotent — same shape as admin's 1801400000000) then drops the table + its orphaned periodtype enum behind a no-dependents gate. `MODULE_SCHEMAS['billing']` updated both ways (table removed, backup ledger registered). Handler regression spec added.
**Residual (pre-existing, explicitly NOT widened by this PR):** the metering pipeline is wired but unfed — no domain producer calls `recordUsage` yet, and `performRollup`/`cleanupOldAggregations` have no scheduler despite `rollupConfigs[].aggregateOnSchedule: true` — so `usage_aggregations` also holds 0 live rows today. That is a missing-producer/scheduler gap in the ONE remaining model, not a second model; tracked as part of the metering activation work (BILLING-CRITICAL-003 Redis-backed redesign lineage). **Owner:** billing-expert. **Deadline:** 2026-09-30.
**Update (2026-07-13, Billing Revival Faz E):** the SCHEDULER half of the residual is now closed — see ORPHAN-MEDIUM-405. `performRollup` is driven by an hourly `@Cron` (`runScheduledRollups`) that reads `rollupConfigs[].aggregateOnSchedule`, and the persistence/cleanup timers moved from raw `setInterval` to Nest `@Interval`s. The scheduler is idempotent and no-ops until data exists. Usage-INGESTION (no domain producer calls `recordUsage`) remains the OPEN half — see ORPHAN-MEDIUM-405's ingestion carve-out; owner billing-expert, deadline 2026-10-15.
**Owner:** billing-expert + data-expert. **Status:** RESOLVED (this PR).

## ORPHAN-MEDIUM-383 — subscription pricing fork collapsed: billing is sole price authority (A5 / DB-IDENT-MEDIUM-003)

**Discovered:** 2026-07-11 by the Lane-D database E2E audit (db-audit-identity-billing, DB-IDENT-MEDIUM-003). Subscription pricing was forked: `auth.modules` carried a `price` column (plus `is_core`) whose entity comment claimed "Billing resolver sums module prices to compute total plan cost" — versus billing's own `billing.plans.basePrice` + `billing.subscription_module_items`. Platform rule D14 makes billing the subscription/pricing SSoT.
**Scout truth (read-only + live droplet verification, 2026-07-13):** the claimed "billing resolver" does not exist — NO total-cost computation reads `auth.modules.price` anywhere. The live per-module price catalog is `admin.module_pricing` (BASE_PRICE + usage metrics + tier multipliers; managed by admin-api `ModulePricingService`, consumed by `PricingCalculatorService` and the CreateTenantPage quote flow), and plan/subscription pricing lives in `billing.plans`/`billing.subscriptions`. `auth.modules.price` was NULL for ALL 6 production rows — a dead write-path fork: admin module CRUD (`admin-panel → admin-api → NATS → auth-admin-nats.handler`) could write a price that nothing ever charged, while the admin ModulesPage badge displayed that dead value instead of the real catalog price.
**RESOLVED (this PR):** `price` removed from the auth surface — `Module` entity field + GraphQL `@Field` (the Module ObjectType is schema-orphaned; published GraphQL surface unchanged, verified against `infrastructure/apollo-router/codegen-schema.generated.json`), `AuthModuleSnapshot`/`AdminCreateModuleCommand`/`AdminUpdateModuleCommand` contract fields, auth NATS module CRUD, admin-api pass-through DTOs, admin-panel API params; auth migration `1807200000000-DropModulePriceFromAuthModules` drops the column (all-NULL values archived in the migration docblock). Admin-api `ModuleDto.price` is repointed to the module-pricing catalog (currently-effective active `admin.module_pricing` row's `base_price` metric) so the module list shows the SAME price the quote flow charges; regression spec pins the source. **`is_core` intentionally KEPT in auth:** judged catalogue metadata (admin-UI classification flag: filter/badge/stats; consumed by zero pricing logic), not a price input.
**Owner:** data-expert + billing-expert. **Status:** RESOLVED (this PR).

## ORPHAN-HIGH-384 — billing-service module-metadata lookups are structurally unresolvable and tenant provisioning creates no subscriptions (0 rows live)

**Discovered:** 2026-07-13 during the A5 pricing-SSoT scout (this PR's read-only live verification; NOT fixed here — contract-level fix required).
**Defect 1 — unresolvable relation:** `apps/billing-service/src/billing/event-handlers/tenant-subscription-requested.handler.ts:388` and `apps/billing-service/src/billing/handlers/billing-admin-nats.handler.ts:717` both run `SELECT id, code, name FROM modules WHERE id = ANY($1)` with an UNQUALIFIED table name. billing-service pins `schema: 'billing'` and no `billing.modules` table exists (verified live: `SET search_path TO billing; SELECT … FROM modules` → `relation "modules" does not exist`). Schema-qualifying to `auth.modules` would ALSO fail: the `billing_service` role has NO grant on `auth.modules` (verified via `information_schema.table_privileges` — only `admin_service`/`auth_service`/owner roles hold SELECT). Consequence: the saga path (`reconcileSubscriptionModuleItems`, inside a SERIALIZABLE transaction) hard-fails whenever `moduleIds` is non-empty; the legacy event path swallows the error in a try/catch and silently creates ZERO `billing.subscription_module_items` (0 rows live).
**Defect 2 — pricing placeholders:** the legacy event handler prices module items at a hardcoded `$25` base (`tenant-subscription-requested.handler.ts:272,400`) and the saga path inserts `subtotal=0,total=0` line items — neither reads the module-pricing catalog, so subscription module items would misstate revenue even if the metadata lookup worked.
**Defect 3 — live orphan tenants:** production has 3 `auth.tenants` rows, 0 `billing.subscriptions`, 0 `billing.command_receipts`, empty retry queue — tenant provisioning is not producing billing state at all; every live tenant is subscription-orphaned.
**Root-cause direction:** billing must not read other services' schemas. Carry module catalogue metadata (`{moduleId, code, name}`) inside `BillingTenantProvisioningCommand` / `TenantSubscriptionRequestedEvent` (the admin-api sender already holds it), and price module items from billing-owned data (plan pricing or a quote payload computed by the pricing calculator and bound into the command), then re-verify end-to-end provisioning creates subscription + module items for a modules-enabled tenant.
**Owner:** billing-expert (+ admin-expert for the sender side). **Deadline:** 2026-08-15.

## ORPHAN-MEDIUM-385 — auth-service DI-resolution spec red on main: AuditModule does not provide DataSource

**Discovered:** 2026-07-13 during A5 verification (this PR ran the auth tenant-domain unit suites). `apps/auth-service/src/modules/tenant/handlers/__tests__/auth-user-query-nats.handler.spec.ts` › "resolves via NestJS DI with the AuditModule-provided AuditLogService" fails with `Nest can't resolve dependencies of the AuditLogService (AuditLogRepository, ConfigService, ?) — DataSource at index [2] is missing in AuditModule`. Verified PRE-EXISTING: fails identically on a pristine `origin/main` working tree (git stash → run → pop), 1 failed / 205 passed across the tenant-domain suites. The spec exists precisely to catch AuditModule wiring regressions in production DI — a red run means either the module genuinely under-provides `DataSource` for `AuditLogService` or the spec's testing-module imports drifted from the app graph; either way the guard is currently asserting nothing.
**Owner:** auth-security-expert. **Deadline:** 2026-08-15.

## ORPHAN-HIGH-386 — a THIRD live edge alarm engine bypassed the shared decision kernel, plus kernel boundary/edge-case gaps

**Discovered:** 2026-07-13 by an adversarial verification of the ORPHAN-HIGH-383 alarm-core kernel (three lenses: completeness / correctness / integration). The completeness lens caught a THIRD alarm engine that ORPHAN-HIGH-383 never touched: the IEC 62682 `AlarmManager` in `sens-api-gateway/src/alarms.rs`, which runs concurrently with the migrated `alarm_engine.rs` in the same edge crate (`AppState.alarm_manager`, invoked on every IO-poll tick via `AlarmManager::process_source`). Its `AlarmDefinition::is_triggered` (High/Low/Deviation) and `AlarmInstance::process_value` delay gate re-implemented the SAME condition/deadband/delay math by hand, and had drifted on the exact axis the kernel exists to kill: it cleared active High/Low alarms on the INCLUSIVE deadband boundary (`value > setpoint - deadband`) while the kernel is EXCLUSIVE. Not biting today only because the wired seeding path uses `deadband=0`/`delay=0`, but the `with_deadband`/`with_delay_ms` builders and the `Deviation` branch are live code that would drift the moment a non-zero band is configured. The correctness lens additionally found the kernel's `!=` deadband boundary was INCLUSIVE (`<=`), contradicting the module's own "boundaries are EXCLUSIVE" contract, and that NaN values, negative deadband, and the exact epsilon edge were unpinned by any fixture.

**Fix (this PR):** `alarms.rs::is_triggered` and the delay gate now route ALL value/threshold/time math through `alarm_core` (`evaluate_condition`, `is_outside_deadband`, `delay_elapsed`); the inline math is deleted. Only this engine's own concerns stay local (the `enabled` gate, trigger-vs-clear edge selection, the `Deviation` two-level hysteresis band, and the 7-state machine). The kernel's `!=` deadband boundary is corrected to EXCLUSIVE (`<`) so `==`/`!=` stay exact complements at the edge; NaN fail-safe behaviour, the non-negative-deadband contract (`debug_assert!`), and the epsilon/`!=` edges are pinned by new shared fixtures + Rust/TS unit tests. Also fixed a cluster of latent CI-blockers on the WASM-adoption branch that CI had not exercised (CI stopped triggering mid-branch, and the nx project-name collision below broke the graph so `nx affected` never ran the affected suites since Phase 2):

- the `crates/protocol-codec` nx project name collided with the new `libs/protocol-codec` TS lib (renamed the crate's nx project to `protocol-codec-rs`, matching the `event-contracts-rs` convention; cargo package name unchanged);
- a `f64 as u64` cast in the alarm-core fixture harness that `pedantic + -D warnings` rejects (reads the JSON integers as `u64` directly);
- the `alarm-core` path dependency in `sens-api-gateway/Cargo.toml` + `crates/alarm-core-wasm` carried no version, which the edge `deny.toml` `wildcards = "deny"` ban rejects — pinned to `version = "0.1.0"` (lock unchanged);
- `libs/alarm-core` + `libs/protocol-codec` were absent from ADR-028's lib inventory (registered both); the `libs/protocol-codec` façade had no local specs, failing its jest target (added a façade smoke test that cross-checks the CRC-16 trailer + typed error path);
- `wasmi` (Phase 3, behind the off-by-default `wasm-codec` feature) pulls the unmaintained `paste` crate, which cargo-audit `--deny warnings` flags (RUSTSEC-2024-0436) — added EDGE-CI-008 as a tracked ignore across all synced audit surfaces (`.cargo/audit.toml` + `deny.toml` + both workflow `--ignore` steps), matching the existing unmaintained-signal ignores;
- the `three-store` registry invariant hard-failed on finding P0-CRITICAL-001's deep Phase-0 closing-SHA (`32839e24`) being unreachable in a shallow CI checkout — made the SHA-existence check shallow-tolerant (skips a genuinely-unfetched deep SHA on a shallow clone, stays strict on a full clone);
- the `banned-construct` gate flagged `as unknown as` casts in the Phase-1 SCADA-sandbox specs (partial jest mocks injected into a constructor). Replaced them with a type-checked generic (`mockOf<T>(impl: DeepPartial<T>): T` — the "write a generic" escape CLAUDE.md sanctions) and, for the private-method reach, `Reflect.get(...) as ScriptValidator` — no double-assertions remain.

**Owner:** edge-expert + sensor-expert. **Deadline:** RESOLVED (this PR). The `alarms.rs` behaviour change (INCLUSIVE→EXCLUSIVE clear at the band edge) is edge-only and lands the same canonical alignment ORPHAN-HIGH-383 applied to `alarm_engine.rs`.

**Out of scope (flagged, not fixed here — different domains, confirmed by the verification):** `sens-api-gateway/src/scripting/triggers.rs` uses `f64::EPSILON` for `==`/`!=` (ADR-017 script-trigger engine, not alarms); `apps/alert-engine/src/alert/services/alert-evaluation.service.ts` compares equality with `===` on a scaled sensor float (NATS cloud alert pipeline, no deadband/delay — a latent never-matches bug worth its own finding); `apps/alert-engine/src/rules-engine/rule-evaluator.service.ts` is `@deprecated` dead code pending deletion.

## ORPHAN-MEDIUM-386 — stray tenant-schema migration journal (migrations_auth in tenant_7f6b08ab) + journal schema-qualification guard — RESOLVED (this PR)

**Discovered:** 2026-07-13, live-verified on the prod droplet: `tenant_7f6b08ab90e246d3.migrations_auth` — a TypeORM migration journal named for the AUTH source schema sitting inside a TENANT schema, 1 row (`Baseline1800000000000`). db-migrate warned on EVERY release: "Tenant schema contains tables registered by NO module — unknownTables:[migrations_auth]". Live enumeration confirmed the tenant's other 7 `migrations_<svc>` tables are the LEGITIMATE fan-out ledgers (farm/sensor/hr/messaging/alert/ai/hydroponics — exactly `TENANT_AWARE_SCHEMAS`); only the auth one is stray. `auth.migrations` (17 rows) is the authoritative auth journal.

**Root cause (forensic, git-verified):** NOT the migration runners' search*path — both runners have always schema-qualified the journal on the live paths (`libs/backend-common/src/database/migration-runner/migration-runner.service.ts` sets `migrationsSchema` + `driver.buildTableName(ledger, schema)` on the executor; `apps/db-migrate/src/migration-orchestrator.ts` pins `schema` on the per-run DataSource so TypeORM builds `"<schema>"."<ledger>"`; every raw ledger DML in both files is `"${schema}"."${table}"`-qualified). The creator was the RETIRED runtime seeding path: `SchemaManagerService.seedMigrationsHistory` (renamed to the `migrations*<src>`ledger naming in 87d1bb116, 2026-05-19) iterated EVERY module a tenant requested — including the platform-level`auth`module resolved from`auth.tenant*modules`by the era`tenant-provisioning.service.ts`— with no tenant-scoped filter, creating`tenant*….migrations_auth`and copying`auth.migrations` into it (exactly 1 row at that time: the post-baseline-reset ledger). That path is dead (`createTenantSchema`/`syncTenantSchema`refuse runtime DDL; provisioning is owned by aqua-db-migrate), and every CURRENT journal creator is gated on`TENANT_AWARE_SCHEMAS`(db-migrate backfill + fan-out, tenant-schema-provisioner, Nest runner Phase 2). The one remaining structural hole:`createMigrationRunnerService('<platform-schema>', { tenantAware: true })` was a documented manual opt-in that would re-mint stray journals in every tenant schema.

**Fix (this PR):**

1. **Make it impossible** — `createMigrationRunnerService` now throws at factory time when `tenantAware: true` is forced on a schema outside the `TENANT_AWARE_SCHEMAS` SSoT (docblock updated; opt-out stays legal). Spec pins the gate + the journal's explicit schema-qualification for both source and tenant phases against a schema-less driver (`migration-runner.factory.spec.ts`).
2. **Make it automatic (self-heal)** — new db-migrate phase (`apps/db-migrate/src/stray-tenant-journal-heal.ts`, wired before the tenant-privilege gate in `main.ts`) drops `migrations_<svc>` journals inside `tenant_*` schemas ONLY when (a) the name matches `^migrations_[a-z_]+$` in a `tenant_<uuid16>` schema, (b) the source schema is NOT tenant-aware (fan-out ledgers are never touched), and (c) the owning source schema still has its authoritative `migrations` journal. Idempotent; logs every dropped journal with its destroyed entries; unattributable candidates are kept + warned (fail-closed). Non-fatal by contract. London-school spec: `stray-tenant-journal-heal.spec.ts`.

**Owner:** data-expert. **Status:** RESOLVED (this PR — heal removes the live stray on the next deploy).

## ORPHAN-HIGH-387 — SchemaVersionGate hand-copied shared-table literal crash-looped every backend service after ADR-042's drop (prod outage) — RESOLVED (this PR)

**Discovered:** 2026-07-13 ~05:25Z. After the eed7b368d deploy applied ADR-042's `shared.user_permissions` drop, db-migrate honestly recorded `shared_table_count=4` in `platform.bootstrap_signal` — but `schema-version-gate.service.ts:296` carried a hand-copied `EXPECTED_SHARED_TABLE_COUNT = 5`, so EVERY service running the gate refused boot ("partial bootstrap") and crash-looped (auth 24 restarts, ai 35, admin-api 27, farm 16). The A3 retirement swept every textual `user_permissions` reference; a bare numeric literal is invisible to that sweep — the exact Potemkin-copy class the shared-schema-canonical invariant exists to prevent, but the gate file was not one of its enforced sources.
**Fix (this PR):** the expectation is now DERIVED from `PROTECTED_TABLES`' `shared.*` prefix count (runtime canonical copy, parity-enforced against generate-init-schemas + 006 SQL); shared-schema-canonical.spec gains an assertion that the gate contains no numeric literal and references PROTECTED_TABLES.
**Recovery:** fix-forward release (rollback correctly refused by the ORPHAN-381 guard — 8 migrations applied; restoring old images was structurally impossible and undesirable).
**Owner:** platform-kernel-expert. **Deadline:** RESOLVED on merge.

## ORPHAN-HIGH-388 — messaging-partition privilege recipe was hand-mirrored across the TS provisioner helper and Stage 010 SQL — RESOLVED (#920)

Severity: HIGH (recurrence enabler for the 2026-07-07 messaging outage class). #919 fixed the runtime-DML-grant bug in BOTH mirrored copies of the recipe — `grantTenantMessagingPartitionAuthority` (`libs/backend-common/src/database/messaging-partition-privileges.ts`) and the Stage 010 backfill (`apps/db-migrate/src/sql/platform-bootstrap/010-messaging-partition-definer.sql`) — but LEFT the mirror itself: the re-own + schema-grant + runtime-DML + default-privileges recipe existed twice, so adding a partitioned relation or a grant meant editing both copies, and the exact outage class (a grant present in one copy, absent in the other) stayed structurally possible.

**Fix (merged as #920, Tier-1 make-it-impossible):** the COMPLETE recipe collapses into ONE bootstrap-owned SQL function `platform.grant_messaging_partition_authority(text)` (schema USAGE+CREATE to the definer role, re-own of the partition-managed relations, runtime DML grant, ALTER DEFAULT PRIVILEGES forward cover, structured relations[] return). Both callers delegate: Stage 010 backfills every existing tenant schema via `PERFORM`, the TS provisioner helper issues a single `SELECT platform.grant_messaging_partition_authority($1)`. The `deploy-ssot-contract` invariant pins the function's grant surface and that BOTH callers delegate; the helper unit spec pins the single-call shape. The one-off live remediation script applied during the 2026-07-07 incident (`scripts/ops/remediate-messaging-partition-grants.sql`, never committed) is fully subsumed by the function — verified grant-for-grant — and was deleted from the shared checkout.

**Provenance note (2026-07-13 branch-triage):** this record originated on PR #938's branch as "ORPHAN-HIGH-338"; that number was independently claimed on main by the Lane-D db-audit coverage finding while the branch aged (NNN collision), and #938's code content had already landed byte-identical via #920. The record is renumbered to 388 and carried by the branch-triage PR; #938 is closed as superseded.

**Owner:** data-expert. **Deadline:** closed by #920's merge (2026-07-11).

## ORPHAN-MEDIUM-388 — support threads vs support tickets double-model ruled a declared boundary, not a consolidation target (A7 / DB-IDENT-MEDIUM-001) — RESOLVED (this PR)

**Discovered:** 2026-07-11 by the Lane-D database E2E audit (db-audit-identity-billing, DB-IDENT-MEDIUM-001). auth-service models SuperAdmin↔TenantAdmin conversations twice: `auth.message_threads`/`auth.messages` (SupportMessageThread/SupportMessage, `apps/auth-service/src/modules/messaging/entities/`) AND `auth.support_tickets`/`auth.ticket_comments` (`apps/auth-service/src/modules/support/entities/`), both live and FE-reachable from tenant-admin (`MY_THREADS_QUERY` + `MY_TICKETS_QUERY`, `web/modules/tenant-admin/src/graphql/communication-queries.ts:15,150`).
**RESOLVED (this PR) — ADR-043 (`docs/adr/043-support-conversation-boundary.md`):** the boundary is declared, consolidation explicitly rejected. Ticket = SLA-tracked formal issue lifecycle (5-state machine, response/resolution deadlines + breach predicates, triage/assignment, satisfaction capture, human-readable `ticketNumber`); thread = free-form chat (open/closed/archived, per-side unread counters, no completion obligation). Routing rule: obligations → ticket; conversation → thread. The denormal counters (`messageCount`/`unreadCount*`/`commentCount`) are documented as display-only denormals maintained by atomic `increment()` with child-row `COUNT(*)` as SSoT; promoting one to a gating input requires same-transaction increments + reconciliation per the ADR. Rejection grounds: disjoint lifecycle semantics (a merged table is a null-riddled union type with entangled state machines) + live FE usage of both surfaces (consolidation = breaking GraphQL change for zero integrity gain — no double-write between the families exists).
**Owner:** auth-service domain (admin-expert reviewer). **Status:** RESOLVED (this PR — ADR only; no code change required).

## ORPHAN-MEDIUM-389 — AI conversation content ownership ruled (messaging owns, ai runner-context only) + INC-MSG-1 cross-service UPDATE-in-catch removed (A9 / DB-PEOPLE-MEDIUM-004) — RESOLVED (this PR)

**Discovered:** 2026-07-11 by the Lane-D database E2E audit (db-audit-people-messaging, DB-PEOPLE-MEDIUM-004 + incidental INC-MSG-1). AI in-channel conversations were double-modelled: `messaging.messages` (relational per-turn, `isAiGenerated`, receipts, retention/legal-hold/GDPR machinery) vs `ai.agent_conversations.messages` (jsonb blob for agent rehydration). INC-MSG-1: messaging's `gdpr.service.ts` anonymize path issued a DIRECT cross-service `UPDATE agent_conversations` inside a broad `catch {}` — a schema-ownership violation whose every failure class was swallowed as "table not found". The cascade event it also emitted was contract-violating: off-contract fields (`targetService`, `targetEntity`, `anonymizedAt`) and missing contract-required `requestId`/`fulfilByIso` (schemas validate `additionalProperties:false`); the sibling `UserDataAnonymized` emission likewise omitted required `method`/`initiatedBy`.
**RESOLVED (this PR) — ADR-044 (`docs/adr/044-ai-conversation-content-ownership.md`):** `messaging.messages` is the OWNER of AI in-channel conversation content (the compliance source); `agent_conversations.messages` is runner working-context ONLY (may be truncated/summarised; never a compliance/export source). Erasure crosses the boundary by EVENT only: the direct UPDATE + swallow is deleted; both outbox emissions are now typed against their contracts (compiler-enforced exact shape) and enqueued inside the erasure transaction; enqueue failure FAILS LOUD (error log + new `messaging_gdpr_cascade_emit_failure_total` metric + rethrow → rollback, so erasure never commits without its cascade request). The cascade `requestId` is minted and recorded in the same-transaction `compliance_audit_log` row (request-of-record). Consumer verdict: ai-service consumer ALREADY EXISTS — `ConversationPrivacyEventHandler` (`apps/ai-service/src/conversation/conversation-privacy-event.handler.ts:29`, `eventBus.subscribeWildcard` in `onModuleInit`, registered in `ConversationModule`) hard-deletes the blob via `ConversationService.eraseForUser` (stronger than anonymise; legal because the blob is disposable context under ADR-044). London-school specs added on both sides (5 new messaging assertions incl. no-agent_conversations-SQL + fail-loud; 6-test ai handler spec).
**Owner:** messaging-expert + ai-service domain. **Status:** RESOLVED (this PR).

## ORPHAN-MEDIUM-390 — edge-device v1/v2 coexistence documented + single-writer-per-model guard (A11 / DB-SENSOR-MEDIUM-003) — RESOLVED (this PR)

**Discovered:** 2026-07-11 by the Lane-D database E2E audit (db-audit-sensor, DB-SENSOR-MEDIUM-003). sensor-service registers BOTH edge-device generations in `edge-device.module.ts`: v1 `edge_devices` (`EdgeDevice` — production-active: all runtime writes in `edge-device.service.ts`/`provisioning.service.ts`/`mqtt-auth.service.ts` plus the GraphQL surface) and the v2 family per ADR-034 (`devices`/`policies`/`licenses`/`firmware_releases`/`provisioning_records`/`witnesses`/`audit_archive_v1` — migration-created and module-registered, NO runtime write path yet). Disclosed WIP mid-cutover, not drift — but the coexistence contract was undocumented and nothing prevented an interleaved dual-write from creeping in.
**RESOLVED (this PR):** v1 is NOT retired. The coexistence contract is written into the `EdgeDeviceV2` docblock (`apps/sensor-service/src/edge-device/entities/v2/device-v2.entity.ts`) and a new "Coexistence" section of ADR-034 (`docs/adr/034-edge-schema-sensor-per-tenant-ownership.md` — the active edge-schema ADR of the family; the audit's "per ADR-025" label predates the ADR-025→034 renumber pinned by `edge-v2-plan-contract.spec.ts`; no ADR renumbered). New invariant `tests/invariants/edge-device-dual-model-guard.spec.ts` (registered in the registry shard): grep-based per-file classification of repository acquisition (`@InjectRepository`/`Repository<>`/scoped-lookup patterns, v1 `EdgeDevice` vs v2 `EdgeDeviceV2` disambiguated) + ORM write calls + raw-SQL writes against `edge_devices`/`devices`; asserts (1) detector witness — the patterns still recognise the real v1 write path in edge-device.service.ts (anti-theater self-check), (2) no file acquires both models' repositories, (3) no file writes both device rows, (4) allowlist entries (WHY + tracked residual required) must exist and genuinely touch both models. **Divergent-write scan result: NONE found** — v2 entities are referenced only by module registration and migrations (verified 2026-07-13), so the allowlist ships empty; the guard is green on real evidence, not by construction.
**Owner:** sensor-expert. **Status:** RESOLVED (this PR).

## ORPHAN-HIGH-391 — event-store-service shipped with no NATS wiring; latent until the config fail-fast surfaced it (event persistence down, critical_health false-negatives) — RESOLVED (this PR)

**Discovered:** 2026-07-13 while auditing the #965 release's red `critical_health` gate: `aqua-event-store` was crash-looping (restarts 55+) on `NATS_URL must be set to a non-localhost address in production (event-bus config SSoT)`. The compose service block had NO NATS env and no cert mounts — unlike every other backend service — even though event-store was onboarded to the GDPR erasure cascade (INFRA-HIGH-003) and publishes proof/outbox events. The CN was already minted (`infrastructure/nats/services.yaml` `event_store_service`) and the client cert pair ships in the deploy checkout; only the compose wiring was missing. The gap stayed invisible while the NATS client silently defaulted; today's event-bus config fail-fast (correct behaviour) turned it into a crash-loop, which in turn made the #965 release's `critical_health` gate fail (rollback correctly refused — 1 migration applied — fix-forward).
**Fix (this PR):** `x-nats-event-store-env` anchor (mTLS cert pair per ADR-015) + `<<:` into the event-store environment, `nats-ca`/`nats-clients` volume mounts, and a `nats: service_healthy` depends_on.
**Owner:** infra-expert. **Deadline:** RESOLVED on merge.

## ORPHAN-MEDIUM-392 — event-store Redis wiring also missing (localhost-fallback retry spam; Redis-backed paths silently degraded) — RESOLVED (this PR)

**Discovered:** 2026-07-13 immediately after the ORPHAN-HIGH-391 NATS fix deployed: the recreated event-store logs `Redis connection error ... ECONNREFUSED 127.0.0.1:6379` ~30×/min. `app.module.ts:72` registers `RedisModule` (`buildRedisOptions(..., 'optional')`); with no `REDIS_URL` in the compose block the client falls back to localhost and retries forever — no fail-fast exists for Redis (unlike the event-bus SSoT that surfaced 391), so the gap is pure log-spam + silent degradation of whatever rides Redis in the service. Same root family as 391: the event-store compose block was never fully provisioned when the service became a first-class platform participant.
...
## ORPHAN-CRITICAL-393 — billing subscription provisioning silently rolls back on a schema-unqualified cross-schema `modules` query (3 tenants / 0 subscriptions live) — RESOLVED (this PR)

**Discovered:** 2026-07-13 (Billing Revival Faz A). Live prod: `auth.tenants`=3, `billing.subscriptions`=0. Root cause (file:line verified): the provisioning saga step `create_subscription` (`apps/admin-api-service/src/tenant/services/tenant-provisioning-workflow.service.ts` → `createTenantSubscription` → `billingCommandClient.provisionTenantSubscription`) sends NATS `PROVISION_TENANT_SUBSCRIPTION`, handled by `apps/billing-service/src/billing/handlers/billing-admin-nats.handler.ts` `provisionTenantSubscription`, which runs a SERIALIZABLE tx: create subscription THEN `reconcileSubscriptionModuleItems`. That method executed `SELECT id, code, name FROM modules WHERE id = ANY($1::uuid[])` — schema-UNQUALIFIED. `billing.modules` does not exist (live-verified `to_regclass('billing.modules')` = NULL) and the `billing_service` role has NO grant on `auth.modules` (live-verified: zero rows in `role_table_grants`), so the query threw → `NotFoundException` → the whole tx (including the just-inserted subscription row) ROLLED BACK, while the `auth.tenants` row (reserved earlier at `reserve_auth_tenant`) survived. Every tenant creation lost its subscription silently.
**Fix (this PR, Tier-1 make-it-structurally-impossible):** admin-api OWNS `admin.module_pricing` + `PricingCalculatorService` + module code/name resolution (`auth.modules` via its own grant). The provisioning command now carries a resolved, priced `moduleItems[]` (`{moduleId, code, name, quantities, lineItems, subtotal, discountAmount, total}`); billing writes the module rows directly from the command — the cross-schema `modules` SELECT is DELETED. billing no longer reaches across a schema boundary it has no grant for, so the rollback class cannot recur. The residual empty/missing-`moduleItems` case is a boundary VALIDATION_ERROR raised before the transaction opens (no partial subscription possible).
**Owner:** billing-expert. **Status:** RESOLVED (this PR).

## ORPHAN-HIGH-394 — subscription module items were written with subtotal/discount/total hardcoded to 0 (invented $0 price; admin.module_pricing never read) — RESOLVED (this PR)

**Discovered:** 2026-07-13 (Billing Revival Faz A). `reconcileSubscriptionModuleItems` inserted every `billing.subscription_module_items` row with `subtotal, discount_amount, total` = `0, 0, 0` (hardcoded literals) and `line_items = '[]'`, ignoring `admin.module_pricing` entirely — even on the paths where the tx did not roll back, a provisioned subscription would carry $0 module lines and the subscription's own `pricing.basePrice` reflected only the catalog plan base, never the selected modules. The recurring-invoice scheduler bills solely off `subscription.pricing.basePrice`, so tenants would have been invoiced for the plan base with the module charges structurally invisible (revenue leak).
**Fix (this PR):** admin-api computes each module's real price via `PricingCalculatorService.calculatePricing` (reads `admin.module_pricing`, honours `moduleQuantities` + tier) and passes the per-module `subtotal/discountAmount/total/lineItems` in the command. billing writes those real values. The subscription's `pricing.basePrice` is wired to `sum(moduleItems.total)` so the stored monthly price and the scheduler's recurring invoice reflect the actual module-based charge. Modules with no `admin.module_pricing` catalog entry legitimately price at 0 (free/core tier) WITHOUT throwing — the fix that removed the rollback (393) is not reintroduced as a hard failure.
**Owner:** billing-expert. **Status:** RESOLVED (this PR).

## ORPHAN-MEDIUM-395 — dead event-driven subscription-provisioning path (unregistered handler + $25 hardcode + @Cron retry loop + retry table with zero emitters) — RESOLVED (this PR)

**Discovered:** 2026-07-13 (Billing Revival Faz A). `TenantSubscriptionRequestedHandler` (`apps/billing-service/src/billing/event-handlers/tenant-subscription-requested.handler.ts`) was registered NOWHERE (`billing.module.ts` `EventHandlers: never[] = []`) and the `TenantSubscriptionRequested` event had ZERO emitters repo-wide (the live provisioning path uses the NATS request-reply command, not this event). The dead handler carried its own `$25`-per-module hardcode, a `@Cron('*/2 * * * *')` retry loop, and a `billing.subscription_provisioning_retries` table (live-verified 0 rows). This is a Potemkin second pricing/provisioning model competing with the real one.
**Fix (this PR):** deleted the handler + its `mapToPlanTier`/`DEFAULT_PRICING`/retry machinery. Removed `subscription_provisioning_retries` from `MODULE_SCHEMAS['billing'].tables` and dropped the table (migration `1801800000000-DropSubscriptionProvisioningRetries`, no archive — live 0 rows, evidence stated in the docblock). Removed the orphaned `TenantSubscriptionRequestedEvent` contract + its JSON-Schema validator + `ModuleQuantityConfig` (all provably unused after the handler delete) and updated the two stale doc-comments that named the deleted class. `SubscriptionProvisioningFailedEvent` retained (see ORPHAN-LOW-396) as it is a distinct contract; the tenant-provisioning-ssot invariant already asserts `billing.module.ts` contains no `TenantSubscriptionRequestedHandler`.
**Owner:** billing-expert. **Status:** RESOLVED (this PR).

## ORPHAN-LOW-396 — SubscriptionProvisioningFailedEvent orphaned after the dead-handler delete (its only emitter is gone; no consumers) — OPEN

**Discovered:** 2026-07-13 (Billing Revival Faz A, incidental). `SubscriptionProvisioningFailedEvent` (`libs/event-contracts/src/billing-events.ts`) was emitted ONLY by the now-deleted `TenantSubscriptionRequestedHandler` and has no consumer/subscriber anywhere in the repo. Removing it is additional event-contract-union blast radius orthogonal to the subscription-break fix, so it is left in place and tracked rather than removed in this PR. It is now a dead contract (an admin-alerting failure event that nothing emits and nothing reads).
...
**Fix (this PR, Tier-1/2):** FREE is now a first-class billing tier end-to-end. `PlanTier` enum + event-contracts `PlanTier` type gain `free`; migration `1801900000000-AddPlanTierFreeEnumValue` adds `'free'` to both billing enums (additive, `transaction=false`, `IF NOT EXISTS`, fail-closed postcondition). `plan-seed` seeds a permanent $0 `Free` plan (limits projected from PLAN_CATALOG FREE via `billingPlanLimitsFor`). `metered-billing` gains a FREE model where every meter is priced at 0 on a single open-ended tier (no usage bills), `includedUnits` sourced from `resolvePlanLimits(TenantPlan.FREE)`. Both coercion mappers pass `free` through. `PricingCalculatorService.calculateModulePrice` short-circuits FREE to an all-zero breakdown (also removes the latent 0-multiplier divide-by-zero). The provisioning handler (`billing-admin-nats.handler.ts`) enforces FREE invariants as the billing SSoT: status always `active` (never `trial`), no trial window, and every price clamped to $0 (subscription base + each module item), regardless of what the command carried. NO Stripe object is created on the provisioning path (verified: `createProvisioningSubscription` is a pure INSERT, so the Stripe-gating concern in the task was a no-op).

**FREE-module-charge decision:** FREE clamps ALL module charges to $0, not just the platform fee. Rationale: the user decision is "FREE = permanent, fully free"; the billing subscription is the SSoT for billing state (D14), and the invoice scheduler bills off `pricing.basePrice`, so a non-$0 module total on a FREE subscription would produce real charges. Enforced at THREE layers (make-it-impossible): admin pricing calculator ($0 breakdown produces $0 resolved items), billing handler clamp (base + items), FE `calculatedTotal` short-circuit.
...
## ORPHAN-MEDIUM-405 — metering rollup SCHEDULER was dead (performRollup uncalled, aggregateOnSchedule unread, raw setInterval); scheduler wired, usage-INGESTION still deferred (Billing Revival Faz E) — RESOLVED (this PR)

**Discovered:** 2026-07-13 (Faz E). The billing metering module's rollup scheduler was a Potemkin pipeline: `UsageAggregatorService.performRollup` had **zero callers**, and every `RollupConfig.aggregateOnSchedule` flag (set `true` for HOURLY→DAILY, DAILY→WEEKLY, DAILY→MONTHLY in `initializeDefaultRollupConfigs`) was **written but never read** — so no coarser-granularity `usage_aggregations` row was ever materialised even once data existed. Separately, all periodic work in the module ran on hand-rolled `setInterval` handles (aggregator 30s persistence; metering 5s flush / 10s Redis-sync / 1h cleanup; metered-billing hourly stale-FX warn — the last one leaked, its handle never stored or cleared) despite the billing app already wiring `ScheduleModule.forRoot()`, so none of these timers were owned by Nest's SchedulerRegistry (unmanaged lifecycle, undiscoverable, untestable without wall-clock advancement).

**Fix (this PR):** `performRollup` is now driven by `UsageAggregatorService.runScheduledRollups`, an hourly `@Cron` that reads `rollupConfigs`, and for every tenant (via the secondary tenant index, not an all-tenant scan) and every active meter rebuilds BOTH the in-progress and the just-closed target period for each configured rollup, in declared order so HOURLY→DAILY materialises before DAILY→MONTHLY consumes it in the same pass. `performRollup` rebuilds each target by SUMMING sources and OVERWRITING the keyed row, so the pass is idempotent and no-ops (returns `null`) until data exists. All `setInterval`s became Nest timers: aggregator persistence → `@Interval('metering-aggregator-persist', 30s)`; metering flush/Redis-sync/cleanup → `@Interval`s; metered-billing stale-FX warn → `@Cron(EVERY_HOUR)` (also fixing the leak). `performRollup`'s logic was reviewed and found sound (correct window containment, overwrite-not-accumulate) — no data-corruption bug; no behaviour change needed beyond wiring.
**Tests:** new `usage-aggregator.scheduler.spec.ts` (London-style) proves the `@Cron`/`@Interval` metadata is registered, that `runScheduledRollups` drives `performRollup` for the full config chain, materialises the daily total from hourly buckets, is idempotent across repeated passes, and no-ops safely with no ingested usage. The pre-existing `usage-aggregator.service.spec.ts` (was red — missing `DataSource` provider) and `usage-metering.service.spec.ts` (3 stale `toBeInstanceOf(Date)` assertions on ISO-string fields + fake-timer-driven flush) were repaired to match the converted code.
**Ingestion carve-out (still OPEN — the big deferred item):** usage-INGESTION remains dead — no domain producer calls `UsageMeteringService.recordUsage`, so `usage_aggregations` still holds 0 live rows and the now-live scheduler will run and no-op in production until ingestion lands. Faz E deliberately wired only the SCHEDULER (per scope decision); wiring recordUsage callers is a separate tracked initiative. **Owner:** billing-expert. **Deadline:** 2026-10-15.
**Owner:** billing-expert. **Status:** RESOLVED (this PR) for the scheduler; ingestion carve-out OPEN.

## ORPHAN-LOW-406 — secret-rotation runbook documented the wrong Stripe outbound-key env var (`STRIPE_API_KEY` vs canonical `STRIPE_SECRET_KEY`) (Billing Revival Faz E) — RESOLVED (this PR)

**Discovered:** 2026-07-13 (Faz E). `docs/runbooks/secret-rotation.md` documented the Stripe server-side/outbound key as `STRIPE_API_KEY` (section header + rotation-cadence table), but the canonical env var read by the client factory is `STRIPE_SECRET_KEY` (`libs/backend-common/src/billing/stripe-client.factory.ts`, `STRIPE_SECRET_KEY_ENV = 'STRIPE_SECRET_KEY'`). `STRIPE_API_KEY` is the historical wrong name the factory never reads — the same env-name fracture that left Stripe silently unconfigurable and contributed to the 2026-06 Suderra billing incident (see the 2026-06-27 runtime correction earlier in this file). An operator following the runbook verbatim would rotate a variable the service ignores.
**Fix (this PR):** runbook section + cadence table repointed to `STRIPE_SECRET_KEY`, with a callout that `STRIPE_API_KEY` is the historical wrong name (do-not-use) and a pointer to grep deploy manifests for stray `STRIPE_API_KEY`. `STRIPE_WEBHOOK_SECRET` section left correct (it is the right name).
**Owner:** billing-expert / infra-expert. **Status:** RESOLVED (this PR).

## ORPHAN-LOW-407 — admin `tenant_billing_info` + `custom_plans` satellite tables conceptually overlap billing.subscriptions/plans (Billing Revival Faz E incidental) — OPEN

**Discovered:** 2026-07-13 (Faz E, incidental). admin-api-service owns two billing-shaped tables — `admin.tenant_billing_info` (`apps/admin-api-service/src/tenant/entities/tenant-activity.entity.ts:103`) and `admin.custom_plans` (`apps/admin-api-service/src/billing/entities/custom-plan.entity.ts:74`) — that model per-tenant billing state and plan definitions. Platform rule D14 makes `billing.subscriptions` the SSoT for per-tenant subscription state and `billing.plans` the SSoT for plan/pricing definitions. The admin satellites risk a second billing authority (the same split-brain class ORPHAN-MEDIUM-382/383 collapsed for usage-metrics and module pricing) if they carry state that billing also owns, rather than being a thin read-projection/enrichment.
**Recommendation:** consolidation review — audit each column of `tenant_billing_info` and `custom_plans` for overlap with `billing.subscriptions`/`billing.plans`/`billing.subscription_module_items`; keep only admin-local enrichment (activity/audit metadata), and repoint anything that duplicates billing state to read from billing (query/event) rather than storing a fork. This is a design review, not a mechanical drop — CUSTOM plan definitions may legitimately live where they are consumed, but the SSoT boundary must be explicit.
**Owner:** billing-expert. **Deadline:** 2026-10-30. **Status:** OPEN.

## ORPHAN-MEDIUM-408 — MeteredBillingService.clearCache(subscriptionId) never matched the reshaped tenant-scoped cache key, silently breaking subscription-change invalidation (Billing Revival Faz E incidental) — RESOLVED (this PR)

**Discovered:** 2026-07-13 (Faz E, incidental — surfaced by a red `metered-billing.service.spec.ts` case when the scheduler conversion brought the suite back under test). `calculateBilling` builds its cache key in the layer-1 tenant-scoped shape `cache:metered-billing:<tenantId>:billing-calculation:<subscriptionId>-<start>-<end>` (the BILLING-MEDIUM-006 cure), but `clearCache(subscriptionId)` still matched entries with `key.startsWith(subscriptionId)`. Since the key starts with `cache:metered-billing:...`, that predicate matched **nothing** — so the MED-02 `@OnEvent('subscription.created'|'updated'|'cancelled') handleSubscriptionChange → clearCache(subscriptionId)` path never actually evicted a subscription's cached calculation. A plan upgrade/downgrade/cancel therefore kept serving the pre-change billing calculation until the 5-minute TTL lapsed — the exact staleness MED-02 was written to prevent.
**Fix (this PR):** `clearCache(subscriptionId)` now matches the exact `:billing-calculation:<subscriptionId>-` key segment (the trailing `-` delimiter prevents `sub-1` from matching `sub-12`). Root-cause fix of the key/predicate mismatch, not a test tweak; the previously-red "should clear cache by subscription" spec now passes for the right reason.
**Owner:** billing-expert. **Status:** RESOLVED (this PR).
