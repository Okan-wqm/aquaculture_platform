...
FORCE=false
[ "${1:-}" = "--force" ] && FORCE=true
if [ "${FORCE}" = true ] && [ -n "${DEPLOY_CERTS_DIR:-}" ]; then
  echo "error: production certificate rotation requires a staged set and explicit consumer recreate" >&2
  exit 2
fi
GENERATED_STAGE=''
...
# Legacy shared client cert — retained on disk for rollback compatibility and
# validated with the same fail-closed file contract. Canonical production
# Compose does not mount this identity into any container.
generate_client_cert() {
  local name="$1" cn="$2" dir="${CERTS_DIR}/${1}"
  local key_path="${dir}/client-key.pem"
  local cert_path="${dir}/client-cert.pem"
  local key_stage csr_stage cert_stage cert_serial
  assert_certificate_directory "${dir}" "${name} legacy client directory"
  if [ "$FORCE" = false ] && certificate_set_has_any_path "${key_path}" "${cert_path}"; then
    validate_existing_client_set \
      "${key_path}" \
      "${cert_path}" \
    ...
    echo "  [skip] ${name} client (legacy shared)"
    return
  fi
  require_safe_generation_target "${key_path}" "${name} legacy client private key"
  require_safe_generation_target "${cert_path}" "${name} legacy client certificate"
  create_stage_file "${dir}/.client-key.pem.XXXXXX"
  key_stage=${GENERATED_STAGE}
  create_stage_file "${dir}/.client.csr.XXXXXX"
  csr_stage=${GENERATED_STAGE}
  create_stage_file "${dir}/.client-cert.pem.XXXXXX"
  cert_stage=${GENERATED_STAGE}
  cert_serial=$(new_certificate_serial)
  ...
  openssl x509 -req -days 365 -in "${csr_stage}" \
    ...
    -set_serial "0x${cert_serial}" -out "${cert_stage}" 2>/dev/null
  chmod 0644 "${key_stage}" "${cert_stage}"
  validate_existing_certificate_asset "${key_stage}" 0644 "staged ${name} legacy client private key"
  validate_existing_certificate_asset "${cert_stage}" 0644 "staged ${name} legacy client certificate"
  validate_certificate_key_pair \
    "${key_stage}" \
    "${cert_stage}" \
    "O=Aquaculture Platform,CN=${cn}" \
    "${CERTS_DIR}/ca/ca-cert.pem" \
    sslclient \
    "staged ${name} legacy shared client"
  # Publish the private key last; it is the usable-identity commit member.
  mv -fT -- "${cert_stage}" "${cert_path}"
  mv -fT -- "${key_stage}" "${key_path}"
...
# Legacy shared client cert remains a validated rollback artifact only. Every
# canonical production consumer mounts its exact per-service identity below.
