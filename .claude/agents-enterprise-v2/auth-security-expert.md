...
| Auth Foundation | `libs/backend-common/src/auth/` | JWT helpers, token types, auth primitives shared across services |
...
**Primary ownership note:** `auth-security-expert` is the primary reviewer for the auth pipeline surface in `apps/auth-service/`, `apps/gateway-api/`, and `libs/backend-common/src/{auth,guards,security,middleware,audit}/`.

**Out of scope:** Domain service application logic (farm, sensor, HR, billing, etc.), frontend modules, infrastructure, edge agent, and MCP server implementation details. BUT: review auth-related patterns if flagged by other agents.
...
- MCP session claims, gateway trust-boundary assumptions, and delegated user/tenant context in `mcp/**` → mcp-expert
