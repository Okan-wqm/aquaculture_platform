...
export class Baseline1800000000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "notification"."notification_logs_channel_enum" AS ENUM('email', 'sms', 'push', 'webhook', 'in_app', 'system'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;`);
        await queryRunner.query(`DO $$ BEGIN CREATE TYPE "notification"."notification_logs_status_enum" AS ENUM('pending', 'sent', 'failed', 'retrying', 'bounced', 'dead_letter'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;`);
