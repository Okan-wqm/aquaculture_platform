  ...
  it('renders the per-cause grid READ-ONLY when mortality is from records', () => {
    render(
      <MortalityStep
        formData={formData}
        onChange={vi.fn()}
        prefill={prefillWith({ path: '/mortality', provenance: 'RECORDS', sourceRecordCount: 3, blocking: false })}
      />,
    );
  ...
  it('keeps the per-cause grid editable when mortality is MANUAL_REQUIRED', () => {
    render(
      <MortalityStep
        formData={formData}
        onChange={vi.fn()}
        prefill={prefillWith({ path: '/mortality', provenance: 'MANUAL_REQUIRED', blocking: false })}
      />,
    );
    const counts = screen.getAllByRole('spinbutton');
