...
export class FeedingWindowReadinessService {
  ...
  async evaluate(
    ...
    const guarded = entries.filter(
      (entry) => typeof entry.minDissolvedOxygen === 'number' && isValidUUID(entry.unitId),
    );
    if (guarded.length === 0) {
      return [];
    }
  ...
  private async loadOxygenObservations(
    ...
    const freshnessCutoff = new Date(Date.now() - DO_READING_FRESHNESS_MINUTES * 60_000);
    ...
    for (const row of rows) {
      observations.set(row.unitId, {
        ...
        dissolvedOxygen: row.dissolvedOxygen === null ? null : Number(row.dissolvedOxygen),
