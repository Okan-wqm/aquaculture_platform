...
const WidgetCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      {isFav && <Star className="w-3 h-3 text-warning-400 fill-warning-400 flex-shrink-0" />}
      {pw > 0 && (
...
export const UnifiedLeftPanel: React.FC<UnifiedLeftPanelProps> = ({
    ...
    (entry: FuxaWidgetCatalogEntry) => {
      ...
      addWidget(activeScreenId, {
        ...
        config: {
          label: entry.name,
          catalogId: entry.id,
          category: entry.subcategory ? `${entry.category} > ${entry.subcategory}` : entry.category,
        },
      });
  ...
  // Debounce search
  useEffect(() => {
    const t = setTimeout(() => setDq(query), 150);
  ...
  // Persist categories
  useEffect(() => {
    lsWrite(LS_CAT, Array.from(expCat));
  }, [expCat]);
  ...
  const trackDrop = useCallback((k: string) => {
    setRecent((p) => {
      const n = [k, ...p.filter((x) => x !== k)].slice(0, 8);
      lsWrite(LS_RECENT, n);
      return n;
    });
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return allW.filter(({ w, cat }) => {
      const sub = w.defaultConfig?.equipmentSubType as string | undefined;
      return (
        fuzzy(dq, w.label) || fuzzy(dq, cat) || fuzzy(dq, w.type) || (sub ? fuzzy(dq, sub) : false)
      );
    });
  }, [dq, allW]);
  ...
  const onDrag = useCallback(
    (e: React.DragEvent, w: PaletteWidgetDef) => {
      startDrag(e, w);
  ...
  const onCtx = useCallback(
    (e: React.MouseEvent, w: PaletteWidgetDef) => {
      e.preventDefault();
      toggleFav(paletteWidgetKey(w));
    },
    [toggleFav],
  );
  ...
  const onAdd = useCallback(
    (w: PaletteWidgetDef) => {
      ...
      const sub = w.defaultConfig?.equipmentSubType as string | undefined;
      const sd =
        (sub ? EQUIPMENT_SUBTYPE_SIZES[sub as EquipmentSubType] : undefined) ??
        WIDGET_SIZES[w.type];
      ...
      if (filtered.length === 0)
        return (
          <div className="py-8 text-center text-xs text-gray-500 dark:text-gray-400">
            No widgets match &ldquo;{dq}&rdquo;
          </div>
        );
      ...
      return (
        <div className="py-1 px-1 space-y-1">
          {Array.from(byCat.entries()).map(([cat, ws]) => (
            <div key={cat}>
              <div className="px-2 py-1 text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {cat} ({ws.length})
              </div>
              <div className="space-y-1 px-1">
                {ws.map((w) => (
                  <WidgetCard
                    key={paletteWidgetKey(w)}
                    w={w}
                    isFav={favs.has(paletteWidgetKey(w))}
                    onDrag={onDrag}
                    onCtx={onCtx}
                    onAdd={onAdd}
                  />
      ...
      <div>
        ...
        {recentW.length > 0 && (
          <div className="border-b border-gray-100 dark:border-gray-700 pb-1">
            <div className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
            ...
            <div className="flex gap-1.5 px-2 pb-1.5 overflow-x-auto">
              {recentW.map(({ w }) => (
                <button
                  ...
                  onContextMenu={(e) => onCtx(e, w)}
                  onClick={() => onAdd(w)}
                  className="flex-shrink-0 flex items-center gap-1 px-2 py-1 rounded-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 hover:border-info-400 hover:bg-info-50 cursor-grab text-xs text-gray-600 dark:text-gray-400 transition-colors"
                  title={w.label}
                >
                  {icon(w.iconKey, 'w-3 h-3')}
                  <span className="max-w-[60px] truncate text-[10px]">{w.label}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {PALETTE_CATEGORIES.map((cat) => (
          <div key={cat.name}>
            <Button variant="secondary" size="sm" onClick={() => toggleCat(cat.name)}>
              <span>
                {cat.name}{' '}
          ...
          <div className="flex-1 overflow-hidden flex flex-col min-h-0">
            ...
            <div className="px-2 pb-1.5 flex-shrink-0">
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 h-8 text-xs font-medium text-success-700 dark:text-success-300 bg-success-50 dark:bg-success-900/20 border border-success-200 dark:border-success-800 rounded-md hover:bg-success-100 dark:hover:bg-success-900/50 hover:border-success-300 dark:hover:border-success-600 transition-colors"
      ...
      <div className="flex-shrink-0">
        {!layColl && (
          <button
            ...
            className={`h-1.5 cursor-row-resize flex items-center justify-center border-t border-gray-200 dark:border-gray-700 hover:bg-info-50 transition-colors ${resizing ? 'bg-info-100 dark:bg-info-900/40' : ''}`}
          >
            <GripHorizontal className="w-4 h-4 text-gray-300" />
          </button>
        )}
        <Button variant="secondary" size="xs" onClick={() => setLayColl((p) => !p)}>
          {layColl ? (
            <ChevronRight className="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" />
          ) : (
            <ChevronDown className="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" />
