...
impl CommandHandler {
    ...
    ///
    /// Downloads and installs a new version of the edge-agent binary from GitHub releases.
    /// Sends MQTT progress updates at each stage.
    ///
    /// # Task Handle
    /// The spawned task is intentionally not tracked because:
    /// 1. The agent will be restarted after successful update
    /// 2. We must return the ACK response before the update begins
    /// 3. Progress is reported via MQTT at each stage
    async fn cmd_update_firmware(&self, command: &CommandMessage) -> (bool, Value, Option<String>) {
        info!("Executing update_firmware command");

        let params = &command.params;

        // Parse target_version (required)
        ...
        // Optional: GitHub repo override (validated to prevent URL injection)
        let repo = params
            .get("repo")
            .and_then(|v| v.as_str())
        ...
        // Get device_id for progress responses
        let device_id = {
        ...
        let _ = tokio::spawn(async move {
            ...
            // Extract version number from tag (agent-v1.5.3 -> 1.5.3)
            ...
            // Extract tarball to a temp dir, then find the binary
            let extract_dir = update_dir.join("extract");
            let _ = fs::remove_dir_all(&extract_dir); // clean any previous extract
            if let Err(e) = fs::create_dir_all(&extract_dir) {
                send_failed(format!("Failed to create extract directory: {}", e)).await;
                return;
            }

            // Extract tarball using tar command (async to avoid blocking the runtime)
            let tar_status = tokio::process::Command::new("tar")
                .args(["xzf", &tarball_path.to_string_lossy().to_string(), "-C", &extract_dir.to_string_lossy().to_string()])
                .status()
                .await;
            ...
            // Find the extracted binary (could be at extract_dir/suderra-agent or nested)
            ...
            // chmod +x the extracted binary before install
            ...
            if let Err(e) = chmod_status {
                ...
                // Non-fatal on most setups, but log it
            }

            // Atomic install: copy to staging path, then rename (rename is atomic on same fs)
            ...
            if let Err(e) = fs::rename(&staging_path, &binary_path) {
                // rename failed (possibly cross-device), fall back to copy
            ...
            // Cleanup extract dir and downloaded files
            ...
            // Restart agent via systemctl (async to avoid blocking)
    ...
    ///
    /// Receives a list of device configurations and updates the LoRa actor.
    /// Existing devices not in the new list are removed.
    #[cfg(feature = "lorawan")]
    async fn cmd_update_lora_devices(&mut self, params: &Value) -> (bool, Value, Option<String>) {
        ...
        // Diff: yeni listede olmayan cihazlari kaldir
        // Mevcut cihaz listesini config'den al
        let current_devices: Vec<String> = {
            let state_r = self.state.read().await;
            state_r.config.lorawan.as_ref()
                ...
                match crate::lora::types::DevEui::from_hex(existing_eui) {
                    Ok(dev_eui) => {
                        if let Err(e) = lora_handle.remove_device(dev_eui).await {
                            ...
                            removed += 1;
                            info!("LoRa cihaz kaldirildi (diff): dev_eui={}", existing_eui);
                        }
                    }
                    Err(e) => {
        ...
        if errors.is_empty() {
            ...
            warn!("LoRa device update partially failed: {} added, {} removed, {} errors",
                added, removed, errors.len());
            (
                added > 0 || removed > 0,
    ...
    #[cfg(feature = "lorawan")]
    async fn cmd_lora_downlink(&mut self, params: &Value) -> (bool, Value, Option<String>) {
        ...
        // Tek sayili hex string kontrolu — gecersiz uzunluk erken reddedilir
        if payload_hex.len() % 2 != 0 {
            return (false, json!(null), Some("Payload hex string must have even length".to_string()));
        }
...
/// Validate that a GitHub repo string is safe (owner/repo format, no path traversal).
...
/// Validate that a version/tag string contains only safe characters.
...
///
/// Accepts:
/// - "latest" → fetches latest agent-v* tag from GitHub releases
/// - "agent-v1.5.3" → used as-is
/// - "1.5.2" → prefixed with "agent-v"
async fn resolve_firmware_version(target: &str, repo: &str) -> anyhow::Result<String> {
    if target == "latest" {
        ...
        // Assume bare version number like "1.5.2"
...
///
/// Filters releases to only those with tag names starting with "agent-v",
/// returning the first match (GitHub returns releases newest first).
async fn fetch_latest_agent_tag(repo: &str) -> anyhow::Result<String> {
...
/// Download a file from a URL to a local path with a 300-second timeout.
...
/// - `<hash>` (hash only)
fn read_checksum_file(path: &Path) -> anyhow::Result<String> {
    ...
    // Validate it looks like a hex SHA256 (64 hex chars)
