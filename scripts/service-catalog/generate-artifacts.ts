  ...
  return {
    ...
    contents: jsonArtifact({
      ...
      excludedFederatedServices: PLATFORM_SERVICE_CATALOG.filter(
        ...
        owner: entry.federationExclusion?.owner ?? 'platform-service-catalog',
        removeAfterRelease: entry.federationExclusion?.removeAfterRelease ?? '2026.09',
        reason:
          entry.federationExclusion?.reason ??
...
function catalogDeployEnvArtifact(): Artifact {
  ...
  return {
    path: 'infrastructure/deploy/service-catalog.deploy.env',
    contents: `${header('yaml')}
