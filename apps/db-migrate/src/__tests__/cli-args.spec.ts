...
describe('parseArgs — default (no flags)', () => {
  ...
  it('rejects bare positional args', () => {
    expect(() => parseArgs(['ignoreme', 'positional'])).toThrow(/unexpected positional/i);
  });
});

describe('parseArgs — --down N --schema <name> happy path', () => {
  it('parses --down 1 --schema admin', () => {
    const out = parseArgs(['--down', '1', '--schema', 'admin', '--confirm-release', 'release-1']);
    expect(out).toEqual({
      down: 1,
      schema: 'admin',
      confirmRelease: 'release-1',
    });
  });

  it('parses --schema first then --down N', () => {
    // Flag order should not matter.
    const out = parseArgs(['--schema', 'sensor', '--confirm-release', 'release-1', '--down', '3']);
    expect(out).toEqual({
      down: 3,
      schema: 'sensor',
      confirmRelease: 'release-1',
  ...
  it('accepts --down 100 (large count)', () => {
    const out = parseArgs(['--down', '100', '--schema', 'admin', '--confirm-release', 'release-1']);
...
describe('parseArgs — --down rejects malformed argument', () => {
  ...
  it('rejects --down 0 (must be positive)', () => {
    expect(() =>
      parseArgs(['--down', '0', '--schema', 'admin', '--confirm-release', 'release-1']),
    ).toThrow(/positive integer/i);
  });

  it('rejects --down -1 (negative)', () => {
    expect(() =>
      parseArgs(['--down', '-1', '--schema', 'admin', '--confirm-release', 'release-1']),
    ).toThrow(/positive integer/i);
  });

  it('rejects --down abc (non-numeric)', () => {
    expect(() =>
      parseArgs(['--down', 'abc', '--schema', 'admin', '--confirm-release', 'release-1']),
    ).toThrow(/positive integer/i);
  });
  ...
  it('rejects --down 1.5 (non-integer — parseInt would truncate silently)', () => {
    ...
    expect(() =>
      parseArgs(['--down', '1.5', '--schema', 'admin', '--confirm-release', 'release-1']),
    ).toThrow(/positive integer/i);
  });
});
...
describe('parseArgs — --schema rejects malformed argument', () => {
  it('rejects --schema with no argument', () => {
    expect(() => parseArgs(['--down', '1', '--schema'])).toThrow(/requires a schema name/i);
  });
  ...
  it('rejects --schema when the next token is another flag', () => {
    expect(() => parseArgs(['--schema', '--down', '1'])).toThrow(/requires a schema name/i);
  });
});
...
describe('parseArgs — cross-flag validation', () => {
  ...
  it('rejects --schema without --down (schema is a rollback-only modifier)', () => {
    expect(() => parseArgs(['--schema', 'admin'])).toThrow(/only accepted alongside --down/i);
  });

  it('rejects --down without --confirm-release', () => {
    expect(() => parseArgs(['--down', '1', '--schema', 'admin'])).toThrow(
      /requires --confirm-release/i,
    );
  });
