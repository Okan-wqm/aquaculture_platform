      ...
      <div className="space-y-6">
        <Card title="Multi-Factor Authentication (MFA)">
          <div className="space-y-4">
            {/* ADR-046: "Require MFA for all users" is managed by the tenant's
                own administrator (tenant-admin module -> auth-service policy),
                not by SUPER_ADMIN. The old checkbox wrote a fabricated field
                that nothing persisted and nothing enforced. */}
            <p className="text-sm text-gray-500">
              MFA enforcement for all users is managed by the tenant&apos;s own
              administrator (Tenant Admin &rarr; Security Policy).
            </p>
            <div className="flex items-center">
              <input
                type="checkbox"
        ...
        <Card title="Login & Session Security">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {/* ADR-046: the tenant idle-session timeout is managed by the
                tenant's own administrator (auth-service policy — it clamps the
                refresh-token TTL at every mint); the old input wrote a
                fabricated field that nothing enforced. */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Session Timeout (min)</label>
              <p className="text-sm text-gray-500">
                Managed by the tenant&apos;s administrator (Tenant Admin &rarr; Security Policy).
              </p>
            </div>
            <div>
