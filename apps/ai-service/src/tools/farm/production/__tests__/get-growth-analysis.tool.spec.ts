...
import { GetGrowthAnalysisTool } from '../get-growth-analysis.tool';
...
describe('GetGrowthAnalysisTool', () => {
  let send: jest.Mock;
  let tool: GetGrowthAnalysisTool;
  ...
  beforeEach(() => {
    ...
    tool = new GetGrowthAnalysisTool({ send });
  });

  it('is a module-scoped farm read tool', () => {
    const metadata = tool.getMetadata();
    expect(metadata.name).toBe('get_growth_analysis');
  ...
  it('applies defaults and sends the contract subject with the context tenant', async () => {
    ...
    expect(send).toHaveBeenCalledWith(FARM_AI_QUERY_SUBJECTS.GROWTH_ANALYSIS, {
      ...{ batchId: TANK },
