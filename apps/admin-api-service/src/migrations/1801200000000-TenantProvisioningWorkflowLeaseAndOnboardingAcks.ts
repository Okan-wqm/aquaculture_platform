 ...
 *
 * This is the same "ledger says applied, DB says lagged" class the SAVEPOINT and
 * entity-diff gates close from other angles. CLAUDE.md: "Never hand-edit
 * migration files — generate a new one." So this is a NEW forward migration that
 * brings any database stuck at the pre-edit shape up to the desired state; it is
 * fully idempotent (IF NOT EXISTS / DO-EXCEPTION guards), so on a fresh database
 ...
 * auth (docs/reviews/orphan-findings.md ORPHAN-HIGH-133). This migration is
  ...
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // PG has no `ADD CONSTRAINT IF NOT EXISTS`; wrap DROP-then-ADD in a
    // DO/EXCEPTION guard so the migration is replay-safe (no 42710).
    await queryRunner.query(`
      DO $$
      BEGIN
        ALTER TABLE "admin"."tenant_provisioning_runs"
    ...
    // (see the class docblock + ORPHAN-HIGH-133).
    await queryRunner.query(`
