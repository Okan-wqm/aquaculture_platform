...
import {
  ...
  ActionTokenPurpose,
  ActionTokenStatus,
} from '../../modules/authentication/entities/action-token.entity';
...
import {
  ...
  JwtPayload,
} from '../../modules/authentication/services/token.service';
import {
  WebAuthnRegisterCredentialInput,
  WebAuthnVerifyLoginInput,
} from '../../modules/authentication/dto/webauthn.dto';
...
} from '../../modules/authentication/services/credential-state';
...
describe.each(['fresh', 'populated'] as const)('authentication state: %s database', (path) => {
  ...
  beforeAll(async () => {
    ...
    migrator = new DataSource({
      ...
      migrations:
        path === 'populated'
          ? migrations.filter((name) => !name.includes('1819100000000'))
          : migrations,
    });
    ...
    if (path === 'populated') {
      await migrator.query(
        `INSERT INTO auth.users (id,email,password,role,"updatedAt")
    ...
    try {
      ...
      await applyTenantRlsToSchema(hardeningRunner, {
        schemaOverride: 'auth',
        ...rlsOptions,
        excludeTables: [...(rlsOptions.excludeTables ?? []), ...ledgers],
      });
    ...
    await harness.dataSource.query(
      `CREATE ROLE auth_service LOGIN PASSWORD 'auth-contract-test-only' NOSUPERUSER NOBYPASSRLS`,
    );
    ...
    orm = new DataSource({
      ...
      password: 'auth-contract-test-only',
      schema: 'auth',
      entities: [
        User,
        Tenant,
  ...
  async function newUser(): Promise<User> {
    const user = orm.manager.create(User, {
  ...
  it('ignores client-assigned versions on INSERT and UPDATE; bookkeeping preserves the proof', async () => {
    ...
    await orm.manager.query(
      `UPDATE auth.users SET "credentialVersion" = 100,
      "lastLoginAt" = CURRENT_TIMESTAMP, "updatedAt" = '2026-09-01 12:00:00.123456+00' WHERE id = $1`,
      [user.id],
    );
    ...
    const rows = await orm.query<Array<{ credentialVersion: number }>>(
  ...
  it('rejects an authenticated proof after a concurrent password change', async () => {
    ...
    await orm.manager.update(
      User,
      { id: user.id },
      { password: await hashPassword('Changed-Credential-73!') },
    );
    await expect(
      orm.transaction((manager) => LockedAuthContext.lock(manager, proof)),
  ...
  it('increments once for multiple credential fields and excludes pending MFA bookkeeping', async () => {
    ...
    await orm.manager.update(
      User,
      { id: user.id },
      { mfaSecret: 'pending', lastUsedTotpStep: '1' },
    );
    expect((await orm.manager.findOneByOrFail(User, { id: user.id })).credentialVersion).toBe(1);
    await orm.manager.update(
      User,
      { id: user.id },
      { mfaEnabled: true, mfaSecret: 'enabled', isActive: false },
    );
    expect((await orm.manager.findOneByOrFail(User, { id: user.id })).credentialVersion).toBe(2);
  ...
  it('keeps cutoff monotonic and rolls credential changes back with their transaction', async () => {
    ...
    expect(
      (await orm.manager.findOneByOrFail(User, { id: user.id }))
        .accessTokenInvalidBeforeEpochSeconds,
    ).toBe(100);
    await expect(
      orm.transaction(async (manager) => {
  ...
  async function authenticationStack(
    hashed: boolean,
    maxSessions = 3,
  ): Promise<{
    ...
    const config = new ConfigService({
      ...
      JWT_KEY_ID: 'authentication-contract',
      JWT_EXPIRES_IN: '15m',
      MFA_ENCRYPTION_KEY: '11'.repeat(32),
      JWT_AUDIENCE: 'aquaculture-platform',
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        ...repositories.map((entity) => ({
          ...
          useValue: new Repository<ObjectLiteral>(entity, orm.manager),
        })),
        ...
        { provide: JwtService, useValue: jwt },
        {
          ...
          useValue: new OutboxPublisher(AuthOutbox, {
        ...
        },
        {
          ...
          useValue: new BestEffortEventPublisher({
    ...
    return {
      ...
      account: module.get(AccountService),
      internal: module.get(InternalAuthController),
      outbox: module.get(OutboxPublisher),
      tokens: module.get(TokenService),
      jwt,
      audit: module.get(AuditLogService),
      invalidation: module.get(DurableUserTokenInvalidationService),
      accessInvalidation: module.get(DurableAccessTokenInvalidationService),
      lifecycle: module.get(TenantProvisioningCommandService),
  ...
  async function principal(role: Role): Promise<User> {
    const tenant =
      role === Role.SUPER_ADMIN
        ? null
        : await orm.manager.save(
            Tenant,
            orm.manager.create(Tenant, {
  ...
  async function seedResetAction(user: User): Promise<ActionToken> {
    return new BypassRlsService().withBypass('auth-contract:seed-reset', () =>
      orm.transaction(async (manager) => {
  ...
  it.each([
    ...
    'runs actual %s login, refresh and reset under production RLS (hashed=%s)',
    async (role, hashed) => {
      ...
      const historyBeforeReset = await migrator.query<
        Array<{ familyId: string; isRevoked: boolean }>
      ...
      expect(
        await requestContextStorage.run({ tenantId: unrelatedTenant }, () =>
          orm.manager.count(RefreshToken, { where: { userId: user.id } }),
        ),
      ).toBe(0);
      if (user.tenantId) {
        expect(
          await requestContextStorage.run({ tenantId: user.tenantId }, () =>
            orm.manager.count(RefreshToken, { where: { userId: user.id } }),
          ),
        ).toBe(2);
      }
      ...
      expect(await authentication.resetPassword(action.id, 'Recovered-Credential-73!')).toEqual({
        success: true,
        loginRequired: true,
      });
      ...
      expect(resetUser.accessTokenInvalidBeforeEpochSeconds).toBeGreaterThanOrEqual(
        claims.iat ?? 0,
      );
      ...
      const oldHistory = await migrator.query<Array<{ isRevoked: boolean; revokedReason: string }>>(
        'SELECT "isRevoked", "revokedReason" FROM auth.refresh_tokens WHERE "userId" = $1',
        [user.id],
      );
      expect(oldHistory).toHaveLength(2);
      expect(
        oldHistory.every((row) => row.isRevoked && row.revokedReason === 'Password reset'),
      ).toBe(true);
      ...
      const fresh = await authentication.login({
        email: user.email,
        password: 'Recovered-Credential-73!',
      });
      ...
      expect(
        await migrator.query(
          'SELECT action FROM auth.audit_logs WHERE "entityId" = $1 AND action = $2',
          [user.id, 'PASSWORD_RESET_SUCCESS'],
        ),
      ).toHaveLength(1);
      mint.mockRestore();
  ...
  async function verifiedSession(
    jwt: JwtService,
    accessToken: string,
  ): Promise<OriginatingAccessSession> {
  ...
  it.each([Role.TENANT_ADMIN, Role.SUPER_ADMIN])(
    'logs out %s history under authenticated RLS even after account deactivation',
    async (role) => {
      const user = await principal(role);
      ...
      expect(history.every((row) => row.isRevoked && row.revokedReason === 'User logged out')).toBe(
        true,
      );
      expect(
        await orm.manager.find(AuthOutbox, {
          where: { eventType: 'AccessTokenInvalidationRequested' },
        }),
      ).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ payload: expect.objectContaining({ targetJti: session.jti }) }),
        ]),
      );
      await expect(authentication.refreshToken(first.refreshToken)).rejects.toThrow();
  ...
  it('revalidates platform cleanup identity under the User lock before changing history', async () => {
    ...
    await expect(
      authentication.logout({ ...session, role: Role.SUPER_ADMIN, tenantId: null }),
    ).rejects.toThrow();
    expect(
      await migrator.query(
        'SELECT id FROM auth.refresh_tokens WHERE "userId" = $1 AND NOT "isRevoked"',
        [user.id],
      ),
    ).toHaveLength(1);
  });
  ...
  it('platform password change revokes full history and persists cutoff under its authenticated credential proof', async () => {
    ...
    await expect(
      account.changeMyPassword(user.id, {
        currentPassword: password,
        newPassword: 'Changed-Platform-91!',
      }),
    ...
    expect(history).toHaveLength(2);
    expect(history.every((row) => row.isRevoked && row.revokedReason === 'Password changed')).toBe(
      true,
    );
    const current = await orm.manager.findOneByOrFail(User, { id: user.id });
    ...
    await expect(authentication.refreshToken(second.refreshToken)).rejects.toThrow();
    expect(
      await migrator.query('SELECT id FROM auth.audit_logs WHERE "entityId" = $1 AND action = $2', [
        user.id,
        'PASSWORD_CHANGED',
      ]),
    ).toHaveLength(1);
  ...
  function notificationRequest(tenantId: User['tenantId']): TenantRequest {
    return stub<TenantRequest>({
      verifiedIdentity: {
  ...
  it.each([Role.TENANT_ADMIN, Role.SUPER_ADMIN])(
    'persists %s recovery and resolves its emailed action under canonical RLS',
    async (role) => {
      const user = await principal(role);
      ...
      const actions = await migrator.query<Array<{ id: string }>>(
        'SELECT id FROM auth.action_tokens WHERE "userId" = $1 AND status = $2',
        [user.id, ActionTokenStatus.ACTIVE],
      );
      expect(actions).toHaveLength(1);
      const action = actions[0];
      if (!action) throw new Error('Reset request did not persist its action');
      const events = await orm.manager.find(AuthOutbox, {
        where: { aggregateId: user.id, eventType: 'PasswordResetRequested' },
      });
      ...
      await expect(
        internal.getActionTokenUrl(action.id, notificationRequest(user.tenantId)),
      ).resolves.toEqual({ actionUrl: `https://app.example.test/reset-password/${action.id}` });
      await expect(
        internal.getActionTokenUrl(action.id, notificationRequest(randomUUID())),
      ).rejects.toThrow();
      if (user.tenantId) {
        await expect(
          internal.getActionTokenUrl(action.id, notificationRequest(null)),
        ).rejects.toThrow();
      }
      expect(await orm.manager.count(ActionToken, { where: { userId: user.id } })).toBe(0);
      await expect(
        authentication.resetPassword(action.id, 'Recovered-Credential-73!'),
  ...
  it('rolls back the platform recovery credential when its durable delivery event cannot be appended', async () => {
    ...
    const failure = jest
      ...
      .mockRejectedValueOnce(new Error('Recovery delivery unavailable'));
    // The public response remains non-enumerating even when persistence fails.
    await authentication.initiatePasswordReset(user.email);
    expect(
      (await orm.manager.findOneByOrFail(User, { id: user.id })).passwordResetToken,
    ).toBeNull();
    expect(
      await migrator.query('SELECT id FROM auth.action_tokens WHERE "userId" = $1', [user.id]),
    ).toHaveLength(0);
  ...
  it('rolls back the actual refresh INSERT and successful-login bookkeeping when the audit append fails', async () => {
    ...
    const failure = jest
      .spyOn(audit, 'log')
      .mockRejectedValueOnce(new Error('audit append unavailable'));
    await expect(authentication.login({ email: user.email, password })).rejects.toThrow(
      'audit append unavailable',
    );
    expect(
      await migrator.query('SELECT id FROM auth.refresh_tokens WHERE "userId" = $1', [user.id]),
    ).toHaveLength(0);
  ...
  function transactionPause(): TransactionPause {
    let identify: (runner: QueryRunner) => void = () => {
      throw new Error('Pause was not initialized');
    };
    const ready = new Promise<QueryRunner>((resolve) => {
      identify = resolve;
    });
    const gate = signal();
    return {
    ...
    const spy = jest.spyOn(audit, 'log').mockImplementation(async (dto, manager) => {
      ...
      if (dto.action === action) {
        if (!(manager instanceof EntityManager))
          throw new Error('Audit is outside its mutation transaction');
  ...
  function pauseAccessInvalidation(
    service: DurableAccessTokenInvalidationService,
  ): TransactionPause {
    const pause = transactionPause();
    const enqueue = service.enqueue.bind(service);
    const spy = jest.spyOn(service, 'enqueue').mockImplementation(async (manager, intent) => {
  ...
  it('a login that wins the User lock commits first and reset terminally revokes its complete family', async () => {
    ...
    const [login, reset] = await orderedRace(
      ...
      pauseAudit(audit, 'LOGIN_SUCCESS'),
  ...
  it('a reset that wins the User lock rejects a login authenticated against the old password', async () => {
    ...
    const [reset, login] = await orderedRace(
      () => authentication.resetPassword(action.id, 'Reset-Racing-Credential-91!'),
      pauseAudit(audit, 'PASSWORD_RESET_SUCCESS'),
      () => authentication.login({ email: user.email, password }),
    );
    expect(valueOf(reset)).toEqual({ success: true, loginRequired: true });
    expect(login).toEqual({
      status: 'rejected',
      reason: expect.objectContaining({ message: expect.stringContaining('credentials changed') }),
    });
    expect(
      await migrator.query('SELECT id FROM auth.refresh_tokens WHERE "userId" = $1', [user.id]),
    ).toHaveLength(0);
    expect(
      await migrator.query('SELECT status FROM auth.action_tokens WHERE id = $1', [action.id]),
  ...
  it('suspension waits for the winning login, retries its serializable snapshot and revokes that committed family', async () => {
    ...
    const command = {
      ...
      reason: 'Authentication lock contract',
    };
    ...
    expect(
      await migrator.query(
        'SELECT status FROM auth.tenant_command_receipts WHERE "operationId" = $1',
        [command.operationId],
      ),
  ...
  it('a suspension that wins the Tenant lock prevents a waiting login from minting', async () => {
    ...
    const [suspension, login] = await orderedRace(
      () =>
        lifecycle.suspendTenant({
          operationId: randomUUID(),
          tenantId,
          actor: { id: randomUUID(), type: 'user' },
          reason: 'Authentication lock contract',
        }),
      pauseUserInvalidation(invalidation),
      () => authentication.login({ email: user.email, password }),
    );
    expect(valueOf(suspension)).toMatchObject({ status: TenantStatus.SUSPENDED });
  ...
  it('logout that wins the User lock closes old history while a later password login establishes one new family', async () => {
    ...
    const original = await authentication.login({ email: user.email, password });
    const origin = await verifiedSession(jwt, original.accessToken);
    const [logout, login] = await orderedRace(
      () => authentication.logout(origin),
      pauseAccessInvalidation(accessInvalidation),
      () => authentication.login({ email: user.email, password }),
    );
    expect(valueOf(logout)).toBe(true);
    const admitted = valueOf(login);
    ...
    const history = await migrator.query<
      Array<{ familyId: string; isRevoked: boolean; revokedReason: string }>
    >(
      'SELECT "familyId", "isRevoked", "revokedReason" FROM auth.refresh_tokens WHERE "userId" = $1',
      [user.id],
    );
    expect(history).toHaveLength(2);
    ...
    await expect(authentication.refreshToken(admitted.refreshToken)).resolves.toMatchObject({
      accessToken: expect.any(String),
    });
  });

  async function webAuthnServiceFor(
    user: User,
    challenge: string,
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            findOne: (options: FindOneOptions<WebAuthnCredential>) =>
              orm.manager.findOne(WebAuthnCredential, options),
          },
        },
        { provide: ConfigService, useValue: new ConfigService({ WEBAUTHN_RP_ID: 'localhost' }) },
        {
          provide: AuditLogService,
          useValue: auditService ?? { log: jest.fn().mockResolvedValue(undefined) },
        },
        {
          provide: TokenService,
          useValue: tokenService ?? {
            assertOriginatingSessionInContext: jest.fn().mockResolvedValue(undefined),
            generateTokensInContext: jest
              .fn()
              .mockRejectedValue(new Error('Stale ceremony reached mint')),
          },
        },
        {
          ...
          useValue: {
            getdel: jest
              ...
              .mockResolvedValueOnce(
                JSON.stringify({ challenge, userId: user.id, type, createdAt: Date.now() }),
              ),
          },
        },
        { provide: DataSource, useValue: orm },
  ...
  it('public MFA proof binds tenant RLS before actual refresh mint', async () => {
    ...
    const session: OriginatingAccessSession = {
      sub: claims.sub,
      role: claims.role,
      tenantId: claims.tenantId,
      jti: claims.jti,
    ...
    expect(await jwt.verifyAsync<JwtPayload>(authenticated.accessToken)).toMatchObject({
      sub: user.id,
      tenantId: user.tenantId,
      mfaVerified: true,
    });
  ...
  it('public WebAuthn proof binds tenant RLS before actual refresh mint', async () => {
    ...
    await orm.manager.insert(WebAuthnCredential, {
      userId: user.id,
      credentialId,
    ...
    jest.mocked(verifyAuthenticationResponse).mockResolvedValueOnce({
      verified: true,
      authenticationInfo: {
    ...
    expect(await jwt.verifyAsync<JwtPayload>(result.accessToken)).toMatchObject({
      sub: user.id,
      tenantId: user.tenantId,
      mfaVerified: true,
    });
  ...
  it.each(['registration', 'authentication'] as const)(
    'reset serializes against a verified WebAuthn %s and rejects the old proof',
    async (ceremony) => {
      const user = await newUser();
      ...
      const registration: WebAuthnRegisterCredentialInput = {
        challenge,
        credentialId: randomUUID(),
        currentPassword: password,
      ...
      const authentication: WebAuthnVerifyLoginInput = {
        challenge,
        credentialId: credential.credentialId,
        authenticatorData: 'YXV0aA',
        clientDataJSON: 'e30',
      ...
      const attempt =
        ceremony === 'registration'
          ? service.registerCredential(session, registration)
          : service.verifyLogin(authentication);
      ...
      try {
        await Promise.race([
          verified.promise,
          attempt.then(() => {
            throw new Error('Ceremony completed before verification');
          }),
        ]);
        await LockedAuthContext.lock(reset.manager, user.id);
        await reset.manager.update(User, user.id, {
          password: await hashPassword('Reset-Racing-Credential-91!'),
        });
        ...
        expect(await outcome).toEqual({
          error: expect.objectContaining({
            message: expect.stringContaining('credentials changed'),
          }),
        });
  ...
  it('a credential removed under the User lock cannot be recreated by a verified login', async () => {
    ...
    const attempt = service.verifyLogin({
      challenge,
      credentialId: credential.credentialId,
      authenticatorData: 'YXV0aA',
      clientDataJSON: 'e30',
    ...
    try {
      ...
      expect(await outcome).toEqual({
        error: expect.objectContaining({ message: expect.stringContaining('Credential changed') }),
      });
      expect(await orm.manager.count(WebAuthnCredential, { where: { userId: user.id } })).toBe(0);
    } finally {
      allowVerification.release();
  ...
  it('replays without resetting positive versions and rejects an incompatible named constraint', async () => {
    ...
    try {
      ...
      await runner.query(
        'ALTER TABLE auth.users DROP CONSTRAINT "CHK_users_credential_version_positive"',
      );
      await runner.query(
        'ALTER TABLE auth.users ADD CONSTRAINT "CHK_users_credential_version_positive" CHECK ("credentialVersion" > -1)',
      );
