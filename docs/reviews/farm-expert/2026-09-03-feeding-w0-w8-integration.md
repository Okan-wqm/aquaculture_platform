...
The PR body listed ten findings as still open. Seven of them ARE closed by commits on the branch and
the trailers are carried through the rebase unchanged; they are recorded below with the commit that
closes each, so a reader is not sent looking for work that is already done. Three are not closed by
any commit on the branch and are not closed here either. This document states, for each of those
three, the owner, the deadline and the criterion a closure must satisfy.
...
The blocker is the ceremony, not the engineering. Each entry below is written so the fix can be made
in one sitting once the rows are in the registry.
...
kilograms rewrites the cost of the original meal.** State: OPEN. Owner: `farm-expert`. Deadline:
2026-09-19.
...
Root cause: cost is recomputed from a mutable master-data field instead of being treated as a ledger
fact that accretes. A feed whose price moved between the meal and its correction therefore restates
history, and the restatement flows into batch `totalFeedCost` and every finance derivation that
reads it.
...
unreachable through the DTO, and `countDerived` is never read by any caller.** State: OPEN. Owner:
`farm-expert`. Deadline: 2026-09-26.
...
constraint), an event-contract change with its JSON Schema, and regenerated client artifacts — which
is why this one carries the later deadline of the three.

## Closed on the branch — recorded here so the PR body's list is not read as current

| ID              | Closed by                                                                                                                                                           |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FARM-HIGH-311   | `fix(farm): band/yem/FCR çözümünü tek servise indir, canlı çözümü plana yaz`                                                                                        |
| FARM-HIGH-312   | `fix(farm): yem düşümünü çok-lotlu FEFO tahsisine çevir…` + `fix(farm): make the unit-growth lock protocol impossible to compose wrongly`                           |
...
## Integration note (2026-09-04): renumbered ids, rewritten commit hashes, one dead-letter route

Nine of this branch's registry ids had been allocated by main to unrelated findings after the fork,
so the branch's rows and every `Closes:` trailer were renumbered before the merge:
`FARM-HIGH-243→309`, `246→310`, `247→311`, `248→312`, `249→313`, `FARM-MEDIUM-297→314`,
`FARM-HIGH-300→315`, `SEC-HIGH-055→157`, `SENSOR-MEDIUM-058→107`. The rewrite changed every commit
hash on the branch; the hashes in this document are the merged ones.

W7's inbound dead-letter shelf (`DeadLetterModule`, `TypeormDeadLetterSink`,
`message-disposition.ts`, the alert/notification `event_dlq` migrations) is NOT carried. Between
this branch's fork and its merge, PR #1338 gave `NatsEventBus` a platform-wide terminal route: after
`NATS_DLQ_AFTER_DELIVERIES` deliveries the bus publishes a `DlqEnvelope` to the `AQUACULTURE_DLQ`
stream and acks only on PubAck (`handleMessageFailure`, replay tooling in
`tools/scripts/telemetry-dlq-replay.ts`, SENSOR-HIGH-093). Two terminal sinks for one failure would
be two authorities, so the bus route is the only one. What W7 established still holds and is kept:
the `reproducible` / durable classification in `event-delivery-semantics.ts`, the handlers that
rethrow only for durable signals, and the in-app delivery receipt index (FARM-LOW-282).
FARM-MEDIUM-260 is therefore closed by the platform route, not by a per-service table.
