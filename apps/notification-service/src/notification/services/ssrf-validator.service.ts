 ...
 * @description Validates URLs for webhook/notification calls to prevent
 * Server-Side Request Forgery (SSRF) attacks.
 *
 * Defense layers:
 * 1. Protocol allowlist (https only; http allowed in development)
 ...
 * 5. Redirect policy: never follow redirects (redirect: 'error')
 *
 ...
 *
 * @see PLAT-CRITICAL-006 (SSRF finding)
 */
...
/** Development-only: also allow http. */
const DEV_ALLOWED_PROTOCOLS = new Set(['http:', 'https:']);
...
const IPV4_DENY_CIDRS: Array<{ network: number; mask: number }> = [
  ...
  { network: ipToNumber('198.18.0.0'), mask: cidrMask(15) },
...
export class SsrfValidatorService {
  ...
  async validateUrl(url: string): Promise<SsrfValidationResult> {
    ...
    if (!allowedProtocols.has(parsed.protocol)) {
      return {
        safe: false,
        reason: `Protocol "${parsed.protocol}" not allowed. Only https: is permitted.`,
      };
    }
