...
// ============================================================================
const S = {
  pH: 8.5, tan: 4.0, h2s: 0.5, tempC: 20, sal: 0, vol: 1,
  alkMg: 100,
  nh3Lim: 0.0125, h2sLim: 5, co2Lim: 20,
};
...
describe('Risk Minimizasyonu — pH taraması', () => {
  ...
  it('Hassas pH taraması — optimal bölge etrafında 0.01 adımla', () => {
    ...
    for (let pH = 6.0; pH <= 8.5; pH += 0.01) {
      const r = riskAt(pH, alkMeq);
      if (r.maxR < bestMax) { bestMax = r.maxR; bestPH = pH; }
    }

    console.log('\n══════════════════════════════════════════════════════════════════');
    console.log(`  HASSAS TARAMA — pH ${(bestPH - 0.3).toFixed(2)} — ${(bestPH + 0.3).toFixed(2)} arası`);
    console.log('══════════════════════════════════════════════════════════════════');
    console.log('  pH    | NH₃ mg/L  | ratio | H₂S µg/L | ratio | CO₂ mg/L | ratio | maxR  | baskın risk');
    console.log('  ──────|───────────|───────|──────────|───────|──────────|───────|───────|───────────');

    for (let pH = bestPH - 0.3; pH <= bestPH + 0.3; pH += 0.02) {
      const r = riskAt(pH, alkMeq);
      const dominant = r.rNH3 >= r.rH2S && r.rNH3 >= r.rCO2 ? 'NH₃'
        : r.rH2S >= r.rNH3 && r.rH2S >= r.rCO2 ? 'H₂S' : 'CO₂';
    ...
    for (let pH = 6.0; pH <= 8.5; pH += 0.001) {
      ...
      if (diff < eqDiff) { eqDiff = diff; eqPH = pH; }
    }
  ...
  it('Dozlama: mevcut pH → optimal pH için kaç gram ne eklemeliyiz', () => {
    ...
    for (let pH = 6.0; pH <= 8.5; pH += 0.001) {
      const r = riskAt(pH, alkMeq);
      if (r.maxR < optMax) { optMax = r.maxR; optPH = pH; }
    }
    ...
    for (const frac of dilutions) {
      ...
      console.log(`    NH₃        : ${nh3.toFixed(6)} mg/L (${(nh3 / S.nh3Lim).toFixed(2)}x limit)`);
      console.log(`    H₂S        : ${h2s.toFixed(4)} µg/L (${(h2s / S.h2sLim).toFixed(2)}x limit)`);
      console.log(`    CO₂        : ${co2.toFixed(4)} mg/L (${(co2 / S.co2Lim).toFixed(2)}x limit)`);
      console.log(`    CO₂ dozlama: ${dose > 0 ? dose.toFixed(1) + ' gram' : 'gerekmiyor'}`);
      console.log(`    ${allSafe ? '✅ TÜM LİMİTLER ALTINDA — GÜVENLİ' : '⚠️  Hala limit aşımı var'}`);
    }
  });
  ...
  it('TAN dilüsyon eğrisi — kaç mg/L TAN\'da güvenli bölge açılır', () => {
    ...
    console.log('══════════════════════════════════════════════════════════════════');
    console.log('  TAN mg/L | opt pH | NH₃ mg/L   | NH₃/lim | H₂S µg/L | H₂S/lim | CO₂ mg/L | maxR  | Durum');
    console.log('  ────────|────────|────────────|─────────|──────────|─────────|──────────|───────|──────');
  ...
  it('SONUÇ RAPORU', () => {
    ...
    let safePH = 7.0, safeMax = Infinity;
    for (let pH = 5.5; pH <= 9.0; pH += 0.001) {
      const nh3 = calcNH3(safeTAN, pH, S.tempC, S.sal);
      const h2s = calcH2S(totalSulfide, pH, S.tempC, S.sal);
      const co2 = co2Level(alkMeq, pH, S.tempC, S.sal);
      const mx = Math.max(nh3 / S.nh3Lim, h2s / S.h2sLim, co2 / S.co2Lim);
      if (mx < safeMax) { safeMax = mx; safePH = pH; }
    }
    ...
    console.log(`║     NH₃: ${opt.nh3.toFixed(4)} mg/L → ${(calcNH3(S.tan, S.pH, S.tempC, S.sal) / S.nh3Lim).toFixed(1)}x limit aşımı                      ║`);
    console.log('║                                                                   ║');
    console.log('║  📍 SADECE pH AYARI İLE (su değişimi yok):                        ║');
    console.log(`║     Optimal pH     : ${optPH.toFixed(3)}                                        ║`);
    console.log(`║     NH₃            : ${opt.nh3.toFixed(6)} mg/L (${opt.rNH3.toFixed(2)}x limit)             ║`);
    console.log(`║     H₂S            : ${opt.h2s.toFixed(4)} µg/L (${opt.rH2S.toFixed(2)}x limit)               ║`);
    console.log(`║     CO₂            : ${opt.co2.toFixed(4)} mg/L (${opt.rCO2.toFixed(2)}x limit)               ║`);
    console.log(`║     Risk azalma    : ${((1 - optMax / (calcNH3(S.tan, S.pH, S.tempC, S.sal) / S.nh3Lim)) * 100).toFixed(0)}%                                       ║`);
    console.log('║                                                                   ║');
    console.log('║  ✅ GÜVENLİ BÖLGEYE GİRMEK İÇİN:                                 ║');
    console.log(`║     TAN eşiği      : ≤ ${safeTAN.toFixed(2)} mg/L                                  ║`);
    console.log(`║     Su değişimi    : %${waterChange.toFixed(0)} (${(waterChange / 100 * S.vol * 1000).toFixed(0)} litre)                              ║`);
    console.log(`║     Sonra pH ayarı : ${safePH.toFixed(3)} (${co2Dose.toFixed(1)} g CO₂ ekle)                      ║`);
    console.log(`║     Sonuç NH₃      : ${safeNH3.toFixed(6)} mg/L (${(safeNH3 / S.nh3Lim).toFixed(2)}x limit)             ║`);
    console.log(`║     Sonuç H₂S      : ${safeH2S.toFixed(4)} µg/L (${(safeH2S / S.h2sLim).toFixed(2)}x limit)               ║`);
    console.log(`║     Sonuç CO₂      : ${safeCO2.toFixed(4)} mg/L (${(safeCO2 / S.co2Lim).toFixed(2)}x limit)               ║`);
    console.log('║                                                                   ║');
    console.log('║  🔧 ACİL AKSİYON PLANI:                                           ║');
    console.log(`║     1. %${waterChange.toFixed(0)} su değişimi yap (${(waterChange / 100 * S.vol * 1000).toFixed(0)}L temiz su)                  ║`);
    console.log(`║     2. ${co2Dose.toFixed(1)} gram CO₂ ekle (pH ${S.pH} → ${safePH.toFixed(2)})                      ║`);
