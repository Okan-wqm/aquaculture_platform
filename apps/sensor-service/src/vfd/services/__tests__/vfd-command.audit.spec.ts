...
function makeService(repo: Repo, writeRegister?: jest.Mock<Promise<WriteResult>>) {
  ...
  const registerMapping = {
    getControlWordMapping: jest
      .fn()
      .mockResolvedValue({ registerAddress: 49999, scalingFactor: 1 }),
    getSpeedReferenceMapping: jest
      .fn()
      .mockResolvedValue({ registerAddress: 50000, scalingFactor: 1 }),
  };
