...
describe('INVARIANT: tenant creation route SSOT', () => {
  ...
  it('keeps the public accepted response narrow and canonical', () => {
    ...
    const workflow = readRepoFile('apps/admin-api-service/src/tenant/services/tenant-provisioning-workflow.service.ts');
  ...
  it('does not expose auth-service GraphQL createTenant as an external create path', () => {
    const resolver = readRepoFile('apps/auth-service/src/modules/tenant/resolvers/tenant.resolver.ts');
...
describe('INVARIANT: auth-service owns tenant lifecycle commands', () => {
  ...
  it('registers auth-service NATS handlers for the canonical lifecycle subjects', () => {
    const handler = readRepoFile('apps/auth-service/src/modules/tenant/handlers/auth-admin-nats.handler.ts');
  ...
  it('persists auth tenant command receipts for idempotency and payload-hash conflict checks', () => {
    const migration = readRepoFile('apps/auth-service/src/migrations/1800100000000-TenantCommandReceiptLedger.ts');
    const service = readRepoFile('apps/auth-service/src/modules/tenant/services/tenant-provisioning-command.service.ts');
  ...
  it('does not let admin-api or auth GraphQL bypass tenant command receipts', () => {
    const updateHandler = readRepoFile('apps/admin-api-service/src/tenant/handlers/update-tenant.handler.ts');
    const resolver = readRepoFile('apps/auth-service/src/modules/tenant/resolvers/tenant.resolver.ts');
...
describe('INVARIANT: billing provisioning is confirmed by owner receipt evidence', () => {
  it('uses billing-service request-reply command receipts instead of subscription-requested outbox enqueue', () => {
    ...
    const migration = readRepoFile('apps/billing-service/src/database/migrations/1800400000000-BillingCommandReceipts.ts');
    const billingModule = readRepoFile('apps/billing-service/src/billing/billing.module.ts');
    const billingHandler = readRepoFile('apps/billing-service/src/billing/handlers/billing-admin-nats.handler.ts');
    const workflow = readRepoFile('apps/admin-api-service/src/tenant/services/tenant-provisioning-workflow.service.ts');
    ...
    expect(billingHandler).toContain('@MessagePattern(BILLING_ADMIN_COMMAND_SUBJECTS.PROVISION_TENANT_SUBSCRIPTION)');
    ...
    expect(billingHandler).toContain('Active billing subscription exists but its module digest does not match');
    expect(billingHandler).toContain('markBillingReceiptSucceeded');
    expect(billingHandler).toContain('Enterprise provisioning requires an approved billing quote or custom plan');
    ...
    const createSubscriptionIndex = billingHandler.indexOf('const subscription = await this.createProvisioningSubscription');
    ...
    expect(workflow).toContain('Billing provisioning completed without subscription receipt evidence');
    expect(workflow.indexOf("'create_subscription'")).toBeLessThan(workflow.indexOf("'activate_tenant'"));
    expect(workflow.indexOf("'activate_tenant'")).toBeLessThan(workflow.indexOf("'publish_tenant_provisioned'"));
...
describe('INVARIANT: notification commands have real consumers and ACL parity', () => {
  it('boots notification-service NATS command consumers for all canonical outbound channels', () => {
    ...
    const handler = readRepoFile('apps/notification-service/src/notification/event-handlers/notification-command.handler.ts');
    const taskHandler = readRepoFile('apps/notification-service/src/notification/event-handlers/task-event.handler.ts');
    const module = readRepoFile('apps/notification-service/src/notification/notification.module.ts');
    const dispatcher = readRepoFile('apps/notification-service/src/notification/services/notification-dispatcher.service.ts');
    const receiptMigration = readRepoFile('apps/notification-service/src/database/migrations/1800100000000-CreateNotificationCommandReceipts.ts');
    ...
    for (const subject of [
      ...
      'commands.notification.sendSms',
      'commands.notification.sendWebhook',
    ]) {
      expect(contracts).toContain(subject);
    ...
    for (const constant of ['SEND_EMAIL', 'SEND_PUSH', 'SEND_SMS', 'SEND_WEBHOOK']) {
    ...
    expect(handler).toContain('logRef: `userId:${recipient.ref}`');
  ...
  it('uses recipient and template references instead of raw delivery payloads', () => {
    ...
    expect(handler).toContain('Notification command recipientRef must not contain raw recipient material');
...
describe('INVARIANT: platform event registry is the lifecycle SSOT', () => {
  ...
  it('orders onboarding ack before final tenant provisioned aliases', () => {
    const workflow = readRepoFile('apps/admin-api-service/src/tenant/services/tenant-provisioning-workflow.service.ts');
    const migration = readRepoFile('apps/admin-api-service/src/migrations/1800400000000-TenantProvisioningWorkflow.ts');
    const farmHandler = readRepoFile('apps/farm-service/src/water-quality/event-handlers/tenant-onboarding.event-handler.ts');
    ...
    expect(farmHandler).toContain("createBaseEvent<TenantOnboardingFailedEvent>('TenantOnboardingFailed'");
...
describe('INVARIANT: admin surfaces do not carry raw invite or reset token material', () => {
  ...
  it('keeps tenant-admin invite command results tokenless and delivery-event based', () => {
    const contracts = readRepoFile('libs/event-contracts/src/tenant-commands.ts');
    const handler = readRepoFile('apps/auth-service/src/modules/tenant/handlers/auth-admin-nats.handler.ts');
    const lifecycle = readRepoFile('apps/auth-service/src/modules/tenant/services/user-lifecycle.service.ts');
    const actionTokenEntity = readRepoFile('apps/auth-service/src/modules/authentication/entities/action-token.entity.ts');
    const internalAuth = readRepoFile('apps/auth-service/src/modules/authentication/controllers/internal-auth.controller.ts');
...
describe('INVARIANT: destructive tenant schema cleanup requires workflow proof', () => {
  it('requires CleanupDropProof at the shared deleteTenantSchema boundary', () => {
    const schemaManager = readRepoFile('libs/backend-common/src/database/schema-manager.service.ts');
  ...
  it('keeps SchemaManagerService schema deletion fail-closed under db-migrate authority', () => {
    ...
    expect(schemaManager).toContain('runtime services must write a cleanup request ledger entry instead');
  });

  it('mints rollback and deprovision proofs before admin cleanup callers delete schemas', () => {
    const provisioning = readRepoFile('apps/admin-api-service/src/tenant/services/tenant-provisioning.service.ts');
    const provisionerSql = readRepoFile('apps/db-migrate/src/sql/platform-bootstrap/009-tenant-schema-provisioner.sql');
    const provisionerWorker = readRepoFile('apps/db-migrate/src/tenant-schema-provisioner.ts');
    const backupController = readRepoFile('apps/admin-api-service/src/database-management/controllers/backup.controller.ts');
    const backupService = readRepoFile('apps/admin-api-service/src/database-management/services/backup-restore.service.ts');
    ...
    expect(provisionerSql).toContain('GRANT EXECUTE ON FUNCTION platform.request_tenant_schema_deletion(UUID, UUID, TEXT, JSONB) TO admin_service');
    expect(provisionerSql).not.toContain('GRANT EXECUTE ON FUNCTION platform.request_tenant_schema_deletion(UUID, UUID, TEXT, JSONB) TO auth_service');
...
describe('INVARIANT: password reset state is auth-service owned', () => {
  ...
  it('registers auth-service handlers for public password reset commands', () => {
    const contracts = readRepoFile('libs/event-contracts/src/tenant-commands.ts');
    const handler = readRepoFile('apps/auth-service/src/modules/authentication/controllers/auth-public-nats.handler.ts');
    const module = readRepoFile('apps/auth-service/src/modules/authentication/authentication.module.ts');
    ...
    expect(handler).toContain('@MessagePattern(AUTH_PUBLIC_COMMAND_SUBJECTS.REQUEST_PASSWORD_RESET)');
