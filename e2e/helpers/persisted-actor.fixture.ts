...
import {
  ...
  FIXTURE_PASSWORD,
  loginFixtureUser,
} from './real-auth.fixture';

type PersistedActorOptions = Pick<
  TestTokenOptions,
  'userId' | 'sub' | 'email' | 'role' | 'roles' | 'tenantId'
>;
...
export async function issueTestToken(options: PersistedActorOptions = {}): Promise<string> {
  ...
  try {
    if (tenantId) {
      const existing = await db.query<{ id: string }>('SELECT id FROM auth.tenants WHERE id = $1', [
        tenantId,
      ]);
    ...
    const existing = await db.query<{ email: string; role: string; tenantId: string | null }>(
      'SELECT email, role, "tenantId" FROM auth.users WHERE id = $1',
      [userId],
    );
    const user = existing.rows[0];
    if (user) {
      if (user.role !== role || user.tenantId !== tenantId)
        throw new Error('Fixture identity disagrees with persisted actor');
