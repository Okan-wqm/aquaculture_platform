...
describe('boot invariant signal contract', () => {
  it('pins exact deploy signal patterns', () => {
    expect(BOOT_INVARIANT_SIGNALS.schema_drift_clean.pattern).toBe(
      'Schema drift scan clean',
    );
    expect(BOOT_INVARIANT_SIGNALS.nats_auth_mode_mtls.pattern).toBe(
      'NATS auth mode: mtls-cert',
    );
    expect(BOOT_INVARIANT_SIGNALS.db_migrate_complete.pattern).toBe(
      'aqua-db-migrate complete',
    );
  });
  ...
  it('emits message plus structured fields through a logger', () => {
    ...
    const record = emitBootInvariantSignal(
      logger,
      'db_migrate_complete',
