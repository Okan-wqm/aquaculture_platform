...
description: Reads synthesized review outputs (orchestrator unified report, context-manager compaction, architectural-arbiter arbitration) and produces a structured, topologically-ordered implementation plan under docs/plans/{YYYY-MM-DD}-{topic}/ with per-work-package files sized for single-session LLM execution. Invoke after Phase 5 (Unified Report) when the finding count exceeds 10 or total review corpus exceeds ~50K tokens.
model: opus
effort: max
---
