 ...
 *
 * This cross-runtime contract is the single authority consumed directly by
 * config-service, its seed migration, and the admin panel. It deliberately
 * lives outside those runtimes so neither can become a second author.
 *
 ...
 * imports the same immutable value and literal types. Neither side re-declares
 * or generates a key, so drift cannot be expressed.
 *
...
export const TENANT_SETTINGS = [
  ...
  },
  {
    ...
    section: 'storage',
    valueType: 'json',
    defaultValue: '["pdf","doc","docx","xls","xlsx","csv","jpg","jpeg","png","gif"]',
...
export type TenantSettingStringKey = Extract<TenantSetting, { valueType: 'string' }>['key'];

/** Keys whose stored value is a number. */
export type TenantSettingNumberKey = Extract<TenantSetting, { valueType: 'number' }>['key'];

/** Keys whose stored value is a boolean. */
export type TenantSettingBooleanKey = Extract<TenantSetting, { valueType: 'boolean' }>['key'];

/** Keys whose stored value is a JSON string list. */
export type TenantSettingListKey = Extract<TenantSetting, { valueType: 'json' }>['key'];
