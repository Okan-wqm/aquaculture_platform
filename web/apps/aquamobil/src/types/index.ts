...
import type { AccessType, ClockMethod, AttendanceStatus, LeaveRequestStatus, TaskCategory, TaskPriority, TaskStatus, StorageItemType } from '@/generated/graphql';

export type { AccessType, ClockMethod, AttendanceStatus, LeaveRequestStatus, TaskCategory, TaskPriority, TaskStatus, StorageItemType };
 ...
 */
export interface LiceCountPayload {
  siteId: string;
  tankId: string;
  batchId?: string;
...
export interface WelfareAssessmentPayload {
  siteId: string;
  tankId: string;
  batchId?: string;
...
export interface EscapeIncidentPayload {
  siteId: string;
  tankId?: string;
  batchId?: string;
 ...
 */
export interface ChecklistItemSetPayload {
  taskId: string;
  itemId: string;
  isCompleted: boolean;
}
...
/** Messaging offline payloads — sendMessage uses SendMessagePayload, editMessage uses { id, content },
...
export type OperationPayload = (
  ...
  | { id: string }
  | ChecklistItemSetPayload
  | TransferPayload
  | CreateWaterQualityPayload
  | StockMovementPayload
  | StockTransferPayload
  | LiceCountPayload
  | WelfareAssessmentPayload
  | EscapeIncidentPayload
...
/**
 * Mirrors `input CreateWaterQualityPayload` in apps/farm-service/schema.graphql.
 *
