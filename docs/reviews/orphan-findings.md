...
**Orphan observed (separate item, not among the 9):** `ListSubEquipmentTypes` (`useSubEquipment.ts`) also selects the non-existent `SubEquipmentTypeResponse.category` (same root cause as the farm items) — tracked here for a follow-up slice.
