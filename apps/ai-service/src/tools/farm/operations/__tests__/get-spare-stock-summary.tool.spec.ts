...
import { GetSpareStockSummaryTool } from '../get-spare-stock-summary.tool';
...
describe('GetSpareStockSummaryTool', () => {
  let send: jest.Mock;
  let tool: GetSpareStockSummaryTool;
  ...
  beforeEach(() => {
    ...
    tool = new GetSpareStockSummaryTool({ send });
  });

  it('is a module-scoped farm read tool', () => {
    const metadata = tool.getMetadata();
    expect(metadata.name).toBe('get_spare_stock_summary');
  ...
  it('applies defaults and sends the contract subject with the context tenant', async () => {
    ...
    expect(send).toHaveBeenCalledWith(FARM_AI_QUERY_SUBJECTS.MAINT_STOCK_SUMMARY, {
      ...{},
