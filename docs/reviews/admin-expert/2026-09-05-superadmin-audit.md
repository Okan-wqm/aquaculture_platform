...
## SEC-CRITICAL-161 — Two internet-reachable ingresses; kernel edge controls mounted on the wrong one (ARCH-CRITICAL-000, R11)
...
**Depends on:** DATA-CRITICAL-016 (retention) — mounting AccessLog on admin-api without a working `shared.access_logs` policy grows an unbounded table.
...
## SEC-HIGH-166 — Login rate-limit tier keyed to REST paths while login is a GraphQL mutation (SA-009)
...
**Fix:** the tier is bound to the GraphQL login operation name resolved from the parsed document, sharing one declaration with the auth-service resolver; `TRUST_PROXY` becomes required for public services (SEC-CRITICAL-161).
...
## BILLING-CRITICAL-012 — Raw SQL against subscriptions; dead Stripe reconciliation; no idempotency (R8, C14)
...
## CONTRACT-CRITICAL-004 — No machine-readable FE↔BE contract; interface DTOs disarm validation (R9, C1, C2)
...
## ADMIN-CRITICAL-102 — Actor from client strings; 273 mutations unaudited; audit writer swallows (C8)
...
## ADMIN-HIGH-105 — No shared admin-panel query/mutation primitive; page-local pagination and stats (C3, C17)
...
**Fix (ordered):** mask the message path (OBS-CRITICAL-008) → ship logs → OTLP tracing → admin-api SLO rules on existing RED metrics → in-app health surfaces return `{status: 'ok' | 'unavailable'}` → delete fabricated dashboards only after Grafana replacements exist.
...
Critical path: `INFRA-HIGH-165 → DATA-CRITICAL-016 → SEC-CRITICAL-161 → SEC-CRITICAL-162 → {SEC-CRITICAL-163, SEC-HIGH-164}` and `INFRA-HIGH-165 → CONTRACT-CRITICAL-004 → BILLING-CRITICAL-011 → BILLING-CRITICAL-012`, with INFRA-CRITICAL-164 gating the destructive half of every wave.
...
**Delete with a coupled decision:** TenantConfigurationPage, ProvisioningSettingsPage, MessagingMonitoring / AiDashboard / AiPersonas pages (delete the route first); ErrorTrackingPage (delete unless a reporter is wired); `useAsyncData` (after W6); `login_attempts` / `user_sessions` / `api_usage_logs` + detectors (ADMIN-HIGH-109); `slow_query_logs` / `database_metrics` (with the `pg_stat_statements` decision); `maintenance_modes` (if maintenance moves to the gateway); `password-reset.controller.ts` (second un-rate-limited pre-auth ingress); `tenants.ts deactivate/archive` (adopt with `@Destructive` or delete both sides); 168 dead backend routes as a set after W3; K8s alert rule files (port useful rules to `droplet/rules` first); Grafana dashboards (rebuild).
...
3. **Quarantine owners and expiries:** every remaining `knownUnstableProjects` entry now names an owner and an expiry; the fleet-wide lint entries were assigned to the repository owner with a 2026-12-31 expiry and INFRA-HIGH-165 as their finding.
