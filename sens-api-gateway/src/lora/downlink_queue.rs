                ...
                .map(|(i, q)| {
                    (
                        ...
                        evictability(
                            q.item.confirmed,
                            q.item.priority,
                            now.duration_since(q.enqueued_at),
                        ),
                    )
                })
...
mod tests {
    ...
    fn per_dev_addr_cap_evicts_stale_for_a_more_valuable_newcomer() {
        ...
        let a = [0, 0, 0, 7];
        assert_eq!(q.enqueue(item_pc(a, 1, 9, false)), EnqueueOutcome::Accepted);
        assert_eq!(q.enqueue(item_pc(a, 2, 9, false)), EnqueueOutcome::Accepted);
        ...
        assert!(
            found_critical,
            "kritik onaylanmis komut cikarilmis olmamali"
        );
    }
}
