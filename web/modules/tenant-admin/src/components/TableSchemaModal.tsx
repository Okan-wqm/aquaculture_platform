  ...
  const indexInfoColumns: DataTableColumn<IndexInfo>[] = [
    {
      ...
      header: 'Index Name',
      render: (_value, idx) => (
        <span className="font-mono text-sm text-gray-700">{idx.indexName}</span>
      ),
    },
    {
      key: 'column',
      header: 'Column',
      render: (_value, idx) => (
        <span className="font-mono text-sm text-gray-600">{idx.columnName}</span>
      ),
    },
    {
      key: 'unique',
  ...
  const columnInfoColumns: DataTableColumn<ColumnInfo>[] = [
    {
      ...
      render: (_value, col) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-sm font-medium text-gray-900">{col.columnName}</span>
        </div>
      ),
    },
    {
        ...
        <>
          <span
            className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-medium ${
              col.isNullable ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'
            }`}
          >
            {col.isNullable ? 'Y' : 'N'}
          </span>
