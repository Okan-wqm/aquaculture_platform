 ...
 * a config entry with no consumer (SEC-HIGH-166). The upload bucket named
 ...
 *
 * Finding: docs/reviews/admin-expert/2026-09-05-superadmin-audit.md#SEC-HIGH-166
 */
...
describe('gateway rate-limit buckets resolve to registered routes and mutations (SEC-HIGH-166)', () => {
  // A typed double, not an unchecked cast: `stub<ConfigService>` is verified
  // against the interface it stands in for, so a signature change breaks the
  // spec instead of letting it go green on a value that checks nothing.
  const config = buildGatewayEdgeConfig(
    stub<ConfigService>({
