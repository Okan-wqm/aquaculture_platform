...
| ADMIN-HIGH-010, ADMIN-LOW-008                      | `feat(web): give the tenant admin a way to turn MFA enforcement on, and the user a way to comply` | tenant-admin Security screen + shell `MfaSetupScreen`.                                          |
...
| ADMIN-HIGH-008, ADMIN-MEDIUM-006, ADMIN-MEDIUM-007 | `feat(admin-panel): fill in the billing dashboard's five null metrics and its placeholder charts` | `GET /billing/payments/stats`, AreaChart, LineCharts, shared MetricCard.                        |
| ADMIN-MEDIUM-003, ADMIN-MEDIUM-018                 | `feat(shared-ui): make toasts actually reach the screen, from any remote`                         | `ToastProvider`, `Avatar`, parametrizable Table/ConfirmModal labels.                            |
| ADMIN-HIGH-020                                     | `security(tenant-admin): put the tenant back in two cache keys that lost it`                      | Split out of ADMIN-MEDIUM-020 — see below.                                                      |

## Deliberately not landed, with owners

### ADMIN-MEDIUM-020 — the typed-transport half of CRIT-04
...
### ADMIN-MEDIUM-009 — English-only panel strings + the CI gate
...
### ADMIN-MEDIUM-021, ADMIN-LOW-003 — the admin-panel confirm/toast sweep

**State:** IN-PROGRESS. **Owner:** `admin-expert`. **Deadline:** 2026-10-05.

The `ToastProvider` these depend on now exists (ADMIN-MEDIUM-003), but the sweep itself — replacing
