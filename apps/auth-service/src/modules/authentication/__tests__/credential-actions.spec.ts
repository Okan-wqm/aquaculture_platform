...
describe('credential action completion', () => {
  async function setup(purpose: ActionTokenPurpose): Promise<{
    ...
    const tenant = Object.assign(new Tenant(), { id: randomUUID(), status: TenantStatus.SUSPENDED, enforceMfa: true });
    const user = Object.assign(new User(), { id: randomUUID(), email: 'action@example.test',
      role: Role.MODULE_USER, tenantId: tenant.id, isActive: true, credentialVersion: 4,
      accessTokenInvalidBeforeEpochSeconds: 0, invitationToken: 'stored-action-hash',
      passwordResetToken: 'stored-action-hash', passwordResetExpires: new Date(Date.now() + 60_000) });
    const action = Object.assign(new ActionToken(), { id: randomUUID(), userId: user.id, tenantId: tenant.id,
      purpose, tokenHash: 'stored-action-hash', status: ActionTokenStatus.ACTIVE, expiresAt: new Date(Date.now() + 60_000) });
    const invitation = Object.assign(new Invitation(), { id: randomUUID(), userId: user.id, tenantId: tenant.id,
      token: action.tokenHash, email: user.email, role: user.role, status: InvitationStatus.PENDING,
    ...
    const update = jest.spyOn(manager, 'update').mockImplementation(async (entity, _criteria, values) => {
    ...
    const mfa = { isMfaAvailable: jest.fn().mockReturnValue(false), generateMfaSetupToken: jest.fn() };
    const invalidation = { enqueue: jest.fn().mockResolvedValue(undefined), applyImmediately: jest.fn() };
    const outbox = { enqueue: jest.fn().mockResolvedValue(undefined) };
    const module = await Test.createTestingModule({ providers: [
      AuthenticationService,
      ...[User, RefreshToken, ActionToken, Tenant, WebAuthnCredential].map((entity) => ({ provide: getRepositoryToken(entity), useValue: {} })),
      { provide: getRepositoryToken(Invitation), useValue: { findOne: jest.fn().mockResolvedValue(invitation) } },
      { provide: DataSource, useValue: { manager, transaction: jest.fn(async (operation: (active: typeof manager) => Promise<unknown>) => operation(manager)) } },
      { provide: JwtService, useValue: {} },
      { provide: ConfigService, useValue: { get: (_key: string, fallback?: unknown): unknown => fallback } },
      ...
      { provide: BypassRlsService, useValue: { withBypass: async <T>(_reason: string, operation: () => Promise<T>): Promise<T> => operation() } },
    ] }).compile();
    return { service: module.get(AuthenticationService), action, user, outbox, tokens, mfa, invalidation, update, remove };
  }

  it.each([ActionTokenPurpose.INVITATION, ActionTokenPurpose.PASSWORD_RESET])(
    'completes %s without new-session admission, Redis application or MFA availability', async (purpose) => {
      const test = await setup(purpose);
      const result = purpose === ActionTokenPurpose.INVITATION
      ...
      expect(test.update).toHaveBeenCalledWith(RefreshToken, { userId: test.user.id }, expect.objectContaining({ isRevoked: true }));
  ...
  it('validates an emailed opaque invitation id and rejects it after consumption', async () => {
    ...
    expect(await test.service.validateInvitation(test.action.id)).toMatchObject({ valid: true, email: test.user.email });
    await test.service.acceptInvitation(test.action.id, 'New-Credential-42!');
    expect(await test.service.validateInvitation(test.action.id)).toEqual({ valid: false });
    await expect(test.service.acceptInvitation(test.action.id, 'Another-Credential-73!')).rejects.toThrow('Invalid or expired');
  });
});
