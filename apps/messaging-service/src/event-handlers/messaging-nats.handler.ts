...
import { TenantValidatingConsumer } from '@aquaculture/backend-common/nats';
...
import { MarkReadCommand } from '../message/commands/mark-read.command';
import { REDIS_CLIENT } from '../shared/redis.provider';
import { AiPrivacyService } from '../ai/services/ai-privacy.service';
...
interface AiConsentAllowedPayload {
  tenantId: string;
  userId: string;
  purpose: string;
}

interface ResolvedNotificationRef {
  channelId: string;
  messageId: string;
}

interface UserDeletedPayload {
  /** BaseEvent.userId is requester/actor only; never use it as deletion target. */
  userId?: string;
 ...
 */
@Controller()
export class MessagingNatsHandler extends TenantValidatingConsumer {
  ...
  /**
   * Persist a WebSocket mark-read request through the same command path as
   * GraphQL mutations. The gateway has already authenticated the socket, but
   * this service remains authoritative for tenant/channel membership checks.
   */
  @MessagePattern('request.messaging.markRead')
  async markRead(@Payload() data: MarkReadPayload): Promise<boolean> {
    if (
      ...
      this.logger.warn('Rejected request.messaging.markRead with invalid UUID payload');
      return false;
    }

    return this.commandBus.execute(
      new MarkReadCommand(data.tenantId, data.userId, data.channelId, data.messageId),
    );
  }

  @MessagePattern('request.messaging.aiConsentAllowed')
  async aiConsentAllowed(@Payload() data: AiConsentAllowedPayload): Promise<boolean> {
    if (
      !TENANT_ID_REGEX.test(data.tenantId) ||
      !TENANT_ID_REGEX.test(data.userId) ||
      typeof data.purpose !== 'string' ||
      data.purpose.length === 0 ||
      data.purpose.length > 64
    ) {
      this.logger.warn('Rejected AI consent check with invalid payload');
      return false;
    }

    return this.aiPrivacyService.canAnalyzeMessage(data.tenantId, data.userId);
  }
  ...
  async resolveNotificationRef(
    ...
    if (
      !TENANT_ID_REGEX.test(data.tenantId) ||
      !TENANT_ID_REGEX.test(data.userId) ||
      typeof data.notificationRef !== 'string' ||
      data.notificationRef.length > 128
    ) {
      this.logger.warn('Rejected notificationRef resolution with invalid payload');
      return null;
    }

    const key = `msg:push:ref:${data.tenantId}:${data.notificationRef}`;
    ...
    if (
      parsed.tenantId !== data.tenantId ||
      parsed.recipientUserId !== data.userId ||
      !parsed.channelId ||
      !parsed.messageId
    ) {
      this.logger.warn('Rejected notificationRef resolution due to owner mismatch');
      return null;
    }
  ...
  async handleUserDeleted(
    ...
    const validation = this.validateTenantFromSubject(data, context.getSubject());
        ...
        async (queryRunner) => {
          ...
          if (userMessageIds.length > 0) {
            ...
            await queryRunner.query(
             ...
             WHERE "tenantId" = $1::uuid AND "sourceMessageId" = ANY($2::uuid[])`,
              [data.tenantId, userMessageIds],
            );
          }
  ...
  async handleTenantProvisioned(
    ...
    if (
      !TENANT_SCHEMA_REGEX.test(data.schemaName) ||
      data.schemaName !== getTenantSchemaName(data.tenantId)
    ) {
      this.logger.error(
