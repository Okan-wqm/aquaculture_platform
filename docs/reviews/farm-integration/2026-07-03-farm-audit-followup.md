...
BIOMASS_REPORT_QUERY + backend biomassReport query/handler exist with no caller. Contested harm; already in the dead-contract baseline. The concrete latent harm: the wizard is fixed to the previous month, so re-opening it on a period that already has a DRAFT started blank and the create-or-update-if-draft mutation silently overwrote the saved draft on submit. Fix: wire the pre-fill (chosen over deleting a working backend read path) — `useBiomassReport(siteId, month, year)` fetches the persisted payload, `hydrateFormFromPayload` reverses the submit mapping to continue the draft, a finalised (SUBMITTED, immutable) period blocks the editable wizard, and the now-live query leaves the dead-contract baseline honestly (56→55). The query's pre-fill comment is now truthful.
