/**
 * MFA setup screen — the pre-session enrollment path (ADR-046).
 *
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) {
          setLoadError(
            err instanceof Error ? err.message : 'Could not start MFA setup. Please sign in again.',
          );
        }
      }
    })();
    ...
    return (
      <AuthFormShell titleKey="login.title" icon={<LockIcon />}>
        <div className="space-y-5">
          <Alert type="success">
            Multi-factor authentication is now enabled for your account. Please sign in again using
      ...
      <AuthFormShell titleKey="login.title" icon={<LockIcon />} error={loadError}>
        <div className="space-y-5">
          <p className="text-sm text-[var(--surface-muted-fg)]">
            Your organization requires multi-factor authentication, but setup could not be started.
    ...
    <AuthFormShell titleKey="login.title" icon={<LockIcon />} error={verifyError}>
      <div className="space-y-5">
        <p className="text-sm text-[var(--surface-muted-fg)]">
          ...
          authenticator app (e.g. Google Authenticator or 1Password), then enter the 6-digit code to
          finish setup.
        </p>
