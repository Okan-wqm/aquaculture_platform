...
export class SecuritySignalProjectionHandler {
  ...
  async onLoginFailed(event: AuthLoginFailedEvent): Promise<HandlerOutcome> {
    return this.project(event, false, event.reason);
  }
  ...
  async onLoginSuccess(event: AuthLoginSuccessEvent): Promise<HandlerOutcome> {
    return this.project(event, true, undefined);
  }
  ...
  async onRateLimitExceeded(event: RateLimitExceededEvent): Promise<HandlerOutcome> {
    ...
    if (!written) {
      ...
      return HandlerOutcome.ack('already projected');
    }
  ...
  private async project(
    ...
    if (!email) {
      ...
      return HandlerOutcome.terminate('login signal carried no email');
    }
