...
**Implementation note — the three raw-SQL lifecycle blocks (landed 2026-09-06):** cancel, reactivate and extend-trial each ran a raw `UPDATE billing.subscriptions` in the admin NATS handler while the corresponding CQRS handler sat unused. Every one of them told Stripe nothing, wrote no outbox event, projected nothing onto `auth.tenants` and validated no state transition under a lock — and each `WHERE tenant_id = $n AND is_deleted = false` named no subscription id, so a tenant with more than one row had all of them written.

Concretely: a "reactivated" tenant kept its cancelled entitlements while Stripe went on stopping the subscription at period end; a tenant granted another fourteen trial days was invoiced on the ORIGINAL date, mid-trial, because Stripe never heard about the extension; and that same statement set `current_period_end = trial_end`, which on a monthly plan silently moved the next invoice date and on a plan whose period ran past the trial moved it BACKWARDS.

`CancelSubscriptionCommand` gained the `cancelImmediately` the admin path had always offered — and the Stripe idempotency key now carries it, so a scheduled cancellation followed by an immediate one is not replayed as the first. `ReactivateSubscriptionHandler` and `ExtendSubscriptionTrialHandler` are new: Stripe-aware (`cancelAtPeriodEnd: false` and `trial_end` respectively, both added to the canonical client's `updateSubscription`, which mirrors Stripe's own), outbox-writing, projecting, and re-checking the state transition under the row lock rather than in a read that raced the write. The trial extension adds days in UTC arithmetic instead of `setDate`, which gains or loses an hour across a daylight-saving boundary, and sets `proration_behavior: 'none'` so moving a trial end does not raise an invoice.

Twenty tests were added across the three handlers, each one asserting something the raw statements did not do.

**Implementation note — provisioning gets a Stripe object (landed 2026-09-06):** admin tenant provisioning raw-`INSERT`ed into `billing.subscriptions` with `stripe_customer_id` and `stripe_subscription_id` left NULL, while `CreateSubscriptionHandler` minted both. **Every tenant an operator provisioned therefore had a subscription this platform believed in and Stripe had never heard of**: nothing charged them, no Stripe webhook could resolve to them, and the divergence was invisible until someone reconciled by hand.

The two paths differ in exactly two ways, and neither is the write itself — where the numbers come from (a GraphQL input vs. the priced module items of a provisioning command) and which transaction they belong to (the handler's own vs. the provisioning receipt's SERIALIZABLE one). So the write became `SubscriptionWriterService.createWithin(manager, args)`, taking a resolved shape and the CALLER's `EntityManager`, and the Stripe half became `ensureStripeObjects`, called before either transaction opens (SSOT-C-12 — never hold a pool connection across a network call). Its idempotency keys derive from the tenant and plan, so the provisioning receipt's own replay-on-retry reuses the same Stripe customer and subscription rather than minting a second pair. The FREE invariants (always `active`, never `trial`, every price clamped to 0) and the period-end arithmetic — `addMonthsClamped`, which existed byte-identically in both files, because `Date.setMonth` turns Jan 31 + 1 month into Mar 3 and with it an invoice raised two days late every February — moved in with it.

**Gate (all three parts of the ADR now):** part (iii) forbids a raw `INSERT`/`UPDATE`/`DELETE` against `billing.subscriptions` anywhere in billing-service outside migrations, requires each of the three lifecycle transitions to go through a `@CommandHandler`, and requires both subscription-creating paths to use the one writer. Verified by reverting the admin NATS handler — two assertions fail. `stripe-calls-via-canonical-client.spec.ts` was updated to follow the moved calls rather than pinned to the file that no longer holds them, and gained an assertion that both paths reach Stripe.

**Implementation note — the command contract (landed 2026-09-06):** `BillingAdminCommandMeta` carried only an `actorId`, so nothing on the consumer side could tell a retry from a new request. This is not an operator-discipline problem: admin-panel's own `apiFetch` retries 502/503/504 three times with backoff, and `BillingAdminCommandClientService` maps a billing NATS timeout to `BadGatewayException` — 502. **The browser was already re-submitting refunds by itself.** `RefundPaymentHandler` derives its Stripe idempotency key from the already-refunded total, so the second submission carried a DIFFERENT key (`refund:p1:0:5000`, then `refund:p1:5000:5000`) and Stripe issued a second real refund; the handler's own guard passes, because 50 + 50 ≤ 100.

The key had to come from somewhere that survives a retry. Deriving it from the command's arguments is wrong — two legitimate $50 refunds of one payment are distinct operations that must not collapse, and cancel → reactivate → cancel would be blocked by its own history — and minting it per send is no key at all. So it is the caller's `Idempotency-Key` header, minted ONCE per call by `apiFetch` **outside** its retry loop (`X-Request-ID` stays per-attempt: it identifies the attempt, this identifies the operation), composed with a per-command operation scope the compiler now demands at all 29 send sites. `Idempotency-Key` was added to admin-api's CORS allow-list in the same commit; without it the browser would have dropped every billing mutation at the preflight.

Receipt identity moved from `(operationId, tenantId, commandType, idempotencyKey)` to `(tenantId, commandType, idempotencyKey)`. `operationId` led the old unique index and the provisioning workflow mints a fresh one on retry, so **the idempotency key was decorative** — a retry inserted a second receipt and provisioned again. `tenantId` is now nullable for the platform-scoped catalogue commands under `NULLS NOT DISTINCT`, because a sentinel UUID would make the row lie about its owner; the index is partial on `"supersededAt" IS NULL`, so pre-existing duplicate receipts are retired as evidence rather than deleted.

Writing the receipt is a `BillingCommandReceiptInterceptor` bound to all five NATS controllers rather than a `runOnce(...)` line hand-written into 32 handler methods — 32 chances to forget the one line that stops a refund executing twice. Skipping it costs an explicit decorator: `@NonMutatingBillingCommand()` on quote / validate / generate (a repeat must RECOMPUTE — a stale quote is its own defect) and `@OwnsBillingCommandReceipt()` on provisioning, whose receipt commits inside the same SERIALIZABLE transaction as the subscription and is therefore strictly stronger than a before/after pair. A reply with `success: false` is filed FAILED so a transient refusal can be retried; `success: true` is filed SUCCEEDED and replayed verbatim, which is exact because the stored summary IS the wire form. `BillingAdminCommandResult` is now declared once and extended by all fifteen reply interfaces, so the field that rule branches on cannot be forgotten by a new command. A receipt left STARTED by a dead process fails closed — the outcome is unknowable, and guessing it is how money moves twice; the operator releases it by superseding the row, which is the one legitimate write of that column.

Eight tests cover the interceptor, including the two that matter most: the second identical refund does not reach the handler, and a refusal does not become a cached failure. Five spec files across billing-service and admin-api needed the new meta on their command literals — the compiler found every one.

**Implementation note — the invoice charged more than the quote (landed 2026-09-06):** what a longer billing cycle costs was written TWICE, and the two copies disagreed. `ModulePricingService.quote` multiplies the monthly total by the months and takes the commitment discount off — 5% quarterly, 10% semi-annual, 15% annual, declared as commercial terms in `BILLING_CYCLE_DISCOUNT_RATE`. That is the number an operator approves. `BillingSchedulerService` multiplied by the months and took **nothing** off. So an annual tenant was invoiced 15% above the price they signed, every year, and a quarterly one 5% above — silently, because both figures are plausible and nobody reconciles a quote against an invoice by hand.

Both now go through one `cycleAmountFor(monthly, cycle, currency)`, which rounds the gross and the discount at the currency boundary because both appear on the invoice as real money. The invoice line says what the discount was instead of quietly folding it in. The scheduler's own third copy of the months table (a `switch` beside `BILLING_CYCLE_MONTHS` and the quote's multiplier) is gone: adding a cycle must not mean remembering three places, one of which decides how long a customer goes unbilled.

`annualTotal` on a quote was wrong in the other direction — it applied whichever rate the REQUESTED cycle carried, so the annual figure shown beside a monthly quote used a 0% rate and overstated the annual price by 15%, making the commitment look worse than it is. It is now `cycleAmountFor(monthly, 'annual', …)`.

The existing scheduler spec asserted `100 x 3 = 300` for a quarterly subscription — it encoded the defect, so it was corrected to 285 rather than kept. Six tests were added; against the pre-fix code the two behavioural ones fail and `module-quote.spec.ts` does not compile, which is the evidence they pin behaviour rather than describe it. The unknown-cycle case is asserted as TOTALITY over `BILLING_CYCLES` — adding a cycle without a commercial term now fails here rather than at the first invoice, where a missing term would silently bill one month.

**Implementation note — three of the four billing cycles could not be sold, and no custom plan could be activated (landed 2026-09-06):** provisioning resolved a catalogue plan by `{tier, billingCycle}` against `billing.plans`, which demands a row whose DEFAULT cycle equals the requested one — and the seed wrote MONTHLY rows only. **Quarterly, semi-annual and annual provisioning every answered CATALOG_MISSING.** W4b had already normalised the per-cycle matrix into `plan_cycle_prices`, so the fix is not four plan rows per tier (that would re-fragment what W4b joined): one plan is purchasable on a cycle exactly when it carries a price row for it, and `plans.billing_cycle` is the plan's DEFAULT. The GraphQL path already resolved by tier alone; the two paths now agree. The requested cycle also reaches `ensureStripeObjects`, which selects `stripe_price_ids[cycle]` and is part of the Stripe idempotency key — the plan's default would have minted the wrong price on any non-default sale.

The same resolver held a second break. `customPlanId` is a `billing.custom_plans` id and it was being looked up in `billing.plans` — different tables, so it could never match: **every custom-plan activation died with CATALOG_MISSING**, one call after admin-api's own `approved` guard had passed, and nothing said the id had been resolved against the wrong table. A custom plan is not a catalogue row; it is a negotiated price FOR a tenant travelling as `enterprise` plus its own module items. The catalogue plan is resolved by tier, and the custom plan is now verified separately — it exists, it belongs to the tenant being provisioned, and it is approved. None of that was checked before, so a command naming ANOTHER tenant's plan, or a rejected one, was indistinguishable from a typo.

The seed was rewritten around both: one row per tier priced for every cycle through `cycleAmountFor` (so the catalogue cannot state a price the platform would not charge, and `plan_cycle_prices.discount_percent` records the commitment discount actually applied), matched on TIER rather than name — a rename through the catalogue UI used to make the seed insert a second "Starter" — and it now FAILS THE BOOT instead of `logger.warn`ing, because a catalogue that failed to seed makes every provisioning answer CATALOG_MISSING: a healthy service that sells nothing. Already-seeded catalogues are brought up to every cycle without overwriting an operator's edited price.

Two more copies of the months table went with it. `subscription-writer.service.ts` reads `BILLING_CYCLE_MONTHS`, and `billing-admin-nats.handler.ts` lost `calculatePeriodEnd`/`cycleToMonths`/`addMonthsClamped` entirely — dead since the raw `INSERT` was removed. `GetTenantBillingHandler.calculateMonthlyPrice` was a third defect found on the way: it DIVIDED `pricing.basePrice` by the cycle months, but that column is the monthly rate by contract (the scheduler multiplies it), so an annual tenant on $49/month was shown $4.08.

Six handler tests and six seed tests were added; against the pre-fix handler all six of the handler ones fail.

**Implementation note — the discount the catalogue displayed and nothing billed (landed 2026-09-06):** `billing.plan_cycle_prices.discount_percent` was written by `PlanCatalogService` from operator input and read back into the catalogue snapshot, but no pricing path read it — the commitment discount actually charged came from the platform-wide `BILLING_CYCLE_DISCOUNT_RATE`. Two numbers claimed to be the same thing and only one of them billed, so an operator editing a plan's annual terms changed a figure the platform ignored.

The obvious closure — have the invoice read the plan's row — is the wrong one, and worse than the defect: an operator editing a plan's annual terms would silently re-price every customer already on it. A contracted term is snapshotted at the sale, like the rest of `pricing`. So `billing.subscriptions` gained `commitment_discount_percent` (migration `1802500000000`, `NUMERIC(5,2)` under a `[0, 100]` CHECK), written by `SubscriptionWriterService.createWithin` from the plan's `plan_cycle_prices` row for the cycle being sold. Both creation paths supply it: provisioning from the cycle row it already resolved, and the GraphQL path from the same `cyclePriceOf` helper — which also gave that path the cycle check it never had. Existing rows backfill to the platform default for their cycle, which is exactly the rate the previous commit began charging them, so the backfill records what is already true rather than moving anyone's price.

`cycleAmountFor` no longer looks the rate up at all: it takes `discountPercent` as a required argument. A lookup inside is precisely how the row became decorative — the function always had an answer of its own. The invoice passes the subscription's snapshot; the quote passes the plan's row, resolved through `PlanCatalogService.commitmentDiscountsFor(tier)`, so what an operator is shown and what the sale will store are one number. `BILLING_CYCLE_DISCOUNT_RATE` is now reachable only through `defaultCommitmentDiscountPercent`, whose sole caller is the seed.

Crossing back out of the entity enum needed the same exhaustive boundary map the catalogue already had in the other direction (`WIRE_VALUE_BY_CYCLE`) — a TypeScript string-enum member is not assignable to the contract's string-literal union, and assuming otherwise is the recurring drift class in this codebase.

Three more byte-identical `PERCENT_TRANSFORMER` copies went with it (plan cycle prices, custom plans, discount codes). A percentage multiplies money, so a float there reaches the invoice as surely as a float in the price would: `@PercentColumn()` now sits beside `@MoneyColumn()` in `@aquaculture/backend-common/monetary`.

**Gate:** nothing outside `module-quote.ts` names `BILLING_CYCLE_DISCOUNT_RATE`, the seed is the only caller of the accessor that exposes it, `cycleAmountFor` takes the discount as an argument, the quote asks the catalogue, the invoice asks the subscription, and the writer snapshots it. Three tests pin the quote's sourcing (a plan on 22% quotes 22%, not the platform's 15%), one pins the sale's snapshot, and `cycleAmountFor` refuses a rate outside [0, 100] rather than inverting the price.

**Gate (parts i + ii of the ADR's three):** the new invariant derives every one of the 32 `BILLING_ADMIN_COMMAND_SUBJECTS` members' sender, consumer `@MessagePattern` and `services.yaml` grant from the one subject map (96 assertions), and holds the metadata key to a single declaration: the producer may not write it as a literal, the webhook consumer may not reach into a Stripe object's metadata by hand, and no Stripe-surface file may read the old key. Verified by reverting both files — three assertions fail. Part (iii), "no raw write to `billing.subscriptions` outside a `@CommandHandler`", lands with the provisioning move: the three lifecycle `UPDATE`s are gone, and the one remaining raw statement is the provisioning `INSERT` that ADR-0014's first decision replaces. A gate carrying a carve-out for the exact statement it exists to forbid would be theatre, so it lands when the carve-out would not be needed.
...
`parseFloat` on the way in (ADMIN-MEDIUM-017). They are billing-owned DDL, but
...
NOT a uuid problem — see ADMIN-MEDIUM-016.

**Fleet-wide:** the same baseline reversal stands in auth-service and
billing-service. Their DDL, their wave.

## ADMIN-MEDIUM-016 — An actor column is a union with no type (W6 split)
...
## ADMIN-MEDIUM-017 — The billing mirrors widen money through parseFloat (W6 split)
...
**The finding understated it in one place and overstated it in another, and both corrections are load-bearing.**
...
in-app surfaces are gone (see ADMIN-HIGH-014).
...
| **W6 — physical types**                   | ADMIN-HIGH-012 (all five classes landed) → ADMIN-MEDIUM-016, ADMIN-MEDIUM-017                                                                       | the type is the cheapest constraint; the decorator is what a baseline squash reads                                                                                      |
...
**Delete with a coupled decision:** TenantConfigurationPage, ProvisioningSettingsPage, MessagingMonitoring / AiDashboard / AiPersonas pages (delete the route first); ~~ErrorTrackingPage (delete unless a reporter is wired)~~ (W5 wired the reporter: the fleet-wide `ServiceErrorCaptured` ingress); `useAsyncData` (after W6); ~~`login_attempts` / `user_sessions` / `api_usage_logs` + detectors~~ (ADMIN-HIGH-014 — decided in W5: the first and third are projected from the security stream, `user_sessions` is deleted); `slow_query_logs` / `database_metrics` (with the `pg_stat_statements` decision); `maintenance_modes` (if maintenance moves to the gateway); `password-reset.controller.ts` (second un-rate-limited pre-auth ingress); `tenants.ts deactivate/archive` (adopt with `@Destructive` or delete both sides); 168 dead backend routes as a set after W3; K8s alert rule files (port useful rules to `droplet/rules` first); Grafana dashboards (rebuild).
