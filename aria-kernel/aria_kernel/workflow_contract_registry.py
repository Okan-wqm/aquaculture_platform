...
WORKFLOW_CONTRACTS: dict[str, WorkflowContract] = {
    ...
    "aria-daily-report": WorkflowContract(
        ...
        job_contracts=(
            ...
            WorkflowJobContract(
                ...
                preflight_step="Persist enterprise workflow preflight",
                first_governed_mutation_step="Publish idempotent daily report PR",
                ...
                upload_artifact_name_pattern=(
                    r"^automation-publication-result-\$\{\{\s*"
                    r"steps\.publication\.outputs\.commit\s*\}\}$"
                ),
                upload_artifact_path_patterns=(
                    rf"^{_RUNNER_TEMP}/aria-daily-report-preflight\.json$",
                    rf"^{_RUNNER_TEMP}/aria-daily-report-publication\.json$",
                ),
                retention_days=90,
                required_permissions=(("contents", "read"),),
                token_source="github_actions:artifact+github_app:installation",
    ...
    "finding-state-sweep": WorkflowContract(
        ...
        job_contracts=(
            WorkflowJobContract(
                ...
                upload_artifact_name_pattern=rf"^finding-state-sweep-input-{_RUN_ID_ATTEMPT}$",
                upload_artifact_path_patterns=(
                    rf"^{_RUNNER_TEMP}/finding-state-sweep-preflight\.json$",
                    rf"^{_RUNNER_TEMP}/finding-state-sweep-plan\.txt$",
                ),
                retention_days=90,
                required_permissions=(
                    ("actions", "read"),
                ...
                token_source="github_actions:actions_read+oidc+github_app:installation",
    ...
    "finding-registry-authority": WorkflowContract(
        workflow_id="finding-registry-authority",
        workflow_file=".github/workflows/finding-registry-authority.yml",
        job_contracts=(
            WorkflowJobContract(
                job_id="mutate",
                preflight_step="Persist governed-write preflight",
                first_governed_mutation_step="Apply canonical mutation",
                allowed_write_path_patterns=(r"^docs/reviews/_registry/findings\.jsonl$",),
                preflight_artifact_path_pattern=rf"^{_RUNNER_TEMP}/finding-registry-authority-preflight\.json$",
                ...
                token_source="github_actions:actions_read+oidc+github_app:installation",
                network_policy=("github_oidc", "github_api"),
                dlp_artifact="finding-registry-authority-preflight.json",
                clean_worktree_policy="pre_and_post",
                external_root_allowlist=("RUNNER_TEMP",),
            ),
        ),
    ...
    "rule-health-report": WorkflowContract(
        ...
        job_contracts=(
            WorkflowJobContract(
                ...
                upload_artifact_name_pattern=rf"^rule-health-report-input-{_RUN_ID_ATTEMPT}$",
                upload_artifact_path_patterns=(
                    rf"^{_RUNNER_TEMP}/rule-health-input$",
                ),
                retention_days=90,
                required_permissions=(("actions", "read"), ("contents", "read")),
                token_source="github_actions:actions_read+artifact+github_app:installation",
                network_policy=("github_api", "github_artifact"),
...
AUDITED_WORKFLOW_EXCLUSIONS: dict[str, AuditedWorkflowExclusion] = {
    "automation-publication-admission": AuditedWorkflowExclusion(
        workflow_id="automation-publication-admission",
        reason="protected-main pull_request_target admission gate; checks out and executes only "
        "the exact PR base verifier, treats the PR head solely as authenticated API data, "
        "holds read-only permissions, and uploads no governed artifact",
