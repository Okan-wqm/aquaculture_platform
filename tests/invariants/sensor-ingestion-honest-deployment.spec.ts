...
describe('INVARIANT (Cluster 4): sensor-ingestion deploy honesty', () => {
  ...
  it('the restored REAL pipeline is present: per-tenant sink + outbox + JetStream PubAck (Task 3)', () => {
    ...
    // The schema derivation goes through the 16-hex SSoT. Asserted on the
    // shared golden fixture the Rust crate's integration test consumes
    // (crates/tenant-context/tests/schema-golden.rs), NOT on a source
    // substring: the previous `full[..16]` pin named a local binding that
    // has since been renamed, so it asserted spelling rather than shape.
    const golden = JSON.parse(read('crates/tenant-context/tests/schema-golden.json')) as Array<
      Record<string, string>
    >;
    expect(golden.length).toBeGreaterThanOrEqual(4);
    for (const row of golden) {
      expect(row['schema_name']).toMatch(/^tenant_[0-9a-f]{16}$/);
    }
    // ...and that fixture is what the crate derives against.
    expect(read('crates/tenant-context/tests/schema-golden.rs')).toContain(
      'SchemaName::from_tenant_id',
    );
  });
