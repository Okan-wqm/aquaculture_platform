...
Status: RESOLVED — `.github/workflows/deploy-digitalocean.yml` now wraps `docker login` in a 3-attempt shell retry loop with 5s/10s/15s backoff (single commit on `fix/ghcr-login-retry`). The third-party `docker/login-action` dependency was removed in the same change; the supply-chain surface narrowed by one action. Tier 1 property: single-flake transient is absorbed inside the build step; sustained outage still fails loud with the rc of the third attempt.
