...
export const securityApi = {
  ...
  getUserActivities: (userId: string, params?: PaginationParams & DateRangeParams) =>
    apiFetch<PaginatedResult<BackendActivityLog>>(
      `/security/activities/user/${userId}?${buildQueryString(params || {})}`,
      platformScope,
    ),
  ...
  generateComplianceReport: (
    ...
    apiFetch<BackendComplianceReport>('/security/compliance/reports', {
      ...platformScope,
      method: 'POST',
      body: JSON.stringify({
        complianceType,
  ...
  createDataRequest: (
    data: Omit<BackendDataSubjectRequest, 'id' | 'status' | 'createdAt' | 'dueDate'>,
  ) =>
    apiFetch<BackendDataSubjectRequest>('/security/compliance/data-requests', {
      ...platformScope,
      method: 'POST',
  ...
  processDataRequest: (
    ...
    if (action === 'approve') {
      return apiFetch<BackendDataSubjectRequest>(
  ...
  resolveSecurityEvent: (id: string, resolvedBy: string, notes?: string) =>
    apiFetch<BackendSecurityEvent>(`/security/monitoring/events/${id}/status`, {
      ...platformScope,
      method: 'PUT',
  ...
  updateSecurityIncident: (id: string, data: Partial<BackendSecurityIncident>) =>
    apiFetch<BackendSecurityIncident>(`/security/monitoring/incidents/${id}`, {
      ...platformScope,
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  ...
  addThreatIndicator: (
    data: Omit<
      BackendThreatIndicator,
      'id' | 'firstSeenAt' | 'lastSeenAt' | 'createdAt' | 'updatedAt'
    >,
  ) =>
    apiFetch<BackendThreatIndicator>('/security/monitoring/threat-intelligence', {
      ...platformScope,
      method: 'POST',
      body: JSON.stringify(data),
    }),
  ...
  getComplianceChecks: (framework: string) =>
    ...
    >(`/security/compliance/checks/${framework}`, platformScope),
};
