...
`max_file_store` (threshold 1.5GiB against today's 2GiB in `infrastructure/docker/nats/nats.conf`)
for 15 minutes.
...
2. Which stream owns the bytes? Until Task 2 lands `AQUACULTURE_TELEMETRY`, the only stream is
   `AQUACULTURE_EVENTS` (`events.>`, 1.5GiB cap, Discard Old) — a full events stream means consumer
   backpressure: check which durable consumer stopped acking (`/jsz?consumers=true`).
...
message-size drift beyond the measured 600–750B, or the Task 2 telemetry stream landing without
raising `max_file_store` and the threshold together.
