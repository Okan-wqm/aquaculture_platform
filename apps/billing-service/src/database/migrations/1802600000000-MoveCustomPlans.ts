 ...
 */
export class MoveCustomPlans1802600000000 implements MigrationInterface {
  name = 'MoveCustomPlans1802600000000';
  ...
  private async copyFromAdmin(queryRunner: QueryRunner): Promise<void> {
    ...
    if ((missing[0]?.missing ?? '0') !== '0') {
      throw new Error(
        `MoveCustomPlans1802600000000: ${missing[0]?.missing} admin.custom_plans rows did not copy into billing.custom_plans. Resolve the source rows before re-running.`,
      );
    }
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // DESTRUCTIVE: down()-only rollback of MoveCustomPlans1802600000000 — drops the three tables this same migration created; the admin source rows are dropped by admin migration 1809200000000, so roll that one back first
