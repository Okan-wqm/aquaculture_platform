/**
 * ChatThread — one open conversation: its history, and the composer under it.
 *
 * WHY THIS IS A COMPONENT AND NOT PART OF ChatRoomPage. Two shells now open the
 * same conversation. The handheld pushes a full screen at `/messages/:channelId`
 * with a back chevron; the cabin board holds the thread in its right column
 * beside the conversation list, because that is what the extra width buys over
 * push-navigation. What differs between them is the HEADER — a back button and a
 * presence line versus a column heading — and nothing else.
 *
 * Everything below the header is identical and is not cheap: optimistic send,
 * edit and delete routing, infinite scroll with scroll-anchor restoration, the
 * read-cursor rules (at the bottom AND foregrounded, deduped), attachment upload
 * with an offline binary lane, voice notes, forwarding, typing indicators. A
 * second copy of that for the board would be a second place for MSG-CRITICAL-055
 * (the cache key that never matched), MSG-MEDIUM-053 (edit mode) and
 * MSG-MEDIUM-055 (attachments discarded offline) to come back, in a file nobody
 * would think to check when fixing the first one. So the thread is written once
 * and both shells render it; the header is what each shell supplies.
 *
 * `channelId` is `string | undefined` to match the messaging hooks it wraps —
 * every one of them takes `string | undefined` and disables itself without an
 * id. It is not a nullable value being papered over; it is this layer's contract.
 */

import { useQueryClient } from '@tanstack/react-query';
import { AlertCircle, ChevronDown, Send, X } from 'lucide-react';
...
export interface ChatThreadProps {
  /** The conversation to open. Undefined disables every query, as the hooks do. */
...
export function ChatThread({ channelId }: ChatThreadProps): ReactElement {
  ...
  // message in the newly-opened channel always triggers a mark-read. On the
  // board this fires without a remount: picking another conversation in the left
  // column only changes this prop.
  useEffect(() => {
  ...
  return (
    // The keyboard offset lives HERE rather than on the page: the composer is
    // inside this component, so both the phone's full screen and the board's
    // right column lift it above an on-screen keyboard without either shell
    // having to know a keyboard exists. `min-h-0` is what lets the scroller
    // below actually scroll inside a flex parent.
    <div
