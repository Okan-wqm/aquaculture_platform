  ...
  it('renders species rows READ-ONLY with no add/load affordances when stock is from records', () => {
    render(
      <BiomassStep
        formData={formData}
        onChange={vi.fn()}
        prefill={prefillWith({ path: '/currentBiomass', provenance: 'RECORDS', sourceRecordCount: 2, blocking: false })}
      />,
    );
  ...
  it('keeps species rows editable with add/load affordances when stock is MANUAL_REQUIRED', () => {
    render(
      <BiomassStep
        formData={formData}
        onChange={vi.fn()}
        prefill={prefillWith({ path: '/currentBiomass', provenance: 'MANUAL_REQUIRED', blocking: false })}
      />,
    );
