...
// Pin the clone authority once. Runtime mutation of globalThis must not change
// which proxy/non-cloneable inputs this protocol accepts after module load.
const PINNED_STRUCTURED_CLONE = resolveStructuredClone();
...
/** Exact C0 + DEL rejection shared by every canonical wire-text boundary. */
...
function normalizeUnicodeScalarString(
  value: string,
  state: SnapshotState,
  allowWireLoneSurrogates: boolean,
): string {
  for (let index = 0; index < value.length; index += 1) {
    const codeUnit = value.charCodeAt(index);
    if (codeUnit >= 0xd800 && codeUnit <= 0xdbff) {
      const next = value.charCodeAt(index + 1);
      if (!allowWireLoneSurrogates && (!Number.isInteger(next) || next < 0xdc00 || next > 0xdfff)) {
        throw new TypeError('CANONICAL_JSON_INVALID_UNICODE');
      }
      if (next >= 0xdc00 && next <= 0xdfff) index += 1;
    } else if (!allowWireLoneSurrogates && codeUnit >= 0xdc00 && codeUnit <= 0xdfff) {
      throw new TypeError('CANONICAL_JSON_INVALID_UNICODE');
    }
  }
  state.sourceBytes += utf8Bytes(value).byteLength;
...
function snapshot(
  ...
  try {
    ...
    for (const key of Object.keys(descriptors).sort(compareUtf16CodeUnits)) {
      ...
      normalizeUnicodeScalarString(key, state, state.mode === 'wire-v1');
...
function createDocument(
  ...
  const clone = PINNED_STRUCTURED_CLONE;
...
/**
 * Explicit legacy wire policy: Date→ISO, object undefined omitted, array
 * undefined→null, and lone surrogates retain JSON.stringify escape identity.
 */
...
function serializeCanonicalValue(value: CanonicalJsonValue): string {
  ...
  return `{${Object.keys(record)
    ...
    .map((key) => {
      ...
      if (entry === undefined) throw new TypeError('CANONICAL_JSON_INTERNAL_INVARIANT');
      return `${JSON.stringify(key)}:${serializeCanonicalValue(entry)}`;
    })
    .join(',')}}`;
}

function indexedNumber(values: ArrayLike<number>, index: number): number {
  const value = values[index];
  if (value === undefined) throw new TypeError('CANONICAL_JSON_INTERNAL_INVARIANT');
  return value;
}
...
export function sha256Hex(value: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index++) {
      const previous15 = indexedNumber(words, index - 15);
      const previous2 = indexedNumber(words, index - 2);
      const sigma0 = rotateRight(previous15, 7) ^ rotateRight(previous15, 18) ^ (previous15 >>> 3);
      const sigma1 = rotateRight(previous2, 17) ^ rotateRight(previous2, 19) ^ (previous2 >>> 10);
      words[index] =
        (indexedNumber(words, index - 16) + sigma0 + indexedNumber(words, index - 7) + sigma1) >>>
        0;
    }

    let a = indexedNumber(state, 0);
    let b = indexedNumber(state, 1);
    let c = indexedNumber(state, 2);
    let d = indexedNumber(state, 3);
    let e = indexedNumber(state, 4);
    let f = indexedNumber(state, 5);
    let g = indexedNumber(state, 6);
    let h = indexedNumber(state, 7);
    for (let index = 0; index < 64; index++) {
      ...
      const temporary1 =
        (h +
          sigma1 +
          choice +
          indexedNumber(ROUND_CONSTANTS, index) +
          indexedNumber(words, index)) >>>
        0;
    ...
    state[0] = (indexedNumber(state, 0) + a) >>> 0;
    state[1] = (indexedNumber(state, 1) + b) >>> 0;
    state[2] = (indexedNumber(state, 2) + c) >>> 0;
    state[3] = (indexedNumber(state, 3) + d) >>> 0;
    state[4] = (indexedNumber(state, 4) + e) >>> 0;
    state[5] = (indexedNumber(state, 5) + f) >>> 0;
    state[6] = (indexedNumber(state, 6) + g) >>> 0;
    state[7] = (indexedNumber(state, 7) + h) >>> 0;
  }
...
/**
 * Versioned bare content digest for byte-identities that predate domain
 * separation. New protocols must prefer canonicalJsonSha256 with an explicit
 * authority. This named wire API preserves existing persisted identities
 * without weakening the strict document/hash boundary above.
 */
export function canonicalWireJsonContentSha256V1(
  value: unknown,
  limits?: Partial<CanonicalJsonLimitsV1>,
): string {
  return sha256Hex(canonicalWireJsonStringifyV1(value, limits));
}
