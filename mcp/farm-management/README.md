...
```typescript
import {
  definition as newMetricDef,
  handler as newMetricHandler,
} from './math/calculate-new-metric.js';
```
...
- **FARM-LOW-329 (sonra):** formüllerin ve dört matematik aracının (`calculate_oxygen_budget`,
  `calculate_carrying_capacity`, `calculate_growth_metrics`, `predict_feeding_impact`) aritmetiği
  `@platform/aquaculture-engines`'e taşındı (`oxygen/`, `growth/`, `feeding/`); `utils/formulas.ts`
  silindi. MCP araçları yalnızca girdi doğrular, motoru çağırır ve yuvarlayıp açıklar — aynı
  motorlar ai-service'in `tools/aquaculture-math/` araçlarını da besler (tek kaynak).
...
- **103 unit test**, 5 dosya, vitest ile:
  - `formulas.test.ts` (18 test) — Weiss DO referans değerleri (USGS) — FARM-LOW-329 ile `libs/aquaculture-engines/src/oxygen/__tests__/`e taşındı
