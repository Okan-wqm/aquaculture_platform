...
const GaugeRenderer: React.FC<WidgetRendererProps> = ({ config, value, width, height, isEditing }) => {
  ...
  const label = config.label ?? 'Gauge';
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      <text x={cx} y={125} textAnchor="middle" fontSize={10} fill="#9ca3af">
        {label}
      </text>

      {/* Min / Max */}
      <text x={arcX(startAngle) - 2} y={arcY(startAngle) + 14} textAnchor="end" fontSize={9} fill="#9ca3af">
        {min}
      </text>
      <text x={arcX(endAngle) + 2} y={arcY(endAngle) + 14} textAnchor="start" fontSize={9} fill="#9ca3af">
        {max}
      </text>
    </svg>
  );
