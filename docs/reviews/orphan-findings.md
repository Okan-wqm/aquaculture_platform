...
## ORPHAN-HIGH-122 — in-process `notification.send` bus has no consumer/bridge: every farm in-app notification is silently dropped

Surfaced firsthand during the Phase-4 `feed-reminder-tenant` investigation (2026-06-16). Four farm-service listeners emit the in-process EventEmitter2 event `notification.send` at **11 call sites** — `feeding-completed.listener.ts` (feeding reminder, daily summary, FCR alert, weekly forecast), `low-stock-alert.listener.ts` (3), `maintenance-schedule-due.listener.ts` (4) — but **nothing subscribes to it**: there is no `@OnEvent('notification.send')` handler anywhere in farm-service and no bridge that republishes it to NATS `commands.notification.send*` (the `NotificationSendCommandBase` contract in `libs/event-contracts/src/notification-commands.ts` that `notification-service` actually consumes). So every farm in-app notification (feeding reminders, low-stock alerts, maintenance-due, FCR/daily/weekly) is emitted into a dead in-process bus and dropped. Verified: repo-wide grep for `notification.send` shows only emitters (farm-service) + unrelated doc comments in `notification-service/.../alert-triggered.handler.ts`; zero subscribers.

This is the LARGER sibling of `FARM-HIGH-066` (`feed-reminder-tenant`). FARM-HIGH-066 fixes the `tenantId: undefined` on the reminder branch — necessary and a prerequisite — but does NOT make the notification reach a user, because the bus itself is unwired.

**Why filed as ORPHAN, not folded into Phase 4:** building the bridge is a cross-service feature (a farm-service `@OnEvent('notification.send')` consumer that maps the loose payload onto the typed `NotificationSendCommandBase` and publishes it through outbox→NATS, plus notification-service consumer verification), not a Phase-4 tenant-context/perf line item. Conflating it would bloat the phase. The correct fix: introduce one bridge consumer keyed on `NotificationSendCommandBase` so a missing `tenantId` becomes a compile error platform-wide, and retire the 11 raw `notification.send` emits in favour of typed publishes.

**Architectural fix (highest tier):** Tier-1 — type the in-process notification payload to `NotificationSendCommandBase` (tenantId non-optional) and add the single farm-service bridge consumer that publishes to NATS; Tier-3 — an invariant banning a raw `emit('notification.send', …)` object literal outside the typed publish helper (same shape as the dead-`@OnEvent`-listener ratchet, ORPHAN-MEDIUM-106).

Owner: farm-expert + messaging/notification owner (cross-service). Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-MEDIUM-123 — farm-metrics interceptor derives the Prometheus tenant label from the raw x-tenant-id header

Surfaced by the Phase-4 Lane-A cross-check audit (multi-tenant-saas-expert, 2026-06-16). `apps/farm-service/src/common/metrics/farm-metrics.interceptor.ts:182-183` reads `req.headers['x-tenant-id']` to set the `tenant_id` Prometheus label. Same untrusted-source class as `FARM-MEDIUM-059` (cache-tenant) — a forged/absent header mislabels or omits the tenant on metrics — plus a metric-label-forgery / cardinality-trust concern (an unvalidated `tenant_id` label is an injection vector for label explosion). NOT a cache/isolation boundary, so lower severity than the cache fix, and explicitly outside the `common/cache/**` arch-spec ban. Fix: route through the exported `extractTenantIdSafe` SSoT (now public after FARM-MEDIUM-059) and, per per-tenant-observability cardinality rules, validate/limit the `tenant_id` label. Owner: observability-expert / farm-expert. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-MEDIUM-124 — JsonbPatchService has no tenant-scoped entry point; first cron/NATS caller will reproduce the source-schema no-op

Surfaced by the Phase-4 Lane-A audit (2026-06-16), adjacent to `FARM-HIGH-065`. `apps/farm-service/src/common/jsonb/jsonb-patch.service.ts` injects the bare `DataSource` and calls `this.dataSource.query()`, relying on the request-pool search_path for tenant routing. FARM-HIGH-065 removed the schema qualifier so a REQUEST-context call now routes correctly — but the service has ZERO production callers today, and whichever handler first adopts it from a NON-request context (cron / NATS consumer, e.g. the feeding scheduler which sets `SET search_path` on its OWN QueryRunner) would run on the default pool connection (no tenant search_path) and land on the empty source schema — reproducing exactly the silent no-op FARM-HIGH-065 fixed. Tier-1 fix: make it impossible — require the caller to pass the tenant-scoped `EntityManager`/`QueryRunner` (mirror `tenantManagerRepo(queryRunner.manager, …)` from feeding-scheduler.service.ts) instead of injecting the bare `DataSource`, so the routing context is structurally guaranteed. Owner: farm-expert / data-expert. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-LOW-125 — equipment_types is in BOTH referenceDataTables and per-tenant tables; ListEquipmentHandler reads only the source schema

Surfaced by the Phase-4 Lane-A audit (2026-06-16). `equipment_types` appears in both `MODULE_SCHEMAS[farm].referenceDataTables` (seeded into the source `farm` schema) and `MODULE_SCHEMAS[farm].tables` (cloned per-tenant) in `libs/backend-common/src/database/schema-manager.service.ts`. `apps/farm-service/src/equipment/handlers/list-equipment.handler.ts:367-368` reads only the source-schema copy (`SELECT * FROM "farm"."equipment_types"`). If equipment_types is meant to be tenant-customizable, a source-only read hides per-tenant rows; if it is purely reference data, it should NOT be in per-tenant `tables`. Decide the intended model and align (one membership). Owner: data-expert / farm-expert. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-MEDIUM-126 — equip-list correctness is a FALSE POSITIVE; residual is bounded deep-page over-fetch

Phase-4 investigation flagged `ListEquipmentHandler` (`apps/farm-service/src/equipment/handlers/list-equipment.handler.ts`) for a "wrong total (overlap double-count)" + "deep-page item loss". **Firsthand verification debunked BOTH as correctness false positives (2026-06-16):**
- **No double-count.** The `equipment` and `tanks` tables have DISJOINT ids — tanks are surfaced as Equipment only via IN-MEMORY transforms (`tank-equipment-adapter.service.ts:138`, `allocate-to-tank.handler.ts:402`, `transformTankToEquipment`); nothing persists an equipment-table row with a tank's id (no `equipmentRepository.save` in any tank path). So `dedupeMergedResults` (by id) never removes anything and `total = equipmentResult.total + tankResult.total` is correct (two disjoint counts). The dedupe is dead-defensive.
- **No deep-page item loss.** Each source is capped at `maxRowsNeeded = page * limit`. For any row on page P its global rank is `< P*limit`, and a row's within-source rank ≤ its global rank (merge property), so every page-P row is within each source's fetched cap → no row is dropped. The cap scales with page, which guarantees coverage.

**Residual (real, MEDIUM perf-debt):** the handler loads up to `page*limit` FULL entities (equipment with 8 eager joins) from EACH source, then merges/sorts/slices in JS — an O(page·limit) over-fetch to return ≤limit rows (e.g. page 5 / limit 50 → 500 hydrated entities for 50 shown). Bounded by `MAX_LIMIT=100` and page (DTO max 1000). The architecturally-correct fix is a single SQL `UNION ALL + DISTINCT ON(id) + COUNT + LIMIT/OFFSET` that paginates in the DB and hydrates only the page's ≤limit ids — but it is a large, tenant-scoping-sensitive rewrite NOT justified by a non-correctness, pathological-only (deep-page) perf cost. Deferred. Owner: farm-expert / data-expert. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-HIGH-127 — batch labour cost is silently always zero: work orders are not batch-linked (a missing feature, not a wire-up bug)

`BatchCostCalculatorService.sumLabourCost` (`apps/farm-service/src/batch/services/batch-cost-calculator.service.ts:190-199`) filters work orders by `wo."relatedAsset"->>'batchId' = :batchId`, but **no work order ever carries a `batchId`**: the `RelatedAsset` interface (`work-order.entity.ts:177-182`) and the `RelatedAssetInput` DTO (`create-work-order.dto.ts:95`) have only `assetType`/`assetId`/`assetCode`/`assetName` — no `batchId`; the FE work-order form has no batch field; and the write paths persist `input.relatedAsset` verbatim (`work-order.service.ts:111`). So the predicate matches ZERO rows and the labour-cost axis of every batch cost rollup (`costPerKg`/`costPerFish`/`totalCost` on `batchPerformance`) is silently understated to zero (no warning, unlike `sumTreatmentCost`).

**Why a feature, not a quick fix:** making the data exist requires a cross-stack design decision — either (a) the FE/DTO collect a batch for batch-linked work orders, or (b) the backend attributes labour via the REAL linkage (work order → asset → tank, batch → tank via TankBatch, with a creation-time/time-window capture) since a work order targets an asset, not a batch directly (and a tank can hold mixed batches). The speculative `relatedAsset.batchId` query was written for a feature that was never built end-to-end. The companion perf finding (a btree expression partial index on `("tenantId",(relatedAsset->>'batchId'))` via a @TenantFanOut migration) is MOOT until the column is populated — index after the feature lands, not before. Owner: farm-expert + frontend-expert (cross-stack). Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-HIGH-128 — wq-not-hypertable Tier-B: per-tenant hypertable + continuous aggregate (operator-sanctioned deferral)

Phase-4 deferral, ratified by the operator on 2026-06-16 ("defer wq Tier-B"). `water_quality_measurements` is plain OLTP, not a TimescaleDB hypertable; there is no continuous aggregate or `drop_chunks` retention (current retention is the row-by-row `cleanup_old_water_quality_measurements` cron, 1095d). This is PERF-DEBT, not a correctness bug — the read paths (`WaterQualityService.getTank/SystemStatistics`, `GetFeedingSummaryHandler`) raw-aggregate correct numbers. The genuine acceleration is a TENANT-CORRECT continuous aggregate. **Why deferred:** there is NO precedent for a per-tenant hypertable or a working continuous aggregate anywhere in the platform — sensor-service's hypertables live in the SOURCE schema only and its CAGG is itself unsolved (`OPEN-ADR-030-CAGG`); `TenantSchemaSyncService` clones only BASE TABLEs, so a source-schema hypertable/CAGG would never reach `tenant_<uuid>` (the exact `mv-lost`/FARM-MEDIUM-058 trap). A correct Tier-B needs a shared `backend-common` `createTenantHypertable`/`createTenantContinuousAggregate` helper invoked by BOTH the per-tenant fan-out migration AND `SchemaManagerService.createTenantSchema` (new tenants), plus an entity PK recomposition (`id` → `("id","measuredAt")`) that collides with `wq-unique-lost`/FARM-HIGH-062 — a platform-shared effort gated on real WQ volume (~1200 rows/tenant/day today does not justify hypertable chunking). Owner: data-expert + sensor-expert (shared helper). Status: OPEN (2026-06-16, operator-sanctioned). Registry: orphan-findings.md only.

## ORPHAN-HIGH-129 — regulatory report read-back: no history/status query for 7 of 8 report types (real backend follow-up to fe-reports-mock)

Surfaced by fe-reports-mock (FARM-CRITICAL-052, 2026-06-16). The farm regulatory reporting feature has a REAL submit path (Maskinporten-authenticated Mattilsynet submission mutations) and REAL settings/status/health queries, but NO read-back query returning a tenant's historical submitted reports for sea-lice, smolt, cleaner-fish, slaughter, welfare, disease, or escape (only biomass has `biomassReports`). The dashboard previously faked this with mock fixtures; FARM-CRITICAL-052 removed the fabrication and the 7 tabs now render honest empty history lists. To restore real history (so operators can see what was submitted / what is overdue), build per-type read-back resolvers + a tenant-scoped persisted `regulatory_submissions` history (with server-side overdue/deadline computation), then wire each tab + a real overdue/penalty banner in ReportsPage. Owner: farm-expert + multi-tenant-saas-expert (tenant-scoped persistence). Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-MEDIUM-130 — wire BiomassReportTab history to the existing BIOMASS_REPORTS_QUERY

Surfaced by fe-reports-mock (2026-06-16). Unlike the other 6 non-biomass report types, biomass ALREADY has a real read query (`BIOMASS_REPORTS_QUERY` / `biomassReports(siteId, limit)` in `web/modules/farm-module/src/graphql/regulatory.operations.ts:313` + a farm-service resolver). FARM-CRITICAL-052 left `BiomassReportTab`'s history empty-stated for low-risk uniformity with the other tabs, but it can be wired to real data now with no new backend: add a `useBiomassReports(siteId)` hook (useQuery + graphqlClient.request + createTenantQueryKey, mirroring ReportSettingsModal) and replace the empty `reports` useMemo with the query result (verify the query's response shape maps to the tab's `BiomassReport` type). Owner: farm-expert. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-HIGH-131 — gql-codegen: bind farm-module to generated SDL types + regenerate the checked-in SDL / graphql-types snapshots (prevent FE/BE contract drift)

Surfaced by the Phase-5 contract cluster (close-batch-enum/FARM-HIGH-070, harvest-harvestedby/068, harvest-planid/069, 2026-06-16). Those three were all symptoms of ONE root cause: farm-module GraphQL operations are hand-written UNTAGGED template strings with hand-written TS enums/input types, none bound to the server SDL — so any backend enum/required-arg/field change is invisible to the FE compiler until it fails at runtime in the browser. The cluster fixed the live drift by hand (expanded the backend enum; deleted/added the harvest fields), but the FE types remain hand-written and can re-drift.

Codegen is ALREADY configured (`codegen.ts` emits `web/shared-ui/src/generated/graphql-types.ts`, which already carries the SDL-derived enums/inputs) — it is just UNCONSUMED by farm-module. Two deferred pieces (one toolchain):
1. **Regenerate the snapshots** after this cluster's backend changes: run `schema:emit` (per-subgraph SDL → `apps/farm-service/schema.graphql`) → `apollo-router:compose` → `npm run codegen` (→ `graphql-types.ts`). The runtime SDL is correct (autoSchemaFile), but the checked-in `apps/farm-service/schema.graphql` snapshot + `graphql-types.ts` are now stale (old harvest fields / 5-value BatchCloseReason). Not CI-gated today, but they are codegen's fallback source.
2. **Bind farm-module to the generated types** (Tier-1 make-it-impossible): re-point the hand-written `BatchCloseReason`, `CreateHarvestRecordInput`, etc. in `useBatches.ts` to `import type { … } from` the generated module, so the FE enum/input drift becomes a compile error. Then a Tier-3 lint widening `no-bare-graphql-query-string` to catch UNTAGGED operation strings (it currently only catches `gql`-tagged, blind to farm-module's 207 untagged ops).

The full TypedDocumentNode migration of all 207 untagged operation strings across 35 files is a larger multi-phase initiative (the cited "Phase 8.4 mass-migration" scale) and stays separate. Owner: farm-expert + frontend-expert. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-MEDIUM-132 — sentinelHubService does a raw fetch() to /api (bypasses the central client; same class as fe-upload-bypass)

Surfaced while fixing fe-upload-bypass (FARM-HIGH-071, 2026-06-16). `web/modules/farm-module/src/services/sentinelHubService.ts:388` calls `fetch('/api/sentinel-hub/catalog/search?...')` directly instead of the central `restClient` — so it gets no fresh-token, no refresh-on-401, no lifecycle barrier (it IS a GET, so CSRF is not required, which is why it's lower severity than the upload mutations). Out of scope for FARM-HIGH-071 (the upload-bypass invariant RULE 3 is scoped to `/upload`), but the same architectural class: internal authenticated calls must go through `restClient`. Fix: route it through `restClient.get('/sentinel-hub/catalog/search', params)`. The other sentinelHub/cmems fetches target EXTERNAL URLs (CDSE/proxy/cmems) and are legitimately outside the central client. Owner: frontend-expert. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.

## ORPHAN-MEDIUM-133 — fe-role-gating: useCanMutate applied to only ~13/100 farm-module surfaces (UX/defense-in-depth; backend is fail-closed)

Verified firsthand (access-boundary-auditor, Phase 5, 2026-06-16). `useCanMutate` gates only ~13 of ~100 farm-module files; dozens of destructive surfaces (all setup CRUD tabs, harvest-plan approve/cancel/delete, health-event delete/resolve/quarantine, cleaner-fish remove/deploy/transfer, storage receive/transfer/PO, several production modals) render their controls with NO client-side role gate. **NOT a privilege-escalation hole:** the backend is fully fail-closed — `permission-matrix.guard` is a global APP_GUARD with an EMPTY `UNGATED_OPERATIONS`, so every mutation a regular user clicks gets a server 403. The gap is pure UX (confusing rejected clicks) + defense-in-depth/consistency. Deferred from Phase 5 because the durable fix is large-mechanical.

Recommended architecture (mirrors the backend's drained-UNGATED_OPERATIONS pattern): (1) a `<CanMutate name="...">` wrapper + `useMutationGate(name)` in `web/shared-ui/src/authz` so the zero-effort path for a destructive control is the gated path; (2) expand `FRONTEND_MUTATION_ROLES` to cover every destructive mutation the UI surfaces (~40-60 names; the existing permission-matrix.parity.spec auto-locks each to the backend role set); (3) a make-it-detectable invariant seeded from the backend `MUTATION_ROLES` keys with an allowlist of currently-unwrapped surfaces that must SHRINK, not grow (so new destructive surfaces fail CI and the backlog is tracked); (4) wrap the surfaces module-by-module, draining the allowlist. Owner: frontend-expert + access-boundary-auditor. Status: OPEN (2026-06-16). Registry: orphan-findings.md only.
