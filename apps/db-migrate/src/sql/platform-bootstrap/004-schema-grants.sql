...
--
-- Privilege model:
--   - POSTGRES_USER (superuser) gets ALL on every service schema for
--     diagnostic, backup, and cross-service maintenance flows.
--   - Each *_service role gets ALL on its own schema (USAGE, table CRUD,
--     sequence advance).
...
GRANT ALL PRIVILEGES        ON ALL TABLES IN SCHEMA observability TO observability_service;
GRANT ALL PRIVILEGES        ON ALL SEQUENCES IN SCHEMA observability TO observability_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA observability GRANT ALL ON TABLES    TO observability_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA observability GRANT ALL ON SEQUENCES TO observability_service;

GRANT USAGE                 ON SCHEMA event_store   TO event_store_service;
GRANT ALL PRIVILEGES        ON ALL TABLES IN SCHEMA event_store   TO event_store_service;
GRANT ALL PRIVILEGES        ON ALL SEQUENCES IN SCHEMA event_store TO event_store_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA event_store   GRANT ALL ON TABLES    TO event_store_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA event_store   GRANT ALL ON SEQUENCES TO event_store_service;
