...
/**
 * A page of custom plans. An ALIAS, never a restatement: the field set lives in
 * `@platform/pagination-contracts` (ADMIN-HIGH-004), so a page whose shape this
 * file declared for itself could drift from the one the response interceptor
 * recognises — and an unrecognised page-shaped return is a 500 by design.
 */
export type CustomPlanPage = PaginationResultV1<CustomPlanResponseDto>;
...
export class CustomPlanService {
  ...
  async list(filter: CustomPlanFilter): Promise<CustomPlanPage> {
    ...
    return createStandardPaginatedResult(items.map(toCustomPlanResponse), total, page, limit);
  }
