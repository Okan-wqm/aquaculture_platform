...
class ALossAttestedOnlyByTheCompactionLedgerPublishes(MaintenanceLaneTestCase):
    """ARIA-HIGH-154 — the live shape on 2026-09-17: the published tip still
    claims hot artifacts that compactions BEFORE the ledger existed stripped
    (their governance rows carry no ``pruned_paths``); the next compaction
    strips nothing and backfills ``run-artifacts/compacted.jsonl`` from the
    archives; the continuity gate read only governance ``pruned_paths`` and
    refused the publish (``state_publish_continuity_surfaces_lost``, 18
    surfaces, run 35262206853) — so the heal ARIA-HIGH-117 built could never
    reach ``aria/state``, and every nightly restored the unhealed store."""

    def _record_old_run(self, store) -> str:
        ...
        old_cycle = "cyc-20200101T000000Z-auto"
        base = _run(run_id="run-old", cycle_id=old_cycle)
        record_run(base, base_dir=tools)
        hot = sorted((tools / "run-artifacts" / "hot" / old_cycle).rglob("tool_run.json"))
        self.assertEqual(len(hot), 1, hot)
    ...
    def test_an_absent_artifact_the_ledger_does_not_name_is_still_refused(self) -> None:
        ...
        self.assertTrue(first["published"])
        tools = tools_root(store)
        # Hand-deleted: no archive, no ledger row, no governance prune.
        hot = tools / old_key.split(":", 1)[1]
        hot.unlink()
        with self.assertRaises(StateStoreRefusal) as caught:
            self._publish(store, "snap-2", "cycle-2")
        # The runtime-artifact verification refuses first (no attestation names
        # the artifact, so its refs read as missing); the loss is named either way.
        self.assertTrue(str(caught.exception).startswith("state_publish_"), str(caught.exception))
        self.assertIn(old_key.split(":", 1)[1], str(caught.exception))
