...
**Fix (this PR):** `alarms.rs::is_triggered` and the delay gate now route ALL value/threshold/time math through `alarm_core` (`evaluate_condition`, `is_outside_deadband`, `delay_elapsed`); the inline math is deleted. Only this engine's own concerns stay local (the `enabled` gate, trigger-vs-clear edge selection, the `Deviation` two-level hysteresis band, and the 7-state machine). The kernel's `!=` deadband boundary is corrected to EXCLUSIVE (`<`) so `==`/`!=` stay exact complements at the edge; NaN fail-safe behaviour, the non-negative-deadband contract (`debug_assert!`), and the epsilon/`!=` edges are pinned by new shared fixtures + Rust/TS unit tests. Also fixed three latent CI-blockers on the WASM-adoption branch that CI had not yet exercised (CI stopped triggering mid-branch): the `crates/protocol-codec` nx project name collided with the new `libs/protocol-codec` TS lib (renamed the crate's nx project to `protocol-codec-rs`, matching the `event-contracts-rs` convention; cargo package name unchanged); a `f64 as u64` cast in the alarm-core fixture harness that `pedantic + -D warnings` rejects (reads the JSON integers as `u64` directly); and the `alarm-core` path dependency in `sens-api-gateway/Cargo.toml` (and `crates/alarm-core-wasm`) carried no version, which the edge `deny.toml` `wildcards = "deny"` ban rejects as a wildcard dependency — pinned to `version = "0.1.0"` (matches the crate; the lock is unchanged).
