...
import { useAuth, useToast } from '@aquaculture/shared-ui';
...
const DeleteConfirmDialog: React.FC<{
  ...
  <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="delete-dialog-title" role="dialog" aria-modal="true">
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div className="mt-2">
              <p className="text-sm text-gray-500">
                Are you sure you want to delete the protocol <strong>"{protocol.name}"</strong>?
...
const ProtocolDetailDrawer: React.FC<{
  ...
  <div className="fixed inset-0 z-40 overflow-hidden" aria-labelledby="detail-title" role="dialog" aria-modal="true">
          ...
          <div className="h-full flex flex-col bg-white shadow-xl overflow-y-scroll">
            ...
            <div className="px-4 py-6 bg-blue-600 sm:px-6">
              <div className="flex items-start justify-between">
                <div>
                  <h2 id="detail-title" className="text-lg font-medium text-white">{protocol.name}</h2>
                  <p className="mt-1 text-sm text-blue-200">{protocol.species} - {feedStageLabels[protocol.stage] || protocol.stage}</p>
                </div>
              ...
              <div className="mt-2 flex gap-2">
                ...
                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${protocol.isActive ? 'bg-green-400 text-green-900' : 'bg-gray-400 text-gray-900'}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {protocol.minDissolvedOxygen != null && (
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="text-lg font-semibold text-gray-900">{protocol.minDissolvedOxygen}</p>
                  </div>
                )}
              </div>
              ...
              {protocol.optimalTemperature && (
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">
                    {protocol.optimalTemperature.min} - {protocol.optimalTemperature.max} {protocol.optimalTemperature.unit === 'celsius' ? '\u00B0C' : '\u00B0F'}
                  </p>
                </div>
              )}
                    ...
                    {protocol.temperatureRanges.map((range, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
                        <span className="text-sm text-gray-900">
                          {range.min} - {range.max} {range.unit === 'celsius' ? '\u00B0C' : '\u00B0F'}
                        </span>
                        <span className={`text-sm font-medium ${range.feedingMultiplier >= 1 ? 'text-green-600' : 'text-orange-600'}`}>
                    ...
                    {protocol.defaultSchedule.adjustments && (
                      <div className="mt-2 pt-2 border-t border-gray-200 text-xs text-gray-500">
                        {protocol.defaultSchedule.adjustments.lowOxygenReduction != null && (
                          <p>Low O2 reduction: {protocol.defaultSchedule.adjustments.lowOxygenReduction}%</p>
                        )}
                        {protocol.defaultSchedule.adjustments.postStressReduction != null && (
                          <p>Post-stress reduction: {protocol.defaultSchedule.adjustments.postStressReduction}%</p>
                        )}
                        {protocol.defaultSchedule.adjustments.preMedicationFasting != null && (
                          <p>Pre-medication fasting: {protocol.defaultSchedule.adjustments.preMedicationFasting}h</p>
                        )}
                      </div>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    {protocol.specialConditions.spawningPeriod && (
                      <div><span className="font-medium text-gray-700">Spawning:</span> {protocol.specialConditions.spawningPeriod}</div>
                    )}
                    {protocol.specialConditions.winterFeeding && (
                      <div><span className="font-medium text-gray-700">Winter:</span> {protocol.specialConditions.winterFeeding}</div>
                    )}
                    {protocol.specialConditions.diseaseOutbreak && (
                      <div><span className="font-medium text-gray-700">Disease:</span> {protocol.specialConditions.diseaseOutbreak}</div>
                    )}
                    {protocol.specialConditions.waterQualityIssues && (
                      <div><span className="font-medium text-gray-700">Water Quality:</span> {protocol.specialConditions.waterQualityIssues}</div>
                    )}
                  </div>
                </div>
              )}
            ...
            <div className="flex-shrink-0 px-4 py-4 flex justify-end gap-3 border-t border-gray-200">
              {!protocol.isDefault && protocol.isActive && (
                <button
                  ...
                  className="inline-flex items-center px-3 py-2 border border-yellow-300 shadow-sm text-sm font-medium rounded-md text-yellow-700 bg-yellow-50 hover:bg-yellow-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 disabled:opacity-50"
                >
...
const ProtocolFormModal: React.FC<{
  ...
  const updateTemperatureRange = (index: number, field: keyof TemperatureRange, value: number | string) => {
  ...
  const removeTemperatureRange = (index: number) => {
    updateField('temperatureRanges', form.temperatureRanges.filter((_, i) => i !== index));
  };
  ...
  const removeGrowthStage = (index: number) => {
    updateField('growthStageProtocols', form.growthStageProtocols.filter((_, i) => i !== index));
  };

  // Meal times management
  const addMealTime = () => {
    updateField('mealTimes', [
      ...form.mealTimes,
      { time: '08:00', percentOfDaily: 0 },
    ]);
  };

  const updateMealTime = (index: number, field: keyof FeedingScheduleEntry, value: string | number) => {
  ...
  const removeMealTime = (index: number) => {
    updateField('mealTimes', form.mealTimes.filter((_, i) => i !== index));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="form-dialog-title" role="dialog" aria-modal="true">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
          <form onSubmit={handleSubmit}>
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900 mb-6" id="form-dialog-title">
                {isEdit ? 'Edit Feeding Protocol' : 'Create Feeding Protocol'}
              </h3>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {speciesList.map((sp) => (
                        <option key={sp.code} value={sp.commonName}>{sp.commonName}</option>
                      ))}
                ...
                <div>
                  ...
                  {form.temperatureRanges.length === 0 && (
                    <p className="text-xs text-gray-400 italic">No temperature ranges defined. Click "Add Range" to add one.</p>
                  )}
                  <div className="space-y-2">
                    {form.temperatureRanges.map((range, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
                        <input
                          ...
                          step="0.5"
                          value={range.min}
                          onChange={(e) => updateTemperatureRange(idx, 'min', parseFloat(e.target.value) || 0)}
                        ...
                        <input
                          ...
                          step="0.5"
                          value={range.max}
                          onChange={(e) => updateTemperatureRange(idx, 'max', parseFloat(e.target.value) || 0)}
                          className="w-20 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                          placeholder="Max"
                        />
                        ...
                        <input
                          ...
                          onChange={(e) => updateTemperatureRange(idx, 'feedingMultiplier', parseFloat(e.target.value) || 0)}
                          className="w-20 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                        />
                        <button
                          type="button"
                ...
                <div>
                  ...
                  {form.growthStageProtocols.length === 0 && (
                    <p className="text-xs text-gray-400 italic">No weight ranges defined. Click "Add Weight Range" to add one.</p>
                  )}
                  <div className="space-y-3">
                      ...
                      <div key={idx} className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-2">
                          <input
                            type="number"
                            value={gsp.minWeight}
                            onChange={(e) => updateGrowthStage(idx, 'minWeight', parseFloat(e.target.value) || 0)}
                          ...
                          <input
                            type="number"
                            value={gsp.maxWeight}
                            onChange={(e) => updateGrowthStage(idx, 'maxWeight', parseFloat(e.target.value) || 0)}
                            className="w-24 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                            placeholder="Max wt"
                          />
                          <select
                          ...
                          <input
                            ...
                            onChange={(e) => updateGrowthStage(idx, 'feedPercent', parseFloat(e.target.value) || 0)}
                            className="w-20 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                          />
                          <button
                            type="button"
                ...
                <div>
                  ...
                  {form.mealTimes.length > 0 && (
                    <div className="mb-3">
                      <label className="block text-xs font-medium text-gray-500 mb-1">Total Meals/Day</label>
                      <input
                        type="number"
                        min={1}
                        max={12}
                  ...
                  {form.mealTimes.length === 0 && (
                    <p className="text-xs text-gray-400 italic">No meal times defined. Click "Add Meal" to add one.</p>
                  )}
                  <div className="space-y-2">
                    {form.mealTimes.map((meal, idx) => (
                      <div key={idx} className="flex items-center gap-3 bg-gray-50 rounded-lg p-2">
                        ...
                        <input
                          ...
                          onChange={(e) => updateMealTime(idx, 'percentOfDaily', parseFloat(e.target.value) || 0)}
                          className="w-20 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                        />
                        <div className="flex-1" />
                        <button
...
export const ProtocolsTab: React.FC<ProtocolsTabProps> = () => {
  ...
  const handleSave = useCallback(async (input: CreateFeedingProtocolInput | UpdateFeedingProtocolInput) => {
    try {
      ...
      toast({ title: 'Error', description: 'Failed to save protocol. Please try again.', variant: 'error' });
    }
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Failed to delete protocol. Please try again.', variant: 'error' });
    }
  }, [deletingProtocol, deleteMutation, toast]);

  const handleSetDefault = useCallback(async (protocol: FeedingProtocol) => {
    try {
      await setDefaultMutation.mutateAsync(protocol.id);
      toast({ title: 'Success', description: `"${protocol.name}" set as default for ${protocol.species}.` });
    } catch (err) {
      if (import.meta.env.DEV) console.error('Failed to set default:', err);
      toast({ title: 'Error', description: 'Failed to set default protocol. Please try again.', variant: 'error' });
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex flex-wrap gap-3">
          ...
          >
            ...
            {Array.from(new Set([
              ...uniqueSpecies,
              ...speciesList.map(s => s.commonName),
            ])).sort().map((sp) => (
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-semibold text-gray-900 truncate">{protocol.name}</h3>
                    ...
                    <div className="flex gap-1.5 ml-2 flex-shrink-0">
                      ...
                      <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium ${stageColor}`}>
                  ...
                  {/* Description */}
                  {protocol.description && (
                    <p className="text-xs text-gray-500 mb-3 line-clamp-2">{protocol.description}</p>
                  )}

                  {/* Key Metrics */}
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600 mb-3">
                    {protocol.targetFcr != null && (
                      <span>FCR: <strong>{protocol.targetFcr}</strong></span>
                    )}
                    {protocol.optimalTemperature && (
                      <span>
                        Temp: <strong>{protocol.optimalTemperature.min}-{protocol.optimalTemperature.max}{'\u00B0C'}</strong>
                      </span>
                    )}
                    {protocol.growthStageProtocols && protocol.growthStageProtocols.length > 0 && (
                      <span>{protocol.growthStageProtocols.length} weight range{protocol.growthStageProtocols.length !== 1 ? 's' : ''}</span>
                    )}
                    {protocol.defaultSchedule && (
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {protocol.temperatureRanges.slice(0, 3).map((tr, idx) => (
                        <span key={idx} className="inline-flex items-center px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 text-xs">
                          {tr.min}-{tr.max}{'\u00B0'} ({tr.feedingMultiplier}x)
                        </span>
                      ))}
                      {protocol.temperatureRanges.length > 3 && (
                        <span className="text-xs text-gray-400">+{protocol.temperatureRanges.length - 3} more</span>
                      )}
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                    <span className={`text-xs ${protocol.isActive ? 'text-green-600' : 'text-gray-400'}`}>
                      {protocol.isActive ? 'Active' : 'Inactive'}
                    </span>
                    <div className="flex gap-1">
                      {!protocol.isDefault && protocol.isActive && (
                        <button
                          onClick={(e) => { e.stopPropagation(); handleSetDefault(protocol); }}
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); setDeletingProtocol(protocol); }}
