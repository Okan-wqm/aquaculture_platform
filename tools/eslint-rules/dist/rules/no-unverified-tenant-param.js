...
exports.TENANT_KEYS = new Set(['tenantId', 'tenant_id', 'tenantid']);
const RAW_PARAM_DECORATORS = new Set(['Param', 'Query']);
const VALIDATOR_DECORATOR = /^(Is[A-Z]|Validate|Matches$|Length$|MinLength$|MaxLength$|Min$|Max$|ArrayNotEmpty$|ArrayMinSize$|ArrayMaxSize$|Equals$|NotEquals$|Contains$)/;
 ...
 */
function isTenantIdCarrier(member) {
    const carrier = (member.decorators ?? []).some((d) => decoratorName(d) === 'TenantIdCarrier');
    const annotation = member.typeAnnotation?.typeAnnotation;
    return carrier && annotation?.type === utils_1.AST_NODE_TYPES.TSUndefinedKeyword;
}
function isValidatedInputClass(node) {
    return node.body.body.some((member) => member.type === utils_1.AST_NODE_TYPES.PropertyDefinition &&
...
exports.default = createRule({
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'A handler takes a tenant id only through @TenantParam(), which resolves it against auth.tenants before the handler runs (ADMIN-CRITICAL-009).',
        },
        messages: {
            rawTenantParam: "@{{decorator}}('{{key}}') hands the handler an unverified tenant id. Use @TenantParam('{{source}}') — it resolves the id against auth.tenants and states which lifecycle statuses the route admits.",
            tenantInInputDto: "'{{name}}' on a validated input DTO is an unverified tenant identity. Take it with @TenantParam('body') on the handler and declare the key as `@TenantIdCarrier() readonly {{name}}?: undefined`; a read filter belongs on a *QueryDto.",
        },
        schema: [],
    },
    defaultOptions: [],
    create(context) {
        function checkParameter(param) {
            for (const decorator of param.decorators ?? []) {
                ...
                if (!call || !RAW_PARAM_DECORATORS.has(call.name))
                    continue;
                const first = call.args[0];
                if (!first || first.type !== utils_1.AST_NODE_TYPES.Literal || typeof first.value !== 'string')
                    continue;
                if (!exports.TENANT_KEYS.has(first.value))
                    continue;
                context.report({
                    node: decorator,
        ...
        function checkClass(node) {
            if (!isValidatedInputClass(node))
                return;
            if (node.id && READ_ONLY_DTO.test(node.id.name))
                return;
            for (const member of node.body.body) {
                if (member.type !== utils_1.AST_NODE_TYPES.PropertyDefinition)
                    continue;
                const name = propertyName(member);
                if (!name || !exports.TENANT_KEYS.has(name))
                    continue;
                if (isTenantIdCarrier(member))
                    continue;
...
//# sourceMappingURL=no-unverified-tenant-param.js.map