...
async function bootstrap() {
  const logger = new Logger('HydroponicsService');

  const app = await NestFactory.create(AppModule, {
    logger: new StructuredLoggerService('hydroponics-service'),
  });
  ...
  const port = configService.get<number>('HYDROPONICS_SERVICE_PORT', 4007);
...
bootstrap().catch((error) => {
  bootstrapLogger.error('Hydroponics Service failed to start:', error);
  process.exit(1);
});
