...
function playwrightTransport(body: unknown): { post: jest.Mock } {
  return { post: jest.fn().mockResolvedValue({
...
describe('E2E successful GraphQL operations have one data contract', () => {
  it('returns actual data through the Playwright transport', async () => {
    ...
    await expect(client.executeSuccess({ query: 'mutation { createSite { id } }', token: 'fixture-token' }))
  ...
  it('rejects GraphQL errors even with partial data and HTTP 200', async () => {
    const client = new GraphQLTestClient(playwrightTransport({
      data: { createSite: null }, errors: [{ message: 'Not authorized' }],
    }));
    await expect(client.executeSuccess({ query: 'mutation { createSite { id } }' }))
  ...
  it.each([{}, { data: null }])('rejects an empty successful envelope', async (body) => {
    ...
    await expect(client.executeSuccess({ query: '{ currentUser { id } }' }))
