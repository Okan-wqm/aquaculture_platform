...
### D3 — Per-tenant write = connection-scoped GUC + batched multi-row INSERT (NOT per-tick `runInTenantTransaction`)
...
BUT the review's efficiency point stands: setting the GUC via a full `runInTenantTransaction` per tick per tenant (~5 round-trips × N × 1 Hz) is wasteful. Instead: set `app.current_tenant` **per connection** (`RlsConnectionBootstrap` pattern) and **batch** persistence — because `tenant_id` is a column, one `INSERT … VALUES (t1,…),(t2,…)` flushes all tenants (the `tag-value-fanout.service.ts:165-191` buffered-upsert pattern the codebase already uses). This resolves 414 (writes satisfy the existing policy) at near-zero per-tick cost and needs no new policy class.
