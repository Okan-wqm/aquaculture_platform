...
describe('MealPlanGeneratorService.persistDayPlan — real Postgres', () => {
  ...
  // irrelevant to this suite's surface — persistDayPlan touches neither. They
  // are EMPTY typed doubles rather than nulls: if persistDayPlan ever grows a
  // call into one of them, this suite fails with a MissingDoubleMemberError
  // naming the service and the member instead of a bare `Cannot read
  // properties of null`.
  const service = new MealPlanGeneratorService(
    collaborator<ProtocolRateService>({}, 'ProtocolRateService'),
    collaborator<ProtocolResolutionService>({}, 'ProtocolResolutionService'),
  );
