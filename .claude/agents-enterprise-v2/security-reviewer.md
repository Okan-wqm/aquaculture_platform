...
`SEC-{SEVERITY}-{NNN}` — e.g. `SEC-CRITICAL-001`, `SEC-HIGH-007`. Zero-padded sequential within one report. See `@.claude/agents-enterprise-v2/_shared/output-format.md`. Required by `Closes:` commit convention (CLAUDE.md), context-manager state tracking, implementation-planner package traceability.
