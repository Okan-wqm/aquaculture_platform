...
describe('collaborator', () => {
  ...
  it('throws MissingDoubleMemberError when code touches an un-modelled member', () => {
    ...
    expect(() => applier.lockUnitForGrowth('unit-1')).toThrow(
      MissingDoubleMemberError,
    );
  });
  ...
  describe('runtime and framework probes must not be mistaken for drift', () => {
    const double = collaborator<GrowthApplier>(
      { applyGrowth: jest.fn() },
      'GrowthApplier',
    );
    ...
    it('survives symbol-keyed protocol probes', () => {
      expect(() => (double as { [Symbol.toPrimitive]?: unknown })[Symbol.toPrimitive]).not.toThrow();
