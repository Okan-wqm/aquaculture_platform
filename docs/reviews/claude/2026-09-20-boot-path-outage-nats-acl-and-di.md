...
**Findings:** INFRA-CRITICAL-182, INFRA-CRITICAL-183, SENSOR-CRITICAL-125, BILLING-CRITICAL-017 (all
fixed here); ORPHAN-MEDIUM-829 (baselined by the new invariant); ORPHAN-HIGH-830 (the class-level
gate, tracked with owner and deadline).
...
droplet's `.env` carries `http://89.38.97.90`). That is an operator setting, not a repository
defect, and is outside this cycle.
...
## INFRA-CRITICAL-182 — the scoped inbox prefix ends in a dot
...
**Fix.** No trailing dot — the same convention the two fixed prefixes in `@platform/event-contracts`
(`_INBOXBILLINGCFG`, `_INBOXFARMMARINECFG`) already follow.

## INFRA-CRITICAL-183 — the JetStream manager probes `$JS.API.INFO`
...
**Fix.** `jetstreamManager(connection, { checkAPI: false })`. `setupStream` runs immediately after
and its `STREAM.INFO` request fails just as loudly when JetStream is off, so the probe bought
nothing the boot path did not already check. The signals spec pins the option.

## SENSOR-CRITICAL-125 — SensorErasureModule re-provides MqttAuthService
...
## BILLING-CRITICAL-017 — a type-only import of an injected class
...
- `scripts/nats/boot-path-acl-smoke-harness.sh` + `tools/scripts/nats-boot-path-acl-smoke.ts`: a
  ...
  JetStream sequence (`jetstreamManager({ checkAPI: false })`, then create-or-update of the three
  streams). Wired into `nats-invariants.yml`; the workflow now also triggers on
  `libs/backend-common/src/nats/**`. With either NATS defect restored, all fifteen identities fail.
...
- `tests/invariants/nest-module-provider-duplication.spec.ts`: a module that lists a class another
  ...
  in farm-service and admin-api-service are baselined under ORPHAN-MEDIUM-829; the list only
  shrinks.

## ORPHAN-HIGH-830 — the class-level gap
...
- The droplet's `FRONTEND_URL` must become `https://app.suderra.com` before auth-service boots; the
  deploy environment file is outside the repository and this session cannot read or edit it.
- ORPHAN-MEDIUM-829 (the twelve baselined duplicates) and ORPHAN-HIGH-830 (the dependency-graph
