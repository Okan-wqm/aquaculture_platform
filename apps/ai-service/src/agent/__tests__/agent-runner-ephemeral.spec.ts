...
describe('AgentRunnerService ephemeral runs (FARM-AI 1.2)', () => {
  ...
  const buildHarness = async (providerResponses: LlmChatResult[]) => {
    ...
    // SEC-MEDIUM-075 (main): budget accounting is reserve → settle per call.
  ...
  it('still bills the token budget and appends a ledger row keyed on correlationId + servicePrincipal', async () => {
    ...
    // The reservation is settled to the actual billable usage (input + output
    // + cache creation = 160); cache reads stay excluded.
    expect(harness.reserveBudget).toHaveBeenCalledWith(
      tenantId,
      expect.any(Number),
      expect.any(Number),
    );
    expect(harness.settleReservation).toHaveBeenCalledWith(tenantId, expect.any(Number), 160);
