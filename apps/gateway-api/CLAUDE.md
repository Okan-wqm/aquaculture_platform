...
CI: `tests/invariants/tenant-context-ssot.spec.ts`, `graphql-operation-limit-ssot.spec.ts`, `plan-features-ssot.spec.ts`, `public-service-edge-hardening.spec.ts`, `token-revocation-writer-reader-ssot.spec.ts`, `service-identity-canonical-coverage.spec.ts`. `e2e/tests/tenant-swap-attack.e2e.spec.ts` exists but has no CI runner yet (SENSOR-HIGH-070).
