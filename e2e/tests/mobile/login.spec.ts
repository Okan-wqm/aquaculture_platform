...
test.describe('AquaMobil login', () => {
  ...
  test('a seeded field worker logs in through the real form and lands on the dashboard', async ({
    page,
  }) => {
  ...
  test('an unknown deep link renders the 404 page, not a silent bounce home (MOB-LOW-005)', async ({
    page,
  }) => {
    await loginAsFieldWorker(page, seed.user.email, FIXTURE_PASSWORD);
