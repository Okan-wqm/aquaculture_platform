...
export class MfaService {
   ...
   *   - unusable as a bearer credential (enforceAccessTokenType rejects
   *     everything whose type !== 'access'),
   ...
   * sub-prefix check so no other token shape (access/refresh/mfa_challenge)
   * can ever be accepted here.
   */
  resolveSetupTokenUserId(mfaSetupToken: string): string {
    ...
    try {
      ...
      throw new UnauthorizedException(
        'MFA setup token is invalid or expired. Please login again.',
      );
    }
