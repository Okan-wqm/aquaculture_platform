...
export class TransferBatchHandler implements ICommandHandler<TransferBatchCommand, Batch> {
  ...
  async execute(command: TransferBatchCommand): Promise<Batch> {
    ...
    const transferredBatch = await runInTenantTransaction(
      this.dataSource,
      'farm',
      tenantId,
      async (queryRunner, mutationSession) => {
        ...
        const sourceSiteId = await resolveTankSiteId(
          queryRunner.manager,
          payload.sourceTankId,
          tenantId,
        );
        this.siteAuth.assertSiteAssignment({ caller: siteCaller, siteId: sourceSiteId });
        const destSiteId = await resolveTankSiteId(
          queryRunner.manager,
          payload.destinationTankId,
          tenantId,
        );
        ...
        return this.tankBatchService.withLockedTankBatchSet(
          queryRunner.manager,
          mutationSession,
          tenantId,
          [payload.sourceTankId, payload.destinationTankId],
          async (tankBatchSet) => {
            // Both legs are now one canonical `tankId ASC` lock set. Payload
            // direction cannot influence lock order, and an empty destination
            // is fenced by the set authority before its first insert.
            const sourceTankBatch = tankBatchSet.snapshot(payload.sourceTankId);
        ...
        const availableQuantity =
          batchInSource?.quantity ||
          (sourceTankBatch.primaryBatchId === batchId ? sourceTankBatch.totalQuantity : 0);
        ...
        const quantity = resolvedRemoval.count;
        const biomassKg = resolvedRemoval.biomassKg;
        if (quantity > availableQuantity) {
          throw new BadRequestException(
            `Transfer miktarı (${quantity}) kaynak tank'taki batch miktarından (${availableQuantity}) fazla olamaz`,
          );
        }
        ...
        if (!payload.skipCapacityCheck) {
          const destTankBatch = tankBatchSet.snapshot(payload.destinationTankId);
        ...
        const transferDate = payload.transferredAt ?? mutationInstantDateV1(mutationInstant);
        ...
        const sourceOperation = queryRunner.manager.create(TankOperation, {
          ...
          quantity,
          countDerived: resolvedRemoval.countDerived,
          avgWeightG,
          biomassKg,
        ...
        // CapacityService compiles both post-delta projections from the locked
        // snapshots before either leg mutates; each projection is persisted by
        // the SAME aggregate commit as its stock delta. There is no second
        // capacity-only TankBatch write that could publish an intermediate state.
        const sourceCapacity = this.tankCapacityService.calculate({
          equipment: sourceTank,
          existing: {
            salmonBiomassKg: Math.max(
              0,
              Number(sourceTankBatch.totalBiomassKg) - biomassKg,
            ),
            cleanerBiomassKg: Number(sourceTankBatch.cleanerFishBiomassKg || 0),
          },
          incomingBiomassKg: 0,
        });
        const destCapacity = this.tankCapacityService.calculate({
          equipment: destinationTank,
          existing: {
            salmonBiomassKg: Number(destTankBatch?.totalBiomassKg ?? 0) + biomassKg,
            cleanerBiomassKg: Number(destTankBatch?.cleanerFishBiomassKg || 0),
          },
          incomingBiomassKg: 0,
        });
        const savedSourceTankBatch = await tankBatchSet.applyBatchDelta(
          payload.sourceTankId,
          {
            batchId,
            batchNumber: batch.batchNumber,
          ...
          },
          {
            ...
            capacity: {
              isOverCapacity: sourceCapacity.isOverCapacity,
              utilizationPercent: sourceCapacity.utilizationPercent,
            },
          },
        );

        const savedDestTankBatch = await tankBatchSet.applyBatchDelta(
          payload.destinationTankId,
          {
            batchId,
            batchNumber: batch.batchNumber,
          ...
          },
          {
            ...
            capacity: {
              isOverCapacity: destCapacity.isOverCapacity,
              utilizationPercent: destCapacity.utilizationPercent,
            },
          },
        );
        ...
        if (sourceLookup.isFromTanksTable && sourceLookup.originalTank) {
          await this.batchMutations.replaceTankBiomassProjection(mutationSession, {
            tankId: sourceLookup.originalTank.id,
            currentBiomassKg: newSourceBiomass,
            lifecycle: 'preserve',
          });
        } else {
        ...
        if (destLookup.isFromTanksTable && destLookup.originalTank) {
          ...
          await this.batchMutations.replaceTankBiomassProjection(mutationSession, {
            tankId: destOriginalTank.id,
            currentBiomassKg: newDestBiomass,
            lifecycle: shouldActivate ? 'activate' : 'preserve',
          });
        } else {
          const shouldActivate =
          ...
          await queryRunner.manager
            ...
            .set({
              ...
              ...(shouldActivate ? { status: EquipmentStatus.ACTIVE } : {}),
            })
        ...
        const updatedSourceTankBatch = savedSourceTankBatch;
        const updatedDestTankBatch = savedDestTankBatch;
        ...
        const transferEvent: BatchTransferredEvent = {
          ...createBaseEvent<BatchTransferredEvent>('BatchTransferred', tenantId, {
            aggregateId: batchId,
            aggregateType: 'Batch',
            version: 2,
          }),
    ...
    this.logger.log(
      `Batch ${batchId} transferred: tank ${payload.sourceTankId} → ${payload.destinationTankId}, ` +
        `quantity=${payload.quantity ?? 'derived-from-biomass'}, tenant=${tenantId}`,
    );
