...
describe('sendErrorBannerKey', () => {
  it('maps each contractual code to its specific banner copy key', () => {
    expect(sendErrorBannerKey(clientError('TOO_MANY_REQUESTS'))).toBe(
    ...
    expect(sendErrorBannerKey(clientError('UNAUTHENTICATED'))).toBe(
      'messaging.error.unauthenticated',
    );
  });

  it('falls back to the generic send-failed banner for unknown codes', () => {
    expect(sendErrorBannerKey(clientError('INTERNAL_SERVER_ERROR'))).toBe(
