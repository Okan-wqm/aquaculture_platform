...
import {
  ...
  MortalityCause,
  isMortalityCause,
} from '../entities/mortality-record.entity';
  ...
  async execute(command: RecordMortalityCommand): Promise<Batch> {
    ...
    return runInTenantTransaction(
      ...
      async (queryRunner, mutationSession) => {
        ...
        // FARM-LOW-050: the initial-stock ceiling must use the canonical count,
        // including the explicit count derived by biomass-only mode.
        this.mortalityCullPolicy.assertAggregateWithinInitial({
          batch,
          addedRemoval: quantity,
        });
        ...
        const operation = queryRunner.manager.create(TankOperation, {
          ...
          mortalityReason: isMortalityReason(payload.reason)
            ? payload.reason
        ...
        if (tankLookup.isFromTanksTable && tankLookup.originalTank) {
          await this.batchMutations.replaceTankBiomassProjection(mutationSession, {
            tankId: tankLookup.originalTank.id,
            currentBiomassKg: newBiomass,
            lifecycle: 'preserve',
          });
        } else {
        ...
        const mortalityEvent: MortalityRecordedEvent = {
          ...createBaseEvent<MortalityRecordedEvent>('MortalityRecorded', tenantId, {
            aggregateId: batchId,
            aggregateType: 'Batch',
            version: 2,
          }),
        ...
        await this.auditLogService.logWithManager(queryRunner.manager, {
          ...
          summary:
            `Mortality ${quantity} from batch ${batch.batchNumber} ` +
