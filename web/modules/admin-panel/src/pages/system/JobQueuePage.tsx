...
export const JobQueuePage: React.FC = () => {
  ...
  const scheduledJobColumns: DataTableColumn<BackgroundJob>[] = [
    {
      key: 'jobName',
      header: 'Job Name',
      render: (_value, job) => <span className="font-medium text-gray-900">{job.name}</span>,
    },
    {
      key: 'type',
      header: 'Type',
      render: (_value, job) => <Badge variant="info">{job.jobType}</Badge>,
    },
    {
      key: 'schedule',
      header: 'Schedule',
      render: (_value, job) => (
        <span className="font-mono text-sm text-gray-600">{job.cronExpression || '-'}</span>
      ),
    },
    {
      key: 'nextRun',
      header: 'Next Run',
      render: (_value, job) => (
        <span className="text-sm text-gray-600">{formatDateTime(job.nextRunAt)}</span>
      ),
    },
    {
      key: 'status',
      header: 'Status',
      render: (_value, job) => <Badge variant={getStatusBadge(job.status)}>{job.status}</Badge>,
    },
  ];
  ...
  const backgroundJobColumns: DataTableColumn<BackgroundJob>[] = [
    ...
    },
    {
      key: 'queue',
      header: 'Queue',
      render: (_value, job) => <span className="text-sm text-gray-600">{job.queueName}</span>,
    },
    {
      key: 'status',
      header: 'Status',
          ...
          {job.progress ? (
            <div className="min-w-[120px]">
              ...
              {job.progress.message && (
                <div className="text-xs text-gray-500 line-clamp-1">{job.progress.message}</div>
              )}
            </div>
          ) : job.durationMs ? (
    ...
    },
    {
      ...
      render: (_value, job) => (
        <div className="text-sm text-gray-600">
          ...
          {!job.startedAt && !job.completedAt && <span className="text-gray-500">-</span>}
        </div>
      ),
    },
    {
