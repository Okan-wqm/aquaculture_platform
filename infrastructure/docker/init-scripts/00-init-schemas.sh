...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER:-postgres}" --dbname "${POSTGRES_DB:-postgres}" <<-EOSQL
  ...
  -- NOTE: POSTGRES_USER grants for SCHEMA_REGISTRY schemas (auth, farm,
  -- sensor, hr, messaging, hydroponics, alert, billing, notification, ai,
  -- admin, observability, event_store) are emitted by the GENERATED block
  -- above. The legacy block that used to live here has been removed —
  -- duplicates are idempotent but reviewer-confusing. Gateway-only grants
  -- are declared next to gateway's CREATE SCHEMA above.
