...
function bindSyncBridge(ctx: QuickJSAsyncContext, name: string, fn: SyncBridge): void {
 ...
 */
function bindAsyncBridge(ctx: QuickJSAsyncContext, name: string, fn: AsyncBridge): void {
...
function bindConsole(ctx: QuickJSAsyncContext, console: SandboxBridges['console']): void {
...
async function raceDeadline<T>(promise: Promise<T>, deadline: number): Promise<T> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => reject(new Error('Script execution timed out')), remaining);
  });
  try {
