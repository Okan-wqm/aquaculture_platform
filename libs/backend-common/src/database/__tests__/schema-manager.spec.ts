...
describe('SchemaManagerService', () => {
  ...
  const mockQuery = jest.fn<Promise<unknown>, [sql: string, params?: unknown[]]>();
  ...
  describe('migrateDataToTenantSchema', () => {
    ...
    it('should only fall back to camelCase tenantId when tenant_id column is absent', async () => {
      ...
      expect(defined(insertCalls[0], 'Expected tenant_id INSERT call')[0]).toContain(
        'WHERE tenant_id = $1',
      );
      expect(defined(insertCalls[1], 'Expected tenantId fallback INSERT call')[0]).toContain(
        'WHERE "tenantId" = $1',
      );
    });
    ...
    it('should use ON CONFLICT DO NOTHING for idempotent migration', async () => {
      ...
      const insertCall = defined(
        mockQuery.mock.calls.find((call) => call[0].includes('INSERT INTO')),
