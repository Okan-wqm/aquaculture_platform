...
describe('palette-parity guard', () => {
  it('every palette widget type resolves to a WidgetRenderer', () => {
    ...
    const unrenderable = [
      ...new Set(paletteWidgets.map((w) => w.type)),
    ].filter((t) => !rendererTypes.has(t));
    expect(unrenderable).toEqual([]);
  });

  it('every equipment symbol is reachable from the palette', () => {
    const missing = Object.keys(symbolMap).filter(
      (sub) => !paletteEquipmentSubTypes.has(sub),
    );
  ...
  it('every palette equipment subtype has a registered symbol', () => {
    ...
    const dangling = [...paletteEquipmentSubTypes].filter(
      (sub) => !registered.has(sub),
    );
