...
const PRE_PHASE6_SHAS: ReadonlySet<string> = new Set([
  ...
  // The sibling session's registry ceremony (#1092) claimed ORPHAN-560
  // through the finding-registry CLI - the allocator with a lease behind it -
  // while this commit was already pushed citing the same number from the
  // orphan document, which allocates by hand. This branch's finding moved to
  // 565; the pushed trailer cannot follow without a force-push.
