...
### Assignment-like secret generator specs were scanned as credentials

The droplet secret catalogue encoded every entry as a colon-delimited
`name:command` string and the bootstrap split and `eval`-invoked the command.
That representation was indistinguishable from a literal secret assignment to
the repository's detector. Adding the Sentinel cutover requirement therefore
made the real Gitleaks workflow fail even though the right-hand value was a
function name. Treating the match as an allowlist exception would have retained
both the ambiguous representation and the unnecessary evaluation boundary.

The closure must:

- represent the catalogue as structurally separate name and generator-function
  elements without assignment-like delimiters;
- validate pair cardinality, environment variable names and declared generator
  functions before any consumer uses the catalogue;
- invoke only the validated function directly, never through `eval`;
- derive names and counts from the same catalogue so bootstrap and deployment
  preflight cannot drift;
- preserve the fail-closed, pre-provisioned Sentinel legacy-key behavior.

Verification: Bash syntax checks and catalogue execution prove ten valid
name/function pairs; the farm deployment invariant pins the Sentinel pair and
rejects the former colon encoding; the GitHub Gitleaks workflow is the final
detector-level proof.

## FARM-HIGH-289

### Investor documentation carried a committed super-admin credential literal

The investor-pitch document contained a concrete super-admin password-shaped
value. The file changed in the environmental-monitoring commit, so the value
was reintroduced in that commit's patch and correctly failed the Gitleaks
workflow even though the same unsafe example existed on the default branch.
An allowlist would conceal the exact class of leak the detector exists to stop.

The closure must:

- replace the concrete value with an explicit secret-store placeholder;
- keep the documentation useful without publishing credential material;
- add no path, fingerprint or regex exception to Gitleaks;
- treat the historical value as non-reusable and require operational rotation
  if it was ever assigned to a live account.
