...
## ORPHAN-HIGH-333 — FAILING_CI plan source emits non-repo-verifiable evidence (gh-run-list:<id>); the top-priority pressure source can never converge — IN-PROGRESS
Canlı teşhis (2026-07-06, otonom-aksiyon hattını kanıtlarken): gerçek bir "Quality Gates" CI fail'i varken `rank_candidate_sources` failing_ci candidate'ını seçti (F_FINDING'in ÜZERİNDE öncelik) ve `convert_candidate_to_plan_content` `evidence_refs=["gh-run-list:ci-run-28766786201"]` + `affected_surfaces=[".github/workflows/"]` üretti — sentetik token, repo yolu değil. `classify_evidence_ref` bunu hiçbir target_sha'da repo_verified notlayamaz → convergence gate her envelope'u reddeder → cycle CONVERGED'e asla ulaşmaz. ORPHAN-312 (#860) F_FINDING/ORPHAN kaynakları için, ORPHAN-331 (#861) target_sha notlaması için düzeltti; **EN yüksek öncelikli kaynakta aynı sınıf açık kaldı.** Sonuç: main'de herhangi bir CI koşusu kırmızıyken nightly'nin plan slotu yakınsayamaz failing-CI planına gider ve Stage-D autonomous mode (2026-07-11) PR açamaz (AutonomousV9ImplementationRunner CONVERGED plan şart koşar). **Fix şekli (ORPHAN-312 deseni):** fail'in head_sha + failed job/step dosyalarını gh api ile çöz (türetilebiliyorsa gerçek dosya:satır, değilse workflow dosyasının kendisi), evidence_refs'i o repo yollarına topralama, `gh-run-list:<id>`'yi yalnız metadata olarak taşı; hiçbir repo-verifiable yüzey türetilemezse candidate'ı atla (`plan_candidate_conversion_skipped`) ki iterative fallback F_FINDING'e insin, cycle kilitlenmesin.

## ORPHAN-HIGH-334 — aria-debts/keys signing-key surface stageable (per-cycle ed25519 PRIVATE KEY + token dosyaları tracked dizinde, ignore kuralı yok) — IN-PROGRESS
Stage-D autonomous mode wire edilirken bulundu (bu anahtarları CI runner'da gerçekten mint eden ilk yüzey): `gh_token_factory.mint_signing_key/mint_installation_token` `aria-debts/keys/<cycle_id>` (ed25519 private key, 0600) + `.pub` + `.token` yazıyor, ama `aria-debts/` TRACKED (debt JSON'ları) ve .gitignore'da keys alt-ağacı için kural YOKTU — diğer tüm ARIA runtime yüzeylerinin (aria-tools/, aria-findings/) aksine. Otonom cycle sonrası bir `git add -A` canlı private-key materyalini stage'lerdi; workflow'un clean-worktree politikası da untracked key'leri kir sayardı. **Fix (aynı commit):** .gitignore'a `aria-debts/keys/` (key materyali asla stage'lenemez; aria-debts/ debt JSON'ları tracked kalır) + aria-auto-cycle workflow-kontratına dar `^aria-debts/keys(/.*)?$` write-root (enterprise preflight tam o alt-ağacı kabul eder, aria-debts/'in gerisini etmez). Pin: `test_signing_key_surface_is_gitignored_and_contracted` (aria-kernel/tests/test_autonomous_mode_workflow.py).
