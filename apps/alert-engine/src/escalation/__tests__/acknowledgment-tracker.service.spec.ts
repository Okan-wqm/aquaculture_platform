...
describe('AcknowledgmentTrackerService', () => {
  ...
  describe('createRecord', () => {
    it('should create a new acknowledgment record in Redis', async () => {
      const record = await service.createRecord('alert-1', 'incident-1');
    ...
    it('should emit ack.created event', async () => {
      const record = await service.createRecord('alert-2');
  ...
  describe('acknowledge', () => {
    it('should acknowledge a pending alert', async () => {
      const created = await service.createRecord('alert-1');

      const acked = await service.acknowledge('alert-1', {
        userId: 'user-1',
    ...
    it('should throw when alert has no ack record', async () => {
      await expect(
        service.acknowledge('nonexistent', { userId: 'user-1' }),
    ...
    it('should throw when trying to ack an already acknowledged alert', async () => {
      await service.createRecord('alert-1');
      await service.acknowledge('alert-1', { userId: 'user-1' });

      await expect(
        service.acknowledge('alert-1', { userId: 'user-2' }),
    ...
    it('should remove from pending set when no duration is specified', async () => {
      ...
      expect(redisService.srem).toHaveBeenCalledWith('ack:pending_set', created.id);
    });
  });
  ...
  describe('acknowledgeById', () => {
    it('should acknowledge by record ID', async () => {
      const created = await service.createRecord('alert-1');
      const acked = await service.acknowledgeById(created.id, { userId: 'user-1' });
    ...
    it('should throw for nonexistent record ID', async () => {
      await expect(
        service.acknowledgeById('nonexistent', { userId: 'user-1' }),
  ...
  describe('resolve', () => {
    it('should resolve an acknowledged alert', async () => {
      ...
      const resolved = await service.resolve('alert-1', 'user-1', 'Fixed');
    ...
    it('should emit ack.resolved event', async () => {
      await service.createRecord('alert-1');
      await service.resolve('alert-1', 'user-1');
  ...
  describe('getByAlertId / getById', () => {
    it('should return record by alert ID', async () => {
      const created = await service.createRecord('alert-1');
      const found = await service.getByAlertId('alert-1');
    ...
    it('should return undefined for missing alert', async () => {
      const found = await service.getByAlertId('nonexistent');
  ...
  describe('unacknowledge', () => {
    it('should return alert to pending status', async () => {
      ...
      const unacked = await service.unacknowledge('alert-1', 'user-2', 'Reopening');
    ...
    it('should throw if not in ACKNOWLEDGED status', async () => {
      ...
      await expect(
        service.unacknowledge('alert-1', 'user-1'),
  ...
  describe('manualEscalate', () => {
    it('should escalate an ack record', async () => {
      ...
      const escalated = await service.manualEscalate('alert-1', 'user-1', 'Needs attention');
    ...
    it('should emit ack.escalated event with manual flag', async () => {
      await service.createRecord('alert-1');
      await service.manualEscalate('alert-1', 'user-1');
  ...
  describe('deleteRecord', () => {
    it('should delete the record and all related Redis keys', async () => {
      await service.createRecord('alert-1');
      const result = await service.deleteRecord('alert-1');
    ...
    it('should return false for nonexistent alert', async () => {
      const result = await service.deleteRecord('nonexistent');
  ...
  describe('getPendingAcks', () => {
    it('should return all pending/escalated records', async () => {
      await service.createRecord('alert-1');
      await service.createRecord('alert-2');
      await service.createRecord('alert-3');

      // Acknowledge one
      await service.acknowledge('alert-2', { userId: 'user-1' });

      const pending = await service.getPendingAcks();
  ...
  describe('bulkAcknowledge', () => {
    it('should acknowledge multiple alerts', async () => {
      ...
      const results = await service.bulkAcknowledge(['alert-1', 'alert-2'], {
        userId: 'user-1',
      });
    ...
    it('should collect errors for failed acks', async () => {
      ...
      const results = await service.bulkAcknowledge(['alert-1', 'nonexistent'], {
        userId: 'user-1',
      });
    ...
    it('should reject bulk sizes over 100', async () => {
      ...
      await expect(
        service.bulkAcknowledge(ids, { userId: 'user-1' }),
  ...
  describe('getHistory', () => {
    it('should return history entries for an alert', async () => {
      ...
      const history = await service.getHistory('alert-1');
  ...
  describe('Redis graceful degradation', () => {
    it('should handle Redis errors gracefully in getByAlertId', async () => {
      ...
      const result = await service.getByAlertId('alert-1');
