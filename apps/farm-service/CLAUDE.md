...
- Cross-tenant tables KEEP `schema: 'farm'`. The authoritative set is farm's `MODULE_SCHEMAS[].infrastructureTables` in `libs/backend-common/src/database/schema-manager.service.ts`: <!-- infra-tables:farm -->`migrations`, `farm_outbox`, `outbox_events`, `inbox_messages`, `event_dlq`, `tenant_erasure_audit`, `farm_audit_logs`, `feeding_catalog_revisions`, `feeding_job_catalog_entries`, `feeding_catalog_admission`, `feeding_catalog_admission_history`, `feeding_writer_authority`, `feeding_writer_authority_history`, `feeding_job_runs`, `feeding_job_run_transitions`, `feeding_schedule_dispatches`, `feeding_schedule_dispatch_transitions`, `feeding_scheduler_heartbeat`, `tenant_erasure_target_proofs`<!-- /infra-tables -->. That list is proven against the registry by `tests/invariants/nested-steering-parity.spec.ts` — edit the registry, never this copy. (`farm_outbox` is a legacy registry alias; the live entity `outbox/farm-outbox.entity.ts` declares `name: 'outbox_events'`.)
