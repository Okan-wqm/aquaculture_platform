/**
 * Scheduled operation MealWindowUpcoming toplu şekli (K-2): 500 girdi/event cap + devam
...
import {
  chunkWindowEntries,
  mealWindowRenotifyBefore,
  MEAL_WINDOW_MAX_ENTRIES,
  MEAL_WINDOW_RENOTIFY_MINUTES,
} from '../executors/scheduled-feeding-operation.executor';
...
describe('meal-window reproducible delivery boundary', () => {
  it('derives the re-emission cutoff from the compiled job catalog cadence', () => {
    const observedAt = new Date('2026-07-27T08:15:00.000Z');
    expect(MEAL_WINDOW_RENOTIFY_MINUTES).toBe(15);
    expect(mealWindowRenotifyBefore(observedAt).toISOString()).toBe(
      '2026-07-27T08:00:00.000Z',
    );
    expect(observedAt.toISOString()).toBe('2026-07-27T08:15:00.000Z');
  });

  it('rejects an invalid observation instant instead of disabling re-emission', () => {
    expect(() => mealWindowRenotifyBefore(new Date(Number.NaN))).toThrow('valid observedAt');
  });
});
