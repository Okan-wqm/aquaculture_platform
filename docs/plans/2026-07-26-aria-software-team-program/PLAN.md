...
read `docs/reviews/orphan-findings.md`, which allocates from the same ORPHAN sequence space. The
registry's ORPHAN maximum was 332 while the markdown store was already at 416, so the first nineteen
IDs minted for this programme (333-351) all landed on findings that already existed — eight of them
exactly, so their commit trailers resolved to unrelated findings and the gate went green on a false
link.

CI had been red on the original branch since its second code commit, and the local Python suite was
green throughout; reporting the local suite as though it were the gate is the same defect class this
programme exists to close, so it is written down rather than quietly fixed. The allocator and the
trailer resolver now share one reader over both stores, and the work was replayed onto IDs 417-438.
