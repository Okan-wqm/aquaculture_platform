...
**Fix (fail-closed, this PR):** Removed the "Open Tenant Portal" button — the only control that implied an active impersonated browsing context — so the panel no longer offers a session-entry path that silently runs as the admin. The session-management surface (start/list/extend/end/terminate/audit) remains and is now contract-correct (RBAC-MEDIUM-010), so the feature is honest about what it does: it records governed impersonation _intent_ and audit, and does not grant an unenforced tenant-portal session. **Tracked debt (owner: auth-security-expert; deadline: next impersonation hardening cycle):** wiring a real gateway token-exchange that mints a scoped, `canModifyData`-enforcing impersonation JWT consumed by every subgraph, plus MFA step-up at session start, before any portal-entry control is reintroduced. This is a design-significant multi-service change (gateway + every subgraph guard), explicitly NOT smuggled in here.
...
**Title:** Plan-tier was fully decoupled from capability grants: the catalogue was returned in full to every tenant regardless of `planLevel`/`tenant_modules`; `createRole`/`updateRole`/override writes never checked plan; and the token mint (`getUserResourcePermissions`) folded capabilities with no module/plan intersection — so a tenant could grant itself (or hold from a backfill) a capability for a module it does not license (e.g. a STARTER tenant granting `ai_settings:manage`), and a plan downgrade re-evaluated nothing. The MT-HIGH-057 backfill that seeded messaging/AI caps onto every tenant's default roles irrespective of entitlement was a concrete standing over-grant.

**Layer:** 2 (plan-tier / capability entitlement)
**Evidence:**

- `apps/auth-service/src/modules/tenant/services/permission-catalogue.ts`
- `apps/auth-service/src/modules/tenant/services/capability-authority.ts`
- `apps/auth-service/src/modules/authentication/services/token.service.ts`

**Rule violated:** A tenant's grantable AND mintable capability set must be bounded by the modules its plan licenses; entitlement must be enforced at the write boundary (no non-entitled grant persists) AND at the runtime chokepoint (no non-entitled capability reaches the JWT), so a stale/over-broad stored grant has zero effect.

**Fix (two enforcement points on one SSoT):** Added the entitlement SSoT to `permission-catalogue.ts`: a `CATEGORY_MODULE_REQUIREMENTS` map (the two catalogue categories that map 1:1 to an OPTIONAL `ModuleCode` — `hr`→HR, `ai`→AI; core categories farm/batch/operations/reports/admin and messaging stay always-entitled), the derived `entitledCapabilities(enabledModuleCodes)` set, and `resolveEntitledCapabilities(query, tenantId)` (the shared enabled-module-codes SELECT so both consumers agree). (1) **Write boundary** — `CapabilityAuthorityService.resolveActorAuthority` now resolves the tenant's `entitled` set and both `assert*` methods reject persisting a grant outside it, enforced for admins AND delegates (even a TENANT_ADMIN cannot grant a capability for an unlicensed module — revenue integrity); revokes are exempt so a downgrade can be cleaned up. (2) **Runtime chokepoint** — `getUserResourcePermissions` intersects the effective set with the tenant's entitled set before stamping the JWT `resourcePermissions` claim, so an ALREADY-stored non-entitled grant (backfill or post-downgrade) never reaches any guard/subgraph and has zero runtime effect until the module is (re)licensed. Because the JWT never carries a non-entitled capability, the ai/messaging subgraphs that gate only on the capability are automatically safe. Pinned by new `capability-authority.spec.ts` entitlement tests (admin + delegate rejection, module-named error, revoke-exempt, core-always-entitled) and `token.service.spec.ts` intersection tests (unlicensed dropped, licensed kept). auth-service 512 tests + lint green. (Follow-on: the `permissionCategories` UI query still offers the full catalogue — a fail-closed UX wart since the write now rejects, not a security hole — filtering it by entitlement is tracked separately.)

## RBAC-HIGH-011

**Title:** The three centralized RBAC tables (`auth.tenant_roles`/`tenant_role_permissions`/`user_role_assignments`) had NO TypeORM `@Entity` anywhere (0 matches) — DDL owned by admin-api migrations, DML issued as raw SQL from BOTH auth-service and admin-api — so they were INVISIBLE to the `SchemaDriftValidator` (ADR-012 blind spot: a column-shape change on either writer went undetected until a query crashed). Separately, the only uniqueness was `(tenantId, code) WHERE code IS NOT NULL`; the new model never sets `code`, so duplicate role NAMES per tenant are permitted at the DB layer.

**Layer:** 2 (schema-drift blind spot / ADR-012)
**Evidence:**

- `apps/auth-service/src/modules/tenant/entities/tenant-role.entity.ts` (new)
- `apps/auth-service/src/modules/tenant/entities/tenant-role-permission.entity.ts` (new)
- `apps/auth-service/src/modules/tenant/entities/user-role-assignment.entity.ts` (new)
- `apps/admin-api-service/src/migrations/1800200000000-CreateAdminEntitySurfaceTables.ts` (the DDL these mirror)

**Rule violated:** Every table a service reads/writes must have a canonical `@Entity` in the OWNING runtime service so the schema-drift validator (ADR-012) can catch column-shape drift; a table invisible to drift is an undetectable-corruption surface.

**Fix (the drift-visibility half, landed):** Added the three `@Entity` mappings in the owning runtime service (auth), registered in the tenant module's `forFeature`, each mirroring the migration DDL EXACTLY (column names, uuid types, nullability; `schema: 'auth'` explicit). The boot `SchemaDriftValidator` now checks entity ↔ DB parity for them (it validates columns/uuid-types/nullability/check-constraints/enum-labels — none of which these column-only entities can mismatch, and they declare no relations so FK-presence validation is a no-op). Runtime DML still flows through the existing tenant-pinned raw SQL + `CapabilityAuthorityService` write path — unchanged. Pinned by the DB-independent `centralized-rbac-entities.spec.ts` (table name + schema + exact DB column set per entity), so a future entity-side rename is caught at unit-test time; the live entity↔DB parity runs in the boot validator (CI/prod, needs a DB). auth-service 516 tests + lint + tsc green.

**Tracked debt — DDL hardening (owner: auth-security-expert; requires a live-data migration, deliberately NOT shipped blind here because it can fail on existing dirty data this DB-less environment cannot inspect):**

- **RBAC-MEDIUM-012 (M18) — missing FKs:** `ALTER TABLE auth.user_role_assignments ADD CONSTRAINT fk_ura_user FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE` and `ALTER TABLE auth.tenant_roles ADD CONSTRAINT fk_tr_tenant FOREIGN KEY ("tenantId") REFERENCES auth.tenants(id) ON DELETE RESTRICT` — add `NOT VALID` then `VALIDATE CONSTRAINT` (blue-green safe) after an orphan-row sweep. Hard-deleting a user/tenant currently strands rows.
- **RBAC-MEDIUM-013 (M19) — single-default invariant is app-only:** `CREATE UNIQUE INDEX CONCURRENTLY uk_tenant_roles_one_default ON auth.tenant_roles ("tenantId") WHERE is_default` after de-duping any tenant with two `is_default=true` rows. Today `getDefaultRole` (`LIMIT 1`) is nondeterministic if a race left two.
- **H11 unique-name:** `CREATE UNIQUE INDEX CONCURRENTLY uk_tenant_roles_tenant_lname ON auth.tenant_roles ("tenantId", LOWER(name)) WHERE "tenantId" IS NOT NULL` after de-duping.
- **RBAC-MEDIUM-014 (M20) — migration hygiene:** the DDL migrations above must carry the `SET LOCAL lock_timeout/statement_timeout` envelope, use `CONCURRENTLY` for the indexes, and ship a real reconciling `down()`.

## RBAC-MEDIUM-015

**Title:** Adding a capability to the shipped default roles required a bespoke, name-keyed snapshot backfill migration (the MT-HIGH-057 `1801300000000-BackfillMessagingAiRoleCapabilities` pattern): `seedDefaultRoles` derived permissions live from the `DEFAULT_ROLE_PERMISSIONS` SSoT but ONLY for ABSENT roles — existing default-named roles were never topped-up — so every future capability shipped to the templates was standing debt (a new migration keyed by role NAME that missed renamed/custom roles and re-applied a point-in-time snapshot).

**Layer:** 2 (role seeding / capability upgrade)
**Evidence:**

- `apps/auth-service/src/modules/tenant/services/tenant-role.service.ts`
- `apps/admin-api-service/src/migrations/1801300000000-BackfillMessagingAiRoleCapabilities.ts` (the snapshot pattern replaced)

**Rule violated:** The correct behaviour must be automatic (tier 2): a new default-role capability must reach existing tenants from the single template SSoT with no per-capability migration — a name-keyed snapshot migration per capability is a maintenance trap and a drift surface.

**Fix (catalogue-driven seed/upgrade routine):** `seedDefaultRoles` is now a seed **and** upgrade routine on one SSoT. In the same SERIALIZABLE transaction it (1) creates the ABSENT named defaults as before, and (2) RECONCILES existing default-NAMED, `is_system` roles by UNION-ing any missing template capability into their stored `tenant_role_permissions` — derived live from `DEFAULT_ROLE_PERMISSIONS`, so a future capability needs no migration. The reconcile is additive-only (a prior grant is never removed, so a tenant admin's widening survives; a re-run adds nothing — idempotent), scoped strictly to shipped-default-named `is_system` roles (renamed/custom roles are the admin's to manage, never touched), and change-gated (an UPDATE + a `ROLES_RECONCILED` audit + a token-revocation of active holders fire only when a capability was actually added). Both the create AND the reconcile paths are entitlement-gated (RBAC-HIGH-010): the template is intersected with the tenant's enabled modules via `resolveEntitledCapabilities` before anything is written, so this raw-SQL path never persists a non-entitled capability — matching the `CapabilityAuthorityService` write boundary and the token-mint intersection instead of bypassing them. Pinned by new `tenant-role.service.spec` tests (additive top-up + audit + revoke, idempotent no-op, non-entitled never seeded, entitled seeded). auth-service 519 tests + lint + tsc green. (The MT-HIGH-057 snapshot migration stays as the historical record for tenants provisioned before this routine; new capability rollouts no longer add one.)

## RBAC-LOW-005

**Title:** The `permissionCategories` role-editor query returned the FULL catalogue to every tenant regardless of entitlement (the RBAC-HIGH-010 follow-on): a tenant without the HR/AI module was still offered `employees:*`/`ai_settings:*` toggles in the editor, and only learned at save time that the write boundary rejects them — a fail-closed UX/product-truth defect (not a security hole), and a standing risk that a future UI consumer treats the offered catalogue as the entitled set.

**Layer:** 3 (UI catalogue / entitlement product truth)
**Evidence:**

- `apps/auth-service/src/modules/tenant/services/permission-catalogue.ts`
- `apps/auth-service/src/modules/tenant/services/tenant-role.service.ts`
- `apps/auth-service/src/modules/tenant/resolvers/tenant-role.resolver.ts`

**Rule violated:** Every consumer of the capability catalogue must read through the ONE entitlement SSoT (RBAC-HIGH-010); a surface that offers more than the write boundary accepts is catalogue drift — the exact class of split the permission-catalogue SSoT exists to prevent.

**Fix (third enforcement point on the same SSoT):** Added `entitledPermissionCategories(entitled)` to `permission-catalogue.ts` — a pure filter deriving the role-editor view from the SAME entitlement set the write boundary and token mint consume: actions are filtered per capability, a resource with zero remaining actions is dropped, and a category with zero remaining resources disappears wholesale (no empty groups). `TenantRoleService.getPermissionCategories(tenantId)` now resolves the tenant's entitlement via `resolveEntitledCapabilities` and returns the filtered view; the resolver passes the JWT tenantId (no GraphQL schema change — the query stays argument-free, so no frontend/codegen churn; the client simply renders fewer categories). Pinned by pure-function specs (full entitlement ⇒ catalogue verbatim; core-only ⇒ hr/ai absent + core intact; single module re-admits exactly its category; UI ⊆ entitled invariant) and service specs (tenant-bound entitlement query, unlicensed tenant sees no hr/ai). auth-service 524 tests + lint + tsc green.
