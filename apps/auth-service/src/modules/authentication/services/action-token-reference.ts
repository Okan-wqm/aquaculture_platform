...
export async function resolveActionReference(
  ...
  const isActionId =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(token);
  if (isActionId) {
    const actionToken = await manager.findOne(ActionToken, {
      where: { id: token, purpose },
      ...(lock ? { lock: { mode: 'pessimistic_write' as const } } : {}),
    });
    if (!actionToken || !actionToken.isActive())
      throw new BadRequestException('Invalid or expired action token');
  ...
  return {
    actionToken: null,
    tokenHashes: [createHash('sha256').update(token).digest('hex'), token],
  };
}
