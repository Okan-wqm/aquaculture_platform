...
Found 2026-06-28 (tenant-panel PR-B2). `releaseSocket(url)` computes its pool key from `getTenantId()` AT RELEASE TIME, not from the tenant the caller acquired under. After a tenant switch, an A-bound hook's cleanup `releaseSocket(url)` targets `::B` (the current tenant) and can decrement — and prematurely tear down — tenant B's socket. ORPHAN-206's `onTenantChange` teardown already severs the LEAK (A's sockets), so this is a narrower refcount-accuracy race, not a residency leak. **Status: RESOLVED (2026-06-28) — `releaseSocket(url)` → `releaseSocket(socket: Socket | null | undefined)` in `socketFactory.ts`: the release is now bound to the Socket INSTANCE the caller acquired (matched by `entry.socket === socket`), never re-derived from the ambient tenant, so the post-switch mis-target is structurally impossible (Tier-1 make-it-impossible). Went one step past the suggested `releaseSocket(url, socket?)` — dropped the `url` param entirely so socket identity is the single source of truth, and a null socket (never acquired) is an explicit no-op, which also closes the latent over-decrement where releasing a never-acquired URL wrongly decremented the ambient tenant's entry. Updated all 6 callsites: useAlarmRuntime ×4, useEdgeIoSocket, useScadaLiveData (each already held the socket), and useSensorSocket (now captures `const socket = getSensorSocket()` so its unmount cleanup releases THAT instance). 9/9 vitest (new `socketFactory.identity-release` ×3 + `socketFactory.tenant-teardown` + `useScadaLiveData.tenant-isolation`) + tsc + eslint `--max-warnings=0` clean.**
