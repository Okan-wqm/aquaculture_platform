...
export class DiseaseReportAssembler {
  ...
  async assemble(tenantId: string, siteId: string): Promise<AssembledDraft<DiseasePrefillPayload>> {
    ...
    const affectedPercentage =
      row.affectedPercent != null ? round2(Number(row.affectedPercent)) : null;
    ...
    if (affectedPercentage != null) {
      ...
      fields.push(
        manualRequired(
          '/affectedPercentage',
          'Enter the affected percentage of the population.',
          true,
        ),
      );
    }
  ...
  private async queryLatestOutbreak(tenantId: string, siteId: string): Promise<DiseaseRow | null> {
