...
const VfdMiniWidget: React.FC<WidgetRendererProps> = ({
  ...
  const { reading } = useVfdRealtimeReadings(isEditing ? undefined : vfdDeviceId, {
    enabled: !isEditing && Boolean(vfdDeviceId),
    pollInterval: 3000,
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <text x={8} y={18} fontSize={11} fontWeight={600} fill={themeColors.neutral[700]}>
          {displayName}
        </text>
        ...
        <text x={8} y={46} fontSize={9.5} fill={themeColors.gray[400]}>
          {current}A
        </text>
        <text x={60} y={46} fontSize={9.5} fill={themeColors.gray[400]}>
          {speed}RPM
        </text>
