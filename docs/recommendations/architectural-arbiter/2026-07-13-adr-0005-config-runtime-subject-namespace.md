...
billing-service can read the _effective_ Stripe configuration — including the
...
1. **RPC namespace.** The platform RPC convention is `request.{service}.{...}`
   ...
   deliberate: the secret read must be grantable to _exactly one_ cert CN, and the
