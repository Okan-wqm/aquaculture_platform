...
export class AddTenantSuspensionAudit1807100000000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "auth"."tenants" DROP COLUMN IF EXISTS "suspendedBy"`,
    );
    await queryRunner.query(
      `ALTER TABLE "auth"."tenants" DROP COLUMN IF EXISTS "suspendedReason"`,
    );
    await queryRunner.query(
      `ALTER TABLE "auth"."tenants" DROP COLUMN IF EXISTS "suspendedAt"`,
    );
  }
}
