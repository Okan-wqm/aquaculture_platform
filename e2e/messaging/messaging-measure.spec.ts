...
test.describe('Messaging FAZ measurements @messaging-measure', () => {
  ...
  test('channel switch fires no amplified GraphQL fan-out @messaging-measure', async ({ page }) => {
    ...
    expect(channelMessages, 'one switch must not refetch the thread more than twice').toBeLessThanOrEqual(2);
    expect(myChannels, 'one switch must not refetch the list more than twice').toBeLessThanOrEqual(2);
    collector.stop();
  });
  ...
  test('a WS-delivered message renders without a thread refetch @messaging-measure', async ({
    ...
    test.skip(true, 'needs a second provisioned actor (see lane README)');
    void _page;
  });
