...
`PLAN-{SEVERITY}-{NNN}` — for process findings the planner itself raises (e.g. missing `Closing-Findings` field in a downstream package it reviews).  See `@.claude/agents-enterprise-v2/_shared/output-format.md`.
