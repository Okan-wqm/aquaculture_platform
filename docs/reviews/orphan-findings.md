...
**Root cause (TWO independent legs of the same architectural gap):**

**Leg 1 — TypeORM's built-in runner default.** TypeORM's `DataSource` constructor defaults `migrationsTransactionMode` to `'all'` when the caller omits the option (`node_modules/typeorm/data-source/DataSource.js` lines 261-264). In `'all'` mode, TypeORM's `MigrationExecutor.executePendingMigrations` raises `ForbiddenTransactionModeOverrideError` the instant any pending migration declares an instance-level `transaction` opt-out (the documented escape hatch for `CREATE INDEX CONCURRENTLY`, `DROP INDEX CONCURRENTLY`, and other DDL Postgres rejects inside a transaction block).

`createServiceTypeOrmConfig` (the platform-wide factory) was NOT setting `migrationsTransactionMode`, so every legacy service that kept TypeORM's built-in `migrationsRun: true` (auth-service literal; admin-api-service / event-store-service via `migrationsRunFromEnv` defaulting to `'true'`) inherited the unsafe default. messaging-service is also affected on the production droplet because `docker-compose.droplet.yml` sets `DATABASE_MIGRATIONS_RUN: 'true'`, which flips messaging's `migrationsRunFromEnv: cs.get(..., 'false') === 'true'` to `true` and activates BOTH TypeORM's built-in runner AND the custom `MessagingMigrationRunnerService` at boot. TypeORM's built-in runner fires first at DataSource init, so the crash surfaces before the custom runner gets a chance.

**Leg 2 — Per-service `MigrationRunnerService` outer wrapper.** The platform's per-service runner at `libs/backend-common/src/database/migration-runner/migration-runner.service.ts` was wrapping every migration in an unconditional `await queryRunner.startTransaction()` before calling `executor.executeMigration()`, so any migration declaring `transaction = false` (e.g. `messaging.AddMessageAttachmentIsDeletedIndex1782800000000` whose `up()` runs `CREATE INDEX CONCURRENTLY`) would still crash via Postgres's "cannot run inside a transaction block" rejection — same root-cause class as ORPHAN-CRITICAL-058 in the centralised orchestrator. Only the orchestrator was fixed in PR #245; the per-service runner kept the same bug. (Initial diagnosis claimed the per-service runner was unaffected because TypeORM's `MigrationExecutor` "owned the transaction boundary"; the `runForSchema` path actually calls `executor.executeMigration(migration)` which does NOT consult `executor.transaction` — the outer `startTransaction()` wrapper at runner level is what was crashing.)

This finding closes the second + third legs of the same architectural gap. The first leg (centralised orchestrator) was closed by PR #245. After this commit, all three migration runners in the platform — built-in TypeORM (legacy), platform per-service runner (mainline), centralised db-migrate orchestrator (Phase E) — share one structural rule: instance-level `transaction = false` suppresses both inner and outer transaction wrappers end-to-end.

**Fix (Tier-1 Make-Impossible — both legs):**

1. **`createServiceTypeOrmConfig`**: pin `migrationsTransactionMode: 'each'` in the factory output so EVERY legacy service that relies on TypeORM's built-in migration runner inherits the correct mode automatically. The factory is the SSoT for service TypeORM config — no per-service override is required.

2. **`MigrationRunnerService` (per-service runner)**: gate the outer `startTransaction()` / `commitTransaction()` / `rollbackTransaction()` calls on `migration.instance.transaction !== false`, mirroring PR #245's orchestrator pattern verbatim. The dead-code `executor.transaction = 'each'` line is preserved with an explanatory comment block describing the three-runner topology (so the next reader doesn't try to "clean it up" without understanding why it exists — the property is read by `executePendingMigrations`, NOT by `executeMigration`, so removing it changes nothing today but breaks any future migration to the canonical TypeORM API).

**Verification:**

- `nx build messaging-service admin-api-service` succeeds — both bootstrap targets compile cleanly with the `migrationsTransactionMode: 'each'` factory output.
- New invariant test `tests/invariants/migration-transaction-mode-canonicalised.spec.ts` asserts the factory always emits `migrationsTransactionMode: 'each'` and the per-service runner reads `migration.instance.transaction` before opening the outer transaction, AND the orchestrator (PR #245's gate) keeps the same shape. Either regression flips a literal-source-text check at CI time.

**Status:** RESOLVED — `chore/migration-runner-transaction-each` lands the pin in the factory + the gated transaction wrapper in the per-service runner + the new invariant test that locks both shapes. After redeploy, messaging and admin-api bootstrap cleanly with `DATABASE_MIGRATIONS_RUN: 'true'` (the production-droplet config).
