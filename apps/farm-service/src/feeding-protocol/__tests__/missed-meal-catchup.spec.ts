...
describe('DayPlanRecalcService.applyMissedCatchUp', () => {
  function makeHarness(opts: {
    ...
    // `findOne`, `createQueryBuilder` and `save` are overload sets on
    // EntityManager, so each single-signature double NAMES the member type it
    // stands in for. The member must still exist on EntityManager, and every
    // other member of this double stays fully checked.
    const manager = stub<EntityManager>({
      findOne: stubMember<EntityManager['findOne']>(findOne),
      createQueryBuilder: stubMember<EntityManager['createQueryBuilder']>(() => builder),
      save: stubMember<EntityManager['save']>(
        async (entity: { id?: string; plannedKg?: number }) => {
          saved.push(entity);
          return entity;
        },
      ),
