...
import { GetFishHealthStatsTool } from '../get-fish-health-stats.tool';
...
describe('GetFishHealthStatsTool', () => {
  let send: jest.Mock;
  let tool: GetFishHealthStatsTool;
  ...
  beforeEach(() => {
    ...
    tool = new GetFishHealthStatsTool({ send });
  });

  it('is a module-scoped farm read tool', () => {
    const metadata = tool.getMetadata();
    expect(metadata.name).toBe('get_fish_health_stats');
  ...
  it('applies defaults and sends the contract subject with the context tenant', async () => {
    ...
    expect(send).toHaveBeenCalledWith(FARM_AI_QUERY_SUBJECTS.FH_STATS, {
      ...{},
