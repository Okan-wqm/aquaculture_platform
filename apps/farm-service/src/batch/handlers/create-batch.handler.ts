    ...
    return runInTenantTransaction(
      ...
      async (queryRunner, mutationSession) => {
        ...
        const batchNumber =
          payload.batchNumber ||
          generatedCode?.code ||
          `B-${new Date().getFullYear()}-${Date.now()}`;
        ...
        savedBatch = await this.batchMutations.commitBatchTransition(mutationSession, {
          intent: 'batch_create',
          aggregate: batch,
        });
        ...
        if (payload.initialLocations && payload.initialLocations.length > 0) {
          ...
          if (tankIds.length === 0) {
            ...
            if (tankBatchesToSave.length > 0) {
              await this.batchMutations.commitTankBatchTransitions(mutationSession, {
                intent: 'stock_allocation',
                aggregates: tankBatchesToSave,
              });
            }
            ...
            for (const update of legacyTankUpdates) {
              await this.batchMutations.replaceTankStockProjection(mutationSession, {
                tankId: update.id,
                currentBiomassKg: update.biomassKg,
