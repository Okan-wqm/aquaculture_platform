...
const ConnectionFormModal: React.FC<ConnectionFormProps> = ({
  connection,
  onSubmit,
  onClose,
  isLoading,
    ...
    >
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-primary-300 dark:border-primary-700 bg-primary-50 dark:bg-primary-900/20 px-3 py-2 text-sm font-medium text-primary-700 dark:text-primary-300 hover:bg-primary-100 dark:hover:bg-primary-900/50 disabled:opacity-50"
              ...
              {discoveredEndpoints.length > 0 && (
                <div className="mt-2 rounded-lg border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-900/20 p-3">
                  <h4 className="text-xs font-semibold text-primary-800 dark:text-primary-200 mb-2">
                    Bulunan Endpoint&apos;ler ({discoveredEndpoints.length})
                  </h4>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (ep.serverCertificate)
                            updateField('serverCertificate', atob(ep.serverCertificate));
                        }}
                      >
                        <span className="font-mono">
                          {ep.securityMode}/{ep.securityPolicy}
                        </span>
                        <span className="text-primary-600 dark:text-primary-400">
                          Seviye: {ep.securityLevel}
                        </span>
                      </Button>
                    ))}
                  </div>
            ...
            {form.authMode === 'Certificate' && (
              <div className="space-y-3 rounded-lg border border-warning-200 dark:border-warning-800 bg-warning-50 dark:bg-warning-900/20 p-4">
                <h4 className="text-sm font-medium text-warning-800 dark:text-warning-200">
                  Sertifika Kimlik Dogrulama
                </h4>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            reader.onload = (ev) =>
                              updateField('clientCertificate', ev.target?.result as string);
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <textarea
                        ...
                        rows={3}
                        className={`w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-xs font-mono focus:border-primary-500 focus:ring-1 focus:ring-primary-500 ${!showPrivateKey ? 'text-security-disc' : ''}`}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            const reader = new FileReader();
                            reader.onload = (ev) =>
                              updateField('clientPrivateKey', ev.target?.result as string);
                            reader.readAsText(file);
                          }
                        }}
                      />
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            const reader = new FileReader();
                            reader.onload = (ev) =>
                              updateField('serverCertificate', ev.target?.result as string);
                            reader.readAsText(file);
                          }
                        }}
                      />
        ...
        <div className="border rounded-lg">
          <Button variant="ghost" type="button" onClick={() => setShowAdvanced(!showAdvanced)}>
...
const TestResultModal: React.FC<{
  ...
  >
    <div className="text-center mb-4">
      ...
      <p
        className={`text-sm font-medium ${result.success ? 'text-success-600 dark:text-success-400' : 'text-error-600 dark:text-error-400'}`}
      >
    ...
    <div className="space-y-2 text-sm">
      ...
      {result.error && (
        <div className="mt-2 rounded-lg bg-error-50 dark:bg-error-900/20 p-3">
          <p className="text-sm text-error-700 dark:text-error-300">{result.error}</p>
          {result.errorCode && (
            <p className="text-xs text-error-500 mt-1">Kod: {result.errorCode}</p>
          )}
        </div>
      )}
      <div className="flex justify-between">
...
const PlcConnectionsPage: React.FC = () => {
  ...
  const handleCreate = useCallback(
    async (input: CreatePlcConnectionInput) => {
      try {
  ...
  const handleUpdate = useCallback(
    async (input: UpdatePlcConnectionInput) => {
  ...
  const plcConnectionColumns: DataTableColumn<PlcConnection>[] = [
    ...
    },
    {
      ...
      header: 'Endpoint',
      render: (_value, conn) => (
        <code className="text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded">
          {conn.endpointUrl}
        </code>
      ),
    },
        ...
        return (
          <>
            ...
            {conn.lastError && conn.status === 'ERROR' && (
              <p
                className="text-xs text-error-500 mt-1 max-w-[200px] truncate"
            ...
            <div className="absolute right-0 z-10 mt-1 w-48 rounded-lg border bg-white dark:bg-gray-900 py-1 shadow-lg">
              ...
              {conn.isActive ? (
                <Button
                  variant="ghost"
                  size="sm"
                  leftIcon={<ZapOff className="h-4 w-4" />}
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-4 flex gap-3">
        ...
        <Select
          options={[
            ...
            { value: 'CONNECTING', label: 'Baglaniyor' },
      ...
      {showForm && (
        <ConnectionFormModal
          ...
          onSubmit={
            (editingConnection ? handleUpdate : handleCreate) as (
              data: CreatePlcConnectionInput | UpdatePlcConnectionInput,
            ) => void
          }
          onClose={() => {
            setShowForm(false);
