...
import { useI18n, type MessageKey } from '@aquaculture/shared-ui/i18n';
...
export function RecordFeedingPage(): JSX.Element {
  ...
  // FE-HIGH honesty: the queued op id drives the two-phase status badge so a
  // QUEUED write never looks like a recorded one (mobile-queued-write-honesty).
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    setErrors({});
    try {
      const queued = await addToQueue('recordMealFeeding', {
  ...
  if (showSuccess) {
    return (
      ...
      >
        ...
        {queuedOpId && <QueuedStatusBadge operationId={queuedOpId} />}
        <h2 className="text-head font-bold text-ok">{t('m.feeding.recorded')}</h2>
        {/* Kayıt her zaman önce kuyruğa gider ve arka planda eşitlenir. */}
        <p className="text-ink-2 text-body mt-1">{t('m.feeding.queuedForSync')}</p>
      </div>
    );
  }
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {t(`m.feeding.mealStatus.${meal.status}` as MessageKey)}
                    </span>
                  </div>
