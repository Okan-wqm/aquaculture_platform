...
| RPT-007 | MEDIUM | Harvest `QualityGrade` (PREMIUM/A/B/C/REJECT) ≠ Norwegian kvalitetsklasser (superior/ordinær/produksjonsfisk/utkast) | Two taxonomies grew independently | Phase 2 DONE: `harvest_records.quality_class` added (`1803100000000`), derived from `qualityGrade` at write time — the stored regulatory truth the slakt report reads. **REVISED verdict (Phase 4): the original "drop `qualityGrade`, no dual storage" plan is CANCELLED.** The grade→class map is LOSSY (PREMIUM and GRADE_A both collapse to SUPERIOR), so `qualityGrade` is NOT a resolvable alias — it is the richer 5-level source and is load-bearing for display, harvest statistics (grouped by grade), filters, and the `HarvestCompleted` event. Both columns are kept: `qualityGrade` = operator/commercial source, `quality_class` = its lossy regulatory projection. Retiring the 5-level grade would be a deliberate product change (stats/event/UI), not a mechanical column drop. |
