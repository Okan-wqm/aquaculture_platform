...
export class EventStoreServiceIdentityGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const outcome = verifyServiceIdentityRequest({
      ...
      keyLookup: (kid) => this.lookupKey(kid, serviceIdentity),
      expectedTenantId: tenantId,
      expectedAudience: EVENT_STORE_SERVICE_AUDIENCE,
    });

    if (!outcome.valid || outcome.version !== 'v2') {
      this.logger.warn(
        `Rejected request: ${outcome.valid ? 'legacy-v1' : outcome.reason} from "${serviceIdentity}"` +
  ...
  private lookupKey(kid: string, serviceIdentity: string): string | undefined {
    return this.lookupKeyEntry(kid, serviceIdentity)?.secret;
  }
