  ...
  it('plan-bağlı kayıtta unplannedActualKg + büyüme deltası AYNI tx te yazılır', async () => {
    ...
    const { handler, managerQuery, applyGrowth, recalcForUnit } = makeHarness({
      ...
      dayPlan: {
        id: 'dp-1',
        siteId: 'site-1',
        // Only `expectedFcr` matters to the growth delta under test; naming the
        // resolution type keeps that field checked against the real contract.
        resolution: stub<DayPlanResolution>({ expectedFcr: 1.25 }),
      },
      lockedUnit,
    });
