 ...
 */
export class MergePlanCatalogue1802500000000 implements MigrationInterface {
  name = 'MergePlanCatalogue1802500000000';
  ...
  private async mergeFromAdmin(queryRunner: QueryRunner): Promise<void> {
    ...
    if ((missing[0]?.missing ?? '0') !== '0') {
      throw new Error(
        `MergePlanCatalogue1802500000000: ${missing[0]?.missing} admin.plan_definitions rows did not merge into billing.plans. Resolve the source rows before re-running.`,
      );
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // DESTRUCTIVE: down()-only rollback of MergePlanCatalogue1802500000000 — drops the plan_add_ons table this same migration created; the admin source rows are dropped by admin migration 1809100000000, so roll that one back first
