...
- **Data fetch:** the cross-module pattern is TanStack Query + the tenant-scoped key factory `createTenantQueryKey` (`web/shared-ui/src/utils/tenant-query-keys.ts`) — ALWAYS prefix query keys with `['tenant', tenantId, …]` (FE-CRITICAL-014/015/016: cross-tenant cache leak otherwise). ADR-009/010 describe the admin-panel's legacy `useAsyncData` + styling pattern — they are admin-panel-scoped, NOT the web-wide rule, and `useAsyncData` is being retired: admin-panel reads and writes go through `useAdminQuery` / `useAdminMutation` + `adminKeys` (`web/modules/admin-panel/src/hooks/`), ratcheted to 0 by `tests/invariants/admin-panel-data-layer.spec.ts` (ADMIN-HIGH-121). That invariant also requires every module-scoped `Map` cache in `web/` to reach `registerLogoutCleanup`, and every package importing `@tanstack/react-query` to declare it at the federation-pinned version.

## Enforcement
`tests/invariants/federation-shared-singleton.spec.ts`, `tests/invariants/web-shared-ui-singleton-imports.spec.ts`, `tests/invariants/admin-panel-data-layer.spec.ts`.
