// LOCAL FORK (RUST-CVE-001): rustls-pemfile (unmaintained, RUSTSEC-2025-0134)
// is replaced by rustls-pki-types' first-party PEM decoding. Behaviour is
// preserved: certs are collected the same way, and the first private key of
// any supported encoding (SEC1 / PKCS#1 / PKCS#8) wins, as the old
// read_one/Item loop did.
    ...
    let config = match tls_config {
        TlsConfiguration::Simple {
            ...
            let mut config = if let Some(client) = client_auth.as_ref() {
                ...
                // Read PEM items until we find a valid key (SEC1 / PKCS#1 /
                // PKCS#8). from_pem_reader skips non-key sections exactly
                // like the old rustls_pemfile::read_one loop did.
                let key = match PrivateKeyDer::from_pem_reader(Cursor::new(client.1.clone())) {
                    Ok(key) => key,
                    Err(pem::Error::NoItemsFound) => return Err(Error::NoValidKeyInChain),
