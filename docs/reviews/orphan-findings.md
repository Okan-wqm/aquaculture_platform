...
Status: RESOLVED for slices 1-3 — farm-module (3) + mcp/farm-management (13) + sensor-module (33) = **49 ops, baseline 130 → 81**. Tracked for later: farm sub-equipment `category` (2, backend-gap) + sensor `CancelVfdChangeSet`/`GetSensorChannels` (2) + MCP date-filter input drift. Registry: orphan-findings.md + graphql-fe-drift.baseline.json.
