...
import { ListGrowthMeasurementsTool } from '../list-growth-measurements.tool';
...
describe('ListGrowthMeasurementsTool', () => {
  let send: jest.Mock;
  let tool: ListGrowthMeasurementsTool;

  beforeEach(() => {
    send = jest.fn().mockReturnValue(of({ ok: true, data: { items: [], truncated: false } }));
    tool = new ListGrowthMeasurementsTool({ send });
  });

  it('is a module-scoped farm read tool', () => {
    const metadata = tool.getMetadata();
    expect(metadata.name).toBe('list_growth_measurements');
  ...
  it('applies defaults and sends the contract subject with the context tenant', async () => {
    ...
    expect(send).toHaveBeenCalledWith(FARM_AI_QUERY_SUBJECTS.GROWTH_MEASUREMENTS, {
      ...{ batchId: TANK, limit: 20 },
      tenantId: CTX.tenantId,
    });
  });

  it('forwards explicit fields (limits clamped to the contract cap)', async () => {
    await tool.execute({ batchId: TANK, limit: 99 }, CTX);

    expect(send).toHaveBeenCalledWith(FARM_AI_QUERY_SUBJECTS.GROWTH_MEASUREMENTS, {
      ...{ batchId: TANK, limit: 50 },
      tenantId: CTX.tenantId,
    });
  });
});
