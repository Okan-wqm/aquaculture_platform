 ...
 */
export class CreateModulePriceSheet1802400000000 implements MigrationInterface {
  name = 'CreateModulePriceSheet1802400000000';
  ...
  private async copyFromAdmin(queryRunner: QueryRunner): Promise<void> {
    ...
    if (missingCount !== '0') {
      throw new Error(
        `CreateModulePriceSheet1802400000000: ${missingCount} admin.module_pricing metrics have no row in billing.module_price_metrics. Resolve the source rows before re-running.`,
      );
    }
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // DESTRUCTIVE: down()-only rollback of CreateModulePriceSheet1802400000000 — drops the three billing tables this same migration created; the source rows in admin.module_pricing are dropped by the admin-side migration 1809000000000, so roll that one back first
