...
Pre-fix tools/aria-poc/ci_executor.py invoke_claude_code mock path
...
Plan 024 §B-8 fix:
* invoke_claude_code now takes claim_id, agent_id, role,
...
* main() reads role + must_satisfy from request_envelope (already
  loaded for cost-cap eval) and forwards to invoke_claude_code
  along with the claim_id from claim_request output.

Tests:
1. invoke_claude_code mock without claim_id raises
...
class CiExecutorMockRealLeaseTests(unittest.TestCase):
    def setUp(self) -> None:
        os.environ["CLAUDE_CODE_MOCK"] = "1"

    def tearDown(self) -> None:
        os.environ.pop("CLAUDE_CODE_MOCK", None)

    def test_mock_without_claim_id_raises(self) -> None:
        """Plan 024 §B-8 acceptance (1)."""
        from ci_executor import invoke_claude_code  # type: ignore[import-not-found]
