...
const createRule = utils_1.ESLintUtils.RuleCreator((name) => `https://github.com/Okan-wqm/aquaculture_platform.git/blob/main/tools/eslint-rules/rules/${name}.ts`);
/**
...
exports.default = createRule({
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'Prometheus `labelNames` must not contain unbounded-cardinality identifiers (user_id, request_id, IP, URL, etc.) — each distinct value creates a new time-series and blows up Prometheus storage.',
        },
        schema: [],
        messages: {
            unboundedCardinalityLabel: 'Prometheus label "{{ label }}" has unbounded cardinality — each distinct value creates a new time-series. The observability-expert invariant classifies this as CRITICAL. For request-scoped attribution use structured logs (Loki) or traces (Tempo); for dimensioned metrics use bounded labels like status_code, method, route-template. Reference: docs/plans/2026-04-17-agentic-post-audit-consolidation-plan.md#Phase-2.',
        },
    },
    defaultOptions: [],
    create(context) {
        return {
            Property(node) {
                if (node.key.type !== 'Identifier' || node.key.name !== 'labelNames')
                    return;
...
//# sourceMappingURL=no-high-cardinality-metric-label.js.map