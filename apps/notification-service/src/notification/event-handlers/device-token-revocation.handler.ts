...
@Injectable()
export class DeviceTokenRevocationHandler
  implements IEventHandler<UserDeletedEvent>, OnModuleInit
{
  ...
  async handle(event: UserDeletedEvent): Promise<void> {
    if (
      !UUID_REGEX.test(event.tenantId) ||
      !UUID_REGEX.test(event.deletedUserId)
    ) {
