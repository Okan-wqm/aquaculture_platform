...
- prior reviewed head: `c6065d6dac97306f147de67ef58a96e3a67524ac`
- corrective head, upstream, and remote: `c139f40f69f77c628f0794146a20cb51818bb03d`
...
### D-P1-001 — The admission verifier does not bind the reviewed commit, committed scope, or actual runtime

- **Maps to:** `APP-P1-001`; accepts A-HIGH-001, C-P1-003, ROOT-P1-001/002.
...
- **Test predicate:** every provider body/HTTP combination, 200 merged-vs-enqueued, 409 option mismatch,
  pending poll, expiry 404, unknown status/field, stack membership, and `default` action must fail or
  reconcile exactly as the normative authority specifies.
...
| Root         | Result                 | Binding basis                                                                                     |
| ------------ | ---------------------- | ------------------------------------------------------------------------------------------------- |
| `APP-P1-001` | FAIL                   | D-P1-001 and D-P1-003                                                                             |
| `APP-P1-002` | FAIL                   | D-P1-002                                                                                          |
| `APP-P1-003` | FAIL                   | D-P1-004                                                                                          |
| `APP-P1-004` | FAIL                   | D-P1-005                                                                                          |
| `APP-P1-005` | PASS at contract level | Canonical human/repository/workload lifecycle is explicit; C-P1-001 rejected.                     |
| `APP-P1-006` | PASS at contract level | Eight runtime roles and CLI children have exact containment fields.                               |
| `APP-P1-007` | PASS at contract level | Opaque supervisor volume/handle teardown removes caller-path authority.                           |
| `APP-P1-008` | FAIL                   | D-P1-006                                                                                          |
| `APP-P1-009` | PASS at contract level | Pre-call DLP, quarantine/CAS, encryption/redaction, capture/hold/delete are assigned fail-closed. |
| `APP-P1-010` | FAIL                   | D-P1-007                                                                                          |
| `APP-P1-011` | PASS at contract level | Effective permissions/rules/check/review snapshot and under-lock reread are implementable.        |
| `APP-P1-012` | FAIL                   | D-P1-008                                                                                          |
| `APP-P1-013` | FAIL                   | D-P1-009                                                                                          |
| `APP-P1-014` | FAIL                   | D-P1-010                                                                                          |
| `APP-P1-015` | PASS at contract level | Kill/readiness ordering, S52 sandbox-only, and S55 first production merge are explicit.           |
| `APP-P1-016` | FAIL                   | D-P1-011 and D-P1-012                                                                             |
| `APP-P1-017` | PASS at contract level | Same-origin/browser and clean-host integration matrix is explicit before S41.                     |
| `APP-P2-018` | FAIL                   | D-P2-014                                                                                          |
...
git rev-parse HEAD
git rev-parse '@{u}'
git ls-remote --heads origin docs/new-aria-autonomous-engineering-plan
  => all c139f40f69f77c628f0794146a20cb51818bb03d
...
independent frozen-source/current relation recomputation
  => headings/frozen/matrix=88/88/88, drift=[]
  => P0=24, confirmed=20, partial=[015,017,044], refuted=[026]
  => PLAN/cards/program=72/72/72, gates/roles=9/12

independent hostile temporary-copy controls (all copies removed)
  => coordinated title + unknown finding + junk gate artifacts: verifyMapping errors=[]
  => illegal state continuity with rehash: verifyHistory errors=[]
  => duplicate review role with recomputed evidence/event: verifyHistory errors=[]
  => lexical 1.0 and 1e0: accepted
  => forged runtime metadata: verifyProvenance errors=[]
  => 61-line single-argument arrow: verifyReadability errors=[]

one stock negative suite concurrent with the stock full verifier
  => negative suite exit 0, PASS negative-controls=23
  => full verifier exit 1, exactly 78 PRODUCT_SCOPE errors for sibling fixture

git status --porcelain=v1 after cleanup
