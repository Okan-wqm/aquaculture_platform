...
export type BuildKind = 'node-service' | 'frontend' | 'rust-sidecar' | 'docker-only' | 'one-shot' | 'infra';
...
function buildEntry(input: CatalogEntryInput): ServiceCatalogEntry {
  ...
  const requiredSecrets =
    ...
    input.requiredEnv.filter((name) => SECRET_ENV_PATTERN.test(name));
...
export const PLATFORM_SERVICE_CATALOG: readonly ServiceCatalogEntry[] = [
  ...
  buildEntry({
    ...
    requiredEnv: ['AUTH_SERVICE_DB_PASS', 'PASSWORD_PEPPER', 'MFA_ENCRYPTION_KEY', 'SUPER_ADMIN_PASSWORD'],
    gatewaySubgraph: subgraph('auth', 'auth-service', 'AUTH_SERVICE_URL', 'http://localhost:3001/graphql'),
  }),
  buildEntry({
    ...
    gatewaySubgraph: subgraph('farm', 'farm-service', 'FARM_SERVICE_URL', 'http://localhost:3002/graphql'),
  }),
  buildEntry({
    ...
    gatewaySubgraph: subgraph('sensor', 'sensor-service', 'SENSOR_SERVICE_URL', 'http://localhost:3003/graphql'),
  }),
  buildEntry({
    ...
    deploymentStatus: 'active',
    deployTarget: 'droplet',
    deployProfiles: ['droplet', 'rust-sidecar'],
    criticality: 'required',
    classification: 'internal-service',
    privilegeMode: 'none',
    requiredSignals: [],
  ...
  buildEntry({
    ...
    gatewaySubgraph: subgraph('hr', 'hr-service', 'HR_SERVICE_URL', 'http://localhost:3005/graphql'),
  }),
  buildEntry({
    ...
    gatewaySubgraph: subgraph('hydroponics', 'hydroponics-service', 'HYDROPONICS_SERVICE_URL', 'http://localhost:4007/graphql'),
  }),
  buildEntry({
    ...
    gatewaySubgraph: subgraph('messaging', 'messaging-service', 'MESSAGING_SERVICE_URL', 'http://messaging-service:3000/graphql'),
  }),
  buildEntry({
    ...
    gatewaySubgraph: subgraph('alert', 'alert-engine', 'ALERT_SERVICE_URL', 'http://localhost:3004/graphql', 'alert-engine'),
  }),
  buildEntry({
    ...
    gatewaySubgraph: subgraph('billing', 'billing-service', 'BILLING_SERVICE_URL', 'http://localhost:3006/graphql'),
  }),
  buildEntry({
    ...
    dbRoles: { owner: 'notification_schema_owner', migrator: 'db_migrate', runtime: 'notification_service' },
    privilegeMode: 'dml-only',
    migration: {
    ...
    gatewaySubgraph: subgraph('notification', 'notification-service', 'NOTIFICATION_SERVICE_URL', 'http://localhost:4008/graphql'),
  }),
  buildEntry({
    ...
    federationExclusion: {
      ...
      removeAfterRelease: '2026.09',
      reason: 'admin-api-service is not registered in gatewaySubgraphs(); promotion requires catalog gatewayParticipation change',
    },
    eventStoreTenantScopePolicy: 'all-tenants',
    requiredSignals: ['nats_auth_mode_mtls', 'schema_drift_clean'],
  ...
  buildEntry({
    ...
    requiredSignals: ['schema_drift_clean'],
    requiredEnv: ['CONFIG_SERVICE_DB_PASS', 'SERVICE_IDENTITY_KEYRING', 'SERVICE_IDENTITY_SIGNING_KID', 'CONFIG_ENCRYPTION_KEY'],
    gatewaySubgraph: subgraph('config', 'config-service', 'CONFIG_SERVICE_URL', 'http://localhost:3007/graphql'),
  }),
  buildEntry({
    ...
    dbRoles: { owner: 'event_store_schema_owner', migrator: 'db_migrate', runtime: 'event_store_service' },
    privilegeMode: 'dml-only',
    migration: {
  ...
  buildEntry({
    ...
    dbRoles: { owner: 'observability_schema_owner', migrator: 'db_migrate', runtime: 'observability_service' },
    privilegeMode: 'dml-only',
    migration: {
  ...
  buildEntry({
    ...
    federationExclusion: {
      owner: 'platform-service-catalog',
      removeAfterRelease: '2026.09',
      reason: 'ai-service exposes a dormant federation driver but is not deployed or routed by the gateway',
    },
    requiredSignals: [],
  ...
  ...[
    ...
    'mosquitto',
    'minio',
  ].map((serviceId): ServiceCatalogEntry => buildEntry({
    serviceId,
    nxProject: serviceId.includes('-module') || ['shell', 'dashboard', 'admin-panel', 'tenant-admin'].includes(serviceId)
      ? serviceId
      : undefined,
    imageTarget: serviceId === 'mosquitto' || !['nginx', 'minio'].includes(serviceId) ? serviceId : undefined,
    ...
    criticality:
      serviceId === 'nginx'
        ...
        : serviceId === 'mosquitto' || serviceId === 'minio'
          ? 'ignored'
          : 'warning',
    classification: serviceId === 'nginx' || serviceId === 'mosquitto' || serviceId === 'minio' ? 'infra' : 'frontend',
    privilegeMode: 'none',
    requiredSignals: [],
    requiredEnv:
      serviceId === 'minio'
...
export function gatewaySubgraphs(): readonly GatewaySubgraphCatalogEntry[] {
  return PLATFORM_SERVICE_CATALOG.filter(
    ...
    if (!entry.gatewaySubgraph) {
      throw new Error(`Catalog entry ${entry.serviceId} participates in Apollo gateway without gatewaySubgraph metadata.`);
    }
...
export function requiredRuntimeEnv(): readonly string[] {
  return [...new Set(activeDropletServices().flatMap((entry) => entry.requiredEnv))].sort();
}

export function requiredRuntimeSecrets(): readonly string[] {
  return [...new Set(activeDropletServices().flatMap((entry) => entry.requiredSecrets))].sort();
}
...
export function validateServiceCatalog(
  ...
  for (const entry of catalog) {
    ...
    if (entry.migrationGlobs && entry.migration && entry.migrationGlobs !== entry.migration.globs) {
      errors.push({ serviceId: entry.serviceId, message: 'migrationGlobs must be derived from migration.globs' });
    }
    if (entry.privilegeMode === 'dml-only' && !entry.dbRoles?.runtime) {
      errors.push({ serviceId: entry.serviceId, message: 'dml-only service must declare dbRoles.runtime' });
    }
    if (entry.dbSchema && !entry.dbRoles?.owner) {
      errors.push({ serviceId: entry.serviceId, message: 'schema-owning service must declare dbRoles.owner' });
    }
    if (entry.dbSchema && !entry.dbRoles?.migrator) {
      errors.push({ serviceId: entry.serviceId, message: 'schema-owning service must declare dbRoles.migrator' });
    }
    if (entry.gatewayParticipation === 'apollo-subgraph' && !entry.gatewaySubgraph) {
      errors.push({ serviceId: entry.serviceId, message: 'apollo-subgraph entry must declare gatewaySubgraph' });
    }
    if (entry.serviceId === 'event-store-service' && entry.gatewayParticipation !== 'none') {
      errors.push({ serviceId: entry.serviceId, message: 'event-store-service must not participate in the gateway' });
    }
    if (entry.eventStoreTenantScopePolicy === 'all-tenants' && entry.serviceVisibility === 'public') {
      errors.push({
