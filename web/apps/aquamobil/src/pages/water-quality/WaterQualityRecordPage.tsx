...
import type { CreateWaterQualityPayload } from '@/types';
...
const CREATE_WQ_MUTATION = gql`
  mutation CreateWaterQualityMeasurement($input: CreateWaterQualityPayload!) {
...
export function WaterQualityRecordPage(): JSX.Element {
  ...
  const { mutateAsync: createMeasurement, isPending: isSubmitting } = useMutation({
    mutationFn: async (input: CreateWaterQualityPayload) =>
      graphqlRequest<{
  ...
  const handleSubmit = useCallback(
    async (values: Record<string, FieldValue>, notes: string, weatherConditions?: string) => {
      ...
      const input: CreateWaterQualityPayload = {
        equipmentId: selectedEquipmentId,
      ...
      try {
        if (isOnline) {
          ...
          const queued = await addToQueue('createWaterQuality', input);
