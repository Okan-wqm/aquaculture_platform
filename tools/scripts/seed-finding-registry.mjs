...
const seedEntries = [
  ...
  },
  {
    ...
    severity: 'MEDIUM',
    state: 'RESOLVED',
    title: 'Pre-existing CI failures on agentic: gitleaks-action v2 missing GITHUB_TOKEN + lighthouse-ci-action SHA 2f8dda0c does not resolve',
    layer: 3,
    evidence: [
      '.github/workflows/security-gitleaks.yml:34',
      '.github/workflows/performance-benchmark.yml:52',
    ],
    rule_violated: 'infra-expert GHA rule: every `uses:` must pin a verified commit SHA; every pre-existing workflow must remain functional after a CI-visibility-restoring sweep',
    owner_agent: 'infra-expert',
    raised_in_cycle: AUDIT_CYCLE,
    review_file: AUDIT_FILE,
