...
describe('ScadaPackageService.validateScripts', () => {
  ...
  it('accepts well-formed scripts within limits', () => {
    expect(() =>
      validateScripts({
        scripts: [scriptWith('return 1'), scriptWith('return 2', { mode: 'server' })],
      }),
    ).not.toThrow();
  });
  ...
  it('rejects a script whose code exceeds the size cap', () => {
    ...
    expect(() => validateScripts({ scripts: [scriptWith(huge)] })).toThrow(
  ...
  it('rejects an unknown execution mode', () => {
    expect(() =>
      validateScripts({ scripts: [scriptWith('return 1', { mode: 'daemon' })] }),
    ).toThrow(/'server' or 'client'/);
  });
