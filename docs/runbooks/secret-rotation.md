...
| `CLAUDE_CODE_OAUTH_TOKEN` (Plan ARIA-V3 §A0 INFRA-CRITICAL-002) | 90 days | 14 days | aria oncall | Consumed by `.github/workflows/aria-agent-executor.yml` and `tools/aria-poc/{ci,worker}_executor.py` for `claude code agent` invocations. **Rotation steps:** (1) generate new Claude Code OAuth token via `claude code auth login --output-token` on a clean workstation; (2) update `CLAUDE_CODE_OAUTH_TOKEN` GitHub repo secret; (3) manual-dispatch `aria-agent-executor.yml` with `mock: true` to confirm token loads cleanly; (4) flip `mock: false` once; observe one cycle; (5) close the rotation reminder issue with the literal token `Rotated-at: claude-code-oauth-token` per `INFRA-ROTATION-001`. **Pre-flight invariant:** Plan ARIA-V3 §B1 invariant `I-V3-22b test_claude_oauth_token_preflight_guard_present` asserts the workflow's pre-flight step rejects empty `CLAUDE_CODE_OAUTH_TOKEN` when `CLAUDE_CODE_MOCK=false`. |
| Plan ARIA-V3 `aria-ack-hmac-key` (operator-local) | 90 days | 14 days | aria oncall | Operator-local HMAC key at `aria-tools/secrets/ack_hmac.key` signing the autonomous-loop ack ledger. NEVER committed to git (gitignored + I-V3-19c/d). Rotation via `aria-kernel ack rotate-key --reason ... --operator-approval-ref ...` — full procedure in [aria-ack-key-rotation.md](aria-ack-key-rotation.md). Rolling list retains last 5 keys for historical row verification (I-V3-19e). |
...
**Deferrals are capped at one cadence interval** (per Faz 0 of Plan ARIA-V3, INFRA-ROTATION-001 discipline, paired with INFRA-CRITICAL-002 OAuth rotation). A JWT keypair cannot be deferred twice in a row; the second deferral auto-escalates to the CTO as a CRITICAL finding under INFRA-ROTATION-001.
