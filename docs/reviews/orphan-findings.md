...
## ORPHAN-HIGH-311 — cost attribution hardcodes estimated_usd=0.0 under managed auth; USD caps toothless, ROI reads $0 — IN-PROGRESS
...
## ORPHAN-HIGH-318 — handleFailedLogin misreads TypeORM UPDATE…RETURNING result shape: audit always records "attempt 0" and the CRITICAL ACCOUNT_LOCKED event never fires

**Discovered:** 2026-07-02, live prod (same lockout investigation — audit payload said `Invalid password (attempt 0)` while `auth.users.failedLoginAttempts` was 6).
**Evidence:** `apps/auth-service/src/modules/authentication/services/authentication.service.ts` `handleFailedLogin()` — `dataSource.query('UPDATE … RETURNING …')` with TypeORM's PostgresQueryRunner returns `[rows, affectedCount]` for UPDATE statements, but the code reads `result[0]?.failedLoginAttempts` (the ROWS ARRAY, not the first row) → `undefined ?? 0`. Consequences, both confirmed live: (1) every LOGIN_FAILED_INVALID_PASSWORD audit event logs `attempt 0`; (2) `isNowLocked` requires `updatedAttempts >= maxFailedAttempts` → `0 >= 5` is always false, so the `ACCOUNT_LOCKED` AuditLogSeverity.CRITICAL event and its operator-visible `logger.warn` NEVER fire (today's two lockouts of user 8025339a produced zero ACCOUNT_LOCKED events). The DB-side lock itself works — the SQL is correct; only the return-value read is wrong.
**Root cause:** untyped raw-query result — the hand-written `Array<{…}>` annotation asserts the wrong driver shape and the type system can't catch it.
**Why it matters:** account lockout is the platform's brute-force detection signal; its CRITICAL audit event and accurate attempt counts are exactly what a SOC-2 review and incident response need. Both are silently absent.
**How to fix (architectural):** replace the raw `dataSource.query` with the query builder (`.update().set().where().returning().execute()` → typed `UpdateResult.raw`) or destructure `[rows] = result` with a runtime shape assertion; add a unit test that pins the returned attempt count and an integration test asserting ACCOUNT_LOCKED is emitted on the threshold-crossing attempt. Fix belongs with the ORPHAN-HIGH-308 audit-RLS work (same code path, same test surface).
**Owner:** auth-security-expert. **Deadline:** 2026-07-24.

## ORPHAN-MEDIUM-319 — real client IP never reaches auth-service: gateway forwards no client-IP header and the resolver prefers the socket peer, so every audit row and users.lastLoginIp record the gateway container IP

**Discovered:** 2026-07-02, live prod (same lockout investigation — every LOGIN_* audit payload and `users.lastLoginIp` showed `::ffff:172.18.0.25`, the gateway container; the actual actors — a Windows/Chrome browser at 193.212.164.37 and a curl client at 104.248.134.38 — were only recoverable from nginx access logs by timestamp correlation).
**Evidence:** `apps/gateway-api/src/federation/authenticated-data-source.ts` `willSendRequest()` forwards authorization/cookie/x-tenant-id/correlation/trace headers but no client-IP header; `apps/auth-service/src/modules/authentication/resolvers/auth.resolver.ts:115` computes `request.ip || forwarded` — the socket peer (always the gateway) wins even if x-forwarded-for were present.
**Root cause:** the client IP was never made part of the gateway→subgraph identity contract; the resolver's fallback ordering hides the omission instead of failing loudly.
**Why it matters:** login forensics (who locked this account? which network did failed attempts come from?), lastLoginIp shown to users, and any future geo/IP anomaly detection are all pinned to a constant internal IP. This finding materially slowed today's investigation.
**How to fix (architectural):** carry the nginx-resolved client IP inside the SIGNED gateway→subgraph identity assertion (`generateServiceIdentityHeadersV2` canonical already binds headers; an unsigned x-forwarded-for would be spoofable by anything that can reach the subgraph port) and have `TenantContextMiddleware`/resolvers prefer the verified assertion field over `request.ip`. Same fix serves every subgraph, not just auth.
**Owner:** auth-security-expert + platform-kernel-expert. **Deadline:** 2026-07-31.

## ORPHAN-MEDIUM-320 — account lockout is invisible to the legitimate account owner: generic "Authentication failed" during the lock window, no lockout notification, no operator unlock surface

**Discovered:** 2026-07-02, live prod — the operator spent the morning unable to log in as codex-test-202605240515@suderra.test with the CORRECT password (verified: peppered bcrypt compare matches, and the same credentials succeed end-to-end via the public /graphql once unlocked) because earlier wrong-password submissions from a browser had tripped the 5-attempt/30-minute lock, and every subsequent correct-password attempt returned the same generic `GENERIC_AUTH_ERROR_MSG` as a wrong password.
**Evidence:** `authentication.service.ts` locked-account branch throws `UnauthorizedException('Authentication failed')` (deliberate anti-enumeration choice, commented); no notification of any kind is sent on lock (ACCOUNT_LOCKED event is also currently never emitted — ORPHAN-HIGH-318); admin-panel/tenant-admin expose no "account locked / unlock" surface, so the only remediation today was raw SQL against auth.users.
**Root cause:** the anti-enumeration posture was applied without a compensating legitimate-owner channel — the design conflates "don't tell the ATTACKER" with "don't tell ANYONE".
**Why it matters:** a locked-out legitimate user gets an indistinguishable-from-wrong-password error loop → support burden, operator confusion (this incident), and password-reset attempts that don't clear the lock.
**How to fix (architectural):** keep the generic wire response (correct anti-enumeration + no password oracle during lock), and add the owner-facing channel: (1) on lock trigger, send a notification email/push to the account's verified address ("account locked after N failed attempts, unlocks at T — wasn't you? reset your password"); (2) surface lock state + unlock action in admin-panel (SUPER_ADMIN) and tenant-admin (TENANT_ADMIN for their own users) via admin-api, audit-logged; (3) successful password RESET clears failedLoginAttempts/lockedUntil (verify — reset flow already writes the user row). Depends on ORPHAN-HIGH-318 for the lock-trigger hook.
**Owner:** auth-security-expert + admin-expert. **Deadline:** 2026-07-31.

## ORPHAN-HIGH-321 — farm outbox dispatcher is RLS-blind: forced tenant_isolation_policy on farm.outbox_events hides every pending row from the worker, so farm domain events are written but NEVER dispatched

**Discovered:** 2026-07-02, live prod (event-backbone census during ORPHAN-HIGH-317 diagnosis).
**Evidence:** `farm.outbox_events` holds 28 rows, ALL `publishedAt IS NULL`, `retryCount = 0`, `lastError` empty, `leasedAt/leasedBy` null — newest 2026-07-02 07:44, oldest 2026-06-30 (BatchHarvested, BatchTransferred ×3, MortalityRecorded, WaterQualityMeasurementCreated…). The worker IS running (`OutboxNotifyListener` "LISTEN connected" in boot logs; `@Cron` every 5s in `platform/libs/outbox/src/outbox-worker.service.ts`) yet zero dispatch attempts ever happen. `pg_class` shows `relrowsecurity = t, relforcerowsecurity = t` on `farm.outbox_events` with `tenant_isolation_policy USING (app.bypass_rls = 'on' OR "tenantId" = app.current_tenant)`. The worker polls with NO tenant GUC and NEVER sets `app.bypass_rls` (zero hits in `outbox-worker.service.ts` / `outbox-notify-listener.service.ts`) → its SELECT sees 0 rows → "nothing to do", silently, forever.
**Root cause:** the outbox table got fail-closed RLS (correct hardening — FARM-HIGH-083 closed the fail-open) but the cross-tenant infrastructure sweeper that must read ALL tenants' rows was never given an RLS-compatible read path. Fail-closed + system-path-not-provisioned = silent total outage of the outbox pattern.
**Why it matters:** the transactional-outbox guarantee ("the event WILL eventually publish") is void — farm writes regulatory/GDPR-relevant events (HarvestRegulatoryRecorded, TenantDataErased) into a queue nothing drains. Zero errors anywhere; only a stream census exposes it.
**How to fix (architectural):** the outbox worker's polling/leasing transaction must run with the audited bypass primitive (`BypassRlsService` sets `app.bypass_rls = 'on'` scoped to the transaction — the same pattern the SUPER_ADMIN login path uses), or an equivalent dedicated `outbox_worker` role with a policy carve-out. Add a worker-side metric/alarm: pending-outbox-age > threshold ⇒ alert (make the silent state impossible to re-enter). Verify the same defect in every service's outbox (`<svc>_outbox` / `outbox_events` tables all carry the same forced policy).
**Owner:** platform-kernel-expert + farm-expert. **Deadline:** 2026-07-16.

## ORPHAN-MEDIUM-322 — migration-runner and security-event sinks build non-canonical subjects (`platform.schema-migration.*`, `security.events.*`) that NatsEventBus.normalizeSubject rejects; observability's consumers listen on `events.`-prefixed variants nothing can ever emit

**Discovered:** 2026-07-02 (grants rewrite for ORPHAN-HIGH-317 — the observability subscribe grants name subjects no publisher can produce).
**Evidence:** `libs/backend-common/src/database/nats-migration-event-sink.ts` `subjectFor()` returns `${SCHEMA_MIGRATION_SUBJECT_PREFIX}.applied` = `platform.schema-migration.applied`; `libs/backend-common/src/security/security-event.service.ts` publishes to the bare `securityEventType` subject. `NatsEventBus.normalizeSubject` (platform/libs/event-bus) THROWS for any subject not starting with `events.`/`commands.`/`queries.` — both sinks' publishes die client-side (each swallows the error as best-effort). Meanwhile `observability-service` consumes `events.platform.schema-migration.>` and `events.security.events.>` — subject spaces with no possible publisher. The migration sink is additionally env-gated (`SCHEMA_MIGRATION_EVENTS_ENABLED`, default off), so the defect is latent until someone flips the flag and discovers nothing flows.
**Root cause:** two shared-lib publishers were written against a pre-normalization subject convention; the consumer side was written against the canonical one; no contract test spans the pair.
**How to fix (architectural):** canonicalize the SSoT constants (`SCHEMA_MIGRATION_SUBJECT_PREFIX` → `events.platform.schema-migration`, security events → `events.security.events.<type>`), keep observability's consumers as-is, and pin publisher↔consumer subject agreement with a contract test (same pattern as `nats-subject-contract.spec.ts`). The publish grants for both spaces are already provisioned platform-wide by the ORPHAN-HIGH-317 remediation (`events.platform.schema-migration.>`, `events.security.events.>`), so the code fix lands against a ready broker.
**Owner:** platform-kernel-expert. **Deadline:** 2026-07-24.

## ORPHAN-MEDIUM-323 — the advertised NATS SSoT CI invariant never ran in any workflow AND was 16/18 red against the artifacts it was meant to pin — RESOLVED (this PR)

**Discovered:** 2026-07-02 (wiring check during ORPHAN-HIGH-317).
