...
describe('platform service catalog parity', () => {
  ...
  it('keeps required boot signals in sync for cataloged services', () => {
    const manifest = readYaml<SignalsManifest>(
  ...
  it('keeps generated catalog deploy image targets in sync with the source catalog', () => {
    ...
    expect(generated.deploy.backendImageTargets.sort()).toEqual([...backendImageBuildTargets()].sort());
    expect(generated.deploy.frontendImageTargets.sort()).toEqual([...frontendImageBuildTargets()].sort());
    expect(generated.deploy.infraImageTargets.sort()).toEqual([...infraImageBuildTargets()].sort());
    expect(generated.deploy.applicationImageServices.sort()).toEqual([...imageBuildTargets()].sort());
    expect(generated.deploy.serviceDbRolePrefixes.sort()).toEqual([...serviceDbRolePrefixes()].sort());
  ...
  it('keeps generated deploy shell artifact aligned with catalog image targets', () => {
    ...
    expect(readShellStringList(deployEnv, 'CATALOG_BACKEND_IMAGE_SERVICES')).toEqual([...backendImageBuildTargets()].sort());
    expect(readShellStringList(deployEnv, 'CATALOG_FRONTEND_IMAGE_SERVICES')).toEqual([...frontendImageBuildTargets()].sort());
    expect(readShellStringList(deployEnv, 'CATALOG_INFRA_IMAGE_SERVICES')).toEqual([...infraImageBuildTargets()].sort());
    expect(readShellStringList(deployEnv, 'CATALOG_SERVICE_DB_ROLE_PREFIXES')).toEqual([...serviceDbRolePrefixes()].sort());
    expect(readShellStringList(deployEnv, 'CATALOG_READINESS_SERVICES')).toEqual(
      readinessServices().map((entry) => `${entry.serviceId}:${entry.port}`).sort(),
    );
