...
- @.claude/agents-enterprise-v2/_shared/_conversion-template.md         (canonical agent-file structure)
- @.claude/agents-enterprise-v2/orchestrator.md                         (AUTHORITATIVE runtime review roster — do NOT duplicate here)
...
## Finding ID prefix
`{PREFIX}-{SEVERITY}-{NNN}` — slice convention per @.claude/agents-enterprise-v2/_shared/output-format.md.
...
3. **Draft replacement** preserving EVERY domain-unique invariant verbatim or dense-reformatted; delegate generic content to `@.claude/knowledge/layer-*` + `@.claude/agents-enterprise-v2/_shared/*` references.
...
`PROMPT-{SEVERITY}-{NNN}` — for process findings prompt-writer itself raises (malformed agent prompt, missing finding-ID mandate in a generated agent, roster-drift between orchestrator + individual agent file, unowned repo surface). See `@.claude/agents-enterprise-v2/_shared/output-format.md`.
