...
See `@.claude/agents-enterprise-v2/_shared/operating-modes.md` for the full CATCHER / TEACHER / WRITER contract. Agent-specific overrides:
...
`PLAT-{SEVERITY}-{NNN}` — shared namespace with `platform-services` per finding-registry convention; no collision because each agent maintains its own NNN sequence per cycle. Severity ∈ {CRITICAL, HIGH, MEDIUM, LOW}. Example: `PLAT-CRITICAL-001`. See `@.claude/agents-enterprise-v2/_shared/output-format.md` for the full format.
