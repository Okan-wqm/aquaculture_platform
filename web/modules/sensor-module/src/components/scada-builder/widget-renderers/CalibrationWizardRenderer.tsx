...
const CalibrationWizardRenderer: React.FC<WidgetRendererProps> = ({
  config,
  width,
  height,
  isEditing,
  ...
  const currentStep = (
    isEditing ? (config.demoStep ?? 2) : Number(config.currentStep ?? 0)
  ) as number;
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        style={{ padding: '0 2px 4px', fontSize: 11, fontWeight: 600, color: colors.neutral[700] }}
      >
        {label}
      </div>
          ...
          return (
            <g key={i}>
              ...
              <text
                ...
                fontSize={8}
                fontWeight={700}
                fill={done || active ? 'white' : colors.neutral[400]}
              >
                {done ? '\u2713' : i + 1}
              </text>
              <text
                ...
                textAnchor="middle"
                fontSize={7}
                fill={active ? colors.info[700] : colors.neutral[400]}
              >
                {step}
              </text>
            </g>
