...
**REMAINING under DB-PEOPLE-HIGH-001 (NOT this fix):** the performance/goal operations. hr-module's GOAL fragment (`fragments.ts:569,573`) selects `keyResults` + `milestones` as SCALARS, but `Goal.keyResults?: KeyResult[]` / `Goal.milestones?: GoalMilestone[]` are object arrays — the FE must either sub-select their fields or the ops are over-reaching. That is an FE-fragment restructuring (not the clean BE-@Field pattern) and needs hr-expert judgment on which of the flagged ops are real. Tracked; not closed here.
