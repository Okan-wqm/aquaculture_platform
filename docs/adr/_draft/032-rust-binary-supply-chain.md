# ADR-032: Rust Binary Supply-Chain Hardening (SLSA-3 + cosign/sigstore + SBOM)

**Status:** Proposed
**Date:** 2026-04-22
**Deciders:** platform team, security team, SRE, edge-agent team (informed)
**Owner:** Okan
**Related ADRs:** ADR-011 (schema ownership), ADR-025 (Rust sidecar architecture), ADR-019 (edge firmware signing A/B partition) — pattern parity target
**Related orphan findings:** edge OTA update protocol (tracked separately in ORPHAN register — edge scope)
**Related plans:** `/root/.claude/plans/snappy-sniffing-pine.md` Kör Nokta 9-10
...
- The existing edge gateway (`sens-api-gateway/`) is IEC 62443 SL2 hardened at the dependency level (`deny.toml:1-111`) but does not expose an artifact-signing pipeline — ADR-019 addresses firmware signing + A/B partition; this ADR reuses the same cosign/sigstore primitives on the cloud side.
...
- Secret management for the Rust sidecar (DB password, NATS client cert + key) is undocumented — the plan's secret-config section is blank, and `docs/DEPLOY.md` has no Rust section.
...
5. **SLSA-3 target**: the tooling above covers SLSA-3 (build service, hermeticity where BuildKit allows, provenance, non-falsifiable identity). SLSA-4 (fully hermetic build, two-party review) is not in scope for Faz 0.
...
- Every deploy path — DigitalOcean today + future Kubernetes — must wire in verification. One-time cost; recovered after the first blocked bad artifact.
- Secret rotation runbooks add operator burden.

**Neutral:**
- Edge gateway (`sens-api-gateway/`) is covered by ADR-019 (firmware signing + A/B partition) which uses the same cosign/sigstore primitives. The edge OTA protocol remains tracked separately (orphan ORPHAN-018 in agentic-rust-unified id space) because the edge runtime has different constraints (offline tanks, over-the-air update channel).
