...
## SENSOR-MEDIUM-004 — process-editor canvas hand-manages its dep graph via CDN scripts instead of a build — OPEN (tracked, tier-1)
**Evidence:** `process-editor-canvas.html` is a ~1200-line hand-authored HTML whose React app loads all deps via ordered CDN `<script>` tags with no build-time resolution; it also ships `cdn.tailwindcss.com` in production (its own console warns against this) and runs the iframe `sandbox=allow-scripts allow-same-origin` (browser warns this can escape sandboxing).
**Rule:** a shipped React app's dependency graph must be resolved + pinned at build time by the bundler, not hand-ordered CDN scripts — build-time resolution makes it complete-by-construction, deterministic, offline-capable, and CSP-clean.
**Resolution (deeper structural fix, tracked):** if the canvas were a Vite entry, the jsx-runtime dep would be satisfied by construction and SENSOR-MEDIUM-003's fix would be unnecessary. Extract the inline app into a real Vite build entry, drop the CDN graph, self-host tailwind via PostCSS, tighten the iframe sandbox. Large; unverifiable in this env. Owner: sensor-expert. Deadline: 2026-07-22.
