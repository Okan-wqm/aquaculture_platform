...
exports.default = createRule({
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'queryKey for TanStack Query hooks must use createTenantQueryKey(tenantId, ...) from shared-ui (ADR-009). Bare arrays leak cache across tenant switches.',
        },
        schema: [],
        messages: {
            bareQueryKey: 'queryKey must use createTenantQueryKey(tenantId, ...segments) from @aquaculture/shared-ui. Bare arrays cannot be purged on tenant switch and cause cross-tenant cache leak (FE-CRITICAL-001). Import: `import { createTenantQueryKey } from "@aquaculture/shared-ui";`. Replace `[\'x\', tenantId, filter]` with `createTenantQueryKey(tenantId, \'x\', filter)`.',
            tenantIdNotPrefix: 'tenantId must be the FIRST parameter of createTenantQueryKey. Found array with tenantId in a non-prefix position — this is the exact FE-CRITICAL-001 bleed pattern. Migrate to `createTenantQueryKey(tenantId, ...)`.',
        },
    },
    defaultOptions: [],
    create(context) {
        function isTenantHintedArray(arr) {
...
//# sourceMappingURL=no-bare-tenant-query-key.js.map