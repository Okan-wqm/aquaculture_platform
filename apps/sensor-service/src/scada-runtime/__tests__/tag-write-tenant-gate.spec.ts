...
import { TagDataType, TagDirection, TagIoType } from '../../process/entities/unified-tag.entity';
...
describe('TAG_WRITE tenant + registry gate', () => {
  it('rejects a write to a tag not registered for the connecting tenant', async () => {
    const { gateway, tagManager, tagResolution } = makeGateway({
      resolved: [],
      unresolved: [{ ref: 'EDGE-01/pump.cmd', reason: 'NOT_FOUND' }],
    });
  ...
  it('rejects a write to a read-only (INPUT) tag', async () => {
    const { gateway, tagManager } = makeGateway({
      resolved: [binding(TagDirection.INPUT)],
      unresolved: [],
    });
    const socket = makeSocket();
    seedClient(gateway, socket);
  ...
  it('writes a registered OUTPUT tag with the tenant and ACKs queued (not accepted)', async () => {
    const { gateway, tagManager } = makeGateway({
      resolved: [binding(TagDirection.OUTPUT)],
      unresolved: [],
    });
    const socket = makeSocket();
    seedClient(gateway, socket);
    ...
    expect(tagManager.writeTagValue).toHaveBeenCalledWith(
      'EDGE-01/pump.cmd',
      1,
      'u1',
      'tenant-A',
