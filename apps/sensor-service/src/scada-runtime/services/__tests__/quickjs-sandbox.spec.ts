 ...
 * isolation.
 */
import { runInSandbox, DEFAULT_SANDBOX_LIMITS, type SandboxBridges } from '../quickjs-sandbox';
...
describe('QuickJS sandbox — bounded execution', () => {
  ...
  it('surfaces a guest throw as a rejected promise', async () => {
    await expect(runInSandbox('throw new Error("boom")', emptyBridges())).rejects.toThrow(/boom/);
  });

  it('surfaces a syntax error as a rejected promise', async () => {
    await expect(runInSandbox('return (((', emptyBridges())).rejects.toThrow();
  });
});
...
describe('QuickJS sandbox — host bridges', () => {
  ...
  it('injects resolved params as the `params` global', async () => {
    ...
    const result = await runInSandbox('return params.threshold + params.tag.value', bridges);
