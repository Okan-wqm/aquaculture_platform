...
## Codex managed-auth session (`codex-managed-auth`)

ARIA implementation workflows use Codex CLI with ChatGPT-managed auth on a
trusted runner. `OPENAI_API_KEY` and `CODEX_API_KEY` are explicitly forbidden
for ARIA live mode; rotation here means re-authenticating the runner session and
proving the CLI still reports managed auth.

**Steps:**

1. On the trusted runner, run `codex login` under the operator account approved
   for ARIA execution.
2. Verify `codex login status --json` or `codex doctor --json` reports a
   ChatGPT-managed authenticated session.
3. Confirm ARIA workflows do not expose `OPENAI_API_KEY` or `CODEX_API_KEY` in
   their environment.
4. Run the ARIA executor preflight in mock mode first, then one approved live
   request with proof artifacts retained.

**Rollback:** disable live ARIA execution by setting the mock kill switch and
re-run `codex login` only after the operator confirms the runner identity.
...
| Codex managed-auth session (`codex-managed-auth`)            | 90 days                      | 14 days            | aria oncall         | re-authenticate the trusted runner with `codex login` → verify `codex login status --json` or `codex doctor --json` reports managed auth → confirm `OPENAI_API_KEY` / `CODEX_API_KEY` are absent from ARIA live workflows                                                                                                                                                                                                                                                                                                                                                                                                                          |
