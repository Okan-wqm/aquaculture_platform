...
**Still open under this finding (owner okan, deadline 2026-12-31):** `admin.plan_definitions` and `admin.custom_plans` have not moved. `billing.plans.pricing` is still a jsonb price matrix and `admin.custom_plans.modules` still holds per-module subtotals in jsonb (nine of the twenty-three remaining allowlist entries between them), and `stripeProductId` / `stripePriceIds` still have two writable homes because `admin.plan_definitions` duplicates `billing.plans`.
