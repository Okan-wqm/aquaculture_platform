...
## FARM-HIGH-097 — createFeed `status` enum `@Field` defaultValue → masked Bad Request when status omitted
Same enum-`defaultValue` class as FARM-HIGH-090 (fixed for create-tank in #758), MISSED for feed. `create-feed.input.ts` `status` had `@Field(() => FeedStatus, { defaultValue: FeedStatus.AVAILABLE })`: @nestjs/graphql applies the raw enum KEY (`AVAILABLE`) — not the value (`'available'`) — when the field is omitted, so `@IsEnum(FeedStatus)` (validates the lowercase values) rejects EVERY createFeed that doesn't send status. Masked as "Bad Request" by prod disableErrorMessages — surfaced only after INFRA-MEDIUM-037 added server-side validation logging (`status must be one of: available, low_stock, ...`). The whole "FeedType coercion" investigation was the wrong field; the failing field was `status`. Fix: drop the GraphQL defaultValue (keep nullable + @IsOptional); the handler already applies `input.status ?? FeedStatus.AVAILABLE` (create-feed.handler.ts:125). Confirmed live: createFeed WITH explicit status:AVAILABLE succeeds; omitted fails.
REMAINING (tracked, broader sweep — ORPHAN-HIGH-090b): 16 more enum-`defaultValue` fields across farm create-DTOs (batch BatchInputType, feeding-protocol FeedType, feed-inventory/feeding-program/record/table, species SpeciesStatus, system SystemStatus, harvest-plan, maintenance, fish-health). Each needs nullable+@IsOptional+handler-default verified per field (createSystem works today, so they don't all manifest — must be evidence-tested via the new validation log, not blanket-removed). Feeding/protocol DTOs to be fixed before the protocols flow.
