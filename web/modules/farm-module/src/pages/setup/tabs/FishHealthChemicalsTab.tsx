...
const INITIAL_DATA: FishHealthItem[] = [
  { id: '1', name: 'SLICE Premix', code: 'FH-001', category: 'ANTIPARASITIC', activeIngredient: 'Emamectin Benzoate', concentration: '0.2%', formulation: 'Premix', unit: 'kg', supplierName: 'MSD Animal Health', withdrawalPeriodDays: 175, prescriptionRequired: true, targetConditions: ['Sea lice'], storageRequirements: 'Cool & dry, below 25°C', status: 'AVAILABLE', isActive: true },
  { id: '2', name: 'Alphamax', code: 'FH-002', category: 'ANTIPARASITIC', activeIngredient: 'Deltamethrin', concentration: '10 mg/ml', formulation: 'Liquid', unit: 'L', supplierName: 'PHARMAQ', withdrawalPeriodDays: 7, prescriptionRequired: true, targetConditions: ['Sea lice'], storageRequirements: 'Room temperature', status: 'AVAILABLE', isActive: true },
  { id: '3', name: 'Aqui-S', code: 'FH-003', category: 'ANESTHETIC', activeIngredient: 'Isoeugenol', concentration: '50%', formulation: 'Liquid', unit: 'L', supplierName: 'Aqui-S NZ', withdrawalPeriodDays: 1, prescriptionRequired: false, targetConditions: ['Sedation', 'Anesthesia'], storageRequirements: 'Room temperature, dark', status: 'AVAILABLE', isActive: true },
  { id: '4', name: 'Oxytetracycline 20%', code: 'FH-004', category: 'ANTIBIOTIC', activeIngredient: 'OTC', concentration: '200 mg/g', formulation: 'Powder', unit: 'kg', supplierName: 'PHARMAQ', withdrawalPeriodDays: 80, prescriptionRequired: true, targetConditions: ['Furunculosis', 'Vibriosis'], storageRequirements: 'Cool & dry', status: 'AVAILABLE', isActive: true },
  { id: '5', name: 'PHARMAQ Alpha Ject', code: 'FH-005', category: 'VACCINE', activeIngredient: 'Inactivated A. salmonicida', concentration: 'Standard', formulation: 'Injectable', unit: 'dose', supplierName: 'PHARMAQ', withdrawalPeriodDays: 450, prescriptionRequired: true, targetConditions: ['Furunculosis'], storageRequirements: 'Refrigerated 2-8°C', status: 'AVAILABLE', isActive: true },
  { id: '6', name: 'Bronopol 50%', code: 'FH-006', category: 'ANTIFUNGAL', activeIngredient: 'Bronopol', concentration: '50%', formulation: 'Liquid', unit: 'L', supplierName: 'Novartis', withdrawalPeriodDays: 0, prescriptionRequired: false, targetConditions: ['Saprolegnia'], storageRequirements: 'Room temperature', status: 'AVAILABLE', isActive: true },
];

type FormData = Omit<FishHealthItem, 'id'> & { targetConditionsText: string };

const emptyForm: FormData = {
  name: '', code: '', category: '', activeIngredient: '', concentration: '',
  formulation: '', unit: 'kg', supplierName: '', withdrawalPeriodDays: 0,
  prescriptionRequired: false, targetConditions: [], targetConditionsText: '',
  storageRequirements: '', status: 'AVAILABLE', isActive: true,
};
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filtered.map(item => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.activeIngredient}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                  {item.withdrawalPeriodDays > 0 ? `${item.withdrawalPeriodDays} days` : '-'}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${item.prescriptionRequired ? 'bg-red-50 text-red-700' : 'bg-gray-50 text-gray-600'}`}>
                ...
                <div className="px-6 pt-5 pb-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border-t pt-4 mt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="text" value={formData.activeIngredient} onChange={e => updateField('activeIngredient', e.target.value)}
                            className="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700">Concentration</label>
                          <input type="text" value={formData.concentration} onChange={e => updateField('concentration', e.target.value)}
                      ...
                      <div className="mt-4">
                        ...
                        <select value={formData.formulation} onChange={e => updateField('formulation', e.target.value)}
                          className="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500">
                          <option value="">Select</option>
                    ...
                    <div className="border-t pt-4 mt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="number" min="0" value={formData.withdrawalPeriodDays}
                            onChange={e => updateField('withdrawalPeriodDays', parseInt(e.target.value) || 0)}
                            className="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500" />
                        </div>
                        <div className="flex items-end pb-1">
                          <label className="flex items-center gap-2">
                            <input type="checkbox" checked={formData.prescriptionRequired}
                    ...
                    <div className="border-t pt-4 mt-4">
                      ...
                      <input type="text" value={formData.targetConditionsText}
                    ...
                    <div className="border-t pt-4 mt-4">
                      ...
                      <select value={formData.storageRequirements} onChange={e => updateField('storageRequirements', e.target.value)}
                        className="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:ring-blue-500 focus:border-blue-500">
                        <option value="">Select</option>
