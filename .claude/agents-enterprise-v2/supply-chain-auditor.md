...
See `@.claude/agents-enterprise-v2/_shared/operating-modes.md`. CATCHER default; TEACHER outputs cite the specific CVE + remediation path. WRITER mode NOT supported — supply-chain fixes route to infra-expert (workflow changes) or root maintainer (dep updates).
