  ...
  create(context) {
    function checkParameter(param) {
      for (const decorator of param.decorators ?? []) {
        ...
        if (
          ...
          continue;
        if (!exports.TENANT_KEYS.has(first.value)) continue;
        context.report({
          node: decorator,
    ...
    function checkClass(node) {
      ...
      for (const member of node.body.body) {
        ...
        if (!name || !exports.TENANT_KEYS.has(name)) continue;
    ...
    return {
      MethodDefinition(node) {
        if (node.value.type !== utils_1.AST_NODE_TYPES.FunctionExpression) return;
