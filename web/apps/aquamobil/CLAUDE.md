...
`npm test` (`vitest run`, inferred Nx `test` target) runs every `src/**/*.spec.{ts,tsx}` — the SW build-artifact invariant included — on every affected PR (`ci-affected.yml`) and under coverage in `ci-full.yml`; the LCOV report is inventoried in `tools/quality/coverage-report-inventory.json`. `test:invariant` additionally runs the SW build-artifact spec on its own (a real `vite build`) so the artifact gate is selectable without the whole suite. Both targets are kept reachable by `tests/invariants/test-target-ci-reachability.spec.ts`; until a plain `test` script existed the whole suite was unselectable and ran nowhere (INFRA-HIGH-157 / FARM-MEDIUM-304). The S1 codegen gate covers its GraphQL client. NOTE: the `eslint src` lint baseline is large and pre-existing (ORPHAN-MEDIUM-112) — do not treat it as a clean gate.
