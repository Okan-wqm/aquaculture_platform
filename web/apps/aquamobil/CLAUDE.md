...
`npm test` (`vitest run`, inferred Nx `test` target) runs every `src/**/*.spec.{ts,tsx}` — the SW build-artifact invariant included — on every affected PR (`ci-affected.yml`) and under coverage in `ci-full.yml`; the LCOV report is inventoried in `tools/quality/coverage-report-inventory.json`. Its GraphQL contract is generated end to end — every document under `src/**` (the offline-queue registry included) is a codegen source, `graphqlRequest` takes only a `TypedDocumentNode`, and the queued payload / view types derive from `src/generated/graphql.ts`; `tests/invariants/aquamobil-graphql-contract-ssot.spec.ts` keeps a hand-written `*Input` mirror or an untyped overload from coming back (MOB-HIGH-019). NOTE: the `eslint src` lint baseline is large and pre-existing (ORPHAN-MEDIUM-112) — do not treat it as a clean gate.
