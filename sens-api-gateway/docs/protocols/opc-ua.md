...
A separate OPC UA **server** role (feature `opc-ua-server`, crate `opcua = "0.12"`, `Cargo.toml:266`) is off by default and is covered by `integration/opcua-server.md`; this chapter covers the **client** role only.
