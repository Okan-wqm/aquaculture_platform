...
export const DirtyWaterTankConfig: React.FC<WidgetConfigProps> = ({ config, onChange, deviceId }) => {
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <Input fullWidth type="text" value={config.label || ''} onChange={(e) => onChange({ label: e.target.value })} placeholder="Dirty Water Tank" />
      </div>
      <div>
        <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Demo Level (%)</label>
        <Input fullWidth type="number" min={0} max={100} value={config.demoLevel ?? 55} onChange={(e) => onChange({ demoLevel: Number(e.target.value) })} />
      </div>
      <div>
