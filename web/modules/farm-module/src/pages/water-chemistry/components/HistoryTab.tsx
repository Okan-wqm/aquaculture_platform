  ...
  const historyColumns: DataTableColumn<MeasurementRow>[] = [
    {
      key: 'measuredAt',
      header: 'Date',
      render: (_value, m) => <span className="whitespace-nowrap text-gray-900">{formatDate(m.measuredAt)}</span>,
    },
    {
      key: 'tankId',
      header: 'Tank',
      render: (_value, m) => (
        <span className="whitespace-nowrap text-gray-900">{m.tankId ? tankMap[m.tankId] || m.tankId.slice(0, 8) : '-'}</span>
      ),
    },
    ...visibleConfigs.map((config: ParameterConfig): DataTableColumn<MeasurementRow> => ({
      ...
      render: (_value, m) => {
        ...
        return <span className="whitespace-nowrap text-gray-900">{val != null ? Number(val).toFixed(config.precision) : '-'}</span>;
      },
    })),
    {
      key: 'overallStatus',
    ...
    },
    {
      key: 'source',
      header: 'Source',
      render: (_value, m) => <span className="whitespace-nowrap text-gray-500">{getSourceLabel(m.source)}</span>,
    },
  ];
