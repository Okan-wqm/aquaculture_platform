...
describe('credential proof', () => {
  ...
  it.each([undefined, 0, -1, 1.5, Number.NaN])(
    'rejects absent or invalid version %s',
    (version) => {
      const user = principal();
...
describe('proof-owned pre-auth RLS context', () => {
  it.each([Role.TENANT_ADMIN, Role.SUPER_ADMIN])(
    'binds %s scope before transaction checkout and restores the caller frame',
    async (role) => {
      ...
      const user = Object.assign(new User(), {
        id: 'principal',
        role,
        tenantId: role === Role.SUPER_ADMIN ? null : 'tenant',
        isActive: true,
        credentialVersion: 1,
      });
      ...
      jest.spyOn(runner, 'connect').mockImplementation(async () => {
        expect(getRequestContext()).toMatchObject({
          ...
          bypassRls: role === Role.SUPER_ADMIN,
        });
      });
      ...
      await requestContextStorage.run({ tenantId: 'caller', bypassRls: true }, async () => {
        await withLockedCredentialPrincipal(
          source,
