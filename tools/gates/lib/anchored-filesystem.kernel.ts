...
import { defineErrorCause, errorFromUnknown } from './error-cause';
...
function stableCurrentnessError(
  ...
  return defineErrorCause(failure, cause);
}
...
function runWithAnchoredCleanup<T>(label: string, action: () => T, cleanup: () => void): T {
  ...
  if (outcome.status === 'FAILURE' && cleanupError !== undefined) {
    throw new AggregateError(
      [
        ...
        errorFromUnknown(`${label} cleanup failed`, cleanupError),
      ],
  ...
  if (cleanupError !== undefined) {
    throw errorFromUnknown(`${label} cleanup failed`, cleanupError);
  }
...
function closeAnchoredDirectoryComponents(
  ...
  for (const component of [...components].reverse()) {
    try {
      ...
      failures.push(
        errorFromUnknown(`Failed to close directory descriptor ${component.path}`, error),
      );
    }
  }
...
export function openAnchoredDirectoryChain(
  ...
  try {
    ...
    try {
      ...
      throw new AggregateError(
        [
          errorFromUnknown(`${label} descriptor-chain construction failed`, error),
          errorFromUnknown(`${label} descriptor-chain cleanup failed`, closeError),
        ],
...
interface HermeticExecutableTrustBoundaryV1 {
  readonly path: string;
  readonly ownerUid: bigint;
...
function assertTrustedExecutableGeneration(
  ...
  label: string,
  trustBoundary: HermeticExecutableTrustBoundaryV1,
): void {
  if (
    generation.ownerUid !== trustBoundary.ownerUid ||
    (generation.mode & 0o022n) !== 0n ||
    generation.linkCount !== 1n ||
    (generation.mode & 0o111n) === 0n
  ) {
    throw new Error(
      `${label} must be ${trustBoundary.ownerLabel}-owned, singly linked, executable, and group/world non-writable`,
    );
  }
}
...
function assertTrustedExecutableParents(
  ...
  const boundaryIndex = parents.findIndex((parent) => parent.path === trustBoundary.path);
  if (boundaryIndex === -1) {
    throw new Error(`${label} parent chain does not contain its trusted boundary`);
  }
  for (const parent of parents.slice(boundaryIndex)) {
    if (
      parent.generation.ownerUid !== trustBoundary.ownerUid ||
      (parent.generation.mode & 0o022n) !== 0n
    ) {
      throw new Error(
        `${label} parent is not ${trustBoundary.ownerLabel}-owned and group/world non-writable: ${parent.path}`,
      );
    }
  }
}

function openHermeticExecutableAuthorityAtTrustBoundary(
  ...
  try {
    ...
    assertTrustedExecutableGeneration(generation, contract.label, trustBoundary);
    const parentGenerations = frozenParentGenerations(parent);
    assertTrustedExecutableParents(parentGenerations, contract.label, trustBoundary);
    ...
    const authority = Object.freeze({
      ...
      close: (): void => {
        ...
        try {
          ...
          failures.push(errorFromUnknown(`${contract.label} descriptor close failed`, error));
        }
        try {
          closeAnchoredDirectoryChain(parent);
        } catch (error) {
          failures.push(errorFromUnknown(`${contract.label} parent close failed`, error));
        }
        const [onlyFailure] = failures;
        if (failures.length === 1 && onlyFailure !== undefined) throw onlyFailure;
        if (failures.length > 1) {
    ...
    if (descriptor !== null) {
      try {
        ...
        cleanupFailures.push(
          errorFromUnknown(`${contract.label} descriptor cleanup failed`, closeError),
        );
      }
    }
    try {
      ...
      cleanupFailures.push(errorFromUnknown(`${contract.label} parent cleanup failed`, closeError));
    }
    if (cleanupFailures.length > 0) {
      throw new AggregateError(
        [
          errorFromUnknown(`${contract.label} authority construction failed`, error),
          ...cleanupFailures,
        ],
...
export function openHermeticExecutableAuthority(
  ...
  return openHermeticExecutableAuthorityAtTrustBoundary(
    contract,
    Object.freeze({ path: parse(contract.path).root, ownerUid: 0n, ownerLabel: 'root' }),
    operationDeadlineMs,
  );
}

/**
 * Exact-import adapter for executable lifecycle tests. Production always enters through the
 * root-owned boundary above; tests receive authority only for one direct child of an isolated,
 * process-owned directory and cannot inject a relaxed policy into production construction.
 */
export function openHermeticExecutableAuthorityAtOwnedFixtureBoundary(
  contract: HermeticExecutableContractV1,
  fixtureRoot: string,
  operationDeadlineMs?: number,
): HermeticExecutableAuthorityV1 {
  const canonicalFixtureRoot = requireCanonicalAbsolutePath(fixtureRoot, 'fixture trust root');
  const canonicalExecutablePath = requireCanonicalAbsolutePath(
    contract.path,
    `${contract.label} path`,
  );
  const canonicalTempRoot = realpathSync(tmpdir());
  const fixtureStat = lstatSync(canonicalFixtureRoot, { bigint: true });
  if (
    realpathSync(canonicalFixtureRoot) !== canonicalFixtureRoot ||
    dirname(canonicalFixtureRoot) !== canonicalTempRoot ||
    !fixtureStat.isDirectory() ||
    fixtureStat.isSymbolicLink() ||
    (fixtureStat.mode & 0o777n) !== 0o700n ||
    dirname(canonicalExecutablePath) !== canonicalFixtureRoot
  ) {
    throw new Error(
      'Hermetic executable fixture root must be one canonical mode-0700 OS-temp child and directly own its executable',
    );
  }
  const effectiveUid = process.geteuid?.();
  if (effectiveUid === undefined) {
    throw new Error('Hermetic executable fixture requires a POSIX effective UID');
  }
  return openHermeticExecutableAuthorityAtTrustBoundary(
    contract,
    Object.freeze({
