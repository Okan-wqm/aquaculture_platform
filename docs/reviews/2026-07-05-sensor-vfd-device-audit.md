...
|---|---|---|---|
| SENSOR-CRITICAL-002 | CRITICAL | Güvenlik | HTTP-REST sensör adaptörü operatör kontrollü URL'i doğrulamadan `fetch` ediyor → SSRF ile cloud metadata kimlik hırsızlığı |
| SENSOR-CRITICAL-003 | CRITICAL | Ürün | VFD sekmesi VFD verisi çekmiyor; kayıtlı VFD'ler hiçbir liste/detayda görünmüyor, sekme sensör listesini gösteriyor |
| SENSOR-HIGH-017 | HIGH | Güvenlik | VFD Modbus `host` alanı doğrulanmıyor → iç ağ port-taraması / SSRF |
| SENSOR-HIGH-018 | HIGH | Veri | `dataChannels` sensör kaydında sessizce atılıyor (servis hiç persist etmiyor) |
| SENSOR-HIGH-019 | HIGH | ICS | VFD Modbus-TCP bağlantısı TLS'siz ve circuit breaker'sız |
| SENSOR-HIGH-020 | HIGH | Doğrulama | VFD protokol config gevşek union DTO ile doğrulanıyor → aralık dışı baud/slaveId/IP persist ediliyor |
| SENSOR-HIGH-021 | HIGH | Tenant | `registrationResolver` `'default-tenant'` fail-open fallback + eksik `TenantGuard` |
| SENSOR-HIGH-022 | HIGH | Erişim | `@RequireTenantPermission('edge:manage-io-config')` hiçbir guard'la uygulanmıyor (ölü metadata) |
| SENSOR-HIGH-023 | HIGH | Ürün | `mapSensorToType` parent/child + site/departman alanlarını düşürüyor → grup görünümü + detay geri-okuma bozuk |
| SENSOR-HIGH-024 | HIGH | Veri | Sensör sihirbazı "zorunlu" konum hiyerarşisini (siteId/departmentId) serialize etmiyor |
| SENSOR-HIGH-025 | HIGH | Veri | Parent+children servis create'i `description` alanını düşürüyor |
...
### SENSOR-CRITICAL-002 — HTTP-REST sensör adaptöründe kısıtlanmamış SSRF (metadata kimlik hırsızlığı)
...
### SENSOR-CRITICAL-003 — Kayıtlı VFD cihazları görünmez; "VFD" sekmesi sensör verisi gösteriyor
...
### SENSOR-HIGH-017 — VFD Modbus `host` alanı üzerinden SSRF / iç port-taraması
...
**Kök-neden düzeltme:** SENSOR-CRITICAL-002 ile aynı IP/host allow/deny politikasını `openSocket`'e (ve tüm serial/IoT socket adaptörlerine) uygula; ideal olarak endpoint'leri tenant'ın bildirdiği edge-gateway alt-ağıyla sınırla; istemciye dönen hata mesajlarını tek "connection failed"e normalize et.

### SENSOR-HIGH-018 — Sensör kaydında `dataChannels` sessizce atılıyor
...
### SENSOR-HIGH-019 — VFD Modbus-TCP: transport şifrelemesi ve circuit breaker yok
...
### SENSOR-HIGH-020 — VFD protokol config gevşek union DTO ile doğrulanıyor
...
### SENSOR-HIGH-021 — `registrationResolver`'da `'default-tenant'` fail-open fallback + eksik `TenantGuard`
...
### SENSOR-HIGH-022 — `@RequireTenantPermission('edge:manage-io-config')` hiçbir guard'la uygulanmıyor
...
### SENSOR-HIGH-023 — `mapSensorToType` parent/child + konum alanlarını düşürüyor
...
### SENSOR-HIGH-024 — Sensör sihirbazı "zorunlu" konum hiyerarşisini serialize etmiyor
...
### SENSOR-HIGH-025 — Parent+children servis create'i `description`'ı düşürüyor
...
### SENSOR-HIGH-026 — VFD sihirbazı `modelSeries/pumpId/notes/tags`'i düşürüyor (entity kolonu yok)
...
### SENSOR-HIGH-027 — `tenant_provisioning_keys` şema-yerleşim uyumsuzluğu → self-register akışı kopuk
...
### SENSOR-HIGH-028 — Frontend/backend enum drift (SensorType + VfdChangeSetStatus)
...
- **SENSOR-MEDIUM-009** — `vfd_register_mappings` cross-tenant referans tablosu `schema:` ve `tenantId` olmadan (`entities/vfd-register-mapping.entity.ts:25`) → belirsiz yerleşim, yanlış-register-yazma riski. `schema:'sensor'` + `infrastructureTables`, ya da `tenantId` scope.
- **SENSOR-MEDIUM-010** — `registerParentWithChildren`: commit sonrası fırlatma zaten commit'lenmiş transaction'da `rollbackTransaction()` tetikliyor (`sensor-registration.service.ts:738,753-789,814-821`) → yanlış "failed" + retry'de duplicate cihaz. Commit-sonrası işi try dışına al veya `queryRunner.isTransactionActive` ile gate'le; parent kimliğine idempotency guard.

---

## LOW

- **SENSOR-LOW-001** — `ingestReading`/`batchIngestReadings` rol gate'siz (`sensor/resolvers/sensor.resolver.ts:305-340`) → `MODULE_USER` sahte ölçüm basıp alert'leri tetikleyebilir. `@Roles(TENANT_ADMIN, MODULE_MANAGER)` ekle.
- **SENSOR-LOW-002** — `pingEdgeDevice` rol gate'siz (`edge-device.resolver.ts:214-221`), diğer cihaz komutlarıyla tutarsız. `@Roles` ekle.
- **SENSOR-LOW-003** — `DevicesPage` tüm ayrıcalıklı cihaz aksiyonlarını (ekle/bulk firmware/configure) frontend rol gating olmadan render ediyor (backend fail-closed, ama defense-in-depth + doğru affordance eksik). Paylaşılan auth/rol context'iyle gate'le.
- **SENSOR-LOW-004** — Sensör/VFD sekmelerinde pagination yok + search yalnızca ilk 20 satırda (`useSensorList.ts:120-127`, `DevicesPage.tsx:1050-1147`) → 20+ cihazda başarılı kayıt gizlenebilir. Edge sekmesiyle server-side arama/pagination paritesi.
...
1. **Hemen (CRITICAL):** SENSOR-CRITICAL-002 (SSRF `SafeHttpClient`), SENSOR-CRITICAL-003 (VFD sekmesi görünürlüğü).
2. **Bu PR / sprint (HIGH):** SENSOR-HIGH-017..012. Güvenlik (001,003,006), veri-kaybı (002,007,008,009,010), tenant (005,011), kontrat (012,004).
3. **Takip (MEDIUM/LOW):** owner + deadline + tracked finding ID ile.

Ortak `SafeHttpClient`/socket-guard implementasyonu için SENSOR-CRITICAL-002 + SENSOR-HIGH-017'i `auth-security-expert`/`infra-expert`'e yönlendir; yeni `backend → operatör-tanımlı endpoint` trust boundary'sini `docs/architecture/trust-boundaries.md`'ye ekle.
