...
describe('cross-runtime canonical JSON authority', () => {
  it('implements the RFC 8785 number serialization sample', () => {
    const document = createCanonicalJsonDocumentV1([
      Number('333333333.33333329'), 1e30, 4.5, 2e-3, 1e-27, -0,
    ]);
    expect(canonicalJsonStringify(document)).toBe('[333333333.3333333,1e+30,4.5,0.002,1e-27,0]');
    expect(() => createCanonicalJsonDocumentV1([Number.NaN])).toThrow(
      'CANONICAL_JSON_NON_FINITE_NUMBER',
    );
    expect(() => createCanonicalJsonDocumentV1([Number.POSITIVE_INFINITY])).toThrow(
      'CANONICAL_JSON_NON_FINITE_NUMBER',
    );
  });

  it('implements RFC 8785 UTF-16 property ordering including astral keys', () => {
    const document = createCanonicalJsonDocumentV1({
      '\u20ac': 'Euro Sign',
      '\r': 'Carriage Return',
  ...
  it('separates strict JSON data from the bounded legacy wire normalizer', () => {
    ...
    expect(canonicalWireJsonContentSha256V1({ text: '\ud800' })).toBe(
      '7d38e2388498cec03881027e7753b07826c5af2d61dd589b4c1caaab14ec2cc4',
    );
  ...
  it('pins structuredClone authority at module initialization', () => {
    const original = Reflect.get(globalThis, 'structuredClone');
    try {
      Reflect.set(globalThis, 'structuredClone', () => {
        throw new Error('mutated clone authority');
      });
      expect(canonicalJsonStringify(createCanonicalJsonDocumentV1({ stable: true }))).toBe(
        '{"stable":true}',
      );
    } finally {
  ...
  it('matches published SHA-256 and astral UTF-8 vectors in every runtime', () => {
    ...
    expect(canonicalWireJsonContentSha256V1({ b: 2, a: 1 })).toBe(
