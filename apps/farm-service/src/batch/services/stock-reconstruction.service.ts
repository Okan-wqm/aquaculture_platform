...
export class StockReconstructionService {
  ...
  static fold(rows: TankBatchReconstructionRow[]): ReconstructedSiteStock {
    ...
    for (const row of rows) {
      ...
      const qty =
        Number(row.inflowSigned) - Number(row.mortality) - Number(row.cull) - Number(row.harvest);
      if (qty < 0) {
        return {
          ...empty,
    ...
    const speciesBreakdown: ReconstructedSpeciesStock[] = Array.from(bySpecies.values()).map(
      (acc) => ({
        ...
        avgWeightG:
          acc.quantity > 0 ? Math.round(((acc.biomassKg * 1000) / acc.quantity) * 100) / 100 : 0,
      }),
    );
    ...
    return {
      ...
      totalBiomassKg:
        Math.round(speciesBreakdown.reduce((sum, s) => sum + s.biomassKg, 0) * 100) / 100,
