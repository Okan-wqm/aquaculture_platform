...
export class TagValueFanoutService implements OnModuleInit, OnModuleDestroy {
  ...
  async fanoutMetric(metric: IngestedMetricForFanout): Promise<void> {
    try {
      const fqns = await this.resolveFqns(metric.tenantId, metric.sensorId, metric.channelId);
      if (fqns.length === 0) {
        this.unmappedCount++;
        return;
      }
  ...
  private async resolveFqns(
    ...
    const fqns = await this.unifiedTagService.findFqnsBySensorSource(tenantId, sensorId, channelId);
