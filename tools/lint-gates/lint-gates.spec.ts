...
async function gateRuleIds(filePath: string, code: string): Promise<readonly string[]> {
  ...
  const messages = linter.verify(code, {
    // WHY: ESLint 9's `Linter` defaults to flat-config mode, where parser
    // settings live under `languageOptions` — a top-level `parserOptions`
    // (eslintrc shape) is rejected with "appears to be in eslintrc format
    // rather than flat config format". The repo migrated to flat config
    // (eslint.config.mjs), so the gate harness must speak flat too.
