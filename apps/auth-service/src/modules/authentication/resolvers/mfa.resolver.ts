...
export class MfaResolver {
   ...
   * neither.
   *
   * Precedence: an authenticated identity wins (the token arg is ignored),
   * so a setup token can never redirect an authenticated user's enrollment
   * to another account. Without a session, the mfa_setup token — minted by
   * login after password validation when the tenant enforces MFA — is the
   * credential (MfaService positively requires type === 'mfa_setup').
   * These two operations are the ONLY consumers of the setup token.
   */
  private resolveMfaSubject(context: GqlContext, mfaSetupToken?: string | null): string {
    ...
    if (!mfaSetupToken) {
      throw new UnauthorizedException(
        'Authentication or an MFA setup token is required',
      );
    }
   ...
   */
  // SECURITY: pre-session surface → velocity-limited per setup token (the
  // SEC-CRITICAL-002 pattern verifyMfaLogin established). Authenticated calls
  // have no token arg, so the identifier dimension falls back to user/ip.
  @RateLimit({
    name: 'mfa-setup-init',
    limit: 5,
    windowMs: 15 * 60_000,
