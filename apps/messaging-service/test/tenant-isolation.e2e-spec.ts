...
describe('Tenant Isolation (E2E) — SECURITY-CRITICAL', () => {
  ...
  it('Tenant B cannot read Tenant A messages', async () => {
    ...
    if (res.body.errors) {
      expect(res.body.errors[0].message).toMatch(/forbidden|not a member|not found/i);
    } else {
  ...
  it('Tenant B cannot join Tenant A channel', async () => {
    ...
    expect(res.body.errors[0].message).toMatch(/forbidden|not found|not a member/i);
  });
