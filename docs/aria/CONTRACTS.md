...
- 10 consecutive isolated artifact-bearing `v2-shadow` cycles from a clean worktree, with every expected tool either recorded as `ok` or explicitly failed closed. Lifecycle-only zero-artifact cycles are diagnostic and excluded from promotion evidence.
- `runtime verify-artifacts` and `integrity verify` after every cycle, with non-empty verified artifact index and inventory rows for artifact-bearing gates.
- A short isolated `v2` smoke after shadow soak, followed by retention dry-run, apply, hot-artifact removal, restore, rollback, and final verification.
...
`suppressed_count` and `truncated_count` are derived from runtime run summaries. Real `v2` source-of-truth promotion remains blocked unless the evidence bundle proves those counters match source/emitted record cardinality and every omitted or shortened record resolves through artifact refs.
